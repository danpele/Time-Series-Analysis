...
def chart_hazard_rate():
    ...
    ax.set_title('(B) Crash Becomes Certain as $t \\to t_c$', fontweight='bold')
    ax.set_ylim(0, 1.1); ax.grid(True, alpha=0.3)
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.10), ncol=2, frameon=False)

    fig.text(0.5, -0.02,
             'The hazard rate diverges as $t \\to t_c$: crash becomes increasingly likely\n'
             'This is why bubbles eventually burst—the instability grows without bound',
...
if __name__ == '__main__':
    print('Generating hazard_rate chart...')
    chart_hazard_rate()
    print('Done!')
