...
\begin{frame}{Forecast Evaluation}
    ...
    \begin{block}{Common Metrics}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{RMSE}: $\sqrt{\frac{1}{n}\sum e_{t+h}^2}$
            \item \textbf{MAE}: $\frac{1}{n}\sum |e_{t+h}|$
            \item \textbf{MAPE}: $\frac{100}{n}\sum \left|\frac{e_{t+h}}{Y_{t+h}}\right|$
        \end{itemize}
    \end{block}
...
\begin{frame}{Quiz Question 1: Answer}
    \begin{exampleblock}{Correct Answer: (B)}
        Yes, because all eigenvalues are inside the unit circle.
    \end{exampleblock}

    \begin{block}{Proof/Calculation}
        Eigenvalues of $\bA$: $\det(\bA - \lambda \mathbf{I}) = 0$
        $$(0.8 - \lambda)(0.5 - \lambda) - 0.03 = \lambda^2 - 1.3\lambda + 0.37 = 0$$
        $$\lambda = \frac{1.3 \pm \sqrt{1.69 - 1.48}}{2} = \frac{1.3 \pm 0.458}{2}$$
        $$\lambda_1 = 0.879, \quad \lambda_2 = 0.421$$

        Both $|\lambda_1| < 1$ and $|\lambda_2| < 1$ $\Rightarrow$ \textbf{Stable!}
    \end{block}

    {\footnotesize
    \begin{alertblock}{Why (A) is Wrong}
        Diagonal elements alone don't determine stability. Off-diagonal elements affect eigenvalues.
    \end{alertblock}
    }
\end{frame}
...
\begin{frame}{Quiz Question 2: Answer}
    \begin{exampleblock}{Correct Answer: (B)}
        Past values of $X$ contain useful information for predicting $Y$.
    \end{exampleblock}

    \begin{block}{Explanation}
        Granger causality is about \textbf{predictive content}, not true economic causation:
        $$\E[Y_t | Y_{t-1}, \ldots, X_{t-1}, \ldots] \neq \E[Y_t | Y_{t-1}, \ldots]$$

        Adding $X$'s history improves prediction of $Y$.
    \end{block}

    {\small
    \begin{alertblock}{Why Other Options are Wrong}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item (A): Granger causality $\neq$ economic causality (could be spurious)
            \item (C): Bidirectional Granger causality is possible ($X \leftrightarrow Y$)
            \item (D): Direction of correlation is not implied by Granger test
        \end{itemize}
    \end{alertblock}
    }
\end{frame}
...
\begin{frame}{Quiz Question 3: Answer}
    \begin{exampleblock}{Correct Answer: (C)}
        Which variables can respond contemporaneously to which shocks.
    \end{exampleblock}

    \begin{block}{Illustration with $K=2$}
        Cholesky: $\mathbf{P} = \begin{pmatrix} p_{11} & 0 \\ p_{21} & p_{22} \end{pmatrix}$

        \vspace{0.2cm}
        \begin{itemize}
            \item Variable 1 ordered first: responds only to shock 1 at $t=0$
            \item Variable 2 ordered second: responds to both shocks at $t=0$
        \end{itemize}

        \vspace{0.2cm}
        If we reverse order, the contemporaneous restrictions change!
    \end{block}

    \begin{alertblock}{Example}
        Ordering (GDP, Interest Rate): GDP doesn't respond to monetary shock immediately.

        Ordering (Interest Rate, GDP): Interest rate doesn't respond to output shock immediately.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Quiz Question 5: Answer}
    \begin{exampleblock}{Correct Answer: (B)}
        35\% of variable 1's $h$-step forecast error variance is due to shocks to variable 2.
    \end{exampleblock}

    \begin{block}{FEVD Formula Reminder}
        $$\text{FEVD}_{ij}(h) = \frac{\sum_{s=0}^{h-1} [\boldsymbol{\Theta}_s]_{ij}^2}{\sum_{s=0}^{h-1} \sum_{k=1}^{K} [\boldsymbol{\Theta}_s]_{ik}^2}$$

        \vspace{0.2cm}
        Measures the proportion of variable $i$'s forecast uncertainty at horizon $h$ that is attributable to shocks originating from variable $j$.
    \end{block}

    \begin{exampleblock}{Interpretation}
        If we're forecasting GDP $h$ periods ahead, 35\% of our forecast uncertainty comes from not knowing future monetary policy shocks.
    \end{exampleblock}
\end{frame}
