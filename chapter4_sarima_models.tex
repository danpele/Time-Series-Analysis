...
\begin{frame}{Quiz Question 1: Answer}
    \begin{exampleblock}{Correct Answer: (C)}
        $s = 12$ (12 months per year)
    \end{exampleblock}

    \begin{block}{Common Seasonal Periods}
        \begin{center}
        \begin{tabular}{lcc}
            \toprule
            \textbf{Data Frequency} & \textbf{Seasonality} & \textbf{$s$} \\
            \midrule
            Quarterly & Annual & 4 \\
            Monthly & Annual & 12 \\
            Weekly & Annual & 52 \\
    ...
    {\footnotesize
    \begin{alertblock}{Note}
        Some series have multiple seasonalities (e.g., daily data: weekly $s=7$ AND annual $s=365$).
    \end{alertblock}
    }
\end{frame}
...
\begin{frame}{Quiz Question 2: Answer}
    \begin{exampleblock}{Correct Answer: (B)}
        Computes $Y_t - Y_{t-12}$ (year-over-year change)
    \end{exampleblock}
    ...
    \begin{block}{Explanation}
        ...
        \textbf{Example}: For January 2024 data:
        $$\Delta_{12} Y_{\text{Jan 2024}} = Y_{\text{Jan 2024}} - Y_{\text{Jan 2023}}$$
    \end{block}

    \begin{exampleblock}{Why It Works}
        If $Y_t$ has a seasonal pattern that repeats every 12 months, then $Y_t \approx Y_{t-12}$, so:
        $$\Delta_{12} Y_t = Y_t - Y_{t-12} \approx 0 + \text{noise}$$
        The seasonal pattern is removed!
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz Question 5: Answer}
    \begin{exampleblock}{Correct Answer: (B)}
        The series has annual seasonality that needs seasonal differencing
    \end{exampleblock}

    \begin{block}{Interpretation}
        ACF spikes at lags $s, 2s, 3s, \ldots$ (here 12, 24, 36) indicate:
        \begin{itemize}
            \item Strong correlation between $Y_t$ and $Y_{t-12}$, $Y_{t-24}$, etc.
            \item The series ``remembers'' its value from the same month last year
            \item This is the signature of \textbf{stochastic seasonality}
        \end{itemize}
    \end{block}

    \begin{exampleblock}{Solution}
        Apply seasonal differencing: $W_t = (1 - L^{12})Y_t = Y_t - Y_{t-12}$

        After differencing, the ACF at seasonal lags should be reduced.
    \end{exampleblock}
\end{frame}
