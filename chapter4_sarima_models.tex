...
\begin{frame}{F-test for Seasonal Dummies: Intuition}
    ...
    \begin{block}{The Models Being Compared}
        \textbf{Restricted} (no seasonality): $Y_t = \alpha + \varepsilon_t$ (just a constant)

        \textbf{Unrestricted} (with seasonality): $Y_t = \alpha + \sum_{j=1}^{s-1} \gamma_j D_{jt} + \varepsilon_t$
...
\begin{frame}{Kruskal-Wallis Test: Intuition}
    \begin{block}{What Does This Test Do?}
        ...
        \begin{itemize}
            ...
            \item Then checks if ranks are evenly distributed across seasons
    ...
    \begin{exampleblock}{Why Use It Instead of F-test?}
        \begin{itemize}
            ...
            \item Good for financial data with heavy tails
        \end{itemize}
    \end{exampleblock}
...
\begin{frame}{Kruskal-Wallis Test: Formula and Example}
    ...
    \begin{block}{Test Statistic}
        ...
        where $N$ = total observations, $n_j$ = obs. in season $j$, $R_j$ = sum of ranks in season $j$.
    \end{block}

    \vspace{0.2cm}

    \begin{exampleblock}{Example: Quarterly Sales (n=20, s=4)}
        Data ranked 1-20. Suppose rank sums by quarter:
        \begin{itemize}
            \item Q1: $R_1 = 15$, Q2: $R_2 = 35$, Q3: $R_3 = 70$, Q4: $R_4 = 90$
        \end{itemize}
...
\begin{frame}{HEGY Test: What Problem Does It Solve?}
    ...
    \begin{exampleblock}{Why Not Just Use ADF?}
        ADF only tests for a \textbf{regular} unit root at frequency zero.

        Seasonal data can have unit roots at \textbf{seasonal frequencies} (e.g., annual, semi-annual cycles) that ADF misses!
    \end{exampleblock}

    \vspace{0.2cm}

    \begin{alertblock}{HEGY Tests Multiple Frequencies}
        For quarterly data: tests at 0, $\pi$, and $\pm\pi/2$ frequencies.

        For monthly data: tests at 0, $\pi$, $\pm\pi/6$, $\pm\pi/3$, $\pm\pi/2$, $\pm 2\pi/3$, $\pm 5\pi/6$.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Canova-Hansen Test: The Opposite of HEGY}
    ...
    \begin{alertblock}{Why Does This Matter?}
        \begin{itemize}
            \item HEGY: ``Prove to me there's NO unit root'' (conservative toward differencing)
            \item CH: ``Prove to me there IS a unit root'' (conservative toward dummies)
...
\begin{frame}{Summary: Choosing the Right Seasonality Test}
    ...
    \begin{alertblock}{Key Insight}
        F-test/Kruskal-Wallis answer: ``\textit{Is there seasonality?}''

        HEGY/Canova-Hansen answer: ``\textit{What type of seasonality?}'' (deterministic vs stochastic)
    \end{alertblock}
\end{frame}
