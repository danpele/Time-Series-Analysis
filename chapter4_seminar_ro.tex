...
\begin{frame}{Test 12: Identificarea MA Sezonier}
    ...
    \begin{exampleblock}{Răspuns: A -- SARIMA$(0,1,0) \times (0,1,1)_{12}$}
        {\small
        \textbf{Model}: Lag-uri obișnuite -- fără vârfuri în ACF/PACF; Lag-uri sezoniere -- ACF se întrerupe la $s$, PACF descrește
...
\begin{frame}{Exemplu: Interpretarea ACF/PACF pentru Date Sezoniere}
    \begin{block}{Modele Observate (după diferențiere)}
        \begin{itemize}
            \item ACF: Semnificativ la lag-urile 1, 12; se întrerupe după lag 1 și lag 12
    ...
    \begin{exampleblock}{Interpretare}
        \textbf{Componenta obișnuită}: ACF se întrerupe la 1 $\Rightarrow$ MA(1)
        ...
        \vspace{0.2cm}
        \textbf{Verificare alternativă}: Dacă PACF ar fi arătat întrerupere la lag-urile sezoniere în loc de ACF, considerați termeni AR sezonieri.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Discuție: Sezonalitate Deterministă vs Stocastică}
...
\begin{frame}{Concluzii Cheie din Acest Seminar}
    \begin{block}{Puncte Principale}
        \begin{enumerate}
            \item Diferențierea sezonieră $(1-L^s)$ elimină sezonalitatea stocastică
