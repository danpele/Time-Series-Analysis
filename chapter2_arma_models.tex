...
\subtitle{Autoregressive, Moving Average, and Mixed Models}
...
\begin{frame}{The Box-Jenkins Methodology}
    ...
    \begin{tikzpicture}[node distance=1.5cm, auto,
        box/.style={rectangle, draw=MainBlue, thick, fill=MainBlue!10,
            text width=3cm, text centered, minimum height=1cm, font=\small}]

        \node[box] (id) {1. Identification\\(Choose $p$, $q$)};
        \node[box, right=of id] (est) {2. Estimation\\(Fit parameters)};
        \node[box, right=of est] (diag) {3. Diagnostics\\(Check residuals)};
        \node[box, right=of diag] (fore) {4. Forecasting\\(Predict future)};

        \draw[->, thick, MainBlue] (id) -- (est);
        \draw[->, thick, MainBlue] (est) -- (diag);
        \draw[->, thick, MainBlue] (diag) -- (fore);

        \draw[->, thick, Crimson, dashed] (diag.south) -- ++(0,-0.7) -| (id.south)
            node[pos=0.25, below, font=\scriptsize] {If fails};
    \end{tikzpicture}
    \end{center}
    ...
    \begin{itemize}
        \item Check stationarity (ADF/KPSS tests); difference if needed
        \item Examine ACF and PACF plots
        \item Use information criteria (AIC, BIC) to compare models
    \end{itemize}
\end{frame}
