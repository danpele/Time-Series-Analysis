...
\begin{frame}{AR(1): Visual Illustration}
    ...
    {\footnotesize
    \begin{itemize}
        \item \textbf{Positive $\phi$ (e.g., 0.8)}: Persistent, smooth fluctuations
            \begin{itemize}
                \item Values tend to stay on same side of mean
                \item Gradual mean reversion --- ``trending'' appearance
            \end{itemize}
        \item \textbf{Negative $\phi$ (e.g., -0.8)}: Oscillating behavior
            \begin{itemize}
                \item Values alternate around the mean
                \item Rapid sign changes --- ``choppy'' appearance
            \end{itemize}
...
\begin{frame}{Invertibility: Visual Illustration}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.52\textheight, keepaspectratio]{ch2_def_invertibility.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
    \begin{itemize}
        \item \textbf{Left panel}: Unit circle test for invertibility
            \begin{itemize}
                \item Roots of $\theta(z) = 0$ must lie outside the circle
                \item For MA(1): requires $|\theta| < 1$
            \end{itemize}
        \item \textbf{Right panel}: AR($\infty$) representation weights
            \begin{itemize}
                \item Invertible: weights $\pi_j = (-\theta)^j$ decay to zero
                \item Non-invertible: weights explode or don't decay
            \end{itemize}
        \item \textbf{Practical importance}: Ensures unique model and valid forecasts
    \end{itemize}
    }
    \quantlet{TSA\_ch2\_ma1}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2_ma1}
\end{frame}
...
\begin{frame}{MA(q): Visual Illustration}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.52\textheight, keepaspectratio]{ch2_def_maq.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
    \begin{itemize}
