...
\begin{frame}{AR(1) Stationarity Condition}
    ...
    \textbf{Intuition:}
    \begin{itemize}
        \item If $|\phi| < 1$: shocks decay over time $\succ$ stationary
        \item If $|\phi| = 1$: random walk $\succ$ non-stationary (unit root)
        \item If $|\phi| > 1$: explosive process $\succ$ non-stationary
    \end{itemize}
...
\begin{frame}{Quiz: ACF/PACF Pattern Recognition -- Answer}
    \begin{exampleblock}{Correct Answer: (B) MA(1)}
        ACF cuts off $\succ$ MA process; PACF decays $\succ$ confirms MA(1)
    \end{exampleblock}
    \vspace{0.2cm}
    \begin{center}
...
\begin{frame}{Residual Analysis}
    ...
    \begin{itemize}
        ...
        \item No significant spikes $\succ$ white noise
    \end{itemize}
...
\begin{frame}{Ljung-Box Test}
    ...
    \textbf{Decision:}
    \begin{itemize}
        \item p-value $> 0.05$ $\succ$ fail to reject $H_0$ $\succ$ residuals look like white noise (good!)
        \item p-value $< 0.05$ $\succ$ significant autocorrelation remains $\succ$ model inadequate
    \end{itemize}
    \quantlet{TSA\_ch2\_diagnostics}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_diagnostics}
\end{frame}
