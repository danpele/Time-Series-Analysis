...
\begin{frame}{ACF Comparison: Short Memory vs Long Memory}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.65\textheight, keepaspectratio]{charts/ch8_acf_comparison.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
...
\begin{frame}{Effect of Parameter $d$ on ACF}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.65\textheight, keepaspectratio]{charts/ch8_arfima_d_effect.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
...
\begin{frame}{Hurst Exponent: Visual Interpretation}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.62\textheight, keepaspectratio]{charts/ch8_hurst_interpretation.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
...
\begin{frame}{ARIMA vs ARFIMA: Simulated Comparison}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.62\textheight, keepaspectratio]{charts/ch8_arima_vs_arfima.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
    \textbf{ARIMA} (left): ACF decays \textbf{exponentially} -- shocks are quickly ``forgotten''\\
    \textbf{ARFIMA} (right, $d=0.35$): ACF decays \textbf{hyperbolically} -- shocks persist for long periods
    }
\end{frame}

\begin{frame}{Real Data Example: EUR/RON Long Memory Analysis}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.62\textheight, keepaspectratio]{charts/ch8_eurron_long_memory.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
    \textbf{Returns} (left): $H \approx 0.50$, $d \approx 0$ -- \textbf{short memory} (efficient market)\\
    \textbf{Squared returns} (right): $H \approx 0.65$, $d \approx 0.15$ -- \textbf{long memory} in volatility
    }
\end{frame}
...
\begin{frame}{Estimating the Parameter $d$}
    ...
    {\footnotesize
    In Python: \texttt{arch} package, \texttt{statsmodels.tsa.arima.model.ARIMA} with \texttt{order=(p,d,q)} where $d$ can be fractional.
    }
\end{frame}
...
\begin{frame}{Real Example: Long Memory in Volatility}
    \begin{center}
        \includegraphics[width=0.90\textwidth, height=0.65\textheight, keepaspectratio]{charts/ch8_volatility_long_memory.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
...
\begin{frame}{Preparing Data for Random Forest}
    ...
    \begin{alertblock}{Warning: Data Leakage!}
        \begin{itemize}
            \item No use future information in features
...
\begin{frame}{Feature Engineering: Illustration}
    \begin{center}
        \includegraphics[width=0.90\textwidth, height=0.68\textheight, keepaspectratio]{charts/ch8_feature_engineering.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
...
\begin{frame}{Feature Importance and Interpretation}
    \begin{block}{Feature Importance}
        ...
        \begin{itemize}
            ...
            \item \textbf{Permutation Importance}: How much performance decays when the feature is randomly permuted
        \end{itemize}
    \end{block}
...
\begin{frame}{Random Forest: Forecast Example}
    \begin{center}
        \includegraphics[width=0.90\textwidth, height=0.65\textheight, keepaspectratio]{charts/ch8_rf_prediction.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
...
\begin{frame}{LSTM Cell Architecture}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.65\textheight, keepaspectratio]{charts/ch8_lstm_architecture.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
...
\begin{frame}{Preparing Data for LSTM}
...
\begin{frame}{Evaluation Metrics}
    ...
    \begin{alertblock}{Validation for Time Series}
        \begin{itemize}
            \item \textbf{No} use standard cross-validation!
...
\begin{frame}{Time Series Cross-Validation}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.55\textheight, keepaspectratio]{charts/ch8_timeseries_cv.pdf}
    \end{center}
    ...
    {\footnotesize
    \textbf{Important}: The training set grows progressively, and test is always in the future. This way we avoid data leakage.
    }
\end{frame}
...
\begin{frame}{Model Selection Guide}
    ...
    \begin{tikzpicture}[
        ...
        \draw[arrow] (start) -- node[above right] {No} (nonlin);
        \draw[arrow] (nonlin) -- node[above left] {No} (arfima);
        \draw[arrow] (nonlin) -- node[above right] {Yes} (interp);
        \draw[arrow] (interp) -- node[above left] {Yes} (rf);
        \draw[arrow] (interp) -- node[above right] {No} (lstm);
    \end{tikzpicture}
    \end{center}
    }
\end{frame}
...
\begin{frame}{Model Comparison: Accuracy vs Computational Cost}
    \begin{center}
        \includegraphics[width=0.82\textwidth, height=0.58\textheight, keepaspectratio]{charts/ch8_model_comparison.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
    \textbf{Trade-off}: ML models can have slightly better accuracy, but computational cost increases significantly. For small data or interpretability, ARIMA/ARFIMA remain excellent choices.
    }
\end{frame}
...
\begin{frame}[fragile]{Step 1: Loading and Visualizing Data}
    \begin{block}{Python Code -- Download Data}
        {\footnotesize
\begin{verbatim}
...
\begin{frame}{EUR/RON Series Visualization}
    \begin{center}
        \includegraphics[width=0.90\textwidth, height=0.65\textheight, keepaspectratio]{charts/ch8_case_raw_data.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
    \textbf{Top}: EUR/RON exchange rate -- we observe RON depreciation trend and high volatility periods.\\
    \textbf{Bottom}: Daily returns -- volatility clustering (high volatility periods are followed by similar periods).
    }
\end{frame}

\begin{frame}{ACF Analysis: Returns vs Squared Returns}
    \begin{center}
        \includegraphics[width=0.90\textwidth, height=0.65\textheight, keepaspectratio]{charts/ch8_case_acf_analysis.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
    \textbf{Left}: ACF of returns -- rapid decay, no significant autocorrelation beyond lag 1.\\
    \textbf{Right}: ACF of squared returns -- slower decay indicates \textbf{volatility clustering} (ARCH effects).
    }
\end{frame}
...
\begin{frame}{Long Memory Test Results -- EUR/RON}
    \begin{block}{Typical Output}
        ...
        \texttt{Hurst Exponent (H): 0.47}\\
        \texttt{Estimated d parameter: -0.03}\\
        \texttt{ANTI-PERSISTENT series (mean-reverting)}
        }
    \end{block}
    ...
    \begin{alertblock}{Interpretation}
        \begin{itemize}
            ...
            \item However, \textbf{volatility} can have long memory!
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}{Random Forest: Feature Importance}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.62\textheight, keepaspectratio]{charts/ch8_case_feature_importance.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
    \textbf{Insight}: Recent lags (lag\_1, lag\_2) and volatility rolling are the most important. Calendar features have minor impact for daily returns.
    }
\end{frame}
...
\begin{frame}{LSTM: Learning Curve}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.62\textheight, keepaspectratio]{charts/ch8_case_lstm_training.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
    \textbf{Training Loss}: Decreases quickly in the first epochs, then stabilizes.\\
    \textbf{Validation Loss}: Follows training loss -- no severe overfitting observed.
    }
\end{frame}
...
\begin{frame}{Comparison: Results on EUR/RON}
    ...
    \begin{alertblock}{Conclusions}
        \begin{itemize}
            \item \textbf{Random Forest} achieves the best accuracy (lowest RMSE/MAE)
            \item \textbf{LSTM} performs similarly but requires 10x more training time
            \item \textbf{ARIMA} provides a solid baseline with excellent interpretability
            \item Trade-off: ML models offer better accuracy at cost of complexity
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}{Visualization: Predictions vs Actual Values}
    \begin{center}
        \includegraphics[width=0.90\textwidth, height=0.65\textheight, keepaspectratio]{charts/ch8_case_predictions.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
    All models capture the general pattern, but none perfectly predicts volatility peaks. This reflects \textbf{market efficiency} and \textbf{prediction limits} for financial series.
    }
\end{frame}

\begin{frame}{Model Comparison: Performance Metrics}
    \begin{center}
        \includegraphics[width=0.90\textwidth, height=0.65\textheight, keepaspectratio]{charts/ch8_case_comparison.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
    \textbf{Left}: Error metrics (lower is better) -- Random Forest achieves lowest RMSE and MAE.\\
    \textbf{Right}: Training time (log scale) -- LSTM requires significantly more computational resources.
    }
\end{frame}
...
\begin{frame}{Example 3: Inflation Rate in Romania}
    \begin{block}{Characteristics}
        \begin{itemize}
            ...
            \item \textbf{High persistence} -- shocks persist
...
\begin{frame}{Practical Summary: Model Selection}
    ...
    \begin{tabular}{l|c|c|c|c}
        ...
        Interpretabil & \textbf{Yes} & \textbf{Yes} & Partial & No \\
...
\begin{frame}{Quiz Fast}
    \begin{enumerate}
        ...
        \item What does ``data leakage'' in the context of ML for time series?
    \end{enumerate}
\end{frame}
