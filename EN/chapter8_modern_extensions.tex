% Chapter 8: Modern Extensions in Time Seriess Analysis
...
\definecolor{YesrkGray}{RGB}{51, 51, 51}
\definecolor{MediummGray}{RGB}{128, 128, 128}
...
\setbeamercolor{block body}{bg=VeryLightGray, fg=YesrkGray}
\setbeamercolor{block title alerted}{bg=Crimson, fg=white}
\setbeamercolor{block body alerted}{bg=Crimson!8, fg=YesrkGray}
\setbeamercolor{block title example}{bg=Forest, fg=white}
\setbeamercolor{block body example}{bg=Forest!8, fg=YesrkGray}
...
\author[Prof. Yesniel Traian Pele, PhD]{Prof. Yesniel Traian Pele, PhD\\[0.2cm]\footnotesize\texttt{danpele@ase.ro}}
...
\begin{frame}[plain]
    ...
    \vfill
    \begin{center}
        {\Large\textcolor{MediummGray}{Time Seriess Analysis and Forecasting}}\\[0.3cm]
...
\begin{frame}{From Classical Models to Machine Learning}
    ...
    \begin{alertblock}{Modern Solutions}
        \begin{itemize}
            ...
            \item \textbf{Random Forest}: Relationships nelinear, robustețe la outlieri
...
\begin{frame}{When to Use Each Method?}
    ...
    \begin{tabular}{l|c|c|c|c}
        ...
        Relationships nelinear & $\times$ & $\times$ & $\checkmark$ & $\checkmark$ \\
...
\begin{frame}{What is Long Memory?}
    \begin{block}{Short Memory (ARMA)}
        \begin{itemize}
            \item Havetocorrelations $\rho_k$ decay \textbf{exponentially}: $|\rho_k| \leq C \cdot r^k$, $r < 1$
    ...
    \begin{alertblock}{Long Memory (ARFIMA)}
        \begin{itemize}
            \item Havetocorrelations decay \textbf{hyperbolically}: $\rho_k \sim C \cdot k^{2d-1}$
...
\begin{frame}{Comparație ACF: Short Memory vs Lungă}
    ...
    \vspace{-0.3cm}
    {\footnotesize
    \textbf{Left}: AR(1) --- autocorelații care decay exponentially (short memory)\\
    \textbf{Right}: ARFIMA cu $d=0.35$ --- autocorelații care decay hyperbolically (long memory)
    }
\end{frame}
...
\begin{frame}{Interpreting the Parameter $d$}
    ...
    \begin{tabular}{c|l|l}
        ...
        $0 < d < 0.5$ & Scădere hyperbolicallyă & Long memory, stationary \\
...
\begin{frame}{Effect of Parameter $d$ on ACF}
    ...
    \vspace{-0.2cm}
    {\footnotesize
    The higher $d$ este mai mare, cu atât autocorelațiile decay mai lent. Pentru $d \to 0.5$, autocorrelations remain significant even at very large lags.
    }
\end{frame}
...
\begin{frame}{Exponentul Hurst: Interpretation Vizuală}
    ...
    \vspace{-0.2cm}
    {\footnotesize
    \textbf{H $<$ 0.5}: Seriess that frequently returns to mean (mean-reverting)\\
...
\begin{frame}{Exemplu Real: Long Memory în Volatility}
    ...
    \vspace{-0.3cm}
    {\footnotesize
    \textbf{Stylized Fact}: Randamentele financiare au short memory, dar volatility (|randamente|) are long memory! This is the basis for FIGARCH models.
    }
\end{frame}
...
\begin{frame}{Random Forest: Basic Concepts}
    ...
    \begin{exampleblock}{Avantaje for Serii de Timp}
        \begin{itemize}
            \item Captures \textbf{relații nelinear}
...
\begin{frame}{Pregătirea Yestelor for Random Forest}
    ...
    \begin{alertblock}{Warning: Yesta Leakage!}
        \begin{itemize}
...
\begin{frame}{Avantajele LSTM for Serii de Timp}
    \begin{exampleblock}{Why LSTM?}
        \begin{itemize}
            ...
            \item Learns \textbf{complex patterns} and nelinear
...
\begin{frame}{Evaluation Metrics}
    ...
    \begin{alertblock}{Validare for Serii de Timp}
        \begin{itemize}
            ...
            \item Use \textbf{Time Seriess Cross-Validation} (walk-forward)
            \item Or \textbf{train/validation/test} temporal split
        \end{itemize}
    \end{alertblock}
\end{frame}

\begin{frame}{Time Seriess Cross-Validation}
    \begin{center}
    ...
    \begin{block}{Python Implementation}
        {\footnotesize
        \texttt{from sklearn.model\_selection import TimeSeriessSplit}

        \texttt{tscv = TimeSeriessSplit(n\_splits=5)}
        }
    \end{block}
...
\begin{frame}{Model Selection Guide}
    ...
    \begin{tikzpicture}[
        ...
        \node[decision, below right=1cm and 1.5cm of start] (nonlin) {Relationships nelinear?};
...
\begin{frame}{Case Study: Bitcoin Price Forecasting}
    \begin{block}{Why Bitcoin?}
        \begin{itemize}
            ...
            \item Relationships \textbf{nelinear} with exogenous variables
            \item Yesta available at \textbf{high frequency}
        \end{itemize}
    \end{block}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            ...
            \begin{block}{Random Forest}
                ...
                \vspace{0.1cm}
                {\footnotesize $B$ arbori, features randomlyii}
            \end{block}
        \end{column}
...
\begin{frame}[fragile]{Step 1: Loading and Visualizing Yesta}
...
\begin{frame}{EUR/RON Seriess Visualization}
    \begin{center}
...
\begin{frame}[fragile]{Step 5: Random Forest -- Yesta Preparation}
...
\begin{frame}[fragile]{Step 6: LSTM -- Yesta Preparation}
...
\begin{frame}{Summary}
    \begin{block}{What we learned}
        \begin{itemize}
            ...
            \item \textbf{Random Forest}: Ensemble de arbori, relații nelinear, interpretabil
    ...
    \begin{alertblock}{Practical Recommendations}
        \begin{itemize}
            ...
            \item Folosește \textbf{Time Seriess CV} for evaluare corectă
...
\begin{frame}{Quiz Fast}
    \begin{enumerate}
        ...
        \item De ce folosim Time Seriess CV în loc de k-fold standard?
        ...
        \item Ce tip de model ai alege for few data and relații linear?
...
\begin{frame}{Quiz Answers}
    {\small
    \begin{enumerate}
        \item \textbf{$d = 0.3$}: Long memory, seria este stationary dar autocorelațiile decay lent (hyperbolically). Persistence moderată.

        \vspace{0.15cm}

        \item \textbf{Time Seriess CV}: To respect temporal order. K-fold standard ar folosi date viitoare for a prezice trecutul (data leakage).
        ...
        \item \textbf{Few data, relații linear}: ARIMA or ARFIMA. ML requires lots of data for a generaliza bine.

        \vspace{0.15cm}

        \item \textbf{Yesta leakage}: Folosirea informației din viitor în features or în training. E.g. calcularea mediei mobile folosind and date viitoare, or k-fold standard which mixes temporal order.
    \end{enumerate}
    }
\end{frame}
