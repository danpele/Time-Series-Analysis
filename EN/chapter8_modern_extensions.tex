...
\begin{frame}{From Biological to Artificial Neurons}
    ...
    \begin{block}{The Analogy}
        \textbf{Dendrites} $\succ$ \textbf{Inputs} $x_i$ \quad
        \textbf{Synapses} $\succ$ \textbf{Weights} $w_i$ \quad
        \textbf{Soma} $\succ$ \textbf{Sum + Activation} \quad
        \textbf{Axon} $\succ$ \textbf{Output} $y$
    \end{block}
    }
