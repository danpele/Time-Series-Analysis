...
\begin{frame}{Bitcoin: Evidence for GARCH}
    ...
    \begin{alertblock}{Why GARCH?}
        Squared returns (volatility proxy) show \textbf{significant autocorrelation}. This means past volatility predicts future volatility---exactly what GARCH models!
    \end{alertblock}
\end{frame}
...
\begin{frame}{GARCH: Multi-Step Forecasts Converge}
    ...
    \begin{alertblock}{Problem}
        Multi-step GARCH forecasts converge to $\bar{\sigma}^2 = \omega/(1-\alpha-\beta)$. For long horizons, this gives a \textbf{flat line}---useless for dynamic risk management!
    \end{alertblock}
\end{frame}
...
\begin{frame}{Bitcoin: GARCH Volatility Forecast (Test Set)}
    ...
    \begin{exampleblock}{Result}
        Rolling one-step-ahead GARCH(1,1) forecasts capture the \textbf{dynamic volatility patterns}. The forecast (red line) tracks the realized volatility (blue area), demonstrating the predictability of variance.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Sunspots: The 11-Year Solar Cycle}
    ...
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{Data}
            Period: 1900--2008, 109 years. ACF confirms 11-year periodicity.
        \end{block}

        \column{0.5\textwidth}
        \begin{alertblock}{Challenge}
            SARIMA with $s=11$ needs too many parameters. \textbf{Solution}: Fourier terms.
        \end{alertblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Fourier Terms for Seasonality}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Advantages}
            Only $2K$ parameters (not $s$). $K$ controls flexibility.
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Unemployment: COVID-19 Structural Break}
    \begin{center}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Key Statistics}
            \begin{itemize}
                \item Pre-COVID min: 3.5\%
                \item COVID peak: \textbf{14.8\%}
        ...
        \begin{exampleblock}{Solution}
            \textbf{Prophet} with automatic changepoint detection adapts to structural breaks.
        \end{exampleblock}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Economic Relationships}
            \begin{itemize}
                ...
                \item \textbf{Phillips Curve}: Unemp $\leftrightarrow$ Inflation
        ...
        \begin{alertblock}{Why VAR?}
            Each variable may be \textbf{both cause and effect} of others. Gray: 2008 crisis. Red: COVID.
        \end{alertblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Granger Causality Analysis}
    \begin{columns}[T]
        ...
        \begin{block}{Definition}
            $X$ \textbf{Granger-causes} $Y$ if past values of $X$ help predict $Y$.
        ...
        \begin{exampleblock}{Key Findings}
            \begin{itemize}
                \item Unemployment $\rightarrow$ GDP ($p=0.045$)
                \item Fed Rate $\rightarrow$ Inflation ($p=0.087$)
            \end{itemize}
            Consistent with Okun's Law and Taylor Rule.
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Impulse Response Functions}
    ...
    \begin{exampleblock}{Economic Interpretation}
        GDP shock $\rightarrow$ Unemployment $\downarrow$ (Okun's Law), Inflation $\uparrow$ (demand-pull), Fed Rate $\uparrow$ (Taylor Rule).
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Model Selection Framework}
    ...
    \begin{tikzpicture}[scale=0.8, transform shape,
        ...
        \node[decision, below left=1.2cm and 1.5cm of q1] (q2) {Structural\\breaks?};
        \node[decision, below right=1.2cm and 1.5cm of q1] (q3) {Financial\\data?};
        ...
        \draw[arrow] (start) -- (q1);
        \draw[arrow] (q1) -- node[right] {\small Yes} (var);
        \draw[arrow] (q1) -- node[above left] {\small No} (q2);
        \draw[arrow] (q2) -- node[left] {\small Yes} (prophet);
        \draw[arrow] (q2) -- node[above right] {\small No} (q3);
        \draw[arrow] (q3) -- node[left] {\small Vol.} (garch);
        \draw[arrow] (q3) -- node[right] {\small Level} (arima);
    \end{tikzpicture}
    \end{center}
\end{frame}
...
\begin{frame}{Summary: Model Comparison}
    \begin{columns}[T]
        ...
        \begin{tabular}{llll}
            ...
            \midrule
            Bitcoin & Volatility & GARCH & 2.21 \\
            Sunspots & Seasonality & Fourier & 48.51 \\
