...
\begin{frame}{Quiz 1: Answer}
    ...
    \begin{exampleblock}{Answer: A -- 2 coefficient matrices}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{VAR($p$) model}: $\bY_t = \bc + \bA_1\bY_{t-1} + \bA_2\bY_{t-2} + \cdots + \bA_p\bY_{t-p} + \bvarepsilon_t$
            \item \textbf{VAR(2) with $K=3$}:
        \end{itemize}
        \[
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Key}: $p$ = number of lags = number of matrices
        \end{itemize}
    \end{exampleblock}
...
\begin{frame}{Quiz 2: Answer}
    ...
    \begin{exampleblock}{Answer: C -- 7 parameters per equation}
        ...
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Formula}: Per equation = $1 + K \times p = 1 + 3 \times 2 = 7$. \textbf{Total}: $K(1 + Kp) = 3(1 + 6) = 21$ parameters
        \end{itemize}
        }
    \end{exampleblock}
...
\begin{frame}{Quiz 3: Answer}
    ...
    \begin{exampleblock}{Answer: B -- Past $X$ helps predict future $Y$}
        ...
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Key}: Predictive relationship, NOT true causation!
        \end{itemize}
        }
    \end{exampleblock}
...
\begin{frame}{Quiz 4: Answer}
    ...
    \begin{exampleblock}{Answer: B -- Coefficients on lagged $Y_2$ in $Y_1$ equation = 0}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Null hypothesis}: $H_0: a_{12}^{(1)} = a_{12}^{(2)} = \cdots = a_{12}^{(p)} = 0$
            \item \textbf{Test statistic}: Wald or F-test with $p$ restrictions
            \item \textbf{Interpretation}:
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Reject $H_0$: $Y_2$ Granger-causes $Y_1$
                \item Don't reject: No evidence of predictive relationship
            \end{itemize}
            \item \textbf{Note}: Test $Y_1 \to Y_2$ separately (different coefficients in $Y_2$ equation)
        \end{itemize}
    \end{exampleblock}
...
\begin{frame}{Quiz 5: Answer}
    ...
    \begin{exampleblock}{Answer: C -- All eigenvalues of $\bA_1$ inside unit circle}
        ...
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Stable}: All $|\lambda_i| < 1$ (inside unit circle) $\Rightarrow$ shocks die out over time
        \end{itemize}
        }
    \end{exampleblock}
...
\begin{frame}{Quiz 6: Answer}
    ...
    \begin{exampleblock}{Answer: B -- Effect of shock on all variables over time}
        ...
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{IRF}$_{ij}(h)$: Response of variable $i$ at horizon $h$ to shock in variable $j$
        \end{itemize}
        }
    \end{exampleblock}
...
\begin{frame}{Quiz 7: Answer}
    ...
    \begin{exampleblock}{Answer: B -- BIC (Bayesian Information Criterion)}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Penalty comparison} (for $k$ parameters, $n$ observations):
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item AIC: $-2\ln L + 2k$
                \item BIC: $-2\ln L + k\ln n$
            \end{itemize}
            \item Since $\ln n > 2$ for $n > 8$, BIC penalizes complexity more heavily
...
\begin{frame}{Quiz 9: Answer}
    ...
    \begin{exampleblock}{Answer: B -- Proportion of forecast error variance from each shock}
        ...
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{FEVD}: Shows how much forecast uncertainty comes from each shock at different horizons
        \end{itemize}
        }
    \end{exampleblock}
...
\begin{frame}{Quiz 11: Answer}
    ...
    \begin{exampleblock}{Answer: B -- Variables ordered first affect later ones contemporaneously}
        ...
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Cholesky}: Recursive structure. Ordering matters -- justify by economic theory (most exogenous first)!
        \end{itemize}
        }
    \end{exampleblock}
...
\begin{frame}{Quiz 12: Answer}
    ...
    \begin{exampleblock}{Answer: B -- White noise (no autocorrelation)}
        ...
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Diagnostics}: Residuals should be white noise. Use Portmanteau/LM test. Cross-equation correlation allowed ($\Sigma_u$).
        \end{itemize}
        }
    \end{exampleblock}
...
\begin{frame}{Exercise 2: Parameter Count}
    ...
    \begin{exampleblock}{Solution}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Per equation: $1 + K \times p = 1 + 4 \times 3 = 13$ parameters
            \item Total for $K=4$ equations: $4 \times 13 = \mathbf{52}$ parameters
            \item Plus covariance matrix $\boldsymbol{\Sigma}$: $K(K+1)/2 = 4 \times 5 / 2 = 10$ unique elements
            \item \textbf{Grand total: 62 parameters}
            \item \textit{This is why VARs can be ``over-parameterized'' with limited data!}
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Exercise 3: Granger Causality Interpretation}
    ...
    \begin{exampleblock}{Solution}
        \begin{itemize}\setlength{\itemsep}{2pt}
            ...
            \item \textbf{Conclusion}: Unidirectional causality: Money $\succ$ GDP
            \item \textit{Interpretation}: Past money supply helps predict GDP growth. This is consistent with monetarist views, but remember: Granger causality $\neq$ structural causality!
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
    \quantlet{TSA\_ch6\_case\_granger}{https://github.com/QuantLet/TSA/tree/main/TSA_ch6/TSA_ch6_case_granger}
...
\begin{frame}{Exercise 4: Stability Check}
    ...
    \begin{exampleblock}{Solution}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Find eigenvalues: $\det(\bA_1 - \lambda \mathbf{I}) = 0$
            ...
            \item Both $|\lambda_i| < 1$ $\Rightarrow$ \textbf{Stable!}
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Exercise 5: IRF Computation}
    ...
    \begin{exampleblock}{Solution}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Interpretation}: A unit shock to $Y_2$ at $t$ increases $Y_1$ by 0.22 at $t+2$.
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
    \quantlet{TSA\_ch6\_irf}{https://github.com/QuantLet/TSA/tree/main/TSA_ch6/TSA_ch6_irf}
...
\begin{frame}{Discussion: Granger Causality vs True Causality}
    ...
    \begin{exampleblock}{Conclusion}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Granger causality is about \textbf{prediction}, not \textbf{mechanism}. For structural causality, need theory + identification strategy.
        \end{itemize}
    \end{exampleblock}
    }
    \end{cminipage}
...
\begin{frame}{Discussion: Variable Ordering in IRFs}
    ...
    \begin{exampleblock}{Example: Monetary Policy VAR Ordering}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item 1. Oil prices (exogenous) $\to$ 2. GDP (slow) $\to$ 3. Inflation $\to$ 4. Interest rates (fast)
        \end{itemize}
    \end{exampleblock}
    \begin{block}{Rule}
