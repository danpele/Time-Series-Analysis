...
\begin{frame}{Exercise 1: Autocovariance}
    ...
    \begin{exampleblock}{Solution}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{a) Autocorrelations:} $\rho(h) = \gamma(h)/\gamma(0)$: $\rho(0) = 1$, \quad $\rho(1) = 2/4 = 0.5$, \quad $\rho(2) = 1/4 = 0.25$
            \item \textbf{b)} $\Cov(X_t, X_{t-1}) = \gamma(1) = 2$ \quad (by stationarity, lag 1 covariance)
            \item \textbf{c)} $\Corr(X_5, X_7) = \rho(|7-5|) = \rho(2) = 0.25$
            \item \textbf{d)} For AR(1) with $\phi = \rho(1) = 0.5$:
...
\begin{frame}{Exercise 2: Random Walk Properties}
    ...
    \begin{exampleblock}{Solution}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $X_t = X_0 + \sum_{i=1}^{t} \varepsilon_i$ with $\sigma^2 = 4$
