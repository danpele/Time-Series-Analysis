...
\begin{frame}[fragile]{Python Exercise 1: Import and Visualization\quantlet{TSA\_ch1\_ts\_basics}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_ts_basics}}
...
\begin{frame}[fragile]{Python Exercise 4: Stationarity Testing\quantlet{TSA\_ch1\_unit\_root\_tests}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_unit_root_tests}}
...
\begin{frame}{Case Study: S\&P 500 Index\quantlet{TSA\_ch1\_ts\_basics}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_ts_basics}}
    \begin{columns}[T]
...
\begin{frame}{Time Series Decomposition: Real Example\quantlet{TSA\_ch1\_case\_gdp}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_case_gdp}}
    \begin{columns}[T]
        \column{0.35\textwidth}
        \begin{block}{Observations}
        {\small
...
\begin{frame}{Stationarity Testing: ADF Results\quantlet{TSA\_ch1\_unit\_root\_tests}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_unit_root_tests}}
    \begin{columns}[T]
        \column{0.35\textwidth}
        \begin{block}{Observations}
        {\small
...
\begin{frame}{AI Exercise 1: Stationarity Report --- Find the Errors}
    \begin{block}{AI Analysis of BTC/USD Daily Prices (2020--2024, $T = 1{,}461$)}
        {\scriptsize
        ``ADF on log-prices: stat $= -1.87$, $p = 0.34$ $\to$ non-stationary. \\
        After first diff: ADF $= -12.4$, $p < 0.001$ $\to$ stationary.
        After second diff: ADF $= -28.9$ $\to$ even more stationary. \\
        KPSS on returns: stat $= 0.41$, $p = 0.07$ $\to$ rejects stationarity. \\
        Conclusion: log-returns are i.i.d.\ white noise; no structure to model.''
        }
    \end{block}
    \vspace{-1mm}
    {\small
    ...
    \begin{enumerate}\setlength{\itemsep}{2pt}
        \item Identify at least five methodological or interpretation errors in the analysis above.
        \item For each error, explain why it is wrong and how it should be corrected.
        \item Write a correct stationarity report for the same data.
    \end{enumerate}
    }
\end{frame}

\begin{frame}{AI Exercise 2: The Random Walk Trap}
    \begin{block}{AI Model for EUR/USD Daily Returns ($T = 2{,}500$)}
        {\scriptsize
        ``Fitted ARMA(3,2): $\hat\phi_1 = 0.021$ ($p = 0.04$), $\hat\phi_2 = -0.018$ ($p = 0.03$),
        $\hat\phi_3 = 0.009$ ($p = 0.08$).\\
        $R^2 = 0.0012$. Ljung-Box(20): $p = 0.18$. AIC $= -14{,}892$.\\
        The model captures statistically significant linear patterns in FX returns.''
        }
    \end{block}
    \vspace{-1mm}
    {\small
    \textbf{Tasks}:
    \begin{enumerate}\setlength{\itemsep}{2pt}
        \item Evaluate the explanatory power of the model. Is $R^2 = 0.12\%$ practically relevant?
        \item Analyze the statistical significance of the coefficients accounting for multiple testing.
        \item Discuss whether this model would be profitable in practice, considering transaction costs.
        \item What does the Efficient Market Hypothesis imply for the existence of such patterns?
    \end{enumerate}
    }
\end{frame}
