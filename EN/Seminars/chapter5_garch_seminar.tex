...
\begin{frame}{Quiz 11: Component GARCH (CGARCH)}
    \begin{cminipage}{0.95\textwidth}
    \begin{alertblock}{Question}
        In the Component GARCH (CGARCH) model, what does the \textbf{permanent component} $q_t$ represent?
    \end{alertblock}

    \vspace{0.4cm}

    \begin{block}{Answer choices}
        \textcolor{MainBlue}{\textbf{(A)}} Short-run reaction to news shocks\\[3pt]
        \textcolor{MainBlue}{\textbf{(B)}} The long-run (time-varying) level of variance to which volatility mean-reverts\\[3pt]
        \textcolor{MainBlue}{\textbf{(C)}} The leverage effect coefficient\\[3pt]
        \textcolor{MainBlue}{\textbf{(D)}} The unconditional kurtosis of returns
    \end{block}
...
\begin{frame}{Quiz 11: Answer}
    \begin{cminipage}{0.95\textwidth}
    {\small
    \begin{exampleblock}{Answer: B -- The long-run (time-varying) level of variance to which volatility mean-reverts}

    \vspace{0.1cm}

    \begin{block}{Answer choices}
        \textcolor{MainBlue}{\textbf{(A)}} Short-run reaction to news shocks \incorrect\\[2pt]
        \textcolor{MainBlue}{\textbf{(B)}} \textbf{\textcolor{Forest}{The long-run (time-varying) level of variance to which volatility mean-reverts}} \correct\\[2pt]
        \textcolor{MainBlue}{\textbf{(C)}} The leverage effect coefficient \incorrect\\[2pt]
        \textcolor{MainBlue}{\textbf{(D)}} The unconditional kurtosis of returns \incorrect
    \end{block}

        \vspace{0.1cm}

        \begin{itemize}\setlength{\itemsep}{0pt}
            \item CGARCH decomposes variance: $\sigma_t^2 = q_t + \alpha(\varepsilon_{t-1}^2 - q_{t-1}) + \beta(\sigma_{t-1}^2 - q_{t-1})$
            \item $q_t$ = \textbf{permanent component} (slow-moving trend in volatility)
            \item $(\sigma_t^2 - q_t)$ = \textbf{transitory component} (short-run deviations)
            \item In standard GARCH, $q_t = \bar{\sigma}^2$ is constant; CGARCH allows it to evolve
        \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{Quiz 12: APARCH Nesting Property}
    \begin{cminipage}{0.95\textwidth}
    \begin{alertblock}{Question}
        The Asymmetric Power ARCH (APARCH) model nests many GARCH variants. Which of the following is \textbf{NOT} a special case of APARCH?
    \end{alertblock}

    \vspace{0.4cm}

    \begin{block}{Answer choices}
        \textcolor{MainBlue}{\textbf{(A)}} GARCH(1,1) when $\delta = 2$ and $\gamma_i = 0$\\[3pt]
        \textcolor{MainBlue}{\textbf{(B)}} TGARCH(1,1) when $\delta = 1$\\[3pt]
        \textcolor{MainBlue}{\textbf{(C)}} EGARCH(1,1)\\[3pt]
        \textcolor{MainBlue}{\textbf{(D)}} GJR-GARCH(1,1) when $\delta = 2$
    \end{block}
...
\begin{frame}{Quiz 12: Answer}
    \begin{cminipage}{0.95\textwidth}
    {\small
    \begin{exampleblock}{Answer: C -- EGARCH(1,1)}

    \vspace{0.1cm}

    \begin{block}{Answer choices}
        \textcolor{MainBlue}{\textbf{(A)}} GARCH(1,1) when $\delta = 2$ and $\gamma_i = 0$ \incorrect\\[2pt]
        \textcolor{MainBlue}{\textbf{(B)}} TGARCH(1,1) when $\delta = 1$ \incorrect\\[2pt]
        \textcolor{MainBlue}{\textbf{(C)}} \textbf{\textcolor{Forest}{EGARCH(1,1)}} \correct\\[2pt]
        \textcolor{MainBlue}{\textbf{(D)}} GJR-GARCH(1,1) when $\delta = 2$ \incorrect
    \end{block}

        \vspace{0.1cm}

        \begin{itemize}\setlength{\itemsep}{0pt}
            \item APARCH: $\sigma_t^\delta = \omega + \sum_{i=1}^p \alpha_i(|\varepsilon_{t-i}| - \gamma_i \varepsilon_{t-i})^\delta + \sum_{j=1}^q \beta_j \sigma_{t-j}^\delta$
            \item $\delta = 2$, $\gamma_i = 0$ $\Rightarrow$ \textbf{GARCH}; \; $\delta = 1$ $\Rightarrow$ \textbf{TGARCH}
            \item $\delta = 2$, $\gamma_i \neq 0$ $\Rightarrow$ \textbf{GJR-GARCH}
            \item \textbf{EGARCH} models $\ln(\sigma_t^2)$ --- a fundamentally different structure, not nested
        \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{Quiz 13: FIGARCH and Long Memory}
    \begin{cminipage}{0.95\textwidth}
    \begin{alertblock}{Question}
        What does the fractional differencing parameter $d$ in FIGARCH control?
    \end{alertblock}

    \vspace{0.4cm}

    \begin{block}{Answer choices}
        \textcolor{MainBlue}{\textbf{(A)}} The asymmetry between positive and negative shocks\\[3pt]
        \textcolor{MainBlue}{\textbf{(B)}} The rate of decay of the autocorrelation of squared returns\\[3pt]
        \textcolor{MainBlue}{\textbf{(C)}} The number of ARCH lags\\[3pt]
        \textcolor{MainBlue}{\textbf{(D)}} The degrees of freedom of the innovation distribution
    \end{block}
...
\begin{frame}{Quiz 13: Answer}
    \begin{cminipage}{0.95\textwidth}
    {\small
    \begin{exampleblock}{Answer: B -- The rate of decay of the autocorrelation of squared returns}

    \vspace{0.1cm}

    \begin{block}{Answer choices}
        \textcolor{MainBlue}{\textbf{(A)}} The asymmetry between positive and negative shocks \incorrect\\[2pt]
        \textcolor{MainBlue}{\textbf{(B)}} \textbf{\textcolor{Forest}{The rate of decay of the autocorrelation of squared returns}} \correct\\[2pt]
        \textcolor{MainBlue}{\textbf{(C)}} The number of ARCH lags \incorrect\\[2pt]
        \textcolor{MainBlue}{\textbf{(D)}} The degrees of freedom of the innovation distribution \incorrect
    \end{block}

        \vspace{0.1cm}

        \begin{itemize}\setlength{\itemsep}{0pt}
            \item FIGARCH: $(1 - \beta L)(1 - L)^d \varepsilon_t^2 = \omega + (1 - \beta L - \phi L)(1 - L)^d \varepsilon_t^2$
            \item $d = 0$ $\Rightarrow$ \textbf{GARCH} (short memory); \; $d = 1$ $\Rightarrow$ \textbf{IGARCH} (unit root)
            \item $0 < d < 1$ $\Rightarrow$ \textbf{long memory}: autocorrelations of $\varepsilon_t^2$ decay hyperbolically
            \item Empirically, $d \approx 0.3$--$0.5$ for most financial series
        \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{True or False? --- Questions}
    ...
    \begin{tabular}{p{9cm}c}
        ...
        6. Value at Risk (VaR) can be calculated using GARCH volatility forecasts. & ? \\[0.15cm]
        7. EWMA volatility forecasts mean-revert to a long-run level. & ? \\[0.15cm]
        8. Markov-switching GARCH can explain spurious IGARCH effects. & ? \\[0.15cm]
        9. GARCH-M allows volatility to directly affect expected returns. & ? \\
        \bottomrule
    \end{tabular}
    \end{center}
    \end{cminipage}
...
\begin{frame}{True or False? --- Answers}
    ...
    \begin{tabular}{p{7.5cm}cc}
        ...
        6. Value at Risk (VaR) can be calculated using GARCH volatility forecasts. & \textcolor{Forest}{\textbf{T}} & {\tiny $\text{VaR}_\alpha = z_\alpha \cdot \sigma_{t+1}$} \\[0.08cm]
        7. EWMA volatility forecasts mean-revert to a long-run level. & \textcolor{Crimson}{\textbf{F}} & {\tiny EWMA = IGARCH ($\omega=0$), no mean-reversion} \\[0.08cm]
        8. Markov-switching GARCH can explain spurious IGARCH effects. & \textcolor{Forest}{\textbf{T}} & {\tiny Breaks $\Rightarrow$ upward-biased $\alpha+\beta$} \\[0.08cm]
        9. GARCH-M allows volatility to directly affect expected returns. & \textcolor{Forest}{\textbf{T}} & {\tiny $r_t = \mu + \delta \cdot g(\sigma_t^2) + \varepsilon_t$} \\
        \bottomrule
    \end{tabular}
    \end{center}
    \end{cminipage}
...
\begin{frame}{Exercise 5: IGARCH and EWMA Persistence}
    \begin{cminipage}{0.95\textwidth}
    {\footnotesize
    \begin{alertblock}{Problem}
        An analyst uses EWMA with $\lambda = 0.94$ to forecast volatility. At time $T$:
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Current variance estimate: $\sigma_T^2 = 0.0004$ (i.e., $\sigma_T = 2\%$)
            \item Today's return: $r_T = -0.05$ (5\% drop)
        \end{itemize}
        Calculate: (a) $\sigma_{T+1}^2$ under EWMA; (b) The equivalent IGARCH parameters; (c) Compare with GARCH(1,1) where $\omega = 0.000002$, $\alpha = 0.06$, $\beta = 0.93$; (d) Which model produces a higher long-horizon forecast and why?
    \end{alertblock}

    \vspace{0.1cm}
    \begin{exampleblock}{Solution}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{(a)} $\sigma_{T+1}^2 = 0.94 \times 0.0004 + 0.06 \times (-0.05)^2 = 0.000376 + 0.000150 = 0.000526$; \ Vol: $2.29\%$
            \item \textbf{(b)} EWMA = IGARCH with $\omega = 0$, $\alpha = 1 - \lambda = 0.06$, $\beta = \lambda = 0.94$, \ $\alpha + \beta = 1$
            \item \textbf{(c)} GARCH: $\sigma_{T+1}^2 = 0.000002 + 0.06 \times 0.0025 + 0.93 \times 0.0004 = 0.000524$; \ Vol: $2.29\%$
            \item \textbf{(d)} EWMA: forecast stays at $\sigma_{T+1}^2$ forever (no mean-reversion). GARCH: converges to $\bar{\sigma}^2 = \frac{0.000002}{0.01} = 0.0002$ ($1.41\%$). EWMA gives \textbf{higher} long-horizon forecasts after a large shock.
        \end{itemize}
    \end{exampleblock}
    }
    \end{cminipage}
...
\begin{frame}{Summary: Chapter 5}
    ...
    \begin{exampleblock}{Key Concepts}
        \begin{itemize}\setlength{\itemsep}{2pt}
            \item[\textcolor{MainBlue}{\textbf{1.}}] \textbf{ARCH/GARCH}: conditional variance depends on past shocks and lagged variance
            \item[\textcolor{MainBlue}{\textbf{2.}}] \textbf{IGARCH/EWMA}: unit root in variance ($\alpha + \beta = 1$); no mean-reversion
            \item[\textcolor{MainBlue}{\textbf{3.}}] \textbf{EGARCH/GJR/TGARCH}: capture leverage effect (asymmetry)
            \item[\textcolor{MainBlue}{\textbf{4.}}] \textbf{CGARCH/FIGARCH}: permanent vs transitory volatility; long memory
            \item[\textcolor{MainBlue}{\textbf{5.}}] \textbf{APARCH}: nests GARCH, TGARCH, GJR-GARCH as special cases
            \item[\textcolor{MainBlue}{\textbf{6.}}] \textbf{Key formulas}: $\bar{\sigma}^2 = \frac{\omega}{1 - \alpha - \beta}$, $HL = \frac{\ln(0.5)}{\ln(\alpha + \beta)}$, $\text{VaR}_\alpha = z_\alpha \cdot \sigma \cdot V$
        \end{itemize}
    \end{exampleblock}
