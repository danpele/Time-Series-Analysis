...
\begin{frame}{AI Exercise 1: Decomposition Audit}
    \begin{block}{AI Output (monthly US retail sales, 2015--2023)}
        {\small
        \textbf{Prompt}: ``I have a file with monthly US retail sales from 2015 to 2023. What trends and patterns do you see?''\\[2pt]
        \textbf{AI}: ``Applied additive STL, period=12. Trend captures growth from \$420B to \$580B. Seasonal component is stable across the sample.''
        }
    \end{block}
    \vspace{-1mm}
    {\small
    \textbf{The data contradicts the AI}:
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item Dec--Jan seasonal swing: $\pm\$18$B in 2015, $\pm\$45$B in 2023
        \item Residual std: \$3.2B (2015--2017) vs \$8.7B (2021--2023)
    \end{itemize}

    \vspace{1mm}
    \textbf{Tasks}:
    \begin{enumerate}\setlength{\itemsep}{1pt}
        \item Compute $\frac{\text{seasonal amplitude}}{\text{level}}$ for 2015 vs 2023. If the ratio is approximately constant, what decomposition does this imply?
        \item If $X_t = T_t \cdot S_t \cdot \varepsilon_t$, show that $\ln X_t = \underbrace{\ln T_t}_{\text{trend}} + \underbrace{\ln S_t}_{\text{seasonal}} + \underbrace{\ln \varepsilon_t}_{\text{residual}}$ is additive.
        \item Apply STL to $\ln X_t$. How would you verify constant residual variance? (Hint: Breusch--Pagan or plot $|\hat\varepsilon_t|$ vs $t$.)
        \item What are the consequences of using additive STL on multiplicative data for forecasting?
    \end{enumerate}
    }
\end{frame}

\begin{frame}[fragile]{AI Exercise 2: Data Leakage Detection}
    \begin{block}{AI Code (spot the errors)}
        {\scriptsize
\begin{verbatim}
...
\end{verbatim}
    ...
    \vspace{-2mm}
    {\small
    \textbf{Find three critical errors}:
    \begin{enumerate}\setlength{\itemsep}{1pt}
        \item \texttt{center=True} means $\text{MA}_{12}(t)$ uses $X_{t-5}, \ldots, X_{t+6}$ --- \textbf{six future values} leak into the feature. What is the correct setting?
        \item MAPE is computed on \textbf{in-sample fitted values}, not out-of-sample forecasts. If the true OOS MAPE $= 8.3\%$, what explains the $7\times$ gap?
        \item Additive seasonal model on data with growing amplitude. What diagnostic plot reveals this? (Hint: plot $|S_t|$ vs level $T_t$.)
    \end{enumerate}

    \vspace{1mm}
    \textbf{Fix}: rewrite with \texttt{center=False}, split 80/20, compute MAPE on the test set, and compare with the seasonal na\"ive benchmark $\hat{X}_{t+h} = X_{t+h-12}$.
    }
\end{frame}

\begin{frame}{AI Exercise 3: Smoothing Parameter Forensics}
    \begin{block}{AI Model Comparison (Industrial Production Index)}
        {\small
        \begin{center}
        \begin{tabular}{lccccc}
            \toprule
            \textbf{Model} & $\alpha$ & $\beta$ & $\gamma$ & \textbf{Train RMSE} & \textbf{Test RMSE} \\
            \midrule
            SES & 0.98 & --- & --- & 1.2 & 4.8 \\
        ...
        \end{center}
        \vspace{-2mm}
        AI conclusion: ``Holt-Winters has the lowest RMSE --- best model.''
        }
    \end{block}
    \vspace{-1mm}
    {\small
    \textbf{Tasks}:
    \begin{enumerate}\setlength{\itemsep}{1pt}
        \item Show that SES with $\alpha \to 1$ gives $\hat{X}_{t+1|t} = X_t$ (na\"ive forecast). What does $\alpha = 0.98$ approximate?
        \item Test RMSE \textit{increases} with complexity: $4.8 \to 6.3 \to 9.1$. Name this phenomenon. Which RMSE column should guide model selection?
        \item $\gamma = 0.01$: the seasonal indices barely update from their initial values. What happens if the seasonal pattern shifts (e.g., post-COVID)?
        \item What model would you select, and why? Compute AIC $= T\ln(\text{SSE}/T) + 2k$ for each.
    \end{enumerate}
    }
\end{frame}
