...
\begin{frame}{Proof: Multiplicative Seasonal Structure}
    ...
    \vspace{0.15cm}
    \textbf{Expand the product:}
    \begin{align*}
    (1-\phi L)(1-\Phi L^s)Y_t &= Y_t - \phi L Y_t - \Phi L^s Y_t + \phi\Phi L^{s+1} Y_t \\
    &= Y_t - \phi Y_{t-1} - \Phi Y_{t-s} + \phi\Phi Y_{t-s-1}
    \end{align*}

    \textbf{Result:} The model includes an \textbf{interaction term} $\phi\Phi Y_{t-s-1}$

    \vspace{0.15cm}
    \begin{exampleblock}{Interpretation (Monthly, $s=12$)}
        $Y_t$ depends on:
        \begin{itemize}
            \item $Y_{t-1}$: Last month (short-run dynamics)
            \item $Y_{t-12}$: Same month last year (seasonal effect)
            \item $Y_{t-13}$: Interaction of both effects
        \end{itemize}
    \end{exampleblock}

    \begin{block}{Parsimony}
        Multiplicative form: 2 parameters ($\phi, \Phi$). Additive would need 3+ parameters.
    \end{block}
\end{frame}
...
%=============================================================================
% SECTION 7.5: CASE STUDY - AIRLINE PASSENGERS
%=============================================================================
