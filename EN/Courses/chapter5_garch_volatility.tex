    ...
    \begin{block}{Observed Properties}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Volatility clustering}
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item High volatility periods are followed by high volatility
                    \end{itemize}
            \end{itemize}
        \end{block}
    }
...
\begin{frame}{Example: Bitcoin $\succ$ Evidence for ARCH Effects}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ACF($r_t^2$) is significant at many lags $\succ$ \textbf{ARCH effects present}
            \item Conditional variance is predictable $\succ$ GARCH models are justified
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Properties of the ARCH(1) Model}
    ...
    \begin{block}{ARCH(1): $\sigma_t^2 = \omega + \alpha_1 \varepsilon_{t-1}^2$}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Kurtosis $> 3$ for $\alpha_1 > 0$ $\succ$ \textbf{fat tails}!
        \end{itemize}
    \end{block}
...
\begin{frame}{Derivation: Kurtosis of ARCH(1)}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\kappa > 3$ for any $\alpha_1 > 0$ $\succ$ \textbf{fat tails} (leptokurtosis)
...
\begin{frame}{Quasi-MLE and Robustness}
    ...
    \end{block}
    \vspace{-0.15cm}
    \begin{defn}[QMLE Consistency --- Bollerslev \& Wooldridge (1992)]
        If the conditional mean and variance are correctly specified, QMLE is \textbf{consistent} for $(\omega, \alpha, \beta)$ regardless of the true distribution of $z_t$.
    \end{defn}
    \vspace{-0.15cm}
    ...
    \begin{exampleblock}{When to Use QMLE vs Full MLE}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{QMLE}: safe default --- consistent even under misspecification
...
\begin{frame}{Practical Estimation Challenges}
    ...
    \begin{block}{Common Issues}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Sensitivity to starting values}: Different initializations $\succ$ different estimates
        \end{itemize}
    \end{block}
    \vspace{-0.1cm}
    \begin{exampleblock}{Best Practices}
        \begin{enumerate}\setlength{\itemsep}{0pt}
            ...
            \item Compare results across starting values; consistent estimates $\succ$ reliable model
        \end{enumerate}
    \end{exampleblock}
    \vspace{-0.1cm}
...
\begin{frame}{Typical Values for GARCH(1,1)}
    ...
    \begin{alertblock}{Observations}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\alpha + \beta$ close to 1 $\succ$ \textbf{very persistent volatility}
            \item Small $\alpha$, large $\beta$ $\succ$ slow reaction to shocks, long memory
            \item Bitcoin: larger $\alpha$ $\succ$ faster reaction to news
        \end{itemize}
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{GARCH vs IGARCH: Persistence Comparison}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item IGARCH has no finite mean $\succ$ shocks persist indefinitely
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Volatility Regime Changes}
    ...
    \begin{alertblock}{Practical Implications}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item If structural breaks present, GARCH persistence ($\alpha + \beta$) is \textbf{upward biased} $\succ$ spurious IGARCH
    ...
    {\scriptsize
        ...
        \begin{alertblock}{Problem with GARCH}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Economic intuition: Bad news $\succ$ stock price falls $\succ$ debt/equity ratio rises $\succ$ volatility increases
            \end{itemize}
        \end{alertblock}
...
\begin{frame}{The EGARCH Model --- Nelson (1991)}
    ...
    \begin{block}{EGARCH Advantages}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\gamma < 0$: negative shocks $\succ$ higher volatility
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{EGARCH} and \textbf{GJR-GARCH}: capture asymmetry
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Leverage effect: negative shocks $\succ$ larger impact
                \end{itemize}
        \end{itemize}
    \end{block}
    }
...
\small
    ...
    \begin{block}{Economic Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\delta > 0$: \textbf{risk premium} $\succ$ higher returns when volatility is high
    ...
    \begin{exampleblock}{Typical Example: Equities}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $r_t = 0.02 + \underbrace{0.15}_{\delta} \cdot \sigma_t + \varepsilon_t$ \quad $\succ$ At $\sigma_t = 2\%$: $\E[r_t] = 0.023$ (0.3\% premium)
        \end{itemize}
    \end{exampleblock}
    }
\end{frame}
...
\begin{frame}{Diebold-Mariano Test}
    ...
    \begin{alertblock}{Practical Notes}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $H_0$: equal predictive accuracy; rejection $\succ$ one model is significantly better
...
\begin{frame}{Mincer-Zarnowitz Regression}
    ...
    \begin{defn}[Hypotheses]
        ...
        Test via F-test or Wald test. Rejection $\succ$ forecast is biased or inefficient.
    \end{defn}
    \vspace{-0.1cm}
    ...
    \begin{alertblock}{Caveat}
        Realized variance ($r_t^2$) is a noisy proxy for $\sigma_t^2$ $\succ$ $R^2$ typically low ($< 0.10$). Prefer $RV_t$ when available.
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Realized Volatility and HAR-RV}
    ...
    \begin{alertblock}{Key Advantages}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Caveat: microstructure noise at very high frequency $\succ$ use kernel estimators
        \end{itemize}
    \end{alertblock}
    \end{cminipage}
\end{frame}
    ...
    \begin{block}{Backtesting Principle}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item For a correctly specified model at level $\alpha$:
                ...
                \end{itemize}
            \item VaR 1\% over 250 days $\succ$ expect $\sim$2.5 violations/year
        \end{itemize}
    \end{block}
    \vspace{-0.2cm}
        ...
        \begin{block}{Interpretation}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Red dots: 13 violations out of 250 days ($\hat{p} = 5.2\%$)
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item \textcolor{Crimson}{\textbf{Basel red zone}} $\succ$ model significantly underestimates risk
...
\begin{frame}{VaR Backtesting: Basel Traffic Light}
    ...
    \begin{exampleblock}{Practical Example}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item 3 violations $\succ$ \textcolor{Forest}{\textbf{Green zone}} $\succ$ model acceptable
            \item 7 violations $\succ$ \textcolor{Amber}{\textbf{Yellow zone}} $\succ$ revision needed
            \item 13 violations $\succ$ \textcolor{Crimson}{\textbf{Red zone}} $\succ$ model rejected
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Rolling Window VaR Methodology}
    ...
    \begin{exampleblock}{Step-by-Step Procedure (for each day $t = W+1, \ldots, T$)}
        \begin{enumerate}\setlength{\itemsep}{0pt}
            \item Estimate GARCH on $\{r_{t-W}, \ldots, r_{t-1}\}$ $\succ$ parameters $\hat{\omega}, \hat{\alpha}, \hat{\beta}, \hat{\nu}$
...
\begin{frame}{Rolling Window VaR: Procedure Diagram}
    ...
    \begin{block}{Result}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item We obtain the series $\{\text{VaR}_{\alpha,t}\}_{t=W+1}^{T}$ $\succ$ a \textbf{different} threshold every day
...
\begin{exampleblock}{Typical Output}
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item Number of violations vs expected $\succ$ Basel zone (green/yellow/red)
        \item Kupiec (frequency): $H_0$: $\hat{p} = \alpha$ $\succ$ p-value $> 0.05$ $\succ$ accepted
        \item Christoffersen (independence): $H_0$: violations i.i.d. $\succ$ p-value $> 0.05$ $\succ$ accepted
        \item Combined test: both conditions met $\succ$ \textcolor{Forest}{\textbf{model validated}}
    \end{itemize}
\end{exampleblock}
...
\begin{frame}{Expected Shortfall Backtesting}
    ...
    \begin{block}{Why Backtest ES?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item ES = average loss \textit{given} VaR is breached $\succ$ more informative for tail risk
        \end{itemize}
    \end{block}
    \vspace{-0.15cm}
...
\begin{frame}{Basel IV: From VaR to ES}
    ...
    \begin{exampleblock}{Equivalence under Normality}
        ...
        \]
        $\succ$ Under normality, ES 97.5\% $\approx$ VaR 99\%. Under fat tails, ES $\gg$ VaR.
    \end{exampleblock}
    \vspace{-0.1cm}
...
\small
    \begin{block}{Why Joint Modeling?}
        \textbf{Serial correlation} $\succ$ ARMA for mean; \textbf{Volatility clustering} $\succ$ GARCH for variance.
    \end{block}
    \vspace{-0.1cm}
...
VaR_1pct = norm.ppf(0.01) * sigma_1  # Normal approx
    ...
    \end{block}
    \vspace{-2mm}
    {\scriptsize \textbf{Note}: For Student-$t$ VaR, replace \texttt{norm.ppf} with \texttt{scipy.stats.t.ppf(0.01, df=nu)} scaled by $\sqrt{(\nu-2)/\nu}$.}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Step 1: Data --- S\&P 500 Daily Returns}
    ...
    {\small
        ...
        \begin{exampleblock}{Descriptive Statistics}
            ...
            \vspace{-0.1cm}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Fat tails (kurtosis $\gg 3$) and negative skewness $\succ$ ARCH effects
            \end{itemize}
        \end{exampleblock}
        }
    }
...
\begin{frame}{Step 2: Testing for ARCH Effects}
    ...
    \begin{exampleblock}{Results}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Conclusion}: Strong ARCH effects $\succ$ significant heteroskedasticity
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Step 3: Estimated Parameters --- Comparison}
    ...
    {\scriptsize
        ...
        \begin{alertblock}{Interpretation}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item EGARCH $\gamma = -0.12$ \textbf{significant} $\succ$ \textbf{leverage effect confirmed}
                \item GJR: $\alpha_{\text{neg}} = \alpha + \gamma = 0.148$ vs $\alpha_{\text{pos}} = 0.022$ $\succ$ strong asymmetry
            \end{itemize}
        \end{alertblock}
        }
    }
...
\begin{frame}{Step 4: Model Selection --- AIC/BIC}
    ...
    \begin{exampleblock}{Decision}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Student-t superior to Normal ($\Delta$AIC $\approx 120$) $\succ$ fat tails matter!
...
\begin{frame}{Step 6: Rolling VaR --- S\&P 500}
    \begin{center}
...
\begin{frame}{Step 7: Conclusions --- S\&P 500 Case Study}
    ...
    \begin{block}{Step-by-Step Methodology Summary}
        \begin{enumerate}\setlength{\itemsep}{2pt}
            \item \textbf{Data}: log returns, descriptive statistics $\succ$ fat tails, skewness
            \item \textbf{ARCH test}: ARCH-LM + Ljung-Box on $r_t^2$ $\succ$ significant ARCH effects
            \item \textbf{Estimation}: 4 candidate models (symmetric/asymmetric $\times$ Normal/Student-t)
            \item \textbf{Selection}: AIC/BIC $\succ$ \textbf{EGARCH(1,1)-t} winner
            \item \textbf{Diagnostics}: standardized residuals $\succ$ model adequate
            \item \textbf{VaR}: rolling window + Kupiec/Christoffersen backtesting $\succ$ model \textbf{validated}
        \end{enumerate}
    \end{block}
    \vspace{0.1cm}
...
\begin{frame}{Step 6: Rolling VaR --- Bitcoin}
    \begin{center}
...
\begin{frame}{Step 6: VaR Rolling Window --- Bitcoin}
    \small
    ...
    \begin{exampleblock}{Rolling VaR Characteristics Bitcoin (2018--2024)}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Max VaR: 37.54\% $\succ$ COVID crash March 2020
            \item Min VaR: 2.90\% $\succ$ calm period
...
\begin{frame}{Summary $\succ$ Chapter 5: Volatility Models}
\small
    \vspace{-0.2cm}
    \begin{cminipage}{0.95\textwidth}
...
\begin{frame}{Question 1}
    ...
    \begin{block}{Answer Choices}
        ...
        \textcolor{MainBlue}{\textbf{(C)}} Volatility is constant over time (homoscedasticity)\\[3pt]
...
\begin{frame}{References II}
    ...
    \begin{exampleblock}{Online Resources and Code}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{GitHub TSA}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch5} $\succ$ Python code for this chapter
        \end{itemize}
        }
    \end{exampleblock}
\end{frame}
