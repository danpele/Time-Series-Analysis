...
\begin{frame}{Practical checklist: step-by-step GARCH modeling}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-0.2cm}
        {\footnotesize
        ...
        \begin{alertblock}{Common pitfalls}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Overfitting}: GARCH(1,1) is sufficient in 90\% of cases --- higher orders rarely improve forecasts
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{The DCC-GARCH Multivariate Model}
    \quantlet{TSA\_ch5\_dcc\_garch}{https://github.com/QuantLet/TSA/tree/main/TSA_ch5/TSA_ch5_dcc_garch}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-3mm}
        {\scriptsize
        \setlength{\abovedisplayskip}{2pt}\setlength{\belowdisplayskip}{2pt}
        \begin{block}{Motivation: From Univariate Volatility to Dynamic Correlations}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Univariate GARCH models the volatility of a \textbf{single} series
                \item In practice, portfolios contain \textbf{multiple assets} with time-varying correlations
                \item DCC-GARCH (Engle, 2002) jointly models volatilities \emph{and} correlations
            \end{itemize}
        \end{block}
        \vspace{-1.5mm}
        \begin{block}{Two-Step DCC Structure}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                \item \textbf{Step 1}: Estimate univariate GARCH for each series $\Rightarrow$ $\mathbf{D}_t = \text{diag}(\sigma_{1,t}, \ldots, \sigma_{K,t})$
                \item \textbf{Step 2}: Model conditional correlations:
                $\mathbf{Q}_t = (1 - a - b)\bar{\mathbf{Q}} + a\,\mathbf{u}_{t-1}\mathbf{u}_{t-1}' + b\,\mathbf{Q}_{t-1}$,
                where $\mathbf{u}_t = \mathbf{D}_t^{-1}\boldsymbol{\varepsilon}_t$ are standardized residuals
            \end{enumerate}
        \end{block}
        \vspace{-1.5mm}
        \begin{block}{Conditional Correlation and Covariance Matrices}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Correlation: $\mathbf{R}_t = \mathbf{Q}_t^{*-1}\,\mathbf{Q}_t\,\mathbf{Q}_t^{*-1}$, with $\mathbf{Q}_t^{*} = \text{diag}\!\bigl(\sqrt{q_{11,t}},\, \ldots,\, \sqrt{q_{KK,t}}\bigr)$
                \item Covariance: $\mathbf{H}_t = \mathbf{D}_t\,\mathbf{R}_t\,\mathbf{D}_t$
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{DCC-GARCH Estimation}
...
\begin{frame}{DCC Properties and Extensions}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Positive Definiteness}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\mathbf{H}_t$ is \textbf{positive definite} if $\bar{\mathbf{Q}}$ is positive definite and $a + b < 1$, $a \geq 0$, $b \geq 0$
                \item Normalization through $\mathbf{Q}_t^{*}$ guarantees $\rho_{ij,t} \in [-1, 1]$
            \end{itemize}
        \end{block}
        \vspace{-1.5mm}
        \begin{block}{Asymmetric DCC (Cappiello, Engle \& Sheppard, 2006)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Extension with \textbf{leverage term} to capture increased correlations during crises:
            \end{itemize}
            \vspace{-2mm}
            \[
            ...
            \vspace{-4mm}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\mathbf{n}_t = \mathbb{I}(\mathbf{u}_t < 0) \odot \mathbf{u}_t$ (negative component), $\bar{\mathbf{N}} = \E[\mathbf{n}_t \mathbf{n}_t']$
            \end{itemize}
        \end{block}
        \vspace{-1.5mm}
        \begin{exampleblock}{Applications}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Portfolio optimization}: Markowitz weights with time-varying $\mathbf{H}_t$
                \item \textbf{Systemic risk}: conditional correlations as a measure of financial contagion
                \item \textbf{Dynamic hedging}: optimal hedge ratios $h_t^{*} = \sigma_{12,t} / \sigma_{22,t}$
            \end{itemize}
        \end{exampleblock}
        }
