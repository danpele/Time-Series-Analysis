...
\begin{frame}{Example: DAX -- Volatility Clustering}
    \begin{cminipage}{0.95\textwidth}
    {\small
    \begin{block}{Observations}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item DAX daily returns (2015--2024): pronounced volatility clustering
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Extreme volatility episodes during crises (Brexit 2016, COVID 2020, Ukraine war 2022)
                \end{itemize}
            \item Red zones mark high-volatility regimes (top 10\% of 21-day rolling volatility)
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Calm and turbulent periods alternate --- they do not appear randomly
                \end{itemize}
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{Example: DAX -- Volatility Clustering}
    ...
    {\scriptsize
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Takeaway:} Daily returns show calm/crisis alternation --- a constant-variance model cannot capture this phenomenon
    \end{itemize}
    }
    \begin{center}
...
\begin{frame}{Conditional vs Unconditional Volatility}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Conditional volatility} $\sigma_t$ (GARCH): time-varying, reacts to new information
            \item \textbf{Unconditional volatility} $\bar{\sigma} = \sqrt{\omega/(1-\alpha-\beta)}$: constant long-run level
            \item Red zones: $\sigma_t > \bar{\sigma}$ (stress periods); blue zones: $\sigma_t < \bar{\sigma}$ (calm periods)
        \end{itemize}
    \end{block}
    }
    \end{cminipage}
...
\begin{frame}{GARCH(1,1) Simulation: What Do We Observe?}
    ...
    {\footnotesize
        \begin{block}{Reading the Simulation Plot}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Small $\alpha + \beta$} (0.7): volatility reverts quickly --- series looks almost like white noise
                \item \textbf{Moderate $\alpha + \beta$} (0.95): high-volatility episodes last weeks --- similar to real financial series
                \item \textbf{$\alpha + \beta \approx 1$} (0.99): volatility persists for months --- half-life $\approx 69$ days
            \end{itemize}
        \end{block}
        \begin{exampleblock}{Visual Identification: Half-Life}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Half-life: $HL = \ln(0.5) / \ln(\alpha + \beta)$
                \item $\alpha + \beta = 0.95$: $HL \approx 14$ days (2 wks) \quad $\alpha + \beta = 0.988$: $HL \approx 58$ days (3 mo, typical S\&P 500)
            \end{itemize}
        \end{exampleblock}
        \begin{alertblock}{Practical Rule}
            If the $\pm 2\sigma_t$ bands do not revert to normal within 1--2 months, the market has high persistence ($\alpha + \beta > 0.97$)
        \end{alertblock}
    }
    \end{cminipage}
\end{frame}
...
\begin{frame}{IGARCH: What Do We Observe in the Simulated Series?}
    ...
    {\footnotesize
        \begin{block}{Reading the Comparison Plot}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Standard GARCH} ($\alpha + \beta < 1$): volatility reverts to $\bar{\sigma}^2$ --- ``waves'' with decreasing amplitude
                \item \textbf{IGARCH} ($\alpha + \beta = 1$): no anchor --- after a large shock, the elevated level persists indefinitely
                \item Visually: IGARCH ``wanders'' (random walk in variance), while GARCH oscillates around the mean
            \end{itemize}
        \end{block}
        ...
        \begin{alertblock}{Caution}
            IGARCH may be an artifact of regime changes --- verify with Markov-switching models!
        \end{alertblock}
    }
    \end{cminipage}
\end{frame}
...
\begin{frame}{CGARCH --- Component GARCH (Engle \& Lee, 1999)}
    ...
    {\footnotesize
        ...
        \begin{alertblock}{Interpretation}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $q_t$ varies slowly (macro factors); $\sigma_t^2 - q_t$ reacts quickly to shocks
                \item Different half-lives: permanent $\sim$years, transitory $\sim$weeks. Python: \texttt{arch} with manual spec.
            \end{itemize}
        \end{alertblock}
    }
    \end{cminipage}
...
\begin{frame}{EGARCH Simulation: What Do We Observe?}
    ...
    {\scriptsize
        ...
        \begin{exampleblock}{Numerical Example: Interpreting $\gamma$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item With $\gamma = -0.15$: negative shock $z_{t-1} = -2$ contributes $0.10 \times 2 + (-0.15)(-2) = 0.50$ to $\ln(\sigma_t^2)$; positive $z_{t-1} = +2$ contributes $0.10 \times 2 + (-0.15)(+2) = -0.10$. Difference: \textbf{0.60} on log scale!
            \end{itemize}
        \end{exampleblock}
        \vspace{-1mm}
...
\begin{frame}{EGARCH vs GARCH: EUR/USD --- Model Comparison}
    \begin{center}
...
\begin{frame}{Asset Class Selection Guide}
    \begin{cminipage}{0.95\textwidth}
    {\footnotesize
    \vspace{-0.2cm}
    \begin{block}{Which model for which market?}
        \renewcommand{\arraystretch}{1.15}
        \begin{center}
        \begin{tabular}{lll}
            \toprule
            \textbf{Asset Class} & \textbf{Recommended Model} & \textbf{Why?} \\
            \midrule
            Indices (S\&P 500, DAX) & EGARCH-t / GJR-t & Strong leverage \\
            Individual stocks & GJR-GARCH-t & Leverage + interpretability \\
            Forex (EUR/USD) & GARCH(1,1)-t / FIGARCH & High persistence, no leverage \\
            Crypto (Bitcoin) & EGARCH-t / GJR-t & Leverage + very fat tails \\
            Commodities (oil, gold) & GARCH(1,1)-t & Moderate, no clear asymmetry \\
            Interest rates & IGARCH / CGARCH & Extreme persistence, regimes \\
            \bottomrule
        \end{tabular}
        \end{center}
    \end{block}
    ...
    \begin{alertblock}{Practical Selection Rules}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Step 1}: GARCH(1,1)-t $\rightarrow$ \textbf{Step 2}: sign bias test $\rightarrow$ \textbf{Step 3}: if leverage $\Rightarrow$ EGARCH/GJR
            \item \textbf{Step 4}: if $\hat{\alpha}+\hat{\beta} \approx 1$ $\Rightarrow$ FIGARCH or Markov-switching
            \item \textbf{Step 5}: compare AIC/BIC + Diebold-Mariano out-of-sample
        \end{itemize}
    \end{alertblock}
    }
    \end{cminipage}
...
\begin{frame}{Diagnostic Example --- EUR/USD EGARCH(1,1,1)-t}
    \vspace{-0.2cm}
    {\scriptsize
...
\begin{frame}{Sign-Bias Test --- Engle \& Ng (1993)}
    \begin{cminipage}{0.95\textwidth}
    {\footnotesize
    \begin{block}{Motivation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Tests whether the GARCH model \textbf{correctly captures asymmetry} in volatility
            \item If the test rejects, the symmetric model is inadequate $\Rightarrow$ need EGARCH or GJR
        \end{itemize}
    \end{block}
    \vspace{-1mm}
    \begin{defn}[Sign-Bias Regressions]
        Let $\hat{z}_t = \hat{\varepsilon}_t / \hat{\sigma}_t$ and $S_t^- = \mathbf{1}(\hat{\varepsilon}_t < 0)$. Estimate:
        \vspace{-2mm}
        \[
        ...
        \begin{itemize}\setlength{\itemsep}{-1pt}
            ...
            \item \textbf{Joint test}: $H_0: c_1 = c_2 = c_3 = 0$, statistic $TR^2 \sim \chi^2(3)$
        \end{itemize}
    \end{defn}
    \vspace{-1mm}
    \begin{alertblock}{Practical Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Rejection $\Rightarrow$ model does not capture leverage $\Rightarrow$ switch to EGARCH/GJR
            \item Acceptance on asymmetric model $\Rightarrow$ asymmetry well specified
        \end{itemize}
    \end{alertblock}
    }
    \end{cminipage}
...
\begin{frame}{Nyblom Stability Test (1989)}
    \begin{cminipage}{0.95\textwidth}
    {\footnotesize
    \begin{block}{Motivation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Tests whether GARCH parameters are \textbf{constant over the entire period}
            \item Parameter instability distorts persistence: $\hat{\alpha} + \hat{\beta}$ artificially inflated
        \end{itemize}
    \end{block}
    \vspace{-1mm}
    \begin{defn}[Nyblom Test]
        $H_0$: $\boldsymbol{\theta} = (\omega, \alpha, \beta, \ldots)$ constant; $H_1$: $\boldsymbol{\theta}_t$ follows a random walk.
        \vspace{-2mm}
        \[
        ...
        \]
        \vspace{-2mm}
        $\mathbf{S}_t$ = partial sum of scores; $\hat{\mathbf{V}}$ = estimated information matrix.
    \end{defn}
    \vspace{-1mm}
    \begin{alertblock}{Practical Usage}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Python: \texttt{result.diagnostic\_summary()} (\texttt{arch} package)
            \item Rejection $\Rightarrow$ rolling window or regime-switching GARCH
        \end{itemize}
    \end{alertblock}
    }
    \end{cminipage}
...
\begin{frame}{Estimated Information Matrix}
    \begin{cminipage}{0.95\textwidth}
    {\scriptsize
    \begin{block}{What is the information matrix?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Score}: $\mathbf{s}_t(\boldsymbol{\theta}) = \nabla_{\boldsymbol{\theta}} \ell_t$ --- derivative of log-likelihood w.r.t.\ parameters
            \item \textbf{Fisher information}: $\mathcal{I}(\boldsymbol{\theta}) = \E[\mathbf{s}_t \, \mathbf{s}_t']$ --- measures how much data ``inform'' about parameters
        \end{itemize}
    \end{block}
    \vspace{-1mm}
    \begin{exampleblock}{Estimating the Information Matrix}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{OPG} (Outer Product of Gradients): $\hat{\mathbf{V}} = \frac{1}{T}\sum_{t=1}^T \mathbf{s}_t \, \mathbf{s}_t'$
            \item \textbf{Hessian}: $\hat{\mathbf{V}} = -\frac{1}{T}\sum_{t=1}^T \nabla^2_{\boldsymbol{\theta}} \ell_t$
            \item \textbf{Sandwich} (robust, Bollerslev-Wooldridge): $\hat{\mathbf{V}} = \mathbf{H}^{-1} \mathbf{G} \, \mathbf{H}^{-1}$, $\mathbf{H}$ = Hessian, $\mathbf{G}$ = OPG
        \end{itemize}
    \end{exampleblock}
    \vspace{-1mm}
    \begin{alertblock}{Why does it matter?}
        Standard errors: $\text{se}(\hat{\theta}_j) = \sqrt{[\hat{\mathbf{V}}^{-1}]_{jj} / T}$. The sandwich version is robust to distributional misspecification.
    \end{alertblock}
    }
    \end{cminipage}
\end{frame}
...
\begin{frame}{Loss Functions for Volatility Forecast Evaluation}
    \begin{cminipage}{0.95\textwidth}
    {\footnotesize
    \begin{block}{Why does the loss function matter?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item True volatility $\sigma_t^2$ is \textbf{unobservable} --- we use proxies ($r_t^2$, $RV_t$)
            \item The loss function must be \textbf{consistent}: the ranking does not depend on the proxy (Patton, 2011)
        \end{itemize}
    \end{block}
    \vspace{-2mm}
    \begin{defn}[Standard Loss Functions]
        \vspace{-3mm}
        \begin{align*}
            L_{\text{MSE}} &= (\sigma_t^2 - \hat{\sigma}_t^2)^2 & \text{(sensitive to extremes)}\\
            L_{\text{QLIKE}} &= \ln \hat{\sigma}_t^2 + \sigma_t^2 / \hat{\sigma}_t^2 & \text{(robust, consistent --- \textbf{recommended})}\\
            L_{\text{MAE}} &= |\sigma_t - \hat{\sigma}_t| & \text{(not consistent for $\sigma^2$)}
        \end{align*}
    \end{defn}
    \vspace{-1mm}
    \begin{alertblock}{Recommendation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{QLIKE}: robust to noisy proxy; available in DM and MCS
        \end{itemize}
    \end{alertblock}
    }
    \end{cminipage}
...
\begin{frame}{Model Confidence Set --- Hansen, Lunde \& Nason (2011)}
    \begin{cminipage}{0.95\textwidth}
    {\footnotesize
    \begin{block}{Motivation: from pairwise to multiple comparison}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Diebold-Mariano compares \textbf{2 models}; what about 5, 10, or 20 candidates?
            \item MCS: identifies the \textbf{set of statistically equivalent models} (``the best'')
        \end{itemize}
    \end{block}
    \vspace{-1mm}
    \begin{defn}[Model Confidence Set (MCS)]
        Starting from $\mathcal{M}_0 = \{M_1, \ldots, M_m\}$ (all candidate models):
        \begin{enumerate}\setlength{\itemsep}{-1pt}
            \item Test $H_0$: all models in the set have equal predictive accuracy
            \item If rejected: eliminate the weakest model ($t_{\max}$ or $T_R$)
            \item Repeat until $H_0$ is no longer rejected $\Rightarrow$ $\hat{\mathcal{M}}^*_{1-\alpha}$
        \end{enumerate}
    \end{defn}
    \vspace{-1mm}
    \begin{alertblock}{Advantage over Diebold-Mariano}
        \begin{itemize}\setlength{\itemsep}{-1pt}
            \item Controls \textbf{multiple testing error} (familywise error rate)
            \item Each model receives an \textbf{MCS p-value}; available: \texttt{arch.bootstrap.MCS}
        \end{itemize}
    \end{alertblock}
    }
    \end{cminipage}
...
\begin{frame}{From Modeling to Forecasting and Risk Management}
    \begin{cminipage}{0.95\textwidth}
    {\footnotesize
    \begin{block}{What we have built so far}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Estimated GARCH models (symmetric and asymmetric)
            \item Validated with diagnostic tests (Ljung-Box, ARCH-LM, Sign-bias, Nyblom)
            \item Selected the best model (AIC/BIC, Diebold-Mariano, MCS)
        \end{itemize}
    \end{block}
    \vspace{-0.1cm}
    \begin{alertblock}{Next step: what is the model for?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Volatility forecasting}: $\hat{\sigma}_{T+h}^2$ --- convergence to $\bar{\sigma}^2$
            \item \textbf{Value at Risk (VaR)}: maximum loss at a given probability
            \item \textbf{Expected Shortfall (ES)}: average loss beyond VaR (Basel III/IV)
            \item \textbf{Backtesting}: validating the VaR model on historical data
        \end{itemize}
    \end{alertblock}
    \vspace{-0.1cm}
    \begin{exampleblock}{Section Structure}
        Forecast $\rightarrow$ VaR/ES $\rightarrow$ Backtesting (Kupiec, Christoffersen) $\rightarrow$ Applied case studies
    \end{exampleblock}
    }
    \end{cminipage}
\end{frame}
...
\begin{frame}{Final Comparison: S\&P 500 vs Bitcoin (cont.)}
    \begin{cminipage}{0.95\textwidth}
    {\footnotesize
    \begin{alertblock}{General Conclusion}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Re-estimating GARCH at each step: Kupiec + Christoffersen \textbf{accepted}
            \item Rolling window VaR: \textbf{mandatory} $\Rightarrow$ static VaR is completely inadequate
            \item Student-t + asymmetric model: \textbf{essential} for both markets
        \end{itemize}
    \end{alertblock}
    }
...
% Multivariate GARCH --- see chapter5b_multivariate_garch.tex
...
\begin{frame}{What's Next?}
    ...
    \begin{block}{Chapter 5b: Multivariate GARCH Models}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Conditional covariance}: modeling the joint dynamics of multiple assets
            \item \textbf{DCC-GARCH} (Dynamic Conditional Correlation --- Engle, 2002): dynamic conditional correlations
            \item \textbf{BEKK} (Baba-Engle-Kraft-Kroner, 1995): guaranteeing positivity of $\mathbf{H}_t$
            \item \textbf{CCC-GARCH} (Constant Conditional Correlation --- Bollerslev, 1990): constant conditional correlations
            \item \textbf{Applications}: optimal hedging, portfolio allocation, financial contagion
        \end{itemize}
    \end{block}
    \end{minipage}
    \end{center}
...
\begin{frame}{Question 5}
    ...
    \begin{alertblock}{Applied Computational Question}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} Compute the unconditional variance $\bar{\sigma}^2$ and annualized volatility.\\[3pt]
        \textcolor{MainBlue}{\textbf{(B)}} Compute the half-life of a volatility shock.\\[3pt]
        \textcolor{MainBlue}{\textbf{(C)}} For a EUR\,2\,000\,000 portfolio, compute the 1-day VaR at 1\% under:
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Normal distribution
                    \item Student-$t$ with $\nu = 5$ degrees of freedom
                \end{itemize}
                \vspace{-2pt}(Use $\sigma_{T+1} = \bar{\sigma}$ for simplicity.)
        }
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Question 5: Answer}
    ...
    \textcolor{MainBlue}{\textbf{(A)}} $\bar{\sigma}^2 = \frac{\omega}{1 - \alpha - \beta} = \frac{0.05}{1 - 0.97} = 1.667$ \quad $\Rightarrow$ $\bar{\sigma} = 1.291\%$ daily
            \quad $\sigma_{\text{ann}} = 1.291\% \times \sqrt{252} \approx 20.5\%$\\[3pt]
    \textcolor{MainBlue}{\textbf{(B)}} $HL = \frac{\ln 0.5}{\ln(\alpha + \beta)} = \frac{-0.693}{\ln(0.97)} = \frac{-0.693}{-0.0305} \approx \textbf{23 days}$\\[3pt]
    \textcolor{MainBlue}{\textbf{(C)}} Portfolio value $V = 2{,}000{,}000$ EUR, $\sigma_{T+1} = 1.291\%$:
...
\begin{frame}{References IV --- Realized Volatility}
    \vspace{-0.4cm}
    \begin{block}{High-Frequency and Realized Volatility}
        {\small
        \begin{itemize}\setlength{\itemsep}{-1pt}
    ...
    \end{block}
    \vspace{-0.15cm}
    \begin{exampleblock}{Textbooks and Monographs}
        {\small
        \begin{itemize}\setlength{\itemsep}{-1pt}
            \item Tsay, R.S. (2010). \textit{Analysis of Financial Time Series}, 3rd ed., Wiley.
...
\begin{frame}{References V --- Additional Resources}
    ...
    \begin{exampleblock}{Online Resources and Code}
        {\small
        \begin{itemize}\setlength{\itemsep}{3pt}
            \item \textbf{Quantlet}: \url{https://quantlet.com} --- Platform for quantitative methods code
            \item \textbf{Quantinar}: \url{https://quantinar.com} --- Learning platform for quantitative methods
            \item \textbf{GitHub TSA}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch5} --- Python code for this chapter
            \item \textbf{Online course}: \url{https://danpele.github.io/Time-Series-Analysis/} --- Complete course materials
        \end{itemize}
        }
    \end{exampleblock}
\end{frame}
