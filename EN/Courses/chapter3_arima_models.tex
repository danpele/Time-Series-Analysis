...
\begin{enumerate}\setlength{\itemsep}{0pt}
    \item[\textcolor{MainBlue}{\textbf{1.}}] \textbf{Understand} the concept and implications of non-stationarity
    \item[\textcolor{MainBlue}{\textbf{2.}}] \textbf{Apply} differencing to achieve stationarity in time series
    \item[\textcolor{MainBlue}{\textbf{3.}}] \textbf{Use} the Augmented Dickey-Fuller (ADF) test for unit root detection
    \item[\textcolor{MainBlue}{\textbf{4.}}] \textbf{Build}, estimate, and forecast with ARIMA models
\end{enumerate}
\end{block}
    }
    \end{cminipage}
...
\begin{frame}{Motivating Example: Non-Stationary Data Is Everywhere}
    ...
    \begin{block}{Key Observations}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item The sample mean (red line) is not a consistent estimator for a non-stationary process
...
\begin{frame}{Proof: Random Walk Autocovariance}
    ...
    Only terms with $i = j$ are nonzero (when $i \leq t-k$).
...
\begin{frame}{Random Walk with Drift}
    ...
    \begin{block}{$Y_t = \mu + Y_{t-1} + \varepsilon_t$ \quad $\Leftrightarrow$ \quad $Y_t = Y_0 + \mu t + \sum_{i=1}^{t} \varepsilon_i$}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\E[Y_t] = Y_0 + \mu t$ (mean grows linearly); \quad $\Var(Y_t) = t\sigma^2$ (variance still grows)
            \item \textbf{Without drift} (blue): oscillates randomly around zero; \textbf{With drift} $\mu > 0$ (red): systematic upward trend
            \item Both are non-stationary --- drift adds deterministic trend to stochastic wandering
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{Proof: Differencing Induces Stationarity}
    ...
    \begin{exampleblock}{General Principle}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Differencing removes the ``memory'' that causes variance to accumulate
            \item For $I(d)$ processes, $d$ differences are needed
        \end{itemize}
    \end{exampleblock}
    }
    \end{cminipage}
...
\begin{frame}{Definition of ARIMA}
    ...
    \begin{defn}[ARIMA(p,d,q): $\phi(L)(1-L)^d Y_t = c + \theta(L)\varepsilon_t$]
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{AR}: $\phi(L) = 1 - \phi_1 L - \cdots - \phi_p L^p$; \quad \textbf{MA}: $\theta(L) = 1 + \theta_1 L + \cdots + \theta_q L^q$; \quad \textbf{I}: $d$ differences
            \item \textbf{Top}: original ARIMA series (non-stationary); \textbf{Bottom}: after differencing --- ACF/PACF reveal $p$, $q$
        \end{itemize}
    \end{defn}
    }
...
\begin{frame}{Long-Run Forecasts for ARIMA}
    ...
    \begin{alertblock}{Practical Implication}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ARIMA forecasts are most reliable for short horizons
            \item Long-term forecasts have very wide uncertainty bands
        \end{itemize}
    \end{alertblock}
    }
    \end{cminipage}
...
\begin{frame}{Rolling Forecasting: Fixed vs Expanding Window}
    \begin{center}
    ...
    \vspace{-0.4cm}
    {\scriptsize
    \begin{block}{Out-of-sample forecast accuracy evaluation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Fixed}: window slides forward, constant size $w$ --- adapts to regime changes
            \item \textbf{Expanding}: window grows over time --- uses all historical data
            \item Mimics real-time forecasting scenario; provides multiple forecast errors for evaluation
        \end{itemize}
    \end{block}
    }
\end{frame}
...
\begin{frame}{Step 7: Forecast Evaluation}
    ...
    \begin{block}{Out-of-sample performance (last 12 quarters)}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item RMSE = 0.0486 $\approx$ 4.86\% error
            \item MAE = 0.0430 $\approx$ 4.30\% error
...
\begin{frame}{AI Exercise: Critical Thinking}
    ...
    \begin{block}{\footnotesize Prompt to test in ChatGPT / Claude / Copilot}
        ``Download quarterly US Real GDP from FRED (series GDPC1) for 2000-Q1 to 2024-Q4 (100 observations). Test stationarity, difference if needed, estimate an ARIMA model, and forecast 8 quarters ahead. Give me complete Python code with plots.''
    \end{block}
    \textbf{Exercise}:
    \begin{enumerate}\setlength{\itemsep}{0pt}
