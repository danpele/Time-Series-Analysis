...
\begin{frame}{Ergodicity: the foundation of inference from data}
    ...
    \begin{alertblock}{Counterexample: stationary but non-ergodic}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Let $Z \sim N(0,1)$ and define $X_t = Z$ for all $t$
            \item The process is strictly stationary (finite-dimensional distributions do not depend on $t$)
            \item But $\bar{X}_T = Z$ for all $T$ $\Rightarrow$ the time average does \textbf{not} converge to $\mu = 0$
            ...
            \begin{column}{0.48\textwidth}
                \begin{block}{Interpretation}
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        ...
                        \item AR(1) $\phi > 0$: power concentrated at low frequencies
                        \item MA(1) $\theta > 0$: power concentrated at high frequencies
                    \end{itemize}
                \end{block}
            \end{column}
            \begin{column}{0.48\textwidth}
                \begin{exampleblock}{Connections}
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        ...
                        \item Time domain (ACF) $\equiv$ frequency domain (spectrum)
