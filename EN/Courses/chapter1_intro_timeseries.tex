            ...
            \begin{column}{0.48\textwidth}
                \begin{block}{Interpretation}
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        ...
                        \item MA(1) $\theta > 0$: power at low freq.
                    \end{itemize}
                \end{block}
            \end{column}
            \begin{column}{0.48\textwidth}
...
\begin{frame}{The three types of white noise}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        ...
        \item \textbf{Gaussian}: $\varepsilon_t \stackrel{iid}{\sim} N(0, \sigma^2)$ --- For Gaussian r.v., uncorrelated $\Leftrightarrow$ independent
    \end{itemize}
    \end{exampleblock}
    }
    \hfill\quantlet{TSA\_ch1\_white\_noise}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_white_noise}
...
\begin{frame}{White noise vs random walk: comparison}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{tabular}{l c c}
            \hline
            \textbf{Property} & \textbf{White Noise (WN)} & \textbf{Random Walk (RW)} \\
            \hline
            Model & $X_t = \varepsilon_t$ & $X_t = X_{t-1} + \varepsilon_t$ \\
            Stationary? & \textcolor{Forest}{Yes} & \textcolor{Crimson}{No} \\
            $\Var(X_t)$ & $\sigma^2$ (constant) & $t\sigma^2$ (grows linearly) \\
            $\mathrm{ACF}(h),\; h \neq 0$ & $0$ & $\approx 1$ (slow decay) \\
            Memory & None & Permanent (shocks persist) \\
            Differencing & --- & $\Delta X_t = \varepsilon_t$ (yields WN) \\
            \hline
        \end{tabular}
        \end{center}
        \vspace{0.1cm}
        \begin{block}{Link}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\Delta X_t = \varepsilon_t$ --- differencing a random walk produces white noise
            \end{itemize}
        \end{block}
        }
        \hfill\quantlet{TSA\_ch1\_random\_walk}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_random_walk}
...
\begin{frame}{KPSS test}
    \begin{block}{Model}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item $\varepsilon_t \sim WN(0, \sigma_\varepsilon^2)$ (stationary error), \; $u_t \sim WN(0, \sigma_u^2)$ (random walk innovation)
        \end{itemize}
    \end{block}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{exampleblock}{Hypotheses (opposite of ADF)}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{$H_0$}: $\sigma_u^2 = 0$ \; (series is $I(0)$, stationary)
                    \item \textbf{$H_1$}: $\sigma_u^2 > 0$ \; (series is $I(1)$, unit root)
                \end{itemize}
            \end{exampleblock}
        \end{column}
        \begin{column}{0.48\textwidth}
...
\begin{frame}{Complete stationarity analysis protocol}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-0.2cm}
        {\footnotesize
        \begin{block}{Step-by-step protocol}
            \begin{enumerate}\setlength{\itemsep}{2pt}
                \item[\textcolor{MainBlue}{\textbf{1.}}] \textbf{Visualize}: plot series $+$ ACF/PACF $\Rightarrow$ first intuition (trend? seasonality?)
                \item[\textcolor{MainBlue}{\textbf{2.}}] \textbf{ADF test}: $H_0$: unit root. Specify: constant only \textit{or} constant $+$ trend
                \item[\textcolor{MainBlue}{\textbf{3.}}] \textbf{KPSS test}: $H_0$: stationary. Same specification as ADF
                \item[\textcolor{MainBlue}{\textbf{4.}}] \textbf{Decision}:
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item \textcolor{Forest}{Concordant}: ADF rejects $+$ KPSS fails to reject $\Rightarrow$ \textbf{stationary}
                        \item \textcolor{Forest}{Concordant}: ADF fails to reject $+$ KPSS rejects $\Rightarrow$ \textbf{non-stationary}
                        \item \textcolor{Crimson}{Divergent}: additional tests (PP, DF-GLS) or transformation
                    \end{itemize}
                \item[\textcolor{MainBlue}{\textbf{5.}}] \textbf{Transform} (if non-stationary): differencing ($\Delta y_t$), log $+$ differencing, or detrending
            \end{enumerate}
        \end{block}
        \vspace{0.1cm}
        \begin{alertblock}{Rule of thumb}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Always} apply at least two tests with opposite $H_0$ to avoid false conclusions
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{The DF-GLS Test (Elliott, Rothenberg, Stock 1996)}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-0.3cm}
        {\footnotesize
        \begin{defn}[DF-GLS --- Dickey-Fuller with GLS detrending]
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Principle}: removes deterministic component via \textbf{GLS} \textit{before} applying ADF
                \item \textbf{Step 1}: GLS-detrended series: $\tilde{y}_t = y_t - \hat{\beta}_0 - \hat{\beta}_1 t$, with $\hat{\beta}$ estimated via quasi-differencing ($\bar{\alpha} = 1 - c/T$)
                \item \textbf{Step 2}: ADF on $\tilde{y}_t$: \ $\Delta \tilde{y}_t = \gamma\, \tilde{y}_{t-1} + \sum_{j=1}^{p} \phi_j \Delta \tilde{y}_{t-j} + e_t$
                \item $H_0$: unit root ($\gamma = 0$) \ vs.\ $H_1$: stationary
            \end{itemize}
        \end{defn}
        \vspace{-1mm}
        \begin{block}{Advantages over ADF}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Higher power}: asymptotically most powerful unit root test in its class
                \item \textbf{Reduced size distortion}: less sensitive to lag misspecification
            \end{itemize}
        \end{block}
        \vspace{-1mm}
...
\begin{frame}{Case study: Romanian quarterly GDP}
    ...
    \begin{block}{Initial Analysis}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Data}: Romanian quarterly GDP 1995--2024 (120 obs., Eurostat, chain-linked volumes 2015=100)
            \item \textbf{Observations}: Strong upward trend with structural shocks
            \begin{itemize}
                \item Visible shocks: 1999 crisis, 2009 GFC, COVID-19 2020
            \end{itemize}
...
\begin{frame}{Stationarity testing: ADF and KPSS}
    \begin{columns}[T]
        ...
        \begin{block}{ADF Test}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Hypothesis}: $H_0$: Unit root
                \item \textbf{Result}: ADF stat.: $-0.14$
                \begin{itemize}
                    \item p-value: $0.94$
        ...
        \begin{exampleblock}{KPSS Test}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Result}: KPSS stat.: $0.95^{**}$
                \begin{itemize}
                    \item p-value: $< 0.01$
                    \item Reject $H_0$
                \end{itemize}
...
\begin{frame}{The Zivot-Andrews Test (1992): unit root with breakpoint}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-0.2cm}
        {\footnotesize
        \begin{block}{Model C (shift in mean and trend)}
            \[
            ...
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item $H_0$: $\gamma = 1$ (unit root) \quad vs.\ \quad $H_1$: $|\gamma| < 1$ with breakpoint at $T_B$
            \end{itemize}
        \end{block}
        }
        \vspace{-2mm}
    ...
    \begin{columns}[T]
        ...
        \begin{alertblock}{After Differencing}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \begin{itemize}
                    \item ADF: $-4.95$ (p $<$ 0.001)
                    \item KPSS: $0.10$ (p $>$ 0.10)
                \end{itemize}
            \end{itemize}
        \end{alertblock}
        \column{0.48\textwidth}
...
\begin{frame}{Question 7: PACF and model identification}
    \begin{cminipage}{0.95\textwidth}
        {\small
        \begin{block}{Question}
            What does a PACF that cuts off sharply after lag 2, while the ACF decays exponentially, indicate?
        \end{block}
        \vspace{0.3cm}
        \begin{enumerate}
            \item[\textcolor{MainBlue}{\textbf{(A)}}] An MA(2) process
            \item[\textcolor{MainBlue}{\textbf{(B)}}] An AR(2) process
            \item[\textcolor{MainBlue}{\textbf{(C)}}] An ARMA(2,2) process
            \item[\textcolor{MainBlue}{\textbf{(D)}}] A random walk
        \end{enumerate}
        }
    \end{cminipage}
\end{frame}
...
\begin{frame}{Question 7: Answer}
    \begin{cminipage}{0.95\textwidth}
        ...
        \vspace{-3mm}
        {\small
        \begin{exampleblock}{Answer: (B) --- AR(2) process}
            \begin{itemize}\setlength{\itemsep}{2pt}
                \item \textbf{Identification rule}: PACF with sharp cutoff at lag $p$ $+$ ACF with exponential decay $\Rightarrow$ AR($p$)
                \item PACF $\neq 0$ at lags 1 and 2, then $\approx 0$ for $h > 2$ $\Rightarrow$ \textbf{AR(2)}
                \item Comparison: MA($q$) has ACF cutoff at lag $q$ and PACF with exponential decay (reverse pattern)
            \end{itemize}
        \end{exampleblock}
        }
        \hfill\quantlet{TSA\_ch1\_acf\_patterns}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_acf_patterns}
