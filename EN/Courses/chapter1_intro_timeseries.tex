...
\begin{frame}{Examples: stationary vs.\ non-stationary series}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        ...
        \item Log returns: $r_t = \ln P_t - \ln P_{t-1}$ $\succ$ non-stationary $\to$ stationary
    \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{Weak stationarity: visual illustration}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item $\E[X_t] = \mu$ constant $\succ$ mean does not depend on time
        \item $\Var(X_t) = \sigma^2$ constant $\succ$ variance does not depend on time
        \item $\Cov(X_t, X_{t+h}) = \gamma(h)$ $\succ$ autocovariance depends only on lag $h$
    \end{itemize}
    \end{exampleblock}
    }
    \hfill\quantlet{TSA\_ch1\_stationarity}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_stationarity}
...
\begin{frame}{Counterexample: weakly stationary but NOT strictly stationary}
    ...
    \begin{columns}[T]
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Skewness differs ($0$ vs $>0$) $\succ$ $X_1 \stackrel{d}{\neq} X_2$
        \end{itemize}
        \end{alertblock}
    \end{columns}
    }
...
\begin{frame}{Ergodicity: the foundation of inference from data}
    ...
    \begin{defn}[Ergodicity for Mean]
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \begin{itemize}
                \item $\bar{X}_T = \frac{1}{T}\sum_{t=1}^{T} X_t \xrightarrow{P} \E[X_t] = \mu$ as $T \to \infty$
            \end{itemize}
        \end{itemize}
    \end{defn}
    ...
    \begin{alertblock}{Why does ergodicity matter?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Solution}: Ergodicity allows estimating $\mu$ from $\bar{X}_T$
            \begin{itemize}
                \item The time average converges to the population mean
                \item Without ergodicity, statistical inference is not possible!
            \end{itemize}
        \end{itemize}
    \end{alertblock}
...
\begin{frame}{The Wold decomposition theorem}
    ...
    \begin{thm}[Wold, 1938]
        ...
        \begin{itemize}
            \item $\varepsilon_t \sim WN(0, \sigma^2)$ $\succ$ white noise
            \begin{itemize}
                \item $\psi_0 = 1$, $\sum \psi_j^2 < \infty$
            \end{itemize}
            \item $\eta_t$ $\succ$ deterministic component (perfectly predictable)
        \end{itemize}
    \end{thm}
...
\begin{frame}{The Wold theorem: visual illustration}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        ...
        \item Coefficients $\psi_j$ decay $\succ$ recent shocks have greater impact than distant ones
    \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{Lag operator: visual illustration}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item $LX_t = X_{t-1}$ $\succ$ the lag operator shifts the series back by one period
        \item $L^k X_t = X_{t-k}$ $\succ$ shift by $k$ periods; $L^0 = I$ (identity)
...
\begin{frame}{Differencing}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        ...
        \item $I(1)$: Random walk $\succ$ $\Delta X_t = \varepsilon_t$ is stationary
...
\begin{frame}{Effect of differencing: S\&P 500}
    ...
    \begin{exampleblock}{Interpretation}
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Top}: S\&P 500 prices $\succ$ clear trend, non-stationary ($I(1)$)
        \item \textbf{Bottom}: Log returns $r_t = \ln P_t - \ln P_{t-1}$ $\succ$ fluctuates around mean $\approx 0$, stationary
    \end{itemize}
    \end{exampleblock}
    }
    \hfill\quantlet{TSA\_ch1\_operators}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_operators}
...
\begin{frame}{Random walk process}
    ...
    {\footnotesize $\Var(X_t) = t\sigma^2$ depends on $t$ $\succ$ random walk is \textbf{not stationary}}
    \end{alertblock}
\end{frame}
...
\begin{frame}{Random walk with drift}
    ...
    \begin{alertblock}{Differencing}
        $\Delta X_t = c + \varepsilon_t$ \; --- constant plus white noise $\succ$ the differenced series is stationary
    \end{alertblock}
    \vspace{-2mm}
    \begin{exampleblock}{Practical Importance}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Nominal GDP, stock prices $\succ$ often modeled as RW with drift
...
\begin{frame}{White noise vs random walk: comparison}
    ...
    \begin{columns}[T]
        ...
        \begin{alertblock}{Random Walk}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Non-stationary
                \item $\Var = t\sigma^2$ (grows)
                \item ACF $\approx 1$ (slow)
...
\begin{frame}{ACF comparison: stationary vs random walk}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        ...
        \item \textbf{Rule of thumb}: Slow ACF decay $\succ$ suspect unit root $\succ$ ADF test
    \end{itemize}
    \end{exampleblock}
    }
    \hfill\quantlet{TSA\_ch1\_random\_walk}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_random_walk}
...
\begin{frame}{ACF and PACF patterns}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.98\textwidth, height=0.58\textheight, keepaspectratio]{acf_pacf_examples.pdf}
    \end{center}
    \vspace{-2mm}
    {\footnotesize
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        ...
        \item \textbf{ARMA($p,q$)}: Both decay exponentially $\succ$ identification requires information criteria
    \end{itemize}
    \end{exampleblock}
    }
    \hfill\quantlet{TSA\_ch1\_acf\_patterns}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_acf_patterns}
...
\begin{frame}{ACF decay patterns}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.98\textwidth, height=0.58\textheight, keepaspectratio]{acf_theoretical.pdf}
    \end{center}
    \vspace{-2mm}
    {\footnotesize
    \begin{exampleblock}{Interpretation}
...
\begin{frame}{Augmented Dickey-Fuller (ADF) test}
    ...
    \begin{alertblock}{Decision Rule}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\tau_{ADF} <$ critical value $\succ$ Reject $H_0$ $\succ$ \textcolor{Forest}{Stationary}
            \item $\tau_{ADF} \geq$ critical value $\succ$ \textcolor{Crimson}{Non-stationary (unit root)}
...
\begin{frame}{KPSS test}
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Test Statistic}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    ...
                    \item {\small where $S_t = \sum_{i=1}^{t} \hat{e}_i$}
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
    ...
    \begin{alertblock}{Decision Rule}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $LM >$ critical value $\succ$ Reject $H_0$ $\succ$ \textcolor{Crimson}{Non-stationary}
            \item $LM \leq$ critical value $\succ$ \textcolor{Forest}{Stationary}
        \end{itemize}
    \end{alertblock}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Workflow}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Step 3}: Concordant results $\succ$ OK
                \begin{itemize}
    ...
    \begin{block}{Interpreting the ADF Test}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \begin{itemize}
                ...
                \item $\tau <$ critical value $\succ$ reject $H_0$ $\succ$ stationary series
            \end{itemize}
...
\begin{frame}{Volatility clustering}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        ...
        \item \textbf{Time-varying volatility} $\succ$ ARCH/GARCH models (Ch. 5)
    \end{itemize}
    \end{exampleblock}
    }
    \hfill\quantlet{TSA\_ch1\_ts\_basics}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_ts_basics}
...
\begin{frame}{Case study: Romanian quarterly GDP}
    ...
    \begin{block}{Initial Analysis}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \end{itemize}
            \item \textbf{Hypothesis}: Non-stationary series $\succ$ test with ADF and KPSS
        \end{itemize}
    \end{block}
    }
\end{frame}
...
\begin{frame}{Stationarity testing: ADF and KPSS}
    ...
    \begin{alertblock}{Conclusion: Both Tests Agree}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item The GDP series is \textbf{non-stationary} $\succ$ requires differencing
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}{Experiment: ChatGPT vs Fundamentals}
    \vspace{-2mm}
    \begin{block}{\footnotesize Prompt $\to$ Response}
        {\footnotesize
        \textbf{You}: ``I have daily EUR/RON exchange rate data for the last 5 years. Can you forecast next week?''\\[2pt]
        \textbf{ChatGPT}: ``ADF: $p = 0.67$ $\to$ non-stationary. After differencing: $p < 0.01$ $\to$ stationary.\\
        Fitted ARMA(2,1). Coefficient $\phi_1 = 0.03$ significant ($p = 0.02$). RMSE $= 0.0043$.''
        }
    \end{block}
    \vspace{-1mm}
    {\small
    \textbf{Three errors a trained analyst catches immediately}:
    \begin{enumerate}\setlength{\itemsep}{1pt}
        \item \textbf{Fitted noise}: EUR/RON $\approx$ random walk $\succ$ $\Delta X_t = \varepsilon_t$ is \textbf{white noise}
            \\ {\footnotesize ACF$(k) \approx 0\ \forall k \geq 1$ $\succ$ no model can beat the na\"ive forecast $\hat{X}_{t+1} = X_t$}
        \item \textbf{Spurious significance}: with $T = 5000$, ACF confidence band $= \pm\frac{1.96}{\sqrt{T}} = \pm 0.028$
            \\ {\footnotesize $\hat\rho_1 = 0.03$ barely outside the band $\succ$ fitting this is \textbf{overfitting noise}}
        \item \textbf{ADF misspecified}: series has drift but ADF ran without trend regressor
            \\ {\footnotesize Wrong specification $\succ$ reduced power $\succ$ unreliable conclusion}
    \end{enumerate}
    }
    \vspace{-1mm}
    \begin{alertblock}{}
        {\small \textbf{Discussion}: If prices follow a random walk, can \textit{any} AI model predict them?}
    \end{alertblock}
\end{frame}

\begin{frame}{Quantitative Audit: Verifying the AI}
    \vspace{-2mm}
    {\small
    \begin{block}{Reproduce the AI's claims with actual EUR/RON data (2019--2024, $T \approx 1300$)}
        \vspace{-1mm}
        {\footnotesize
        \begin{center}
        \begin{tabular}{lll}
            \toprule
            \textbf{AI claim} & \textbf{Verification} & \textbf{Verdict} \\
            \midrule
            ADF $p = 0.67$ on levels & \texttt{adfuller(x, regression='ct')} & Plausible (depends on lags) \\
            $\hat\rho_1 = 0.03$ on $\Delta x_t$ & Band: $\pm 1.96/\sqrt{1300} = \pm 0.054$ & \textcolor{Crimson}{Inside the band!} \\
            ARMA RMSE $= 0.0043$ & Na\"ive RMSE on same period & Na\"ive $\approx 0.0044$ \\
            \bottomrule
        \end{tabular}
        \end{center}
        }
    \end{block}
    \vspace{-1mm}
    \textbf{The decisive test}: Diebold--Mariano
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item $H_0$: ARMA and na\"ive have \textbf{equal forecast accuracy}
        \item Typical result: $p > 0.5$ $\succ$ cannot reject $H_0$
        \item The ARMA model adds \textbf{nothing} over $\hat{X}_{t+1|t} = X_t$
    \end{itemize}

    \vspace{1mm}
    \begin{alertblock}{Lesson}
        Always benchmark against the \textbf{na\"ive forecast}. A model with low RMSE is worthless if the random walk achieves the same.
    \end{alertblock}
    }
\end{frame}
...
\begin{frame}{Question 1}
    ...
    \begin{enumerate}[(A)]\setlength{\itemsep}{0pt}
        \item Zero mean, infinite variance, time-dependent covariance
        \item Constant mean, constant variance, autocovariance depends only on lag
        \item Normal distribution, independence, unit variance
        \item Linear trend, constant seasonality, white residuals
    \end{enumerate}
    \end{block}
\end{frame}
...
\begin{frame}{Question 2}
    ...
    \begin{enumerate}[(A)]\setlength{\itemsep}{0pt}
        ...
        \item The series has a unit root (is non-stationary)
        \item The series has no autocorrelation
        \item The series has a normal distribution
    \end{enumerate}
    \end{block}
\end{frame}
...
\begin{frame}{Question 2: Answer}
    ...
    \begin{exampleblock}{Answer: (B)}
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item $H_0$: unit root; $\tau <$ critical value $\succ$ stationary
    \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{Question 3}
    ...
    \begin{enumerate}[(A)]\setlength{\itemsep}{0pt}
        ...
        \item The series is stationary
        \item The series is a random walk
        \item The series has a deterministic trend
    \end{enumerate}
    \end{block}
\end{frame}
...
\begin{frame}{Question 4}
    ...
    \begin{block}{Answer Choices}
    \begin{enumerate}[(A)]\setlength{\itemsep}{0pt}
        \item Variance is constant: $\Var(X_t) = \sigma^2$
        \item Variance grows linearly with time: $\Var(X_t) = t\sigma^2$
        \item Variance decreases with time
...
\begin{frame}{Question 4: Answer}
    ...
    \begin{exampleblock}{Answer: (B)}
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item $\Var(X_t) = t\sigma^2$ grows linearly $\succ$ non-stationary
    \end{itemize}
    \end{exampleblock}
    }
    \hfill\quantlet{TSA\_ch1\_random\_walk}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_random_walk}
...
\begin{frame}{Question 5}
    ...
    \begin{block}{Answer Choices}
    \begin{enumerate}[(A)]\setlength{\itemsep}{0pt}
        \item All values are zero after lag 0
        \item Decays exponentially fast
        \item Decays very slowly (high persistence)
        \item Oscillates between positive and negative
    \end{enumerate}
    \end{block}
\end{frame}
...
\begin{frame}{Question 5: Answer}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item ACF $\approx$ 1 for many lags, slow decay $\succ$ ADF test
    \end{itemize}
    \end{exampleblock}
    }
    \hfill\quantlet{TSA\_ch1\_random\_walk}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_random_walk}
...
\begin{frame}{Question 6}
    ...
    \begin{block}{Answer Choices}
    \begin{enumerate}[(A)]\setlength{\itemsep}{0pt}
        \item Simple differencing: $\Delta P_t = P_t - P_{t-1}$
        \item Log then differencing: $r_t = \ln P_t - \ln P_{t-1}$
        \item Log only: $\ln P_t$
        \item Standardization: $(P_t - \bar{P})/s_P$
    \end{enumerate}
    \end{block}
\end{frame}
...
\begin{frame}{Question 6: Answer}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        ...
        \item First $\ln$ (stabilizes variance), then $\Delta$ (removes trend) $\succ$ stationary series
    \end{itemize}
    \end{exampleblock}
    }
    \hfill\quantlet{TSA\_ch1\_operators}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_operators}
...
\begin{frame}{References}
    ...
    \begin{columns}[T]
        ...
        \begin{alertblock}{Online Resources}
            {\scriptsize
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Quantlet}: \url{https://quantlet.com} $\succ$ statistics code
                \item \textbf{Quantinar}: \url{https://quantinar.com} $\succ$ tutorials
    ...
    \begin{columns}[T]
            ...
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{S\&P 500}: Yahoo Finance
                    \begin{itemize}
                        \item Prices, returns
                    \end{itemize}
                \item \textbf{Romanian GDP}: INS/Eurostat
                    \begin{itemize}
                        \item Quarterly data
                    \end{itemize}
                \item \textbf{Exchange rates}: BNR
            \end{itemize}
        \end{block}
        \column{0.48\textwidth}
        \begin{exampleblock}{Software}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Data}: Yahoo Finance, FRED, Eurostat
            \end{itemize}
        \end{exampleblock}
    \end{columns}
\end{frame}
