...
\begin{frame}{Why ARMA Models?}
    ...
    {\footnotesize
    \begin{itemize}
        \item \textbf{AR processes}: Current value depends on past values $\Rightarrow$ mean-reverting behavior
        \item \textbf{MA processes}: Current value depends on past shocks $\Rightarrow$ short memory
...
\begin{frame}{Model Identification Through ACF Patterns}
    \vspace{-0.3cm}
    \begin{center}
        \includegraphics[width=0.92\textwidth, height=0.60\textheight, keepaspectratio]{ch2_motivation_acf.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
...
\begin{frame}{Recap: Stationarity}
    ...
    \begin{exampleblock}{Why Stationarity Matters for ARMA}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \end{itemize}
            \item \textcolor{Crimson}{Non-stationary data} $\Rightarrow$ difference first (ARIMA, Ch. 3)
        \end{itemize}
    \end{exampleblock}
    \vspace{-0.2cm}
    \begin{alertblock}{Chapter Objective}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Parametric models for stationary series $\Rightarrow$ combining dependence on past observations (AR) with the influence of random shocks (MA)
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}{The Lag Operator: Visual Illustration}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.58\textheight, keepaspectratio]{lag_operator.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\scriptsize
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Key Characteristics}
            \begin{itemize}\setlength{\itemsep}{2pt}
                \item \textbf{Left}: Random fluctuations, no patterns, constant variance
                \item \textbf{Right}: ACF only a spike at lag 0; others within significance bounds $\Rightarrow$ no linear dependence
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{The White Noise Process}
    ...
    \begin{block}{Properties}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Unpredictable}: White noise is \textit{not} predictable $\Rightarrow$ it is purely random
        \end{itemize}
    \end{block}
    }
\end{frame}
...
\begin{frame}{AR(1): Visual Illustration}
    ...
    \begin{block}{Visual Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Larger $|\phi|$ $\Rightarrow$ greater persistence, slower reversion
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{AR(1) Simulations: Effect of $\phi$}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Different values of $\phi$ produce distinct behaviors: larger $|\phi|$ $\Rightarrow$ more persistence
...
\begin{frame}{Theoretical AR(1) ACF}
    ...
    \begin{block}{ACF Pattern}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Formula}: $\rho(h) = \phi^h$ $\Rightarrow$ exponential decay
...
\begin{frame}{AR(1) ACF and PACF: Theory vs Sample}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{PACF}: A single spike at lag 1, then cuts off $\Rightarrow$ identifies AR(1)
...
\begin{frame}{AR(1) Variance as a Function of $\phi$}
    ...
    \begin{block}{Observations}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item As $|\phi| \to 1$, the variance explodes $\Rightarrow$ non-stationarity
...
\begin{frame}{Proof: AR(1) Autocorrelation Function}
    ...
    \begin{exampleblock}{Proof}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item For $h \geq 1$: $\E[\varepsilon_t X_{t-h}] = 0$ $\Rightarrow$ $\gamma(h) = \phi \gamma(h-1)$
...
\begin{frame}{Durbin-Levinson Algorithm for PACF}
    \begin{cminipage}{0.95\textwidth}
        \footnotesize
        \begin{block}{Durbin-Levinson Recursion}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Computes PACF ($\pi_k$) recursively, without inverting the Toeplitz matrix:
            \end{itemize}
            \vspace{-2mm}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                \item \textbf{Initialize}: $\pi_1 = \hat{\rho}(1)$, $v_1 = \hat{\gamma}(0)(1 - \pi_1^2)$
                \item \textbf{Recursion} ($k = 2, 3, \ldots$):
                \vspace{-2mm}
                \[
                    \pi_k = \frac{\hat{\rho}(k) - \sum_{j=1}^{k-1} \phi_{k-1,j}\, \hat{\rho}(k-j)}{1 - \sum_{j=1}^{k-1} \phi_{k-1,j}\, \hat{\rho}(j)}
                \]
                \vspace{-4mm}
                \item \textbf{Update}: $\phi_{k,j} = \phi_{k-1,j} - \pi_k\, \phi_{k-1,k-j}$ for $j = 1, \ldots, k-1$; $\phi_{k,k} = \pi_k$
                \item \textbf{Prediction variance}: $v_k = v_{k-1}(1 - \pi_k^2)$
            \end{enumerate}
        \end{block}
        \vspace{-0.3cm}
        \begin{columns}[T]
            \begin{column}{0.48\textwidth}
                \begin{exampleblock}{Complexity}
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item $O(k^2)$ vs $O(k^3)$ (direct inversion)
                        \item Exploits Toeplitz structure of $\boldsymbol{\Gamma}_k$
                        \item Guarantees $v_k > 0$ if stationary
                    \end{itemize}
                \end{exampleblock}
            \end{column}
            \begin{column}{0.48\textwidth}
                \begin{alertblock}{AR($p$) Identification}
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item $\pi_k = 0$ for $k > p$ $\Rightarrow$ order is $p$
                        \item CI: $|\pi_k| > 1.96/\sqrt{T}$ $\Rightarrow$ significant
                        \item Equivalent to $t$-test on last OLS coeff.
                    \end{itemize}
                \end{alertblock}
            \end{column}
        \end{columns}
...
\begin{frame}{AR(p): Visual Illustration}
    ...
    \begin{block}{Observations}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item PACF cuts off after lag 2 $\Rightarrow$ key identification pattern
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{AR(2) Stationarity: Unit Circle Visualization}
    \begin{center}
        \includegraphics[width=0.85\textwidth, height=0.48\textheight, keepaspectratio]{unit_circle_stationarity.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\scriptsize
    \begin{block}{Characteristic Polynomial and Unit Circle Condition}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Characteristic polynomial} of AR($p$): $\phi(z) = 1 - \phi_1 z - \phi_2 z^2 - \cdots - \phi_p z^p$
...
\begin{frame}{The AR(2) Stationarity Triangle}
    ...
    \begin{block}{Stationarity Region}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Points outside the region $\Rightarrow$ non-stationary or explosive processes
        \end{itemize}
    \end{block}
    }
    \quantlet{TSA\_ch2\_stationarity}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_stationarity}
...
\begin{frame}{MA(1): Visual Illustration}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.92\textwidth, height=0.58\textheight, keepaspectratio]{ch2_def_ma1.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
    \begin{block}{Visual Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Left panel}: MA(1) series $\Rightarrow$ rapid mean reversion
...
\begin{frame}{Proof: MA(1) Variance and Autocovariance}
    ...
    \begin{alertblock}{Autocovariance at lag $h \geq 2$}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item No common $\varepsilon$ terms $\Rightarrow$ $\gamma(h) = 0$
        \end{itemize}
    \end{alertblock}
    }
\end{frame}
...
\begin{frame}{Proof: Maximum ACF for MA(1)}
    ...
    \begin{block}{Claim: $|\rho(1)| \leq 0.5$ for any value of $\theta$}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Differentiate: $\frac{d\rho(1)}{d\theta} = \frac{1-\theta^2}{(1+\theta^2)^2} = 0$ $\Rightarrow$ $\theta = \pm 1$
...
\begin{frame}{MA(1) Simulations: Effect of $\theta$}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item MA(1) is always stationary regardless of $\theta$ $\Rightarrow$ finite memory of only one lag
...
\begin{frame}{MA(1) ACF and PACF: Visual Comparison}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{ACF}: A single spike at lag 1, then cuts off $\Rightarrow$ key MA(1) signature
            \item \textbf{PACF}: Exponential decay $\Rightarrow$ opposite pattern to AR(1)
...
\begin{frame}{Invertibility of MA Models}
    ...
    \begin{exampleblock}{Why Invertibility Matters}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Stationarity} $\Rightarrow$ AR; \textbf{Invertibility} $\Rightarrow$ MA
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{MA(q): Visual Illustration}
    ...
    \begin{block}{Observation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item MA(3) process: key signature $\Rightarrow$ ACF cuts off after lag $q$ ($\rho(h) = 0$ for $h > 3$)
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{ARMA: Visual Illustration}
    ...
    \begin{block}{ARMA(1,1) Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Neither ACF nor PACF cuts off $\Rightarrow$ key identifier for mixed models
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{ARMA Model Structure}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.58\textheight, keepaspectratio]{arma_structure.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\scriptsize
...
\begin{frame}{How ARMA Simulation Works}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.75\textwidth, height=0.58\textheight, keepaspectratio]{arma_simulation_steps.pdf}
    \end{center}
    \vspace{-0.3cm}
    \begin{block}{Steps}
        \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Stationarity and Invertibility Summary}
    ...
    \begin{exampleblock}{Implications}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Causal representation}: $X_t$ depends only on \textit{past} shocks $\Rightarrow$ necessary for forecasting
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Wold's Decomposition Theorem}
    \begin{center}
        \includegraphics[width=0.70\textwidth, height=0.55\textheight, keepaspectratio]{wold_representation.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
...
\begin{frame}{The Box-Jenkins Methodology}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.58\textheight, keepaspectratio]{box_jenkins_flowchart.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\scriptsize
    \begin{block}{Iterative Approach}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Identification $\Rightarrow$ estimation $\Rightarrow$ validation; repeat until residuals are white noise
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{ACF/PACF Identification Rules}
    ...
    \vspace{-0.3cm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.24\textheight, keepaspectratio]{model_identification_table.pdf}
    \end{center}
    \quantlet{TSA\_ch2\_acf\_pacf\_patterns}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_acf_pacf_patterns}
\end{frame}
...
\begin{frame}{AIC vs BIC: Model Selection}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.58\textheight, keepaspectratio]{aic_bic_comparison.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\scriptsize
    \begin{block}{Interpretation}
...
\begin{frame}{Information Criteria}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{AIC (Akaike)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\text{AIC} = -2\ln(\hat{\mathcal{L}}) + 2k$
        ...
        \begin{block}{BIC (Bayesian)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\text{BIC} = -2\ln(\hat{\mathcal{L}}) + k\ln(n)$
    ...
    \end{columns}
    \vspace{0.1cm}
    {\scriptsize \textbf{where:} $\hat{\mathcal{L}}$ = maximum of the likelihood function, $k$ = number of estimated parameters, $n$ = sample size}
    \vspace{0.1cm}
...
\begin{frame}{Parsimony Principle: Bias-Variance Trade-off}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.55\textheight, keepaspectratio]{parsimony_principle.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\scriptsize
    \begin{block}{Bias-Variance Trade-off}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Too simple model $\Rightarrow$ high bias (underfitting)
            \item Too complex model $\Rightarrow$ high variance (overfitting)
...
\begin{frame}{Automatic Model Selection}
    ...
    \begin{alertblock}{Caution}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Automatic selection is not the final answer $\Rightarrow$ verify model validity
            \item Full Auto-ARIMA (including selection of $d$) $\Rightarrow$ Chapter 3
        \end{itemize}
    \end{alertblock}
    }
\end{frame}
...
\begin{frame}{Estimation Methods}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.45\textheight, keepaspectratio]{estimation_comparison.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\scriptsize
    \begin{block}{The Three Main Approaches}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Yule-Walker}: closed-form, AR only; equates sample autocorrelations with theoretical values
            \item \textbf{MLE}: most efficient and consistent; requires distributional assumption (Gaussian)
            \item \textbf{Conditional Least Squares}: compromise; minimizes sum of squared residuals
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{The Yule-Walker Equations for AR(p)}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.58\textheight, keepaspectratio]{yule_walker.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\scriptsize
...
\begin{frame}{Proof: The Yule-Walker Equations}
    ...
    \begin{block}{Goal: Derive $\rho(k) = \phi_1\rho(k-1) + \cdots + \phi_p\rho(k-p)$}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item For $k \geq 1$: $\E[\varepsilon_t X_{t-k}] = 0$ $\Rightarrow$ $\gamma(k) = \phi_1 \gamma(k-1) + \cdots + \phi_p \gamma(k-p)$
...
\begin{frame}{Maximum Likelihood Estimation}
    ...
    \begin{alertblock}{In Practice}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \texttt{statsmodels.tsa.arima.model.ARIMA} $\Rightarrow$ implements exact MLE with automatic initialization
        \end{itemize}
    \end{alertblock}
    }
\end{frame}
...
\begin{frame}{Standard Errors and Inference}
    ...
    \begin{block}{Asymptotic Distribution of MLE}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}\setlength{\parsep}{0pt}
            ...
            \item $\mathbf{I}(\bm{\theta}) = -E\!\left[\frac{\partial^2 \ln L(\bm{\theta})}{\partial \bm{\theta} \, \partial \bm{\theta}'}\right]$ $\Rightarrow$ average curvature of the log-likelihood
    ...
    \begin{exampleblock}{What is the Standard Error (SE)?}
        \vspace{-0.1cm}
        \begin{itemize}\setlength{\itemsep}{0pt}\setlength{\parsep}{0pt}
            \item $SE(\hat{\theta}_j) = \sqrt{\hat{\mathbf{V}}_{jj}} = \sqrt{\text{diag}_j\!\left(\frac{1}{n}\hat{\mathbf{I}}^{-1}\right)}$ $\Rightarrow$ measures estimation uncertainty
...
\begin{frame}{Residual Diagnostics}
    ...
    \vspace{-0.3cm}
    \begin{center}
        \includegraphics[width=0.82\textwidth, height=0.45\textheight, keepaspectratio]{ch2_diagnostics.pdf}
    \end{center}
    \vspace{-0.4cm}
    {\scriptsize
    \begin{block}{If the model is correctly specified, residuals must be white noise}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Residual ACF}: no significant spikes $\Rightarrow$ white noise
            \item \textbf{Q-Q plot}: points on diagonal $\Rightarrow$ normal; heavy tails $\Rightarrow$ non-normal errors
        \end{itemize}
    \end{block}
    \vspace{-0.4cm}
    \begin{alertblock}{Decision}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textcolor{Forest}{$\checkmark$ All checks OK} $\Rightarrow$ adequate model \qquad \textcolor{Crimson}{$\times$ Not satisfied} $\Rightarrow$ return to identification
        \end{itemize}
    \end{alertblock}
    }
    \end{cminipage}
...
\begin{frame}{The Ljung-Box Test: Visual Illustration}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Left}: good model $\Rightarrow$ white noise residuals
            \item \textbf{Right}: inadequate model $\Rightarrow$ residual autocorrelation $\Rightarrow$ re-specification needed
        \end{itemize}
    \end{block}
    }
    \quantlet{TSA\_ch2\_diagnostics}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_diagnostics}
...
\begin{frame}{The Ljung-Box Test}
    ...
    \begin{alertblock}{Decision}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textcolor{Forest}{p-value $> 0.05$} $\Rightarrow$ do not reject $H_0$ $\Rightarrow$ residuals are white noise
            \item \textcolor{Crimson}{p-value $< 0.05$} $\Rightarrow$ residual autocorrelation $\Rightarrow$ inadequate model
        \end{itemize}
    \end{alertblock}
    }
    \quantlet{TSA\_ch2\_diagnostics}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_diagnostics}
...
\begin{frame}{ARMA Forecast with Confidence Intervals}
    ...
    \begin{block}{Observation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item The confidence band widens with the horizon $\Rightarrow$ convergence to the unconditional interval
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{AR(1) Forecast: Mean Reversion}
    ...
    \begin{block}{Properties}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Larger $|\phi|$ $\Rightarrow$ slower reversion; CIs widen with the horizon
        \end{itemize}
    \end{block}
    }
    \quantlet{TSA\_ch2\_forecasting}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_forecasting}
...
\begin{frame}{Forecast Error Variance by Horizon}
    ...
    \begin{block}{Observation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item MSFE increases monotonically with horizon $h$ $\Rightarrow$ convergence to $\Var(X_t)$ (predictability limit)
        \end{itemize}
    \end{block}
    }
    \quantlet{TSA\_ch2\_forecasting}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_forecasting}
...
\begin{frame}{Confidence Intervals for Forecasts}
    ...
    \begin{exampleblock}{Properties}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \begin{itemize}
                \item Larger AR coefficients $\Rightarrow$ wider intervals
            \end{itemize}
...
\begin{frame}{Rolling Window Forecast}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.60\textheight, keepaspectratio]{ch2_rolling_forecast.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
...
\begin{frame}{Rolling vs Multi-Step Forecast}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.60\textheight, keepaspectratio]{ch2_rolling_vs_multistep.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
...
\begin{frame}{Real Data Application: Forecast Comparison}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.60\textheight, keepaspectratio]{real_data_forecast_comparison.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
...
\begin{frame}{Case Study: Sunspots}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.60\textheight, keepaspectratio]{ch2_case_raw_data.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
...
\begin{frame}{Step 1: ACF/PACF Analysis}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.60\textheight, keepaspectratio]{ch2_case_acf_pacf.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
    \begin{exampleblock}{Identification}
        \begin{itemize}
            \item Sinusoidal ACF (AR); PACF with spikes at lags 1, 2, 9 $\Rightarrow$ AR(2) or AR(9); stationary series ($d=0$)
        \end{itemize}
    \end{exampleblock}
    }
    \quantlet{TSA\_ch2\_case\_study}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_case_study}
...
\begin{frame}{Step 2: Model Comparison}
    ...
    \begin{alertblock}{Model Selection}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Compare multiple candidate models using AIC; the \textbf{AR(9)} has the lowest AIC, capturing the 11-year solar cycle
        \end{itemize}
    \end{alertblock}
    }
    \quantlet{TSA\_ch2\_case\_study}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_case_study}
...
\begin{frame}{Step 4: Forecasting}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.60\textheight, keepaspectratio]{ch2_case_forecast.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
...
\begin{frame}{Key Takeaways}
    ...
    \begin{block}{Chapter Summary}
        \begin{itemize}\setlength{\itemsep}{1pt}
            ...
            \item \textbf{Box-Jenkins}: Identification $\Rightarrow$ Estimation $\Rightarrow$ Validation $\Rightarrow$ Forecasting
...
\begin{frame}{AI Exercise: Critical Thinking}
    ...
    \begin{block}{\footnotesize Prompt to test in ChatGPT / Claude / Copilot}
        {\footnotesize
        ``Download monthly US Industrial Production Index from FRED (series INDPRO) for 2010-01 to 2024-12 (180 observations). Compute monthly log-differences (growth rates). Estimate an ARMA model, perform residual diagnostics, and forecast 12 months ahead. Give me complete Python code with plots.''
        }
    \end{block}
    \vspace{-2mm}
    {\footnotesize
    ...
    \begin{enumerate}\setlength{\itemsep}{0pt}
        ...
        \item Does it verify stationarity \textit{before} estimating ARMA? Justify.
        \item How does it choose the orders $p$ and $q$? Does it use ACF/PACF or AIC/BIC?
        \item Are residuals tested correctly? (Ljung-Box, Q-Q plot, heteroscedasticity)
        \item Do forecast confidence intervals converge to the unconditional mean?
    \end{enumerate}
    }
    \vspace{-2mm}
    \begin{alertblock}{}
...
\begin{frame}{Question 1: Answer}
    ...
    \hfill\quantlet{TSA\_ch2\_ar1}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_ar1}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Question 2: Answer}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item ACF cuts off $\Rightarrow$ MA process
        \item PACF decays $\Rightarrow$ confirms MA(1)
    \end{itemize}
    \end{exampleblock}
    }
    \hfill\quantlet{TSA\_ch2\_ma1}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_ma1}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Question 3}
    ...
    \begin{block}{Answer Choices}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} Yes, MA processes are always invertible\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} Yes, because $1.5 > 0$\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} No, because $|\theta| = 1.5 > 1$\\[3pt]

        \textcolor{MainBlue}{\textbf{(D)}} No, MA processes are never invertible
...
\begin{frame}{Question 4: Answer}
    ...
    \hfill\quantlet{TSA\_ch2\_arma}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_arma}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Question 5: Answer}
    ...
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.98\textwidth, height=0.58\textheight, keepaspectratio]{lag_operator.pdf}
    \end{center}
    \vspace{-3mm}
    {\small
    \begin{exampleblock}{Answer: (B)}
    ...
    \hfill\quantlet{TSA\_ch2\_lag\_operator}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_lag_operator}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Question 6: Answer}
    ...
    \hfill\quantlet{TSA\_ch2\_model\_selection}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_model_selection}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Question 7}
    ...
    \begin{block}{Answer Choices}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} The model is adequate, residuals are white noise\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} The model is inadequate, residuals have autocorrelation\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} You need to increase the sample size\\[3pt]
...
\begin{frame}{Question 7: Answer}
    ...
    \hfill\quantlet{TSA\_ch2\_diagnostics}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_diagnostics}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Question 8}
    ...
    \begin{block}{Answer Choices}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} Forecasts increase without bound\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} Forecasts oscillate indefinitely\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} Forecasts converge to the unconditional mean $\mu$\\[3pt]

        \textcolor{MainBlue}{\textbf{(D)}} Forecasts become more precise
...
\begin{frame}{Question 8: Answer}
    ...
    \hfill\quantlet{TSA\_ch2\_forecasting}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_forecasting}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Question 9}
    ...
    \begin{block}{Answer Choices}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} 4.0\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} 5.56\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} 6.25\\[3pt]
...
\begin{frame}{Question 10}
    ...
    \begin{block}{Answer Choices}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} 0.50\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} 0.40\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} 0.25\\[3pt]
...
\begin{frame}{Question 11}
    ...
    \begin{block}{Answer Choices}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} It cuts off after lag 1\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} Exponential decay starting from lag 1, with $\rho(1) \neq \phi$\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} It is zero for all lags\\[3pt]

        \textcolor{MainBlue}{\textbf{(D)}} It exactly follows the pattern $\phi^h$ for all $h \geq 0$
...
\begin{frame}{Question 12}
    ...
    \begin{block}{Answer Choices}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} Yes, it is stationary\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} No, because $\phi_1 + \phi_2 = 1.1 > 1$\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} Cannot be determined without data\\[3pt]

        \textcolor{MainBlue}{\textbf{(D)}} Depends on the value of $\sigma^2$
...
\begin{frame}{Question 12: Answer}
    ...
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.98\textwidth, height=0.50\textheight, keepaspectratio]{ch2_quiz_ar2_check.pdf}
    \end{center}
    \vspace{-3mm}
    {\footnotesize
    \begin{exampleblock}{Answer: (B)}
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item Necessary conditions for AR(2) stationarity: $\phi_1 + \phi_2 < 1$, \; $\phi_2 - \phi_1 < 1$, \; $|\phi_2| < 1$
        \item Here $0.8 + 0.3 = 1.1 > 1$ $\Rightarrow$ the first condition is violated
    \end{itemize}
    \end{exampleblock}
    }
    \hfill\quantlet{TSA\_ch2\_ar2}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_ar2}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Question 13}
    ...
    \begin{block}{Answer Choices}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} Any time series is an AR process\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} Any stationary process can be written as MA($\infty$): $X_t = \sum_{j=0}^{\infty}\psi_j\varepsilon_{t-j}$\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} Any process has finite variance\\[3pt]

        \textcolor{MainBlue}{\textbf{(D)}} ARMA models are always invertible
...
\begin{frame}{Question 14}
    ...
    \begin{block}{Answer Choices}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} It remains constant\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} It decreases to zero\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} It grows toward $2 \times 1.96 \times \sqrt{1/(1-0.81)} \approx 9.0$\\[3pt]

        \textcolor{MainBlue}{\textbf{(D)}} It grows to infinity
...
\begin{frame}{Data Sources and Software}
    \begin{block}{Software Packages}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \texttt{statsmodels} $\Rightarrow$ Statistical models for Python, including ARIMA
            \item \texttt{pmdarima} $\Rightarrow$ Automatic ARIMA selection for Python
            \item \texttt{scipy} $\Rightarrow$ Optimization and statistical functions
            \item \texttt{numpy}, \texttt{pandas} $\Rightarrow$ Data manipulation
            \item \texttt{matplotlib} $\Rightarrow$ Visualization
        \end{itemize}
    \end{block}
...
\begin{frame}{Bibliography II}
    ...
    \begin{exampleblock}{Online Resources and Code}
        ...
        \begin{itemize}
            ...
            \item \textbf{GitHub TSA}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch2} -- Python code for this chapter
        \end{itemize}
        }
    \end{exampleblock}
\end{frame}
