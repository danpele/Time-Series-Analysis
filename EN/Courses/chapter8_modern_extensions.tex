...
\begin{frame}{From Biological to Artificial Neurons}
    ...
    \begin{block}{The Analogy}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Dendrites} $\succ$ \textbf{Inputs} $x_i$ \quad \textbf{Synapses} $\succ$ \textbf{Weights} $w_i$ \quad \textbf{Soma} $\succ$ \textbf{Sum + Activation} \quad \textbf{Axon} $\succ$ \textbf{Output} $y$
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Bitcoin: Price Evolution and Returns}
    ...
    \begin{block}{Key Observations}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Exponential price growth $\succ$ strongly \textbf{leptokurtic} distribution
                \item Daily returns: mean $\approx 0.15\%$, volatility $\approx 3.5\%$
                \item Volatility clustering evident $\succ$ crisis periods (2018, 2020, 2022)
...
\begin{frame}{Bitcoin: ACF and Evidence for Long Memory}
    ...
    \begin{block}{ACF Analysis}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ACF of returns: rapid decay $\succ$ short memory in the mean
            \item ACF of squared returns: \textbf{slow, hyperbolic} decay
                ...
                \end{itemize}
            \item ARFIMA on volatility $>$ ARMA $\succ$ captures shock persistence
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Energy: Why Prophet and TBATS?}
    ...
    \begin{block}{Solution: Models with Multiple Seasonality}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{TBATS}: periods $[24, 168, 8766]$ $\succ$ Fourier for each season
                \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Prophet}: additive/multiplicative seasonality + regressors
                ...
                \end{itemize}
            \item \textbf{Classical ARIMA}: can handle only 1 season $\succ$ MAPE $\approx 8$--$10\%$
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Energy: Conclusions and Practical Recommendations}
    ...
    \begin{block}{Lessons Learned}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item TBATS: best \textbf{out-of-the-box} $\succ$ no hyperparameter tuning needed
        \end{itemize}
    \end{block}
    \vspace{0.1cm}
    ...
    {\footnotesize
    \begin{exampleblock}{}
        \textit{Full details on Prophet and TBATS $\succ$ Chapter 9}
    \end{exampleblock}
    }
    \end{cminipage}
\end{frame}
...
\begin{frame}{Practical Summary: Model Selection}
    ...
    \begin{block}{Recommended Workflow}
        \begin{enumerate}\setlength{\itemsep}{0pt}
            ...
            \item Test for \textbf{long memory} $\succ$ ARFIMA if $d$ is significant
            \item Add \textbf{features} $\succ$ Random Forest
            \item Only with lots of data and resources $\succ$ LSTM
        \end{enumerate}
    \end{block}
    \end{cminipage}
\end{frame}
