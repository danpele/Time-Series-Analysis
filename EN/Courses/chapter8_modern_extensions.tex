...
%=============================================================================
\section{Foundation Models: EUR/RON Use Case}
%=============================================================================

\begin{frame}{Foundation Models: Zero-Shot on EUR/RON}
    \vspace{-0.2cm}
    {\footnotesize
    \begin{block}{Interpretation}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Data}: EUR/RON (2019--2025) --- Classical models vs Chronos/TimesFM (zero-shot, no training)
                \item \textbf{Left}: RMSE and MAE comparison across all models
                \item \textbf{Right}: Relative improvement vs ARIMA baseline
            \end{itemize}
        \end{block}
    }
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.50\textheight, keepaspectratio]{ch8_foundation_comparison.pdf}
    \end{center}
    \quantlet{TSA\_ch8\_foundation\_comparison}{https://github.com/QuantLet/TSA/tree/main/TSA_ch8/TSA_ch8_foundation_comparison}
\end{frame}

\begin{frame}{Chronos and TimesFM: Prediction Detail}
    \vspace{-0.2cm}
    {\footnotesize
    \begin{block}{Interpretation}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Chronos} (Amazon): T5-based, tokenises values into discrete bins; 80\% confidence interval shown
                \item \textbf{TimesFM} (Google): decoder-only with patching; zero-shot point forecast
                \item Both models applied \emph{without any training} on EUR/RON data --- pure transfer learning
            \end{itemize}
        \end{block}
    }
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.50\textheight, keepaspectratio]{ch8_foundation_predictions.pdf}
    \end{center}
...
\begin{frame}{Final Comparison: Classical vs Foundation Models}
    \begin{cminipage}{0.95\textwidth}
    \begin{center}
    {\small
    \begin{tabular}{l|c|c|c|c}
        ...
        MLP/LSTM & 0.0162 & 0.0072 & 0.52 & No \\
        \midrule
        Chronos (zero-shot) & \multicolumn{4}{c}{\textit{Run script to obtain results}} \\
        TimesFM (zero-shot) & \multicolumn{4}{c}{\textit{Run script to obtain results}} \\
        \bottomrule
    \end{tabular}
    }
    \end{center}
    {\scriptsize \textit{Foundation model rows populated by \texttt{generate\_ch8\_eurron\_foundation.py} when packages are installed.}}

    \vspace{0.1cm}

    \begin{alertblock}{Key Takeaways}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Foundation models offer \textbf{zero-shot} forecasting --- no training pipeline needed
            \item On efficient markets (EUR/RON), classical ARIMA is a \textbf{strong baseline}
            \item Foundation models shine on \textbf{cross-domain} tasks and rapid prototyping
        \end{itemize}
    \end{alertblock}
    \end{cminipage}
\end{frame}
