...
\begin{frame}{Learning Objectives}
    ...
    \begin{block}{By the end of this chapter, you will be able to:}
        \begin{itemize}\setlength{\itemsep}{2pt}
            ...
            \item[\textcolor{MainBlue}{\textbf{6.}}] \textbf{Evaluate} foundation models (Chronos, TimesFM) and the zero-shot paradigm
            \item[\textcolor{MainBlue}{\textbf{7.}}] \textbf{Compare} classical vs ML vs foundation model performance
            \item[\textcolor{MainBlue}{\textbf{8.}}] \textbf{Choose} the appropriate method based on data characteristics
        \end{itemize}
    \end{block}
    \end{cminipage}
\end{frame}
...
\begin{frame}{From Classical Models to Machine Learning}
    ...
    \begin{alertblock}{Four Paradigm Shifts}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \end{itemize}
            \item \textbf{Foundation Models}: TimeGPT, Chronos --- zero-shot forecasting
        \end{itemize}
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{When to Use Each Method?}
    ...
    \begin{tabular}{l|c|c|c|c}
        ...
        \midrule
        Long memory & $\times$ & $\checkmark$ & $\sim$\footnote{\scriptsize RF captures persistence through lag features but does not explicitly model long memory structure.} & $\checkmark$ \\
...
\begin{frame}{What is Long Memory?}
    ...
    \begin{block}{Short Memory (ARMA)}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Finite sum: $\sum_{k=0}^{\infty} |\gamma(k)| < \infty$ --- shock effects disappear quickly
        \end{itemize}
    \end{block}
    \vspace{-1mm}
    \begin{alertblock}{Long Memory (ARFIMA)}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Autocorrelations decay \textbf{hyperbolically}: $\rho_k \sim C \cdot k^{2d-1}$ as $k \to \infty$
            \item Infinite sum: $\sum_{k=0}^{\infty} |\rho_k| = \infty$ --- shock effects persist for a long time
        \end{itemize}
    \end{alertblock}
    \vspace{-1mm}
...
\begin{frame}{The ARFIMA(p,d,q) Model}
    ...
    \begin{defn}[ARFIMA --- Granger \& Joyeux (1980), Hosking (1981)]
        ...
        where $d \in \mathbb{R}$ is the \textbf{fractional differencing parameter} (stationarity requires $d < 0.5$).
    \end{defn}
...
\begin{frame}{Interpreting the Parameter $d$}
    ...
    \begin{tabular}{c|l|l}
        ...
        $d = 1$ & Very slow sample ACF decay & Random walk \\
        \bottomrule
    \end{tabular}
    \end{center}
    ...
    \begin{block}{Hurst Parameter $H$}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $H = 0.5$: No long memory (standard ARMA behavior)
...
\begin{frame}{Hurst Exponent: Visual Interpretation}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{H $<$ 0.5}: Mean-reverting \quad \textbf{H = 0.5}: No long memory \quad \textbf{H $>$ 0.5}: Persistent
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{EUR/RON Long Memory Analysis}
    ...
    \begin{block}{Interpretation}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Returns}: $H \approx 0.55$, $d \approx 0.05$ -- near boundary, essentially short memory
                \item \textbf{Squared returns}: $H \approx 0.69$, $d \approx 0.19$ -- long memory in volatility
            \end{itemize}
        \end{block}
    }
    \begin{center}
...
\begin{frame}{Estimating the Parameter $d$}
    ...
    \begin{exampleblock}{Implementation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item In Python: \texttt{arch} package, \texttt{statsmodels.tsa.arfima.ARFIMA} with \texttt{order=(p,d,q)} where $d$ can be fractional
        \end{itemize}
    \end{exampleblock}
    }
    }
...
%=============================================================================
\section{Foundation Models for Time Series}
%=============================================================================

\begin{frame}{Foundation Models for Time Series}
    \vspace{-4mm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{Paradigm: massive pre-training, zero-shot or fine-tuning}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Analogous to GPT/BERT: pre-trained on massive corpus, usable directly without retraining
            \end{itemize}
        \end{block}
        \vspace{-3mm}
        \begin{block}{TimeGPT (Garza \& Mergenthaler-Canseco, 2024)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{100+ billion} time points (finance, energy, retail); Transformer; \emph{zero-shot} via API
            \end{itemize}
        \end{block}
        \vspace{-3mm}
        \begin{block}{Chronos (Ansari et al., 2024, Amazon)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Tokenises values into discrete bins; T5; Mini (20M) $\to$ Large (710M); \emph{zero-shot}
            \end{itemize}
        \end{block}
        \vspace{-3mm}
        \begin{block}{TimesFM (Google, 2024)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{200M params}, 100B real + synthetic points; decoder-only with patching; \emph{zero-shot}
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{Foundation Models: Advantages, Limits, and Perspectives}
    \vspace{-2mm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{exampleblock}{Advantages}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{No domain expertise needed}: the model has already ``seen'' thousands of series types
                \item \textbf{Instant deployment}: a single API call, no training pipeline required
                \item \textbf{Cross-domain transfer learning}: knowledge transferable across domains
            \end{itemize}
        \end{exampleblock}
        \vspace{-2mm}
        \begin{alertblock}{Limitations}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Black box}: less interpretable --- hard to justify in regulated settings
                \item \textbf{Computational cost} of pre-training (but inference is cheap)
                \item \textbf{Distribution shift sensitivity}: performance degrades on novel regimes
            \end{itemize}
        \end{alertblock}
        \vspace{-2mm}
        \begin{block}{Emerging Models}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Lag-Llama} (Rasul+, 2024), \textbf{Moirai} (Woo+, 2024), \textbf{TimesFM} (Google, 2024)
                \item Complementary to traditional econometrics, not a replacement
                \item \textit{For detailed architectures and comparisons, see \textbf{Chapter 11}}
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{Model Selection Guide}
    ...
    \begin{alertblock}{Trade-off}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ML models offer better accuracy but higher computational cost
...
\begin{frame}{Bitcoin: Price Evolution and Returns}
    ...
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.46\textheight, keepaspectratio]{ch10_bitcoin_overview.pdf} % Cross-reference to Ch. 10
    \end{center}
...
\begin{frame}{Bitcoin: GARCH and Risk Management}
    ...
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.41\textheight, keepaspectratio]{ch10_bitcoin_garch.pdf} % Cross-reference to Ch. 10
    \end{center}
...
\begin{frame}{Energy: Demand Visualization and Multiple Seasonality}
    ...
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.47\textheight, keepaspectratio]{ch9_electricity_demand.pdf} % Cross-reference to Ch. 9
    \end{center}
...
\begin{frame}{Energy: Why Prophet and TBATS?}
    ...
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.43\textheight, keepaspectratio]{ch9_multiple_seasonality.pdf} % Cross-reference to Ch. 9
    \end{center}
...
\begin{frame}{Energy: Prophet Decomposition and Results}
    ...
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.38\textheight, keepaspectratio]{ch9_prophet_vs_tbats.pdf} % Cross-reference to Ch. 9
    \end{center}
...
\begin{frame}{Key Formulas -- Summary}
    ...
    {\scriptsize
            \begin{block}{ARFIMA(p,d,q)}
                $\phi(L)(1-L)^d Y_t = \theta(L)\varepsilon_t$ \quad $d \in \mathbb{R}$: long memory (stationary for $d < 0.5$)
            \end{block}
            \vspace{-0.1cm}
...
\begin{frame}{Long Memory Test Results -- EUR/RON}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{Script Output (\texttt{generate\_ch8\_eurron\_foundation.py})}
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \texttt{Phillips-Perron p-value: 0.0000} (returns are stationary)
            \item \texttt{Hurst Exponent (H): 0.5476}
            \item \texttt{Estimated parameter d: 0.0476}
            \item \texttt{Hurst squared returns: H = 0.6896, d = 0.1896}
        \end{itemize}
        }
    \end{block}
    ...
    \begin{alertblock}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item $H \approx 0.55$: near the boundary, slight persistence
            \item $d \approx 0.05$: essentially \textbf{short memory} -- ARMA may be sufficient
            \item \textbf{Squared returns}: $H \approx 0.69$, $d \approx 0.19$ -- long memory in volatility!
        \end{itemize}
    \end{alertblock}
    \end{cminipage}
...
\begin{frame}{Comparison: Results on EUR/RON}
    ...
    \begin{tabular}{l|c|c|c|c}
        ...
        \midrule
        ARIMA(1,1,1) & 0.0139 & 0.0038 & 24.32 & Yes \\
        ARFIMA(1,$d$,1) & 0.0172 & 0.0088 & 0.16 & Yes \\
        Random Forest & 0.0314 & 0.0104 & 0.58 & Partial \\
        MLP/LSTM & 0.0162 & 0.0072 & 0.52 & No \\
        \bottomrule
    \end{tabular}
    }
    \end{center}
    {\scriptsize \textit{Rolling 1-step-ahead forecast on price levels; 70/15/15 temporal split; $d = 0.048$ (Hurst).}}

    \vspace{0.1cm}

    \begin{alertblock}{Conclusions}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ARIMA achieves the \textbf{lowest RMSE} --- hard to beat on an efficient market
            \item ARFIMA and MLP/LSTM are close, but \textbf{not better} than ARIMA
            \item Random Forest overfits slightly on price levels
            \item For EUR/RON, simplicity wins --- \textbf{parsimony principle} confirmed
        \end{itemize}
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Model Comparison: Performance Metrics}
    ...
    \begin{block}{Interpretation}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Data}: EUR/RON exchange rate (Yahoo Finance, 2019--2025) --- ARIMA vs ARFIMA vs RF vs MLP/LSTM
                \item \textbf{Left}: RMSE (lower = better) --- ARIMA achieves the lowest RMSE
                \item \textbf{Right}: Training time (log scale) --- trade-off between accuracy and computation
            \end{itemize}
        \end{block}
    }
    \begin{center}
...
%=============================================================================
% Foundation Models: EUR/RON Use Case
% (Detailed treatment moved to Chapter 11 — see chapter11_llm_foundation_models.tex)
%=============================================================================

\begin{frame}{Foundation Models: EUR/RON Preview}
    \begin{cminipage}{0.95\textwidth}
    \vspace{-2mm}
    {\footnotesize
    \begin{block}{Quick Results: Zero-Shot on EUR/RON}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item We applied \textbf{Chronos} and \textbf{TimesFM} zero-shot to EUR/RON (2019--2025)
            \item Both models forecast \emph{without any training} --- pure transfer learning
            \item On this efficient market, classical \textbf{ARIMA remains a strong baseline}
        \end{itemize}
    \end{block}
    \vspace{-2mm}
    \begin{center}
    ...
    \end{center}
    \vspace{-2mm}
    \begin{alertblock}{For a comprehensive treatment, see Chapter 11}
        {\scriptsize Chapter 11 provides a deep dive into foundation model architectures (Chronos, TimesFM, Lag-Llama, Moirai, TimeGPT), tokenization strategies, zero-shot vs fine-tuning, and extended use cases across multiple domains.}
    \end{alertblock}
    }
    \end{cminipage}
...
\begin{frame}{Case Study 2: Energy Consumption --- Introduction}
    \begin{cminipage}{0.95\textwidth}
    \vspace{-2mm}
    \begin{block}{Dataset: ENTSO-E Hourly Electricity Consumption}
        {\small
        \begin{itemize}\setlength{\itemsep}{1pt}
            \item \textbf{Source}: ENTSO-E Transparency Platform (European electricity data)
            \item \textbf{Variable}: Total hourly electricity consumption (MW) for a European country
            \item \textbf{Period}: 2015--2020, approximately 2{,}160 daily aggregated observations
            \item \textbf{Frequency}: Daily (aggregated from hourly data)
        \end{itemize}
        }
    \end{block}
    \vspace{-2mm}
    \begin{alertblock}{Forecasting Challenge}
        {\small
        \begin{itemize}\setlength{\itemsep}{1pt}
            \item \textbf{Multiple seasonalities}: weekly (5 working days + weekend) and annual (summer/winter)
            \item \textbf{Nonlinear effects}: temperature, holidays, and economic activity drive consumption
            \item \textbf{Structural changes}: COVID-19 lockdowns caused unprecedented demand drops in 2020
            \item \textbf{Goal}: Compare classical (SARIMA, ARFIMA) vs.\ ML (Random Forest, LSTM) approaches
        \end{itemize}
        }
    \end{alertblock}
    \vspace{-2mm}
    \begin{exampleblock}{Why This Dataset?}
        {\small Energy forecasting is a key real-world application: grid operators need accurate short-term forecasts to balance supply and demand and avoid blackouts.}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Example 2: BET Index (Bucharest Stock Exchange)}
    ...
    \begin{exampleblock}{Results on BVB.RO Returns (2019--2025, RMSE in \%)}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ARFIMA(1,$d$,1): 1.35 -- captures long memory ($H = 0.67$, $d = 0.17$)
...
\begin{frame}{What Comes Next?}
    ...
    \begin{block}{Extensions and Advanced Topics}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Prophet} (Meta) for seasonality
...
\begin{frame}{Bibliography II}
    \begin{block}{Threshold, Regime-Switching, and Structural Change Models}
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Petropoulos, F., et al. (2022). Forecasting: Theory and Practice, \textit{Int.\ J.\ Forecasting}, 38(3), 845--1054.
        \end{itemize}
        }
    \end{block}
    \vspace{-1mm}
    \begin{exampleblock}{Online Resources and Code}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{GitHub TSA}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch8} -- Python code for this chapter
        \end{itemize}
        }
    \end{exampleblock}
\end{frame}
