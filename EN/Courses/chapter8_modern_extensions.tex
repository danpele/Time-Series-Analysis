...
\begin{frame}{Long Memory: An Intuitive Analogy}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Short Memory (ARMA)}
                \textbf{Analogy}: You only remember the last few sentences.
                \begin{itemize}\setlength{\itemsep}{0pt}
        ...
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Long Memory (ARFIMA)}
                \textbf{Analogy}: An elephant that never forgets.
                \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{ACF Comparison: Short Memory vs Long Memory}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.55\textheight, keepaspectratio]{ch8_acf_comparison.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
...
\begin{frame}{ARIMA vs ARFIMA: Memory Decay Patterns}
    \begin{center}
        \includegraphics[width=0.88\textwidth, height=0.50\textheight, keepaspectratio]{ch8_arima_vs_arfima.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
    \begin{exampleblock}{Interpretation}
...
\begin{frame}{Real Example: Long Memory in Volatility}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.98\textwidth, height=0.48\textheight, keepaspectratio]{ch8_volatility_long_memory.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
    \begin{exampleblock}{Interpretation}
...
\begin{frame}{Local Whittle Estimator (Robinson, 1995)}
    ...
    \begin{block}{Advantages over GPH}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{More efficient}: $\sqrt{m}(\hat{d} - d) \xrightarrow{d} N(0, \frac{1}{4})$ vs $N(0, \frac{\pi^2}{24})$ for GPH
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{exampleblock}{Advantages}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Most efficient} (Cram\'{e}r-Rao bound)
        ...
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Disadvantages}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Requires \textbf{correct specification}
...
\begin{frame}[fragile]{GPH and Hurst Estimation in Python}
    \vspace{-0.3cm}
    \begin{block}{Hurst Exponent via R/S Analysis}
\begin{verbatim}
...
\end{verbatim}
    \end{block}
    \vspace{-0.2cm}
    \begin{block}{GPH Estimator (Simplified)}
\begin{verbatim}
...
\begin{frame}{LSTM Cell Architecture}
    ...
    \begin{block}{Components}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Cell State} ($C_t$): Long-term memory \quad \textbf{Hidden State} ($h_t$): Short-term memory
            \item \textbf{Gates}: \textcolor{IDAred}{forget}, \textcolor{Forest}{add}, \textcolor{AccentBlue}{transmit}
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{LSTM: When to Use It}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Good Choice When:}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Large datasets ($>$ 1000 obs)
                    \item Complex temporal patterns
                    \item Multivariate inputs
                    \item Accuracy over interpretability
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{NOT a Good Choice When:}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Small data ($<$ 500 obs)
                    \item Linear relationships
                    \item Interpretability required
                    \item ARIMA already performs well
                \end{itemize}
            \end{alertblock}
        \end{column}
    \end{columns}
...
\begin{frame}{Time Series Cross-Validation}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.92\textwidth, height=0.50\textheight, keepaspectratio]{ch8_timeseries_cv.pdf}
    \end{center}
    \vspace{-0.2cm}
    {\footnotesize
    \begin{exampleblock}{Interpretation}
...
\begin{frame}{Case Study: Data Overview}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.98\textwidth, height=0.55\textheight, keepaspectratio]{ch8_data_split.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
...
%=============================================================================
\section{Case Study 2: EUR/RON Exchange Rate}
...
\begin{frame}{EUR/RON Exchange Rate Visualization}
    ...
    \begin{exampleblock}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Data}: EUR/RON daily exchange rate (Yahoo Finance, 2019--2025), 80/20 train/test split
            \item \textbf{Level}: Depreciation trend and periods of high volatility
            \item \textbf{Returns}: Volatility clustering (periods of high volatility are followed by similar periods)
        \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{ACF Analysis: Returns vs Squared Returns}
    ...
    \begin{exampleblock}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Left}: ACF of returns $\rightarrow$ rapid decay, no significant autocorrelation after lag 1
            \item \textbf{Right}: ACF of squared returns $\rightarrow$ slow decay indicates \textbf{volatility clustering} (ARCH effects)
        \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{Random Forest: Feature Importance}
    ...
    \begin{exampleblock}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Calendar features have minor impact for daily exchange rate prediction
        \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{LSTM: Learning Curve}
    ...
    \begin{exampleblock}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Validation Loss}: Tracks training loss $\rightarrow$ no severe overfitting
        \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{EUR/RON: Predictions vs Actual Values}
    \vspace{-0.2cm}
    \begin{center}
    ...
    \begin{exampleblock}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Data}: EUR/RON test period --- ARIMA, Random Forest, MLP/LSTM predictions vs actual
...
\begin{frame}{EUR/RON: Model Performance Comparison}
    \vspace{-0.2cm}
    \begin{center}
    ...
    \begin{exampleblock}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Left}: Error metrics (lower = better) $\rightarrow$ RF achieves the lowest RMSE and MAE
            \item \textbf{Right}: Training time (log scale) $\rightarrow$ ML models require more computational resources
        \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{What Comes Next?}
    \begin{block}{Chapter 9: Multiple Seasonalities}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{The Challenge}: Real data often has multiple seasonal patterns (daily + weekly + yearly)
            \item \textbf{TBATS}: Trigonometric seasonality, Box-Cox, ARMA errors, Trend, Seasonal
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Automatic, handles high-frequency data with Fourier terms
            \end{itemize}
...
\begin{frame}{Key References}
    ...
    \begin{block}{Foundational Papers}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Box \& Jenkins (1970). \textit{Time Series Analysis: Forecasting and Control}. Holden-Day.
            \item Granger \& Joyeux (1980). Long-memory time series models. \textit{J.\ Time Series Analysis}, 1(1).
            \item Hosking (1981). Fractional differencing. \textit{Biometrika}, 68(1).
        \end{itemize}
    \end{block}
    \vspace{-0.1cm}
    \begin{block}{Machine Learning Methods}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Hochreiter \& Schmidhuber (1997). Long short-term memory. \textit{Neural Computation}, 9(8).
        \end{itemize}
    \end{block}
    \vspace{-0.1cm}
    \begin{block}{Forecasting Competitions \& Reviews}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Makridakis et al.\ (2018). The M4 Competition. \textit{Int.\ J.\ Forecasting}, 34(4).
            \item Hyndman \& Athanasopoulos (2021). \textit{Forecasting: Principles and Practice}, 3rd ed.
        \end{itemize}
    \end{block}
    }
\end{frame}
...
\begin{frame}{Online Resources and Code}
    \begin{exampleblock}{}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{GitHub TSA\_ch8}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch8}
        \end{itemize}
        }
    \end{exampleblock}
\end{frame}
