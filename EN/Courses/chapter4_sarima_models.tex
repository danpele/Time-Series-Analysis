    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Data Sources}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    ...
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Retail sales, industrial production
                    \end{itemize}
                    ...
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Seasonal macroeconomic data
                    \end{itemize}
        ...
        \begin{column}{0.48\textwidth}
            \begin{exampleblock}{Python}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \texttt{statsmodels} --- SARIMA models
...
\begin{frame}{What We'll Learn Today}
    ...
    \begin{alertblock}{Key Insight}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item SARIMA = ARIMA applied at \textbf{two frequencies}: non-seasonal (short-term) and seasonal (long-term)
        \end{itemize}
    \end{alertblock}
    \end{cminipage}
    }
...
\begin{frame}{Seasonality: Visual Illustration}
    ...
    \begin{exampleblock}{}
    \begin{itemize}
        \item \textbf{Left}: Monthly data with $s=12$ (annual cycle); \textbf{Right}: Quarterly data with $s=4$
        \item The pattern repeats every $s$ periods $\Rightarrow$ this regularity is exploited by SARIMA
    \end{itemize}
    \end{exampleblock}
    }
    \end{cminipage}
...
\begin{frame}{Box-Cox Transformation: Variance Stabilization}
    ...
    \begin{block}{Box-Cox Family of Transformations}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Formula}: $Y_t^{(\lambda)} = \begin{cases} \frac{Y_t^\lambda - 1}{\lambda} & \text{if } \lambda \neq 0 \\ \ln(Y_t) & \text{if } \lambda = 0 \end{cases}$
            \item \textbf{Special cases}: $\lambda=1$ (no transformation), $\lambda=0$ (logarithm), $\lambda=0.5$ (square root)
        \end{itemize}
    \end{block}
    \vspace{-0.1cm}
    \begin{exampleblock}{Automatic Selection of $\lambda$}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Profile likelihood}: maximizes the log-likelihood as a function of $\lambda$
            \item \textbf{Guerrero method (1993)}: minimizes the coefficient of variation of seasonal sub-series
    ...
    \begin{alertblock}{Why not just logarithm?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Log ($\lambda=0$) assumes variance proportional to level --- not always the case
...
\begin{frame}{Seasonal Difference: Visual Illustration}
    ...
    \begin{itemize}
        ...
        \item \textbf{Right}: After $\Delta_{12} = (1-L^{12})$, seasonal pattern is removed
            \begin{itemize}
...
\begin{frame}{Effect of Differencing Operations}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.78\textheight, keepaspectratio]{ch4_differencing_effect.pdf}
    \end{center}
...
\begin{frame}{ACF Before and After Differencing}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.78\textheight, keepaspectratio]{ch4_acf_differencing.pdf}
    \end{center}
...
\begin{frame}{Case Study: Airline Passengers Data}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.78\textheight, keepaspectratio]{ch4_case_raw_data.pdf}
    \end{center}
...
\begin{frame}{Step 1: Transformations}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.78\textheight, keepaspectratio]{ch4_case_transformations.pdf}
    \end{center}
...
\begin{frame}{Step 2: ACF/PACF Analysis}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.78\textheight, keepaspectratio]{ch4_case_acf_pacf.pdf}
    \end{center}
...
\begin{frame}{Step 3: Model Comparison}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.78\textheight, keepaspectratio]{ch4_case_model_comparison.pdf}
    \end{center}
...
\begin{frame}{Step 4: Residual Diagnostics}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.78\textheight, keepaspectratio]{ch4_case_diagnostics.pdf}
    \end{center}
...
\begin{frame}{Step 5: Forecasting}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.78\textheight, keepaspectratio]{ch4_case_forecast.pdf}
    \end{center}
...
\begin{frame}[fragile]{Python Implementation: Complete SARIMA}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{SARIMA Workflow in Python}
        \scriptsize
        \begin{verbatim}
import numpy as np
...
\begin{frame}{Quick Quiz}
    {\small
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{Check Your Knowledge}
        \begin{itemize}\setlength{\itemsep}{4pt}
            \item[\textcolor{MainBlue}{\textbf{1.}}] What is the difference between deterministic and stochastic seasonality?
            \item[\textcolor{MainBlue}{\textbf{2.}}] Why do we apply seasonal differencing $(1-L^{12})$ and not just regular differencing $(1-L)$?
            \item[\textcolor{MainBlue}{\textbf{3.}}] What ACF pattern indicates the need for seasonal differencing?
            \item[\textcolor{MainBlue}{\textbf{4.}}] How do we identify seasonal MA components from ACF?
            \item[\textcolor{MainBlue}{\textbf{5.}}] Why do we use MASE instead of RMSE as the main metric for seasonal data?
        \end{itemize}
    \end{block}
    \end{cminipage}
    }
...
\begin{frame}{Quiz Answers}
    ...
    \begin{exampleblock}{Answers}
        \begin{itemize}\setlength{\itemsep}{1pt}
            \item[\textcolor{MainBlue}{\textbf{1.}}] \textbf{Types}: Deterministic (dummy variables/Fourier); stochastic (seasonal unit root $\rightarrow$ differencing)
            \item[\textcolor{MainBlue}{\textbf{2.}}] \textbf{Differencing}: $(1-L)$ removes trend; $(1-L^{12})$ removes seasonality. They are complementary, not substitutable.
            \item[\textcolor{MainBlue}{\textbf{3.}}] \textbf{ACF}: Persistent spikes at lags $s, 2s, 3s$ that decay slowly $=$ stochastic seasonality
            \item[\textcolor{MainBlue}{\textbf{4.}}] \textbf{SMA}: ACF spike at lag $s$ that cuts off sharply $=$ SMA(1); spikes at $s$ and $2s$ $=$ SMA(2)
            \item[\textcolor{MainBlue}{\textbf{5.}}] \textbf{MASE}: Scale-free, relative to seasonal naive; RMSE depends on data scale
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
    }
...
\begin{frame}{References II}
    ...
    \begin{exampleblock}{Online Resources and Code}
        {\small
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Quantlet}: \url{https://quantlet.com} $\rightarrow$ Code platform for statistics
            \item \textbf{Quantinar}: \url{https://quantinar.com} $\rightarrow$ Learning platform for quantitative methods
            \item \textbf{GitHub TSA}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch4} $\rightarrow$ Python code for this chapter
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
