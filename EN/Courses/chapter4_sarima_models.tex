    ...
    \begin{block}{What does this test do?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Non-parametric test}: checks whether observations from different seasons come from the same distribution
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item No assumptions needed about the shape of the data distribution
                \end{itemize}
            \item \textbf{Mechanism}: ranks all observations from smallest to largest
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Replaces original values with ranks (1, 2, \ldots, $N$)
                \end{itemize}
            \item \textbf{Check}: whether ranks are uniformly distributed across seasons
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Computes sum of ranks per season and compares with expected distribution
                \end{itemize}
            \item \textbf{Conclusion}: if one season consistently has higher/lower ranks $\succ$ seasonality
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Example: if summer always has high ranks $\succ$ seasonal energy consumption
                \end{itemize}
        \end{itemize}
    \end{block}
    ...
    \begin{exampleblock}{Why use it instead of the F-test?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{No normality assumption} -- works with any distribution
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Ideal for financial data with heavy tails or skewed distributions
                \end{itemize}
            \item \textbf{Robust to outliers} -- extreme values do not distort results
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Ranks bound the influence of any single observation
                \end{itemize}
        \end{itemize}
    \end{exampleblock}
    ...
    \begin{block}{Key question}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Problem}: given a seasonal series, we need to determine the type of differencing
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Wrong choice leads to over-differencing or under-differencing
                \end{itemize}
            \item \textbf{Regular differencing} $(1-L)$? $\succ$ set $d=1$; \textbf{Seasonal differencing} $(1-L^s)$? $\succ$ set $D=1$
            \item \textbf{HEGY}: tests for both types of unit roots simultaneously!
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Proposed by Hylleberg, Engle, Granger and Yoo (1990)
                \end{itemize}
        \end{itemize}
    \end{block}
    \vspace{-0.1cm}
    \begin{exampleblock}{Why not just use ADF?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{ADF}: tests only for a regular unit root at frequency zero
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Completely ignores the seasonal components of the series
                \end{itemize}
    ...
    \begin{alertblock}{HEGY tests multiple frequencies}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Quarterly}: tests at 0, $\pi$, $\pm\pi/2$
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item 3 frequencies $\succ$ one regular root + two seasonal roots
                \end{itemize}
            \item \textbf{Monthly}: tests at 0, $\pi$, $\pm\pi/6$, $\pm\pi/3$, $\pm\pi/2$, $\pm 2\pi/3$, $\pm 5\pi/6$
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item 6 frequencies $\succ$ one regular root + five seasonal roots
                \end{itemize}
        \end{itemize}
    \end{alertblock}
    }
...
\begin{frame}{HEGY Test: Auxiliary Regression (Quarterly)}
    ...
    \begin{block}{Transformed variables}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{$z_{1t}$}: $(1+L+L^2+L^3)y_t = y_t + y_{t-1} + y_{t-2} + y_{t-3}$
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Captures the zero-frequency component (trend)
                \end{itemize}
            \item \textbf{$z_{2t}$}: $-(1-L+L^2-L^3)y_t = -y_t + y_{t-1} - y_{t-2} + y_{t-3}$
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Captures the frequency-$\pi$ component (semi-annual alternation)
                \end{itemize}
    ...
    \begin{alertblock}{Hypotheses}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{$H_0: \pi_1=0$}: unit root at frequency 0 $\succ$ requires $d=1$
            \item \textbf{$H_0: \pi_2=0$}: unit root at frequency $\pi$ $\succ$ requires $D=1$
            \item \textbf{$H_0: \pi_3=\pi_4=0$}: unit root at frequency $\pm\pi/2$ $\succ$ requires $D=1$
        \end{itemize}
    \end{alertblock}
    }
    \end{cminipage}
    ...
    \begin{block}{Box-Cox Family of Transformations}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Special cases}: $\lambda=1$ (no transformation), $\lambda=0$ (logarithm), $\lambda=0.5$ (square root)
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item $\lambda < 1$ compresses large values; $\lambda > 1$ amplifies them
                \end{itemize}
        \end{itemize}
    \end{block}
    \vspace{-0.1cm}
    \begin{exampleblock}{Automatic Selection of $\lambda$}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Profile likelihood}: maximizes the log-likelihood as a function of $\lambda$
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Most commonly used method; provides confidence interval for $\lambda$
                \end{itemize}
            \item \textbf{Guerrero method (1993)}: minimizes the coefficient of variation of seasonal sub-series
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Preferred for seasonal data with varying amplitude
                \end{itemize}
    ...
    \begin{alertblock}{Why not just logarithm?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Log ($\lambda=0$) assumes variance proportional to level --- not always the case
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Example: data with moderate variance may require $\lambda = 0.3$, not $\lambda = 0$
                \end{itemize}
