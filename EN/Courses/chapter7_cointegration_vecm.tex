...
\begin{frame}{Spurious Regression: Visual Example}
    ...
    \begin{alertblock}{Warning}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Result}: Two completely independent random walks show high correlation ($R^2 > 0.8$) purely by chance --- in fact, $R^2 \to 1$ as $T \to \infty$ (Phillips, 1986). This is why we need cointegration analysis
        \end{itemize}
    \end{alertblock}
    }
    \begin{center}
...
\begin{frame}{Intuition: Common Stochastic Trends}
    ...
    \begin{alertblock}{Economic Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Variables may deviate in the short run, but are \textit{pulled back}
...
\begin{frame}{Cointegrating Rank}
    ...
    \begin{block}{How Many Cointegrating Relationships?}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item If $r = k$: All variables are $I(0)$ (inconsistent with the $I(1)$ assumption; use VAR in levels)
        \end{itemize}
    \end{block}
...
\begin{frame}{VECM Representation}
    \begin{block}{Vector Error Correction Model}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\bPi = \sum_{i=1}^{p} \bA_i - \mathbf{I}$ (long-run impact); $\bGamma_j$ (short-run dynamics)
        \end{itemize}
    \end{block}
...
\begin{frame}{VECM Structure}
    ...
    \begin{exampleblock}{Components}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item $\alpha_1, \alpha_2$ = adjustment speeds (signs must ensure error correction toward equilibrium; typically opposite in bivariate systems)
