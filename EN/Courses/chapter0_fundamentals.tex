...
\begin{frame}{Why Do We Study Time Series?}
    ...
    \begin{alertblock}{Main objective: forecasting}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item We use historical patterns to predict future values $\succ$ essential for business planning, risk management, and policy decisions
        \end{itemize}
    \end{alertblock}
    }
\end{frame}
        ...
        \begin{exampleblock}{Fundamental Elements}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \begin{itemize}
                    ...
                    \item Measures temporal dependence
                \end{itemize}
            \end{itemize}
        \end{exampleblock}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Types of Patterns}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Trend}
                \begin{itemize}
                    \item Long-term increase or decrease
                \end{itemize}
                \item \textbf{Seasonal}
                \begin{itemize}
                    \item Regular periodic patterns
                \end{itemize}
                \item \textbf{Cyclical}
                \begin{itemize}
                ...
                \item \textbf{Random}
                \begin{itemize}
                    \item Unpredictable fluctuations
                \end{itemize}
            \end{itemize}
        \end{block}
...
\begin{frame}{Practical Example: Real Financial Data}
    \begin{columns}[T]
        ...
        \begin{block}{S\&P 500 (2024)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Daily frequency}
                \begin{itemize}
                    \item $\approx$ 252 trading days/year
                \end{itemize}
                \item \textbf{Observed characteristics}
                \begin{itemize}
                    \item Upward trend
                    \item Volatility clustering
                    \item Persistence (momentum)
                \end{itemize}
            \end{itemize}
        \end{block}
        \column{0.63\textwidth}
...
\begin{frame}{Examples of Time Series Data}
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Real financial data}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Source}: Yahoo Finance (2019--2025)
                \begin{itemize}
                    \item Normalized to base 100
                \end{itemize}
                \item \textbf{Bitcoin}: most volatile
                \item \textbf{Gold}: most stable
            \end{itemize}
        \end{exampleblock}
        \column{0.63\textwidth}
        \vspace{-0.3cm}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Components Explained}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Original}
                \begin{itemize}
                    \item Observed series
                \end{itemize}
                \item \textbf{Trend-Cycle}
                \begin{itemize}
                    \item Long-term movement
                \end{itemize}
                \item \textbf{Seasonal}
                \begin{itemize}
                    \item Periodic pattern
                \end{itemize}
                \item \textbf{Residual}
                \begin{itemize}
                    \item Random noise
...
\begin{frame}{The Multiplicative Decomposition Model}
    ...
    \begin{block}{Model}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Equation}: $X_t = T_t \times S_t \times \varepsilon_t$ $\succ$ components are multiplied
        \end{itemize}
    \end{block}
    ...
    \begin{alertblock}{Tip}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Log transformation}: multiplicative $\succ$ additive: $\log X_t = \log T_t + \log S_t + \log \varepsilon_t$
        \end{itemize}
    \end{alertblock}
    }
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Key Difference}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \begin{itemize}
                    \item Seasonal component is a \textit{ratio}
                    \item Centered at value 1
                \end{itemize}
                \item \textbf{Additive}
                \begin{itemize}
                    \item Seasonal component in \textit{absolute units}
                    \item Centered at value 0
                \end{itemize}
            \end{itemize}
        \end{block}
        \column{0.63\textwidth}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Properties}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Large window} $\succ$ smoother estimate
...
\begin{frame}{Classical Decomposition Algorithm}
    \begin{block}{Steps for Multiplicative Decomposition}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Step 1} $\succ$ \textbf{Estimate Trend}: $\hat{T}_t = MA_s(X_t)$
            \begin{itemize}
                \item Centered moving average of order equal to the seasonal period
            \end{itemize}
            \item \textbf{Step 2} $\succ$ \textbf{Detrend}: $D_t = X_t / \hat{T}_t$
            \item \textbf{Step 3} $\succ$ \textbf{Estimate Seasonal}: $\hat{S}_j = \text{mean}(D_t \text{ for season } j)$
            \item \textbf{Step 4} $\succ$ \textbf{Normalize}: scale so that $\frac{1}{s}\sum_{j=1}^{s} \hat{S}_j = 1$
            \item \textbf{Step 5} $\succ$ \textbf{Compute Residuals}: $\hat{\varepsilon}_t = X_t / (\hat{T}_t \times \hat{S}_t)$
        \end{itemize}
    \end{block}
    ...
    \begin{alertblock}{Note}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \begin{itemize}
                \item Division $\succ$ subtraction
                \item Multiplication $\succ$ addition
            \end{itemize}
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}{Simple Exponential Smoothing: Effect of $\alpha$}
    ...
    \begin{exampleblock}{Trade-off}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Small $\alpha$} $\succ$ smooth forecasts
            \begin{itemize}
                \item More weight on distant history
            \end{itemize}
            \item \textbf{Large $\alpha$} $\succ$ tracks the data
            \begin{itemize}
...
\begin{frame}{Holt's Method: Visualization}
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Interpretation}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Holt's method}: captures level and trend
                \begin{itemize}
                    \item Projects them into the forecast horizon
                \end{itemize}
                \item \textbf{$\alpha$}: controls level changes
                \item \textbf{$\beta^*$}: controls trend changes
            \end{itemize}
        \end{exampleblock}
        \vspace{0.2cm}
...
\begin{frame}{Holt-Winters: Capturing Seasonality}
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Key Feature}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Complete decomposition}
                \begin{itemize}
                    \item Separates level, trend, and seasonal
                \end{itemize}
                \item \textbf{Seasonal forecasts}
                \begin{itemize}
                    \item Includes both trend and periodic pattern
                \end{itemize}
            \end{itemize}
        \end{exampleblock}
...
\begin{frame}{The ETS Framework: Error-Trend-Seasonality}
    ...
    \begin{exampleblock}{Examples}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{ETS(A,N,N)}: Simple Exponential Smoothing $\succ$ level only, no trend or seasonality
            \item \textbf{ETS(A,A,N)}: Holt's Linear Method $\succ$ level + additive trend
            \item \textbf{ETS(A,A,A)}: Additive Holt-Winters $\succ$ level + trend + additive seasonality
        \end{itemize}
    \end{exampleblock}
    }
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Automatic Selection}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \end{itemize}
                \item \textbf{Optimal selection}
                \begin{itemize}
                    \item Balance between fit and complexity
                \end{itemize}
            \end{itemize}
        \end{exampleblock}
        \column{0.66\textwidth}
...
\begin{frame}{Forecast Accuracy Metrics}
    ...
    \begin{alertblock}{What to Use?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Same series}: RMSE, MAE $\succ$ compare models on the same data
            \item \textbf{Across different series}: MAPE, sMAPE $\succ$ percentage metrics, scale-independent
        \end{itemize}
    \end{alertblock}
    }
\end{frame}
...
\begin{frame}{Forecast Evaluation: Visual Example}
    \begin{columns}[T]
        ...
        \begin{block}{Observations}
            \begin{itemize}\setlength{\itemsep}{2pt}
                \item \textbf{Top}: actual vs.\ forecast
                \begin{itemize}
                    \item Visual assessment of forecast quality
                \end{itemize}
                \item \textbf{Bottom}: residuals
                \begin{itemize}
                    \item Zero mean
        ...
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Why Not Standard CV?}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    ...
                    \item \textbf{Standard k-fold} $\succ$ data leakage
                \end{itemize}
            \end{alertblock}
        \end{column}
        \begin{column}{0.48\textwidth}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Interpretation}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Best}: multiplicative Holt-Winters
                \begin{itemize}
                    \item Ideal for data with growing seasonality
                \end{itemize}
            \end{itemize}
        \end{exampleblock}
        \column{0.66\textwidth}
...
\begin{frame}{Forecast Performance Across Different Datasets}
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Interpretation}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Different series}
                \begin{itemize}
                    \item Require different models
                \end{itemize}
                \item \textbf{Seasonal data}
                \begin{itemize}
                    \item Prefer seasonal methods
                \end{itemize}
                \item \textbf{No universal model}
                \begin{itemize}
                    \item Test multiple approaches
                \end{itemize}
            \end{itemize}
        \end{exampleblock}
        \column{0.66\textwidth}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Comparison}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Dummy variables}
                \begin{itemize}
                    \item Capture any shape
                    \item Require $s-1$ parameters
                \end{itemize}
                \item \textbf{Fourier terms}
                \begin{itemize}
                    \item Only $2K$ parameters
                    \item Smooth, sinusoidal patterns
                \end{itemize}
            \end{itemize}
        \end{block}
        \column{0.63\textwidth}
...
\begin{frame}{Detrending Methods: Comparison}
    \begin{columns}[T]
        ...
        \begin{alertblock}{Key Idea}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Different methods}
                \begin{itemize}
                    \item Produce different residuals
                \end{itemize}
                \item \textbf{Choose by trend type}
                \begin{itemize}
                    \item Consider the analysis objectives
                \end{itemize}
            \end{itemize}
        \end{alertblock}
        \column{0.66\textwidth}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Method Comparison}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \begin{itemize}
                    \item Simple but with lag
                \end{itemize}
                \item \textbf{Polynomial regression}
                \begin{itemize}
                    \item Flexible, parametric
                \end{itemize}
                \item \textbf{HP filter}
                \begin{itemize}
                    \item Macroeconomic standard
                \end{itemize}
            \end{itemize}
        \end{exampleblock}
        \column{0.66\textwidth}
...
\begin{frame}{HP Filter: Effect of $\lambda$}
    \begin{columns}[T]
        ...
        \begin{block}{Trade-off}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Small $\lambda$}: flexible trend
                \begin{itemize}
                    \item Follows the data closely
                \end{itemize}
                \item \textbf{Large $\lambda$}: smooth trend
                \begin{itemize}
                    \item Approaches a linear trend
                \end{itemize}
            \end{itemize}
        \end{block}
        \column{0.63\textwidth}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Application}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \begin{itemize}
                    \item Business cycle extraction
                \end{itemize}
                \item \textbf{Common series}
                \begin{itemize}
                    \item GDP, unemployment, inflation
                \end{itemize}
            \end{itemize}
        \end{exampleblock}
        \column{0.63\textwidth}
...
\begin{frame}{Seasonal Adjustment: Visualization}
    \begin{columns}[T]
        ...
        \begin{block}{Result}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Seasonally adjusted series}
                \begin{itemize}
                    \item Reveals the underlying trend
                    \item Removes periodic fluctuations
                \end{itemize}
            \end{itemize}
        \end{block}
        \column{0.66\textwidth}
...
\begin{frame}{Deterministic vs Stochastic Trend}
    ...
    \begin{alertblock}{Wrong Method = Problems}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Differencing a deterministic trend} $\succ$ over-differencing
            \begin{itemize}
                \item Introduces artificial dependence in the series
            \end{itemize}
            \item \textbf{Regression on a stochastic trend} $\succ$ spurious regression
            \begin{itemize}
...
\begin{frame}{Example: Deterministic Trend}
    \begin{columns}[T]
        ...
        \vspace{-0.3cm}
        \begin{center}
            \includegraphics[width=\textwidth, height=0.78\textheight, keepaspectratio]{deterministic_trend_example.pdf}
        \end{center}
        \vspace{-2mm}
        \quantlet{TSA\_ch0\_detrending}{https://github.com/QuantLet/TSA/tree/main/TSA_ch0/TSA_ch0_detrending}
    \end{columns}
...
\begin{frame}{Example: Stochastic Trend (Random Walk)}
    \begin{columns}[T]
        ...
        \vspace{-0.3cm}
        \begin{center}
            \includegraphics[width=\textwidth, height=0.78\textheight, keepaspectratio]{stochastic_trend_example.pdf}
        \end{center}
        \vspace{-2mm}
        \quantlet{TSA\_ch0\_detrending}{https://github.com/QuantLet/TSA/tree/main/TSA_ch0/TSA_ch0_detrending}
    \end{columns}
...
\begin{frame}{Side-by-Side Comparison}
    \begin{columns}[T]
        ...
        \begin{alertblock}{Remember}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Deterministic trend}: use regression
                \begin{itemize}
                    \item Trend is a predictable function of time
                \end{itemize}
                \item \textbf{Stochastic trend}: use differencing
                \begin{itemize}
                    \item Trend contains a random component
                \end{itemize}
            \end{itemize}
        \end{alertblock}
        \column{0.63\textwidth}
...
\begin{frame}{Experiment: ChatGPT vs Fundamentals}
    ...
    \begin{block}{\footnotesize Prompt $\to$ Response}
        {\footnotesize
        \textbf{You}: ``I have monthly airline passenger data from 1949 to 1960. Analyze it and make a forecast.''\\[2pt]
        \textbf{ChatGPT}: \texttt{seasonal\_decompose(data, model='additive')}\\
        ``Trend extracted. Seasonal pattern identified. MAPE $= 4.2\%$. \textit{The model fits well.}''
        }
    \end{block}
    \vspace{-1mm}
    {\small
    ...
    \begin{enumerate}\setlength{\itemsep}{1pt}
        \item \textbf{Wrong decomposition type}: seasonal amplitude $\times 3$ from 1949 to 1960
            \\ {\footnotesize $\Var(S_t) \neq \text{const}$ $\succ$ additive violated $\succ$ use \textbf{multiplicative} or $\ln X_t$ first}
        \item \textbf{In-sample metric is meaningless}: MAPE $= 4.2\%$ is computed on \textbf{training data}
            \\ {\footnotesize Rolling-origin CV reveals true MAPE $= 8.7\%$ --- the model is \textbf{twice as bad} as reported}
        \item \textbf{Residuals not checked}: ACF of residuals $\neq 0$ at lags $1$--$3$
            \\ {\footnotesize Unexplained systematic pattern remains $\succ$ the decomposition is \textbf{misspecified}}
    \end{enumerate}
    }
    \vspace{-1mm}
    \begin{alertblock}{}
        {\small \textbf{Discussion}: The code runs without errors. The output looks professional. \textit{How do you know it's wrong?}}
    \end{alertblock}
\end{frame}
...
\begin{frame}{Quiz Answers}
    ...
    \begin{exampleblock}{Answers}
        \begin{itemize}\setlength{\itemsep}{1pt}
            ...
            \item[\textcolor{MainBlue}{\textbf{3.}}] \textbf{CV}: standard k-fold ignores temporal order $\succ$ data leakage
            \item[\textcolor{MainBlue}{\textbf{4.}}] \textbf{$\alpha = 0.9$}: high weight on recent observations, reacts quickly but more volatile
            \item[\textcolor{MainBlue}{\textbf{5.}}] \textbf{Trend}: deterministic $\succ$ function of time (regression); stochastic $\succ$ random walk (differencing)
        \end{itemize}
    \end{exampleblock}
    }
\end{frame}
...
\begin{frame}{What's Next?}
    ...
    \begin{block}{Chapter 1: Stochastic Processes and Stationarity}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Fundamental Processes}: white noise and random walk $\succ$ building blocks for ARIMA
