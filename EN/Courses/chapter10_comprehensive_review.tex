...
\begin{frame}{VaR 1\% Backtesting: Model Comparison}
    ...
    \begin{block}{Bitcoin Results --- VaR 1\% (T = 329 days, expected: 3.3 violations)}
        ...
        \begin{tabular}{lcccccc}
            ...
            \textbf{Model} & \textbf{AIC} & \textbf{Violations} & \textbf{Rate} & \textbf{Kupiec $p$} & \textbf{Chr.\ $p$} & \textbf{Conclusion} \\
            \midrule
            GARCH(1,1)-N & 9769 & 1 & 0.3\% & 0.137 & 1.000 & Too conservative \\
            GARCH(1,1)-t & \textbf{9260} & 0 & 0.0\% & 1.000 & 1.000 & Too conservative \\
            GJR-GARCH(1,1)-t & 9260 & 0 & 0.0\% & 1.000 & 1.000 & Too conservative \\
            \rowcolor{Forest!15} Hist.\ Simulation & --- & \textbf{4} & \textbf{1.2\%} & \textbf{0.704} & 1.000 & \textcolor{Forest}{\textbf{Most accurate}} \\
            \bottomrule
        \end{tabular}
        \end{center}
    \end{block}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{VaR 1\% Conclusions}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Parametric models: \textbf{too conservative} at 1\%
                \item GARCH-t/GJR-t: 0 violations $\succ$ tails too heavy
                \item Hist.\ Simulation: 1.2\% $\approx$ 1\% --- best fit
            \end{itemize}
        \end{exampleblock}
        ...
        \begin{alertblock}{Lesson: 5\% vs 1\%}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item At 5\%: Student-t excels (perfect coverage)
                \item At 1\%: Hist.\ Simulation more accurate
                \item \textbf{Conclusion}: optimal model depends on $\alpha$!
            \end{itemize}
        \end{alertblock}
    \end{columns}
\end{frame}
