...
\begin{frame}{Forecast Evaluation Beyond RMSE}
    \begin{alertblock}{RMSE is not universally optimal}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item The metric must be chosen based on the \textbf{economic objective}
        \end{itemize}
    \end{alertblock}

    \vspace{0.1cm}

    \begin{block}{Other relevant metrics}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{MASE} (Mean Absolute Scaled Error) --- comparison with naïve
            \item \textbf{Directional Accuracy} --- correct direction of change
            \item \textbf{Quantile Loss} --- for VaR and probabilistic forecasts
            \item \textbf{CRPS} (Continuous Ranked Probability Score) --- full distribution
        \end{itemize}
    \end{block}

    \vspace{0.1cm}

    \begin{exampleblock}{Example: Quantile Loss}
        \vspace{-0.2cm}
        \[
        QL_{\alpha}(y_t, \hat{q}_t) =
        \begin{cases}
...
\begin{frame}{Formal Forecast Comparison: Diebold--Mariano}
    \begin{alertblock}{Problem}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Lower RMSE $\neq$ statistically significant difference
        \end{itemize}
    \end{alertblock}
    ...
    \begin{defn}[Diebold--Mariano Test]
        ...
        \vspace{-0.2cm}
        \[
        DM = \frac{\bar{d}}{\sqrt{\widehat{\text{Var}}(\bar{d})}} \xrightarrow{d} N(0,1)
        \]
    \end{defn}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Hypotheses}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $H_0$: equal predictive performance
                \item $H_1$: different performance
            \end{itemize}
        \end{block}
        ...
        \begin{exampleblock}{Key message}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Large $|DM|$ $\Rightarrow$ significant difference
                \item Model comparison must be \textbf{statistically grounded}
            \end{itemize}
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{GARCH: Innovation Distributions}
    \begin{block}{Model}
        ...
        \]
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Choices for $z_t$: $\mathcal{N}(0,1)$ (normal) or $t_{\nu}$ (heavy tails)
        \end{itemize}
    \end{block}
    ...
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{alertblock}{Crypto: why it matters}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Kurtosis $> 3$ (heavy tails)
                \item Frequent extreme events
        ...
        \begin{exampleblock}{Practical implication}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Student-t distribution produces \textbf{more realistic} VaR estimates
                \item Degrees of freedom $\nu$ control tail thickness
            \end{itemize}
        \end{exampleblock}
    \end{columns}
\end{frame}

\begin{frame}{Application: Conditional Value-at-Risk}
    \begin{defn}[Conditional VaR at level $\alpha$]
        \vspace{-0.2cm}
        \[
        \text{VaR}_{t+1}^{\alpha} = \mu_{t+1} + \sigma_{t+1} \cdot z_{\alpha}
        \]
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\sigma_{t+1}^2$ comes from the GARCH model
            \item $z_{\alpha}$ = quantile of the chosen distribution (Normal or Student-t)
        \end{itemize}
    \end{defn}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Key insight}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Returns are hard to predict
                \item Volatility is \textbf{predictable}
            \end{itemize}
        \end{block}
        ...
        \begin{exampleblock}{Conclusion}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item GARCH is a \textbf{risk model}, not a return model
                \item VaR = the direct application
            \end{itemize}
        \end{exampleblock}
    \end{columns}
\end{frame}

\begin{frame}{Risk Model Validation: Backtesting}
    \vspace{-0.2cm}
    \begin{block}{Kupiec Test (Unconditional Coverage)}
        ...
        {(1-\hat{p})^{T-x} \hat{p}^x}
        \right) \sim \chi^2(1), \quad x = \text{violations}, \; \hat{p} = x/T
        \]
    \end{block}
    ...
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{Christoffersen Test}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Checks \textbf{independence} of violations
        ...
        \begin{alertblock}{Interpretation}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Too many violations $\Rightarrow$ risk underestimated
                \item Too few $\Rightarrow$ model too conservative
            \end{itemize}
        \end{alertblock}
    \end{columns}
    ...
    \begin{exampleblock}{Principle}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item A risk model must be \textbf{validated}, not just estimated
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Structural Breaks: Formal Approach}
    \begin{columns}[T]
        ...
        \column{0.5\textwidth}
        \begin{exampleblock}{Practical trade-off}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{SARIMA}: stable, parsimonious, but assumes constant parameters
                \item \textbf{Prophet}: flexible, detects changepoints automatically
            \end{itemize}
        \end{exampleblock}
...
\begin{frame}{VAR Model Stability}
    \begin{block}{Stability Condition}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item All eigenvalues of the \textbf{companion matrix} must lie inside the unit circle:
        \end{itemize}
        \[
    ...
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{alertblock}{If not satisfied}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Unstable model
                \item Explosive IRFs
                \item Unsustainable forecasts
            \end{itemize}
        \end{alertblock}

        \column{0.5\textwidth}
        \begin{exampleblock}{Practical check}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \texttt{results.is\_stable()} in statsmodels
                \item Stability verification is \textbf{mandatory} before IRF
            \end{itemize}
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{VAR vs VECM: Cointegration}
    ...
    \begin{block}{Problem}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Johansen test: checks for cointegration relationships
        \end{itemize}
    \end{block}

    \vspace{-0.1cm}

    \begin{defn}[VECM --- Vector Error Correction Model]
        \vspace{-0.3cm}
        \[
    ...
    \begin{alertblock}{Key message}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item VAR is appropriate only for \textbf{stationary} series
            \item Ignoring cointegration produces \textbf{spurious regressions}
        \end{itemize}
    \end{alertblock}
\end{frame}
