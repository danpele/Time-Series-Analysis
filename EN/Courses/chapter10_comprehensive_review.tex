...
\begin{frame}{Bitcoin: Volatility Forecast}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Shaded area: 95\% prediction interval of the volatility forecast
...
\begin{frame}{VaR and ES: Graphical Illustration}
    ...
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.50\textheight, keepaspectratio]{garch_var_illustration.pdf} % Verify \graphicspath in preamble
    \end{center}
...
\begin{frame}{VaR vs Expected Shortfall: Normal vs Student-t}
    ...
    \begin{block}{Interpretation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ES measures average loss when VaR is exceeded; Student-t: VaR and ES are larger than under normal
        \end{itemize}
    \end{block}
    }
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.38\textheight, keepaspectratio]{garch_var_es_comparison.pdf} % Verify \graphicspath in preamble
    \end{center}
    \vspace{-0.3cm}
    \quantlet{TSA\_ch5\_var\_es}{https://github.com/QuantLet/TSA/tree/main/TSA_ch5/TSA_ch5_var_es}
    \vspace{-0.2cm}
    {\scriptsize
    \begin{block}{Formal Definitions}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{VaR}: $\text{VaR}_\alpha = \inf\{l : P(r_t \leq -l) \leq \alpha\}$
            \item \textbf{ES} (preferred under Basel III/IV):
                $\text{ES}_\alpha = \E[-r_t \mid r_t \leq -\text{VaR}_\alpha] = \frac{1}{\alpha}\int_0^{\alpha} \text{VaR}_u \, du$
            \item ES is \textbf{coherent} (subadditive); VaR is \textbf{not} subadditive
        \end{itemize}
    \end{block}
    }
\end{frame}
...
\begin{frame}{Value at Risk --- Numerical Example}
    ...
    \begin{alertblock}{Scaling for Longer Periods}
        $\text{VaR}_{h\text{ days}} = \text{VaR}_{1\text{ day}} \cdot \sqrt{h}$ --- assumes i.i.d.\ returns, which is inconsistent with the serial dependence in conditional variance modeled by GARCH
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{VaR Backtesting: Visualization}
    ...
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.47\textheight, keepaspectratio]{garch_var_backtesting.pdf} % Verify \graphicspath in preamble
    \end{center}
...
\begin{frame}{Application: Rolling VaR with Multiple Models}
    ...
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.52\textheight, keepaspectratio]{ch10_btc_var_multimodel.pdf} % Verify \graphicspath in preamble
    \end{center}
...
\begin{frame}{Application: Rolling VaR 1\% on Multiple Models}
    ...
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.52\textheight, keepaspectratio]{ch10_btc_var_multimodel_1pct.pdf} % Verify \graphicspath in preamble
    \end{center}
    \quantlet{TSA\_ch10\_empirical\_slides}{https://github.com/QuantLet/TSA/tree/main/TSA_ch10/TSA_ch10_empirical_slides}
\end{frame}
...
\begin{frame}{VAR Model Stability}
    ...
    \begin{column}{0.53\textwidth}
        \includegraphics[width=\textwidth, height=0.70\textheight, keepaspectratio]{ch10_var_eigenvalues.pdf} % Verify \graphicspath in preamble
    \end{column}
    \end{columns}
    \quantlet{TSA\_ch10\_empirical\_slides}{https://github.com/QuantLet/TSA/tree/main/TSA_ch10/TSA_ch10_empirical_slides}
\end{frame}
...
\begin{frame}{VAR vs VECM: Cointegration}
    ...
    \begin{columns}[T]
        ...
        \begin{defn}[VECM]
            \vspace{-0.3cm}
            \[
            \Delta y_t = \mu + \Pi y_{t-1} + \sum_{i=1}^{p-1} \Gamma_i \Delta y_{t-i} + u_t, \quad \Pi = \alpha \beta'
            \]
        \end{defn}
...
\begin{frame}{Granger Causality: Empirical Results}
    ...
    {\scriptsize
    \begin{block}{Interpretation}
            Each cell shows p-value for testing whether the row variable Granger-causes the column variable. Green: $p < 0.10$. Interpretation: row variable Granger-causes column variable.
        \end{block}
...
\begin{frame}{Granger Causality: Formal Definition}
    ...
    \begin{defn}[Granger Causality]
        ...
        where $\mathcal{F}_t^{X,Y}$ includes past of both $X$ and $Y$, while $\mathcal{F}_t^{Y}$ includes only past $Y$.
    \end{defn}
    }
    \vspace{-0.2cm}
...
\begin{frame}{Synthesis: From Theory to Practice}
    \begin{cminipage}{0.95\textwidth}
        {\tiny
        \begin{center}
        \begin{tabular}{@{}l l l l@{}}
            \toprule
            \textbf{Chapter} & \textbf{Concept} & \textbf{Formula} & \textbf{Application} \\
            \midrule
            Ch1 & Stationarity & $\gamma(h) = \text{Cov}(X_t, X_{t+h})$ & ADF/KPSS Test \\
            Ch2 & ARMA & $\phi(L)X_t = \theta(L)\varepsilon_t$ & Box-Jenkins \\
            Ch3 & ARIMA & $(1-L)^d X_t \sim$ ARMA & Differencing \\
            Ch4 & SARIMA & $(1-L^s)^D X_t$ & Seasonality \\
            ...
            Ch7 & VECM & $\Delta\mathbf{Y}_t = \boldsymbol{\Pi}\mathbf{Y}_{t-1} + \ldots$ & Cointegration \\
            Ch8 & Extensions & ARFIMA, RF, LSTM & Long memory \\
            Ch9 & Prophet & $y(t) = g(t) + s(t) + h(t) + \varepsilon_t$ & Forecasting \\
            \bottomrule
        \end{tabular}
        \end{center}
        }
...
\begin{frame}{Question 1: Answer}
    ...
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.98\textwidth, height=0.58\textheight, keepaspectratio]{ch10_quiz1_model_selection.pdf} % Verify \graphicspath in preamble
    \end{center}
    \vspace{-3mm}
    {\small
...
\begin{frame}{Question 2: Answer}
    ...
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.98\textwidth, height=0.58\textheight, keepaspectratio]{ch10_quiz2_overfitting.pdf} % Verify \graphicspath in preamble
    \end{center}
    \vspace{-3mm}
    {\small
    \begin{exampleblock}{Answer: (B)}
...
\begin{frame}{Question 3: Answer}
    ...
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.98\textwidth, height=0.58\textheight, keepaspectratio]{ch10_quiz3_train_val_test.pdf} % Verify \graphicspath in preamble
    \end{center}
    \vspace{-3mm}
    {\small
    \begin{exampleblock}{Answer: (B)}
...
\begin{frame}{Question 4: Answer}
    ...
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.98\textwidth, height=0.58\textheight, keepaspectratio]{ch10_quiz4_granger_causality.pdf} % Verify \graphicspath in preamble
    \end{center}
    \vspace{-3mm}
    {\small
    \begin{exampleblock}{Answer: (B)}
...
\begin{frame}{Question 5: Answer}
    ...
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.98\textwidth, height=0.50\textheight, keepaspectratio]{ch10_quiz5_long_seasonality.pdf} % Verify \graphicspath in preamble
    \end{center}
    \vspace{-3mm}
    {\footnotesize
    \begin{exampleblock}{Answer: (C)}
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item SARIMA$_{365}$ requires lag polynomials of order 365 --- infeasible. Fourier terms ($K{=}3$) use only 6 parameters. Prophet and TBATS handle multiple seasonalities automatically.
    \end{itemize}
    \end{exampleblock}
    }
...
%=============================================================================
\section{Integrative Research Project}

\begin{frame}{Project: Forecasting Inflation in Romania}
    \quantlet{TSA\_ch10\_inflation\_forecast}{https://github.com/QuantLet/TSA/tree/main/TSA_ch10/TSA_ch10_inflation_forecast}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Objective}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Apply the \textbf{entire methodology} (Ch.\ 0--9) to forecast Romanian CPI inflation
                \item Complete empirical study: from data collection to forecast evaluation
            \end{itemize}
        \end{block}
        ...
        \begin{columns}[T]
            ...
            \begin{exampleblock}{Data Sources}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{NBR}: monetary policy rate, EUR/RON exchange rate, M3 money supply
                    \item \textbf{NIS}: CPI (consumer price index), quarterly GDP, unemployment rate
                    \item \textbf{Eurostat}: HICP, energy prices, import prices
                    \item \textbf{BSE}: BET index, financial returns
                \end{itemize}
            \end{exampleblock}

            \column{0.5\textwidth}
            \begin{alertblock}{Experimental Design}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Sample}: 2005M01 -- 2024M12 (240 monthly obs.)
                    \item \textbf{Training}: 2005M01 -- 2022M12 (216 obs.)
                    \item \textbf{Out-of-sample}: last 24 months (2023--2024) for evaluation
                    \item \textbf{Target}: annual CPI inflation rate (\% y-o-y)
                \end{itemize}
            \end{alertblock}
        \end{columns}
        }
...
\begin{frame}{Stage 1: Exploratory Analysis (Ch.\ 0--1)}
    \quantlet{TSA\_ch10\_exploratory}{https://github.com/QuantLet/TSA/tree/main/TSA_ch10/TSA_ch10_exploratory}
    \begin{cminipage}{0.95\textwidth}
        {\footnotesize
        \begin{columns}[T]
            \column{0.5\textwidth}
            \begin{block}{Decomposition and Visualization}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{STL} decomposition of inflation: trend + seasonality + residual
                    \item Time plots: CPI, GDP, EUR/RON, M3, BET
                    \item Regime identification: pre-crisis, 2008 crisis, COVID, 2022+ inflation
                \end{itemize}
            \end{block}

            \vspace{0.15cm}

            \begin{exampleblock}{Spectral Analysis}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Periodogram: dominant periodicities (12 months, 6 months)
                    \item Cross-spectrum inflation--exchange rate
                \end{itemize}
            \end{exampleblock}

            \column{0.5\textwidth}
            \begin{alertblock}{Stationarity Tests}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{ADF}: $H_0$: unit root
                    \item \textbf{KPSS}: $H_0$: stationarity
                    \item \textbf{Phillips-Perron}: robust to heteroskedasticity
                    \item Applied to: CPI, GDP, EUR/RON, M3, BET
                    \item Determine integration order $d$ and $D$
                \end{itemize}
            \end{alertblock}

            \vspace{0.15cm}

            \begin{block}{ACF/PACF and Diagnostics}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item ACF/PACF functions: dependence structure
                    \item Normality tests (Jarque-Bera)
                    \item Heteroskedasticity tests (ARCH-LM)
                \end{itemize}
            \end{block}
        \end{columns}
        }
...
\begin{frame}{Stage 2: Univariate Models (Ch.\ 2--4)}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{columns}[T]
            ...
            \begin{block}{ARIMA --- Box-Jenkins}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Identification}: ACF/PACF + information criteria
                    \item \textbf{Estimation}: MLE for $\text{ARIMA}(p,d,q)$
                    \item \textbf{Diagnostics}: Ljung-Box on residuals, Q-Q plot
                    \item Automatic selection: \texttt{auto\_arima} (minimum AIC)
                \end{itemize}
            \end{block}

            \vspace{0.15cm}

            \begin{exampleblock}{SARIMA --- Seasonality}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item $\text{SARIMA}(p,d,q)(P,D,Q)_{12}$
                    \item Capturing monthly seasonality ($s=12$)
                    \item Seasonal differencing: $(1-L^{12})$
                \end{itemize}
            \end{exampleblock}

            \column{0.5\textwidth}
            \begin{alertblock}{Benchmark Models}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Random walk}: $\hat{y}_{t+h} = y_t$
                    \item \textbf{Moving average}: MA(12)
                    \item \textbf{ETS}: Error-Trend-Seasonal (Holt-Winters)
                    \item \textbf{Drift}: random walk with trend
                \end{itemize}
            \end{alertblock}
            ...
            \begin{block}{Out-of-sample Evaluation}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item RMSE, MAE, MAPE on the 2023--2024 window
                    \item Rolling window vs.\ expanding window
                    \item \textbf{Exercise}: test if over-differencing degrades the forecast
                \end{itemize}
            \end{block}
        \end{columns}
        }
...
\begin{frame}{Stage 3: Multivariate Models (Ch.\ 6--7)}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{columns}[T]
            \column{0.5\textwidth}
            \begin{block}{VAR: Inflation + Rate + EUR/RON + M3}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Lag selection}: AIC, BIC, HQ
                    \item \textbf{Granger causality}: which variables predict inflation?
                    \item \textbf{IRF}: inflation response to NBR rate shock
                    \item \textbf{FEVD}: inflation variance decomposition
                    \item Stability: roots $|\lambda_i| < 1$
                \end{itemize}
            \end{block}

            \vspace{0.05cm}

            \begin{exampleblock}{VAR Diagnostics}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Portmanteau tests on multivariate residuals
                    \item Multivariate normality (Jarque-Bera)
                    \item Multivariate ARCH tests
                \end{itemize}
            \end{exampleblock}

            \column{0.5\textwidth}
            \begin{alertblock}{Johansen Cointegration}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Trace} and \textbf{Max Eigenvalue} tests
                    \item $H_0$: $r = 0$ (no cointegration)
                    \item Equilibrium: $\beta' \mathbf{Y}_t = \beta_1 \pi_t + \beta_2 i_t + \beta_3 e_t + \beta_4 m_t$
                    \item Economic interpretation: Taylor rule?
                \end{itemize}
            \end{alertblock}

            \vspace{0.05cm}

            \begin{block}{VECM --- Error Correction}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Cointegration $\Rightarrow$ VECM
                    ...
                    \item $\boldsymbol{\alpha}$: speed of adjustment to equilibrium
                \end{itemize}
            \end{block}
        \end{columns}
        }
...
\begin{frame}{Stage 4: Volatility and Machine Learning (Ch.\ 5, 8--9)}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{columns}[T]
            \column{0.5\textwidth}
            \begin{block}{GARCH on EUR/RON}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Modeling exchange rate \textbf{uncertainty}
                    \item GARCH(1,1), EGARCH, GJR-GARCH
                    \item Distribution: Normal vs.\ Student-$t$ vs.\ Skew-$t$
                    \item Conditional volatility as inflation predictor
                \end{itemize}
            \end{block}
            ...
            \begin{exampleblock}{Prophet / NeuralProphet}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Multiple seasonality + trend
                    \item Automatic changepoints (inflationary regimes)
                    \item NeuralProphet: neural AR components
                \end{itemize}
            \end{exampleblock}
            ...
            \begin{alertblock}{Random Forest / XGBoost}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Features}: inflation lags $(y_{t-1}, \ldots, y_{t-12})$
                    \item Macro: rate, EUR/RON, M3, unemployment
                    \item Calendar features: month, quarter
                    \item Feature importance
                    \item Hyperparameters: temporal CV (TimeSeriesSplit)
                \end{itemize}
            \end{alertblock}

            \vspace{0.1cm}

            \begin{block}{Ensemble and Uncertainty}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Ensemble}: weighted model average
                    \item Weighting: inverse RMSE
                    \item \textbf{Conformal prediction}: model-agnostic intervals with coverage guarantee
                \end{itemize}
            \end{block}
        \end{columns}
        }
...
\begin{frame}{Stage 5: Evaluation and Conclusions}
    \quantlet{TSA\_ch10\_evaluation}{https://github.com/QuantLet/TSA/tree/main/TSA_ch10/TSA_ch10_evaluation}
    \vspace{-0.3cm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{``Horse Race'' Comparison --- Out-of-sample 2023--2024}
            \vspace{-0.1cm}
            \begin{center}
            ...
            \end{center}
            \vspace{-0.2cm}
            \hfill{\tiny \textit{Fill in with your own results. DM test: Diebold-Mariano vs.\ Random Walk.}}
        \end{block}
        }
        ...
        \begin{columns}[T]
            \column{0.5\textwidth}
            \begin{exampleblock}{Formal Testing}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Diebold-Mariano}: statistical superiority of forecast
                    \item \textbf{Model Confidence Set} (Hansen, Lunde \& Nason, 2011): set of equivalently superior models
                    \item Giacomini-White: conditional testing
                \end{itemize}
            \end{exampleblock}

            \column{0.5\textwidth}
            \begin{alertblock}{Conclusions and Deliverable}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Which approach works best for Romanian inflation? Why?
                    \item Does model combination beat individual models?
                    \item \textbf{Deliverable}: 15--20 page report + reproducible Python/R code
                \end{itemize}
            \end{alertblock}
        \end{columns}
        }
...
\begin{frame}{Bibliography II}
    ...
    \begin{exampleblock}{Online Resources and Code}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{GitHub TSA}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch10} -- Python code for this chapter
        \end{itemize}
        }
    \end{exampleblock}
\end{frame}
...
\begin{frame}{What's Next? Chapter 11 --- LLMs and Foundation Models}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{The Frontier of Time Series Forecasting}
        \begin{itemize}\setlength{\itemsep}{2pt}
            \item[\textcolor{MainBlue}{\textbf{1.}}] \textbf{Transformer Architecture} --- self-attention, positional encoding, and their adaptation to time series
            \item[\textcolor{MainBlue}{\textbf{2.}}] \textbf{Foundation Models} --- Chronos, TimesFM, Lag-Llama, Moirai, TimeGPT: architectures and tokenization strategies
            \item[\textcolor{MainBlue}{\textbf{3.}}] \textbf{Zero-Shot Forecasting} --- predicting without training, and when to fine-tune
            \item[\textcolor{MainBlue}{\textbf{4.}}] \textbf{Practical Use Cases} --- EUR/RON, energy demand, and financial volatility with foundation models
            \item[\textcolor{MainBlue}{\textbf{5.}}] \textbf{Benchmarks and Model Selection} --- navigating the growing landscape of pre-trained models
        \end{itemize}
    \end{block}

    \begin{alertblock}{Why It Matters}
        Foundation models are reshaping how we approach forecasting --- enabling zero-shot predictions across domains without task-specific training. Chapter 11 provides the tools to evaluate and apply them.
    \end{alertblock}
    \end{cminipage}
\end{frame}
