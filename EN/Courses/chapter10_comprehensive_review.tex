...
\begin{frame}{Forecast Evaluation Beyond RMSE}
    \begin{columns}[T]
        ...
        \begin{block}{Alternative metrics}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{MASE}: comparison with naïve forecast
                \item \textbf{Directional Accuracy}: correct direction
                \item \textbf{Quantile Loss}: for VaR
                \item \textbf{CRPS}: full distribution
            \end{itemize}
        \end{block}
...
%=============================================================================
% VALUE-AT-RISK: BITCOIN EMPIRICAL APPLICATION
%=============================================================================
...
\begin{frame}{Application: Conditional Value-at-Risk}
    ...
    \end{defn}
    \vspace{-0.2cm}
    \begin{exampleblock}{Bitcoin Results (test: 329 days)}
        \begin{tabular}{lcc}
            & \textbf{Normal} & \textbf{Student-t} \\
            \midrule
            Violations 5\% & 12 (3.6\%) & \textbf{17 (5.2\%)} \\
            Violations 1\% & 1 (0.3\%) & 0 (0.0\%) \\
            Kupiec $p$ (5\%) & 0.238 & \textbf{0.890} \\
        \end{tabular}
    \end{exampleblock}
    \vspace{-0.1cm}
    {\scriptsize Student-t: 5.2\% $\approx$ 5\% --- perfect coverage. Normal: too conservative (3.6\%).}
    }
    \end{column}
...
\begin{frame}{Risk Model Validation: Backtesting}
    ...
    \begin{columns}[T]
        ...
        \end{block}
        \vspace{-0.15cm}
        \begin{block}{Christoffersen (Independence)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Tests \textbf{independence} of violations
                \item $LR_{cc} = LR_{uc} + LR_{ind} \sim \chi^2(2)$
            \end{itemize}
        \end{block}

        \column{0.50\textwidth}
        \begin{exampleblock}{Bitcoin Results (VaR 5\%)}
            \begin{tabular}{lcc}
                & \textbf{Normal} & \textbf{Student-t} \\
                \midrule
                Violations & 12/329 & 17/329 \\
                Rate & 3.6\% & \textbf{5.2\%} \\
        ...
        \begin{alertblock}{Conclusion}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Both pass Kupiec ($p > 0.05$)
                \item Student-t: \textbf{more precise} coverage
            \end{itemize}
        \end{alertblock}
    \end{columns}
    }
