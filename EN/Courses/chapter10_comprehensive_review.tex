...
\begin{frame}{Bitcoin: Evidence for GARCH}
    ...
    \begin{block}{Observation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Squared returns $r_t^2$ exhibit significant autocorrelation $\succ$ GARCH effects
            \item Slow ACF decay $\succ$ high volatility persistence
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{VaR Backtesting: Model Comparison}
    \vspace{-0.1cm}
    {\footnotesize
    \begin{block}{Bitcoin Results --- VaR 5\% (T = 329 days, expected: 16.5 violations)}
        \begin{center}
        \begin{tabular}{lcccccc}
            \toprule
            \textbf{Model} & \textbf{AIC} & \textbf{Violations} & \textbf{Rate} & \textbf{Kupiec $p$} & \textbf{Chr.\ $p$} & \textbf{Conclusion} \\
            \midrule
            GARCH(1,1)-N & 9769 & 12 & 3.6\% & 0.238 & 1.000 & Too conservative \\
            \rowcolor{Forest!15} GARCH(1,1)-t & \textbf{9260} & \textbf{17} & \textbf{5.2\%} & \textbf{0.890} & 0.272 & \textcolor{Forest}{\textbf{Best}} \\
            GJR-GARCH(1,1)-t & 9260 & 17 & 5.2\% & 0.890 & 0.272 & $\gamma \approx 0$ (symmetric) \\
            Hist.\ Simulation & --- & 16 & 4.9\% & 0.909 & 0.216 & Good, non-parametric \\
            \bottomrule
        \end{tabular}
        \end{center}
    \end{block}
    ...
    \begin{columns}[T]
        ...
        \begin{alertblock}{Practical Lessons}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Innovation distribution matters: \textbf{$\Delta$AIC = 509}
...
\begin{frame}{VaR 1\% Backtesting: Model Comparison}
    ...
    \begin{block}{Bitcoin Results --- VaR 1\% (T = 329 days, expected: 3.3 violations)}
        ...
        \begin{tabular}{lcccccc}
            ...
            \textbf{Model} & \textbf{AIC} & \textbf{Violations} & \textbf{Rate} & \textbf{Kupiec $p$} & \textbf{Chr.\ $p$} & \textbf{Conclusion} \\
            \midrule
            GARCH(1,1)-N & 9769 & 1 & 0.3\% & 0.137 & 1.000 & Acceptable \\
            GARCH(1,1)-t & \textbf{9260} & 0 & 0.0\% & 1.000 & 1.000 & Acceptable \\
            GJR-GARCH(1,1)-t & 9260 & 0 & 0.0\% & 1.000 & 1.000 & Acceptable \\
            Hist.\ Simulation & --- & 4 & 1.2\% & 0.704 & 1.000 & Acceptable \\
            \bottomrule
        \end{tabular}
        \end{center}
    \end{block}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{VaR 1\% Conclusions}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{All models pass} Kupiec ($p > 0.05$)
                \item With only 3.3 expected violations, test has \textbf{low statistical power}
                \item Cannot meaningfully distinguish between 0 and 4 violations
            \end{itemize}
        \end{exampleblock}
        ...
        \begin{alertblock}{Lesson: 5\% vs 1\%}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item At 5\% ($\approx$16 violations): powerful test, Student-t best
                \item At 1\% ($\approx$3 violations): weak test, models \textbf{indistinguishable}
                \item \textbf{Conclusion}: test power increases with $\alpha$ and $T$
            \end{itemize}
        \end{alertblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Sunspots: The 11-Year Solar Cycle}
    ...
    \begin{block}{Observation}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Periodic ACF $\succ$ long seasonality, ideal for Fourier terms
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{VAR vs VECM: Cointegration}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Problem}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item If variables are $I(1)$ $\succ$ VAR on levels produces spurious regressions
            \end{itemize}
        \end{block}
        \vspace{-0.15cm}
        \begin{defn}[VECM]
...
\begin{frame}{Impulse Response Functions (IRF)}
    ...
    \begin{block}{Effects}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\uparrow$ GDP $\succ$ $\downarrow$ Unemployment (Okun), $\uparrow$ Inflation (demand), Fed raises rate
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Comprehensive Model Comparison}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Empirical Conclusions}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{GARCH}: Student-t $\succ$ Normal ($\Delta$AIC = 509)
                \item \textbf{Fourier}: $K=3$ harmonics, validated on test set
