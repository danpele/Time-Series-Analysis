...
\begin{frame}{Time Series Decomposition: Visual Example}
    ...
    \end{center}
    \vspace{-0.1cm}
    {\small Each panel isolates one component, making it easy to see how the long-term trend, the recurring seasonal pattern, and the irregular residual combine to produce the observed series.}
...
% Slide "Additive vs Multiplicative: Comparison" removed — redundant with individual model slides
...
% Slide "Moving Average Smoothing" removed — redundant with Moving Average slides in the Decomposition section
...
% Slide "Trend Estimation: Multiple Approaches" removed — redundant with Detrending Methods: Comparison slide
...
\begin{frame}{Deterministic vs Stochastic Trend}
    ...
    \begin{center}
        \includegraphics[width=0.75\textwidth, height=0.30\textheight, keepaspectratio]{trend_comparison_sidebyside.pdf}
    \end{center}
...
\begin{frame}{Example: Deterministic Trend}
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Key}
            Use \textcolor{Crimson}{regression} to remove trend $\succ$ residuals are stationary (ACF decays quickly).
        \end{exampleblock}
        \vspace{0.3cm}
...
\begin{frame}{Example: Stochastic Trend (Random Walk)}
    \begin{columns}[T]
        \column{0.32\textwidth}
        \begin{exampleblock}{Key}
            Use \textcolor{Crimson}{differencing} to remove trend $\succ$ differences are stationary (white noise).
        \end{exampleblock}
        \vspace{0.3cm}
...
% Slide "Side-by-Side Comparison" removed — merged into "Deterministic vs Stochastic Trend" slide above
