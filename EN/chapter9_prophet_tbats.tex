...
\begin{frame}{TBATS Model Structure}
    ...
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{itemize}
            \item $y_t^{(\omega)}$: Box-Cox transformed
            \item $\ell_t$: local level
            \item $b_t$: trend with damping $\phi$
        \end{itemize}

        \column{0.5\textwidth}
        \begin{itemize}
            \item $s_t^{(i)}$: seasonal components
            \item $d_t$: ARMA$(p,q)$ errors
            \item Multiple periods: $m_1, \ldots, m_T$
        \end{itemize}
    \end{columns}
    }
\end{frame}
...
\begin{frame}{Prophet: Trend Component}
    \begin{columns}[T]
        ...
        \begin{block}{Linear Trend with Changepoints}
            ...
            {\small
            \begin{itemize}
                \item $k$: base growth rate
                \item $\bm{\delta}$: rate adjustments
                \item $\bm{a}(t)$: active changepoints
            \end{itemize}
            }
        \end{block}
...
\begin{frame}{Prophet Component Decomposition}
    \begin{center}
...
\begin{frame}{Prophet: Uncertainty Quantification}
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Prediction Intervals}
            \begin{itemize}
                \item Default: 80\% interval
                \item Configurable width
                \item Grows with horizon
            \end{itemize}
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Quiz 1: Multiple Seasonality}
    \begin{center}
        \includegraphics[width=0.85\textwidth, height=0.55\textheight, keepaspectratio]{ch9_quiz1_multiple_seasonality.pdf}
    \end{center}

    \textbf{Question}: Why can't standard SARIMA$(p,d,q)(P,D,Q)_s$ model hourly electricity data with both daily and weekly patterns?

    \vspace{0.2cm}
    {\small \textbf{Answer}: SARIMA handles only \textbf{one} seasonal period $s$. Cannot set $s=24$ and $s=168$ simultaneously.}
\end{frame}

\begin{frame}{Quiz 2: TBATS Components}
    \begin{center}
        \includegraphics[width=0.85\textwidth, height=0.55\textheight, keepaspectratio]{ch9_quiz2_tbats_components.pdf}
    \end{center}

    \textbf{Question}: What does each letter in TBATS represent?

    \vspace{0.2cm}
    {\small \textbf{Answer}: \textbf{T}rigonometric seasonality, \textbf{B}ox-Cox transformation, \textbf{A}RMA errors, \textbf{T}rend, \textbf{S}easonal components.}
\end{frame}

\begin{frame}{Quiz 3: Fourier Harmonics}
    \begin{center}
        \includegraphics[width=0.85\textwidth, height=0.55\textheight, keepaspectratio]{ch9_quiz3_fourier_harmonics.pdf}
    \end{center}

    \textbf{Question}: What happens when we increase the number of Fourier harmonics $K$?

    \vspace{0.2cm}
    {\small \textbf{Answer}: Higher $K$ captures more complex patterns but increases overfitting risk.}
\end{frame}

\begin{frame}{Quiz 4: Prophet Decomposition}
    \begin{center}
        \includegraphics[width=0.85\textwidth, height=0.55\textheight, keepaspectratio]{ch9_quiz4_prophet_decomposition.pdf}
    \end{center}

    \textbf{Question}: What are the main components in Prophet's model $y(t) = g(t) + s(t) + h(t) + \varepsilon_t$?

    \vspace{0.2cm}
    {\small \textbf{Answer}: $g(t)$ = trend with changepoints, $s(t)$ = seasonality, $h(t)$ = holiday effects.}
\end{frame}

\begin{frame}{Quiz 5: Model Comparison}
    \begin{center}
        \includegraphics[width=0.85\textwidth, height=0.55\textheight, keepaspectratio]{ch9_quiz5_prophet_vs_tbats.pdf}
    \end{center}

    \textbf{Question}: What key features does Prophet have that TBATS lacks?

    \vspace{0.2cm}
    {\small \textbf{Answer}: Holiday effects, external regressors, trend changepoints, native missing data handling.}
\end{frame}
...
\begin{frame}{Key Takeaways}
    \begin{columns}[T]
        ...
        \begin{block}{Multiple Seasonalities}
            \begin{itemize}
                \item Real-world data has multiple patterns
                \item SARIMA: one seasonal period
                \item TBATS/Prophet: multiple periods
            \end{itemize}
        \end{block}
        ...
        \begin{exampleblock}{Model Selection}
            \begin{itemize}
                \item TBATS: high-frequency, automatic
                \item Prophet: interpretable, holidays
                \item Both use Fourier terms
            \end{itemize}
        \end{exampleblock}
    \end{columns}
