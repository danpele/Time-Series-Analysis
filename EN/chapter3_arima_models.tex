...
\begin{frame}[plain]
    ...
    \begin{center}
        ...
        {\Large\textcolor{IDAred}{Non-Stationary Time Series}}
    \end{center}
    \vfill
...
\begin{frame}{Random Walk: Visual Illustration}
    ...
    \begin{alertblock}{Key Properties}
        Left: Multiple random walk paths wander unpredictably with no mean reversion. Right: Variance $\Var(Y_t) = t\sigma^2$ grows linearly --- the defining feature of non-stationarity.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Proof: Random Walk Variance}
    ...
    \vspace{0.2cm}
    \textbf{Proof:} By recursive substitution:
    ...
    Since $\varepsilon_t$ are independent (white noise), all covariances are zero:
    $$\Var(Y_t) = \sum_{i=1}^{t} \sigma^2 = \boxed{t\sigma^2}$$

    \begin{alertblock}{Non-Stationarity}
        Variance depends on $t$ $\Rightarrow$ violates stationarity requirement ($\Var(Y_t) = \gamma(0)$ constant).
    \end{alertblock}
\end{frame}
...
\begin{frame}{Integrated Process: Visual Illustration}
    ...
    \end{center}
    \vspace{-0.2cm}
    \small $I(0)$: stationary. $I(1)$: one difference needed. $I(2)$: two differences needed.
\end{frame}
...
\begin{frame}{First Difference: Visual Illustration}
    ...
    \end{center}
    \vspace{-0.2cm}
    \small Left: non-stationary series. Right: after first difference, the series becomes stationary.
\end{frame}
...
\begin{frame}{Differencing in Practice: GDP Example}
    \vspace{-0.3cm}
    \begin{center}
        \includegraphics[width=0.92\textwidth, height=0.55\textheight, keepaspectratio]{ch3_differencing.pdf}
    \end{center}
    \vspace{-0.2cm}
...
\begin{frame}{ADF Test: Visual Illustration}
    ...
    \end{center}
    \vspace{-0.2cm}
    \small Left: stationary series -- ADF rejects unit root. Right: non-stationary -- ADF fails to reject.
\end{frame}
...
\begin{frame}[fragile]{Rolling Forecast: Python Code}
    \vspace{-0.3cm}
    {\scriptsize
...
%=============================================================================
\section{Case Study: US GDP Forecasting}

\begin{frame}{Case Study: Complete ARIMA Analysis}
    \begin{block}{Objective}
        Forecast US Real GDP using the Box-Jenkins methodology
    \end{block}
    ...
    {\small
    \begin{enumerate}
        \item \textbf{Step 1}: Visualize data and check for stationarity
        \item \textbf{Step 2}: Apply unit root tests (ADF, KPSS)
        \item \textbf{Step 3}: Difference if needed, identify $p$ and $q$
        \item \textbf{Step 4}: Estimate ARIMA model
        \item \textbf{Step 5}: Diagnostic checking
    ...
    \begin{alertblock}{Data}
        US Real GDP (FRED: GDPC1), Quarterly, 1990Q1--2024Q2, $n = 138$ observations
    \end{alertblock}
\end{frame}

\begin{frame}{Step 1: Initial Data Analysis}
    \vspace{-0.2cm}
    \begin{center}
    ...
    \begin{block}{Observations}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Clear upward trend $\Rightarrow$ non-constant mean
            \item Variance appears relatively stable (after log transform)
            \item Notable dip in 2020 (COVID-19 pandemic)
            \item \textbf{Conclusion}: Series is non-stationary, needs differencing
        \end{itemize}
    \end{block}
    }
\end{frame}

\begin{frame}{Step 2: Unit Root Testing}
    \vspace{-0.2cm}
    {\small
    \begin{block}{ADF Test on Log GDP Levels}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Test statistic: $-0.91$
            \item Critical values: $-3.48$ (1\%), $-2.88$ (5\%), $-2.58$ (10\%)
            \item p-value: $0.79$
            \item \textbf{Result}: Cannot reject $H_0$ $\Rightarrow$ \textcolor{red}{Unit root present}
        \end{itemize}
    \end{block}

    \vspace{0.1cm}

    \begin{block}{ADF Test on First Difference (Growth Rate)}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Test statistic: $-13.24$
            \item p-value: $< 0.001$
            \item \textbf{Result}: Reject $H_0$ at 1\% $\Rightarrow$ \textcolor{green!50!black}{Stationary after differencing}
        \end{itemize}
    \end{block}
    ...
    \begin{alertblock}{Conclusion}
        GDP is $I(1)$ $\Rightarrow$ Use $d = 1$ in ARIMA model
    \end{alertblock}
    }
\end{frame}

\begin{frame}{Step 3: Model Identification via ACF/PACF}
    \vspace{-0.2cm}
    \begin{center}
    ...
    \vspace{-0.2cm}
    {\footnotesize
    \begin{block}{Analysis of Differenced Series}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ACF: Significant spike at lag 1, then cuts off $\Rightarrow$ suggests MA(1)
            \item PACF: Significant spike at lag 1, decays $\Rightarrow$ suggests AR(1)
            \item \textbf{Candidate models}: ARIMA(1,1,0), ARIMA(0,1,1), ARIMA(1,1,1)
        \end{itemize}
    \end{block}
    }
\end{frame}
...
\begin{frame}{Step 4: Model Estimation}
    \vspace{-0.2cm}
    {\small
    \begin{block}{Comparing Models using Information Criteria}
        \begin{center}
        \begin{tabular}{lccc}
            \toprule
    ...
    \begin{exampleblock}{Selected Model: ARIMA(1,1,1)}
        \vspace{-0.2cm}
        $$(1 - 0.35L)(1-L)Y_t = (1 + 0.58L)\varepsilon_t, \quad \hat{\sigma}^2 = 0.000156$$
        \vspace{-0.3cm}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\hat{\phi}_1 = 0.35$ (SE = 0.09), significant at 1\%
            \item $\hat{\theta}_1 = 0.58$ (SE = 0.08), significant at 1\%
        \end{itemize}
    \end{exampleblock}
    }
\end{frame}

\begin{frame}{Step 5: Diagnostic Checking}
    \vspace{-0.2cm}
    \begin{center}
    ...
    \begin{block}{Residual Analysis}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Ljung-Box test: $Q(10) = 5.8$, p-value $= 0.83$ $\Rightarrow$ No autocorrelation
            \item Jarque-Bera test: $JB = 156.4$, p-value $< 0.001$ $\Rightarrow$ Non-normal (COVID outlier)
            \item \textbf{Conclusion}: Model passes autocorrelation checks; outliers expected
        \end{itemize}
    \end{block}
    }
\end{frame}

\begin{frame}{Step 6: Forecasting with Confidence Intervals}
    \vspace{-0.2cm}
    {\small
    \begin{block}{Last Observed Values (Log GDP)}
        $Y_T = 9.973$ (2024Q2), \quad $Y_{T-1} = 9.956$ (2024Q1)
    ...
    \begin{exampleblock}{1-Step Ahead Forecast (2024Q3)}
        \vspace{-0.2cm}
        \begin{align*}
    ...
    \begin{alertblock}{95\% Confidence Interval}
        \vspace{-0.2cm}
        $$\text{CI} = 9.981 \pm 1.96 \times \sqrt{0.000156} = [9.957, \; 10.006]$$
        In levels: GDP forecast = \$21,652B, CI = [\$21,142B, \$22,175B]
    \end{alertblock}
    }
\end{frame}

\begin{frame}{Step 7: Forecast Evaluation}
    \vspace{-0.2cm}
    \begin{center}
    ...
    \vspace{-0.2cm}
    {\footnotesize
    \begin{block}{Out-of-Sample Performance (Last 12 Quarters)}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item RMSE = 0.0486 (log scale) $\approx$ 4.86\% error
            \item MAE = 0.0430 (log scale) $\approx$ 4.30\% error
            \item Direction accuracy = 91\% (correctly predicted growth/decline)
        \end{itemize}
    \end{block}
    }
\end{frame}
...
\begin{frame}{Case Study: GDP Index (Simulated)}
    \begin{center}
    ...
    \begin{block}{Data Description}
        \textbf{Simulated quarterly GDP index (1960--2010)}: Non-stationary series with upward trend. Demonstrates the need for differencing before ARMA modeling.
    \end{block}
\end{frame}
...
\begin{frame}{Step 2: ACF/PACF Before and After Differencing}
    ...
    \begin{exampleblock}{Identification}
        \textbf{Top}: Slow ACF decay $\Rightarrow$ non-stationarity.
        \textbf{Bottom}: After differencing, ACF and PACF suggest low-order ARMA.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Step 4: Diagnostic Checking}
    ...
    \end{center}
    \vspace{-0.2cm}
    \begin{exampleblock}{ARIMA(1,1,1) Diagnostics}
        Residuals are approximately white noise: no significant autocorrelation, nearly normal distribution.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Step 5: Forecasting}
    ...
    \begin{block}{Results}
        \begin{itemize}
            \item ARIMA(1,1,1) forecasts follow the data trend
            \item Confidence intervals widen with horizon (characteristic of I(1))
            \item Uncertainty grows because errors accumulate through differencing
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{Quiz Question 1: Answer}
    \begin{exampleblock}{Correct Answer: (B) $\Var(Y_t) = t \cdot \sigma^2$}
        Random walk variance grows linearly with time --- this is why random walks are non-stationary.
    \end{exampleblock}
    \vspace{0.2cm}
    \begin{center}
...
\begin{frame}{Quiz Question 2: Answer}
    \begin{exampleblock}{Correct Answer: (C) 2 times}
...
\begin{frame}{Quiz Question 3: Answer}
    \begin{exampleblock}{Correct Answer: (C) Fail to reject $H_0$: series has unit root}
        Test stat $-2.1 > -2.57$ (10\% CV) $\Rightarrow$ Cannot reject at any level. Consider differencing.
    \end{exampleblock}
    \vspace{0.2cm}
    \begin{center}
...
\begin{frame}{Quiz Question 4: Answer}
    \begin{exampleblock}{Correct Answer: (B) Decays exponentially}
        ARIMA(1,1,0) $\Rightarrow$ $\Delta Y_t$ follows AR(1) with ACF $\rho_k = \phi_1^k$ (geometric decay).
    \end{exampleblock}
    \vspace{0.2cm}
    \begin{center}
...
\begin{frame}{Quiz Question 5: Answer}
    \begin{exampleblock}{Correct Answer: (C) They widen without bound}
        For I(1): CI width $\propto \sqrt{h}$ (unbounded). For I(0): CIs converge to a limit.
    \end{exampleblock}
    \vspace{0.2cm}
    \begin{center}
