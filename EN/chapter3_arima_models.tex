...
\begin{frame}{Confidence Interval: Numerical Example}
    ...
    \begin{block}{1-Step Ahead CI}
        \vspace{-0.2cm}
        \begin{align*}
            \Var(e_{T+1|T}) &= \sigma^2 = 4 \\
            \text{95\% CI} &= 111 \pm 1.96 \times \sqrt{4} = 111 \pm 3.92 \\
            &= \boxed{[107.08, \; 114.92]}
        \end{align*}
    \end{block}
    ...
    \begin{block}{2-Step Ahead CI (for $\hat{Y}_{T+2|T} = 112.8$)}
        \vspace{-0.2cm}
        \begin{align*}
            \psi_1 &= 1 + \phi_1 = 1.6, \quad \Var(e_{T+2|T}) = 4(1 + 1.6^2) = 14.24 \\
            \text{95\% CI} &= 112.8 \pm 1.96 \times \sqrt{14.24} = 112.8 \pm 7.40 \\
            &= \boxed{[105.40, \; 120.20]}
        \end{align*}
    \end{block}

    {\footnotesize \textbf{Note}: CI widens as horizon increases!}
    }
\end{frame}
...
rmse = np.sqrt(np.mean((np.array(forecasts)
                        - np.array(actuals))**2))
        \end{verbatim}
    \end{block}
    }
