...
\begin{frame}{Quiz 1: VAR Definition}
    ...
    \begin{exampleblock}{Answer: A}
        VAR($p$) has $p$ coefficient matrices. VAR(2) has $\bA_1$ and $\bA_2$, regardless of the number of variables. Each matrix is $K \times K$ (here $3 \times 3$).
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 2: Number of Parameters}
    ...
    \begin{exampleblock}{Answer: C}
        Each equation has: 1 constant + $K \times p$ = $1 + 3 \times 2 = 7$ parameters. The equation for $Y_{1t}$ includes a constant, plus coefficients on $Y_{1,t-1}, Y_{2,t-1}, Y_{3,t-1}, Y_{1,t-2}, Y_{2,t-2}, Y_{3,t-2}$.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 3: Granger Causality}
    ...
    \begin{exampleblock}{Answer: B}
        Granger causality is about \textbf{predictive content}: past values of $X$ contain useful information for forecasting $Y$, beyond what past $Y$ alone provides. It does NOT imply economic causation.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 4: Granger Causality Test}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        $H_0$: $Y_2$ does NOT Granger-cause $Y_1$ means the coefficients $a_{12}^{(1)} = a_{12}^{(2)} = \cdots = a_{12}^{(p)} = 0$ in the $Y_1$ equation.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 5: VAR Stability}
    ...
    \pause
    \begin{exampleblock}{Answer: C}
        Stability requires all eigenvalues of the coefficient matrix to lie inside the unit circle, i.e., $|\lambda_i| < 1$ for all $i$. This ensures shocks die out over time.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 6: Impulse Response Functions}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        IRFs trace the dynamic response of each variable to a one-time shock to another variable, showing how the effect propagates and eventually dies out (in a stable system).
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 7: Lag Order Selection}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        BIC penalizes model complexity more heavily than AIC ($k\ln n$ vs $2k$), typically selecting fewer lags. For forecasting, AIC may be preferred; for inference, BIC's parsimony helps avoid overfitting.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 9: Forecast Error Variance Decomposition}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        FEVD decomposes the variance of the $h$-step forecast error into contributions from each structural shock. It answers: ``How much of the uncertainty in forecasting $Y$ is due to shocks to $Y$ vs shocks to $X$?''
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 11: Cholesky Decomposition}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        Cholesky imposes a recursive structure: variables ordered first can affect later ones within the same period, but not the reverse. The ordering matters and should be justified by economic theory.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 12: VAR Residual Diagnostics}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        Residuals should be white noise: no autocorrelation at any lag. Use Portmanteau test or LM test for residual autocorrelation. Note: cross-equation correlation is allowed (captured by $\Sigma_u$).
    \end{exampleblock}
\end{frame}
...
\begin{frame}{True/False: Solutions}
    \begin{enumerate}
        ...
        {\small \textcolor{MediumGray}{Each variable is regressed on lags of all variables, including itself.}}

        \vspace{0.15cm}
        \item Granger causality proves true economic causation. \hfill \textcolor{Crimson}{\textbf{FALSE}}

        {\small \textcolor{MediumGray}{It only shows predictive content, not structural causation.}}

        \vspace{0.15cm}
        \item A stable VAR always has eigenvalues inside the unit circle. \hfill \textcolor{Forest}{\textbf{TRUE}}

        {\small \textcolor{MediumGray}{Stability condition: all eigenvalues of companion matrix satisfy $|\lambda_i| < 1$.}}

        \vspace{0.15cm}
        \item FEVD results depend on the ordering of variables. \hfill \textcolor{Forest}{\textbf{TRUE}}

        {\small \textcolor{MediumGray}{Under Cholesky identification, different orderings give different results.}}

        \vspace{0.15cm}
        \item VAR can be estimated by OLS equation by equation. \hfill \textcolor{Forest}{\textbf{TRUE}}

        {\small \textcolor{MediumGray}{With same regressors in each equation, OLS = GLS = ML (under normality).}}

        \vspace{0.15cm}
        \item Impulse responses eventually die out in a stable VAR. \hfill \textcolor{Forest}{\textbf{TRUE}}

        {\small \textcolor{MediumGray}{Stability ensures shocks have transitory effects; IRFs $\to 0$ as $h \to \infty$.}}
    \end{enumerate}
\end{frame}
...
\begin{frame}{Example: Stock Returns and Trading Volume}
    \begin{block}{Scenario}
        You have daily data on stock returns ($R_t$) and trading volume ($V_t$). You want to test for Granger causality in both directions.
    \end{block}
...
\begin{frame}{Example: Interest Rates and Inflation}
    ...
    \begin{exampleblock}{VAR Analysis}
        \begin{itemize}
            \item Does inflation Granger-cause interest rates? (Should, if central bank reacts)
            \item Do interest rates Granger-cause inflation? (Monetary policy transmission)
        \end{itemize}

        \vspace{0.2cm}
        Expected: Bidirectional causality with:
        \begin{itemize}
            \item Quick response: $\pi \rightarrow i$ (policy reaction)
            \item Delayed response: $i \rightarrow \pi$ (policy takes effect)
        \end{itemize}
    \end{exampleblock}
\end{frame}

\begin{frame}[fragile]{Example: Python VAR Analysis}
    {\small
    \begin{block}{Step-by-Step Code}
        \begin{verbatim}
import pandas as pd
from statsmodels.tsa.api import VAR
from statsmodels.tsa.stattools import grangercausalitytests

# Load data (GDP growth, unemployment)
data = pd.DataFrame({'gdp': gdp_growth, 'unemp': unemp_rate})

# Check stationarity first!
# Then fit VAR with optimal lag
model = VAR(data)
results = model.fit(maxlags=8, ic='aic')
print(f"Selected lag order: {results.k_ar}")

# Granger causality tests
print("GDP -> Unemployment:")
grangercausalitytests(data[['unemp', 'gdp']], maxlag=4)
print("\nUnemployment -> GDP:")
grangercausalitytests(data[['gdp', 'unemp']], maxlag=4)
        \end{verbatim}
    \end{block}
    }
\end{frame}
...
\begin{frame}{Discussion: Granger Causality vs True Causality}
    \begin{block}{Key Question}
        If $X$ Granger-causes $Y$, does that mean $X$ actually causes $Y$?
    \end{block}
    ...
    \begin{block}{Discussion Points}
        \begin{itemize}
            \item \textbf{Omitted variable bias}: $Z$ might cause both $X$ and $Y$
                \begin{itemize}
                    \item Example: Weather affects both ice cream sales and drownings
                \end{itemize}
            \item \textbf{Anticipation effects}: Markets anticipate future events
                \begin{itemize}
                    \item Stock prices ``Granger-cause'' earnings announcements
                \end{itemize}
            \item \textbf{Aggregation issues}: Timing of data collection matters
        \end{itemize}

        \vspace{0.2cm}
        \textbf{Conclusion}: Granger causality is about \textbf{prediction}, not \textbf{mechanism}. For structural causality, need theory + identification strategy.
    \end{block}
\end{frame}

\begin{frame}{Discussion: Variable Ordering in IRFs}
    \begin{block}{Key Question}
        Why does the ordering of variables matter for orthogonalized IRFs?
    \end{block}
    ...
    \begin{block}{Explanation}
        ...
        \vspace{0.2cm}
        \textbf{Economic reasoning needed}: Order from ``most exogenous'' to ``most endogenous''

        \vspace{0.2cm}
        Example ordering for monetary policy VAR:
        \begin{enumerate}
            \item Oil prices (exogenous)
            \item GDP (slow to respond)
            \item Inflation
            \item Interest rates (policy responds to all)
        \end{enumerate}
    \end{block}
\end{frame}
...
\begin{frame}{Take-Home Exercises}
    {\small
    \begin{enumerate}
        \item \textbf{Theoretical}: Show that a VAR(1) can be written as a VAR($\infty$) MA representation: $\bY_t = \sum_{i=0}^{\infty} \bA^i \bepsilon_{t-i}$ when stable.

        \vspace{0.2cm}
        \item \textbf{Computation}: For VAR(1) with $\bA = \begin{pmatrix} 0.8 & -0.1 \\ 0.3 & 0.4 \end{pmatrix}$:
            \begin{itemize}
                \item Check stability
                \item Compute IRFs for $h = 0, 1, 2, 3$
        ...
        \item \textbf{Applied}: Download US GDP growth and unemployment data:
            \begin{itemize}
                \item Test both series for stationarity
                \item Estimate a VAR model (select optimal lag)
                \item Test Granger causality in both directions
                \item Compute and interpret IRFs
            \end{itemize}
...
\begin{frame}{Key Takeaways from This Seminar}
    \begin{block}{Main Points}
        \begin{enumerate}
            ...
            \item Test statistic is F-test on coefficient restrictions
            \item \textbf{IRFs} show dynamic propagation of shocks
            \item Variable ordering matters for orthogonalized IRFs
        \end{enumerate}
    \end{block}

    \vspace{0.3cm}
    \begin{alertblock}{Key Practical Points}
        \begin{itemize}
            \item Always check stationarity before estimating VAR
            \item Use information criteria for lag selection
            \item Report Granger tests in both directions
            \item Be careful interpreting as ``true'' causality
        \end{itemize}
    \end{alertblock}
\end{frame}
