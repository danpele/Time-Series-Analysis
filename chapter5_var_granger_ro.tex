% Capitolul 5: Modele VAR și Cauzalitate Granger
% Analiza Seriilor de Timp Multivariate
...
% TITLE INFORMATION
%=============================================================================
\title[Capitolul 5: VAR \& Granger]{Capitolul 5: Modele VAR și Cauzalitate Granger}
...
\begin{frame}[plain]
    ...
    \begin{center}
        ...
        {\Huge\textbf{\textcolor{MainBlue}{Capitolul 5: Modele VAR \& Cauzalitate Granger}}}\\[0.5cm]
...
% MOTIVATION
%=============================================================================
\begin{frame}{Exemplu motivațional: Dinamica macroeconomică}
    \vspace{-0.3cm}
    \begin{center}
...
\begin{frame}{De ce modelele univariate nu sunt suficiente}
    ...
    \begin{exampleblock}{Soluția}
        \textbf{Modelele VAR} captează dinamica comună și efectele de feedback dintre mai multe serii de timp.
    \end{exampleblock}
    }
\end{frame}
...
\begin{frame}{Ce vom învăța astăzi}
    ...
    \begin{exampleblock}{Aplicații}
        \begin{itemize}
            ...
            \item Dinamica piețelor financiare (relații acțiuni-obligațiuni)
            \item Analiza ciclului de afaceri (indicatori avansați)
...
\begin{frame}{VAR(2): Dinamica de ordin superior}
...
\begin{frame}{Forma companion}
    ...
    \begin{block}{De ce este utilă?}
        Staționaritatea, prognoza și IRF sunt mai ușor de analizat în forma companion.
    \end{block}
    }
\end{frame}
...
\begin{frame}{Staționaritatea VAR}
    \begin{block}{Condiția de stabilitate}
        ...
        se află \textbf{în afara} cercului unitate (adică $|z| > 1$).
    \end{block}
...
\begin{frame}{Condiția de stabilitate: Interpretare vizuală}
    ...
    \begin{itemize}
        ...
        \item Dacă vreo valoare proprie este în afara cercului, VAR este exploziv (nestaționar)
    \end{itemize}
    }
\end{frame}
...
\begin{frame}{Structura covarianței pentru VAR(1)}
    ...
    \begin{alertblock}{Rezolvarea ecuației Lyapunov}
        Se poate rezolva prin vectorizare:
...
\begin{frame}{Estimarea VAR}
    \begin{block}{Estimarea OLS}
        Fiecare ecuație poate fi estimată prin \textbf{OLS separat}:
...
\begin{frame}{Selecția lag-ului: Exemplu}
    ...
    \begin{itemize}
        ...
        \item Valori mai mici indică o potrivire mai bună a modelului (penalizată de complexitate)
    \end{itemize}
    }
\end{frame}
...
\begin{frame}{Funcții de răspuns la impuls: Exemplu}
    ...
    \begin{itemize}
        ...
        \item Zonele umbrite reprezintă intervale de încredere (incertitudine în estimări)
...
\begin{frame}{Intervale de încredere pentru IRF}
    ...
    \begin{block}{Surse de incertitudine}
        IRF sunt funcții de parametrii estimați $\hat{\bA}_1, \ldots, \hat{\bA}_p$, deci au \textbf{incertitudine de eșantionare}.
    \end{block}
...
\begin{frame}{Prognoze punctuale din VAR}
    \begin{block}{Prognoza iterativă}
        Pentru VAR(1): $\bY_t = \mathbf{c} + \bA \bY_{t-1} + \bepsilon_t$

        \vspace{0.2cm}
        \textbf{Prognoza la 1 pas}: $\hat{\bY}_{T+1|T} = \mathbf{c} + \bA \bY_T$

        \textbf{Prognoza la 2 pași}: $\hat{\bY}_{T+2|T} = \mathbf{c} + \bA \hat{\bY}_{T+1|T}$

        \textbf{Prognoza la $h$ pași}: $\hat{\bY}_{T+h|T} = \mathbf{c} + \bA \hat{\bY}_{T+h-1|T}$
    \end{block}
...
\begin{frame}{Rezultate VAR estimate}
    ...
    \begin{itemize}
        ...
        \item Valorile criteriilor informaționale pentru compararea modelelor
