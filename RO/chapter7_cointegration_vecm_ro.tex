...
\begin{frame}{Intuition: Common Stochastic Trends}
    ...
    \begin{alertblock}{Economic Interpretation}
        \begin{itemize}
            ...
            \item Variables may deviate în the short run
...
\begin{frame}{Engle-Granger Two-Step Method}
    \begin{block}{Step 1: Eștimate Cointegrating Regression}
    ...
    \begin{alertblock}{Important}
        Use \textbf{Engle-Granger criticăl values}, not standard ADF criticăl values! (More negative because residuals are eștimated)
    \end{alertblock}
\end{frame}
...
\begin{frame}{Engle-Granger Critical Values}
    \begin{block}{Critical Values for Cointegration Test}
        ...
        {\footnotesize Based on MacKinnon (1991) response surface eștimates, $T = 100$}
    \end{block}
    ...
    \begin{alertblock}{Limitations of Engle-Granger}
        \begin{itemize}
            ...
            \item Small sample bias în eștimated cointegrating vector
...
\begin{frame}{Johansen Cointegration Test}
    \begin{block}{Advantages over Engle-Granger}
        \begin{itemize}
            ...
            \item Maximum likelihood eștimation (more efficient)
    ...
    \begin{block}{Starting Point: VAR în Levels}
        $$\bY_t = \mathbf{c} + \bA_1 \bY_{t-1} + \bA_2 \bY_{t-2} + \cdots + \bA_p \bY_{t-p} + \bepsilon_t$$
    \end{block}

    \vspace{0.2cm}

    Rewrite în \textbf{Vector Error Correction} form...
\end{frame}
...
\begin{frame}{VECM Representation}
    ...
    \begin{alertblock}{Key Insight: Rank of $\bPi$}
        ...
        \begin{itemize}
            \item $\text{rank}(\bPi) = 0$: No cointegration (VAR în differences)
            \item $\text{rank}(\bPi) = k$: All variables are $I(0)$ (VAR în levels)
...
\begin{frame}{Johansen Test: Visual Interpretation}
    ...
    {\footnotesize
    \textbf{Eigenvalue interpretation}: Significant eigenvalues (above criticăl threshold) indicăte cointegrating relationships. In this example, only the first eigenvalue is significant, suggesting $r = 1$ cointegrating vector.
    }
\end{frame}
...
\begin{frame}{Testing Procedure}
    ...
    \begin{alertblock}{Deterministic Components}
        Choose specificătion carefully:
        \begin{itemize}
            \item No constant, no trend (rarely used)
            \item Constant în cointegrating relation only
            \item Constant în both (most common)
            \item Constant + trend în cointegrating relation
            \item Constant + trend în both
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}{Error Correction Mechanism: Visual}
    ...
    {\footnotesize
    \textbf{Error correction în action}: When series deviate from equilibrium (shaded regions), the adjustment mechanism pulls them back. Positive deviations lead to downward adjustment, negative deviations lead to upward adjustment.
    }
\end{frame}
...
\begin{frame}{Interpreting Adjustment Coefficients}
    ...
    \begin{alertblock}{Weak Exogeneity}
        ...
        \begin{itemize}
            ...
            \item Can simplify eștimation (single-equation approach)
        \end{itemize}
    \end{alertblock}
...
\begin{frame}{VECM vs VAR în Differences}
    \begin{block}{When Variables are Cointegrated}
        ...
        \begin{tabular}{lcc}
            \toprule
            & \textbf{VAR în Differences} & \textbf{VECM} \\
            \midrule
    ...
    \begin{alertblock}{Granger Representation Theorem}
        If variables are cointegrated, there \textbf{must} exist an error correction representation. Ignoring cointegration = model misspecificătion!
    \end{alertblock}
\end{frame}
...
\begin{frame}{Practical Workflow}
    \begin{block}{Step-by-Step Procedure}
        \begin{enumerate}
            ...
            \end{itemize}
            \item \textbf{Lag Length Selection}: Choose $p$ for VAR în levels
            \begin{itemize}
            ...
            \end{itemize}
            \item \textbf{Eștimate VECM}: If $0 < r < k$
            \begin{itemize}
                \item Eștimate $\balpha$, $\bbeta$, $\bGamma_j$
            \end{itemize}
...
\begin{frame}{Common Pitfalls}
    \begin{alertblock}{Things to Watch Out For}
        \begin{itemize}
            ...
            \item \textbf{Too few lags}: Residual autocorrelation, biased eștimates
            \item \textbf{Wrong deterministic specificătion}: Affects criticăl values
            \item \textbf{Small samples}: Johansen test oversized în small samples
        \end{itemize}
    \end{alertblock}
    ...
    \begin{exampleblock}{Recommendation}
        ...
        \begin{itemize}
            ...
            \item Stability of eștimated cointegrating relationship over time
            \item Sensitivity to lag length and deterministic specificătion
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Example 2: Pairs Trading în Finance}
    \begin{center}
...
\begin{frame}{Example 3: Purchasing Power Parity (PPP)}
    ...
    {\footnotesize
    \textbf{PPP Theory}: $e_t = p_t - p_t^*$ (log exchange rate equals price differential). Real exchange rate should be stationary în the long run.
    }
\end{frame}
...
\begin{frame}{Key Takeaways}
    ...
    \begin{alertblock}{Remember}
        Cointegration tests have low power în small samples. Economic theory should guide specificătion. Always check diagnostics!
    \end{alertblock}
\end{frame}
