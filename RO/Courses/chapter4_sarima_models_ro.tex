...
\begin{frame}{Testul Kruskal-Wallis: intuiție}
    ...
    \begin{block}{Ce face acest test?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Verificare}: dacă rangurile sunt distribuite uniform între sezoane (compară suma rangurilor per sezon)
    ...
    \begin{exampleblock}{De ce să-l folosim în locul testului F?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Fără ipoteza de normalitate} -- ideal pentru date financiare cu cozi groase
            \item \textbf{Robust la valori extreme} -- rangurile limitează influența oricărei observații
        \end{itemize}
    \end{exampleblock}
    \vspace{-0.1cm}
...
\begin{frame}{Testul HEGY: ce problemă rezolvă?}
    ...
    \begin{block}{Întrebarea cheie}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Problemă}: având o serie sezonieră, trebuie să știm tipul de diferențiere (alegerea greșită $\succ$ supra/sub-diferențiere)
    ...
    \begin{alertblock}{HEGY testează frecvențe multiple}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Trimestrial}: testează la 0, $\pi$, $\pm\pi/2$ (3 frecvențe $\succ$ o rădăcină obișnuită + două sezoniere)
...
\begin{frame}{Testul HEGY: Formula de regresie (trimestrial)}
    ...
    \begin{block}{Regresia auxiliară HEGY}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Lag-urile $k$ sunt alese prin AIC/BIC pentru a elimina autocorelația reziduală
        \end{itemize}
    \end{block}
    \vspace{-0.15cm}
...
\begin{frame}{Transformarea Box-Cox: Stabilizarea varianței}
    ...
    \begin{exampleblock}{Selectarea automată a lui $\lambda$}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Verosimilitate profilată}: maximizează log-verosimilitatea în funcție de $\lambda$ (oferă IC pentru $\lambda$)
            \item \textbf{Metoda Guerrero (1993)}: minimizează coef.\ de variație al sub-seriilor sezoniere (preferată pentru date sezoniere)
    ...
    \begin{alertblock}{De ce nu doar logaritm?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Log ($\lambda=0$) presupune varianță proporțională cu nivelul --- nu este întotdeauna cazul (ex: $\lambda = 0.3$)
...
\begin{frame}{Demonstrație: diferențierea sezonieră elimină sezonalitatea deterministă}
    ...
    \begin{block}{Proprietățile lui $\Delta_s Y_t = \varepsilon_t - \varepsilon_{t-s}$}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\E[\Delta_s Y_t] = 0$ (medie constantă); $\Var(\Delta_s Y_t) = 2\sigma^2$ (varianță constantă) -- necesare pentru staționaritate
            \item \textbf{Autocovarianța}: $\gamma(s) = -\sigma^2$, $\gamma(k) = 0$ pentru $k \neq 0, s$ (ACF: un singur spike la lag $s$)
        \end{itemize}
    \end{block}
    \vspace{-0.15cm}
...
\begin{frame}{Strategia de împărțire a datelor}
    ...
    {\scriptsize
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Set de antrenare (70\%)} --- Estimarea parametrilor; estimare coeficienți SARIMA ($\phi$, $\theta$, $\Phi$, $\Theta$)
        \item \textbf{Set de validare (15\%)} --- Selectarea celui mai bun model; comparare candidate, alegere cea mai mică eroare
        \item \textbf{Set de test (15\%)} --- Evaluare finală; performanță out-of-sample imparțială, niciodată folosit în dezvoltare
    \end{itemize}
    }
    \end{cminipage}
\end{frame}
