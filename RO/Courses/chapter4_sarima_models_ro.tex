        ...
        \begin{block}{Ce face acest test?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Test neparametric}: verifică dacă observațiile din diferite sezoane provin din aceeași distribuție
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Nu necesită ipoteze despre forma distribuției datelor
                    \end{itemize}
                \item \textbf{Mecanism}: ordonează toate observațiile de la cea mai mică la cea mai mare
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Înlocuiește valorile originale cu ranguri (1, 2, \ldots, $N$)
                    \end{itemize}
                \item \textbf{Verificare}: dacă rangurile sunt distribuite uniform între sezoane
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Calculează suma rangurilor per sezon și compară cu distribuția așteptată
                    \end{itemize}
                \item \textbf{Concluzie}: dacă un sezon are constant ranguri mai mari/mici $\Rightarrow$ sezonalitate
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Exemplu: dacă vara are întotdeauna ranguri mari $\Rightarrow$ consum de energie sezonier
                    \end{itemize}
            \end{itemize}
        \end{block}
        ...
        \begin{exampleblock}{De ce să-l folosim în locul testului F?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Fără ipoteza de normalitate} -- funcționează cu orice distribuție
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Ideal pentru date financiare cu cozi groase sau distribuții asimetrice
                    \end{itemize}
                \item \textbf{Robust la valori extreme} -- valorile extreme nu distorsionează rezultatele
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Rangurile limitează influența oricărei observații individuale
                    \end{itemize}
            \end{itemize}
        \end{exampleblock}
...
\begin{frame}{Testul HEGY: ce problemă rezolvă?}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Întrebarea fundamentală}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Problemă}: având o serie sezonieră, trebuie să știm tipul de diferențiere
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Alegerea greșită duce la supra-diferențiere sau sub-diferențiere
                    \end{itemize}
                ...
                \item \textbf{HEGY}: testează pentru ambele tipuri de rădăcini unitare simultan!
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Propus de Hylleberg, Engle, Granger și Yoo (1990)
                    \end{itemize}
            \end{itemize}
        \end{block}
        \vspace{-0.1cm}
        \begin{exampleblock}{De ce să nu folosim doar ADF?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{ADF (Augmented Dickey-Fuller)}: testează doar pentru o rădăcină unitară obișnuită la frecvența zero
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Ignoră complet componentele sezoniere ale seriei
                    \end{itemize}
        ...
        \begin{alertblock}{HEGY testează frecvențe multiple}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Trimestrial}: testează la 0, $\pi$, $\pm\pi/2$
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item 3 frecvențe $\Rightarrow$ o rădăcină obișnuită + două sezoniere
                    \end{itemize}
                \item \textbf{Lunar}: testează la 0, $\pi$, $\pm\pi/6$, $\pm\pi/3$, $\pm\pi/2$, $\pm 2\pi/3$, $\pm 5\pi/6$
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item 6 frecvențe $\Rightarrow$ o rădăcină obișnuită + cinci sezoniere
                    \end{itemize}
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
        ...
        \begin{block}{Regresia auxiliară HEGY}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Date trimestriale} ($s=4$): $\Delta_4 y_t = \pi_1 z_{1,t-1} + \pi_2 z_{2,t-1} + \pi_3 z_{3,t-2} + \pi_4 z_{4,t-2} + \sum_{j=1}^{k} \phi_j \Delta_4 y_{t-j} + \varepsilon_t$
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Lag-urile $k$: alese prin AIC (Akaike Information Criterion) / BIC (Bayesian Information Criterion)
                    \end{itemize}
            \end{itemize}
        \end{block}
        \vspace{-0.15cm}
        \begin{block}{Variabile transformate}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{$z_{1t}$}: $(1+L+L^2+L^3)y_t = y_t + y_{t-1} + y_{t-2} + y_{t-3}$
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Captează componenta de frecvență zero (trend)
                    \end{itemize}
                \item \textbf{$z_{2t}$}: $-(1-L+L^2-L^3)y_t = -y_t + y_{t-1} - y_{t-2} + y_{t-3}$
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Captează componenta de frecvență $\pi$ (alternanță semestrială)
                    \end{itemize}
                \item \textbf{$z_{3t}$, $z_{4t}$}: captează componentele de frecvență $\pm\pi/2$ (ciclu anual)
            \end{itemize}
        \end{block}
        \vspace{-0.15cm}
        ...
        \begin{block}{Familia de transformări Box-Cox}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Cazuri speciale}: $\lambda=1$ (fără transformare), $\lambda=0$ (logaritm), $\lambda=0.5$ (rădăcină pătrată)
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item $\lambda < 1$ comprimă valorile mari; $\lambda > 1$ le amplifică
                    \end{itemize}
            \end{itemize}
        \end{block}
        \vspace{-0.1cm}
        \begin{exampleblock}{Selectarea automată a lui $\lambda$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Verosimilitate profilată}: maximizează log-verosimilitatea în funcție de $\lambda$
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Cea mai utilizată metodă; oferă interval de încredere pentru $\lambda$
                    \end{itemize}
                \item \textbf{Metoda Guerrero (1993)}: minimizează coeficientul de variație al sub-seriilor sezoniere
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Preferată pentru date sezoniere cu amplitudine variabilă
                    \end{itemize}
                \item \textbf{Python}: \texttt{boxcox(y)} din \texttt{scipy.stats}
                \item \textbf{R}: \texttt{BoxCox.lambda\_(y)} din pachetul \texttt{forecast}
            \end{itemize}
        \end{exampleblock}
        \vspace{-0.1cm}
        \begin{alertblock}{De ce nu doar logaritm?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Log ($\lambda=0$) presupune varianță proporțională cu nivelul
                \item Nu este întotdeauna cazul: date cu varianță moderată pot necesita $\lambda = 0.3$, nu $\lambda = 0$
        ...
        \begin{block}{Afirmație}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Dacă $Y_t = \mu_t + \varepsilon_t$ unde $\mu_t = \mu_{t-s}$ (medie periodică), atunci $\Delta_s Y_t$ elimină media sezonieră
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Condiția $\mu_t = \mu_{t-s}$ înseamnă că media se repetă exact la fiecare $s$ perioade
                    \end{itemize}
            \end{itemize}
        \end{block}
        \vspace{-0.15cm}
        ...
        \begin{block}{Proprietățile lui $\Delta_s Y_t = \varepsilon_t - \varepsilon_{t-s}$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\E[\Delta_s Y_t] = 0$ (medie constantă); $\Var(\Delta_s Y_t) = 2\sigma^2$ (varianță constantă)
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Ambele proprietăți sunt necesare pentru staționaritate de ordin 2
                    \end{itemize}
                \item \textbf{Autocovarianța}: $\gamma(s) = -\sigma^2$, $\gamma(k) = 0$ pentru $k \neq 0, s$
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item ACF prezintă un singur vârf la lag-ul $s$, apoi zero
                    \end{itemize}
            \end{itemize}
        \end{block}
        \vspace{-0.15cm}
