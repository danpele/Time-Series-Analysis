...
\begin{frame}{De ce studiem seriile de timp?}
    ...
    \begin{alertblock}{Obiectiv principal: prognoza}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Folosim tiparele istorice pentru a prezice valori viitoare $\rightarrow$ esențial pentru planificarea afacerilor, managementul riscului și deciziile de politică
        \end{itemize}
    \end{alertblock}
    }
    \end{cminipage}
...
\begin{frame}{Exemplu practic: date financiare reale}
    ...
    \begin{block}{S\&P 500 (2024)}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Caracteristici observate}: trend ascendent, volatilitate în clustere, persistență (momentum)
        \end{itemize}
    \end{block}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Modelul de descompunere multiplicativă}
    ...
    \begin{block}{Model}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Ecuație}: $X_t = T_t \times S_t \times \varepsilon_t$ $\rightarrow$ componentele se înmulțesc
        \end{itemize}
    \end{block}
    ...
    \begin{alertblock}{Sfat}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Transformare logaritmică}: multiplicativ $\rightarrow$ aditiv: $\log X_t = \log T_t + \log S_t + \log \varepsilon_t$
        \end{itemize}
    \end{alertblock}
    }
    \end{cminipage}
...
\begin{frame}{Selecția modelului ETS}
    ...
    \begin{exampleblock}{Selecție automată}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Selecție optimală}: balanță între ajustare și complexitate
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
        ...
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{De ce nu CV standard?}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    ...
                    \item \textbf{K-fold standard} $\rightarrow$ data leakage
                \end{itemize}
            \end{alertblock}
        \end{column}
        \begin{column}{0.48\textwidth}
...
\begin{frame}{Date reale: compararea prognozelor}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Cel mai bun}: Holt-Winters multiplicativă --- ideal pentru date cu sezonalitate crescătoare
        \end{itemize}
    \end{block}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Estimarea trendului: abordări multiple}
    ...
    \begin{exampleblock}{Comparație metode}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Media mobilă}: simplă dar cu lag
            \item \textbf{Regresie polinomială}: flexibilă, parametrică
            \item \textbf{HP filter}: standard macroeconomic
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Ajustare sezonieră: vizualizare}
    ...
    \begin{block}{Rezultat}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Seria ajustată sezonier}: dezvăluie trendul subiacent, elimină fluctuațiile periodice
        \end{itemize}
    \end{block}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Exercițiu AI: Gândire critică}
    ...
    \begin{block}{\footnotesize Prompt de testat în ChatGPT / Claude / Copilot}
        {\footnotesize
        ``Folosind yfinance, descarcă datele AAPL. Vreau să văd ce tipare există și să prezic unde va fi prețul anul viitor. Fă-l să arate profesional cu grafice.''
        }
    \end{block}
    \vspace{-2mm}
    {\footnotesize
    ...
    \begin{enumerate}\setlength{\itemsep}{0pt}
        ...
        \item Rescrieți analiza corect și comparați cu un benchmark sezonier naiv.
    \end{enumerate}
    }
    \vspace{-2mm}
    \begin{alertblock}{}
...
\begin{frame}{Răspunsuri quiz}
    ...
    \begin{exampleblock}{Răspunsuri}
        \begin{itemize}\setlength{\itemsep}{1pt}
            ...
            \item[\textcolor{MainBlue}{\textbf{3.}}] \textbf{CV}: K-fold standard ignoră ordinea temporală $\rightarrow$ data leakage
            \item[\textcolor{MainBlue}{\textbf{4.}}] \textbf{$\alpha = 0.9$}: pondere mare pe observații recente, reacționează rapid dar e mai volatilă
            \item[\textcolor{MainBlue}{\textbf{5.}}] \textbf{Trend}: determinist $\rightarrow$ funcție de timp (regresie); stochastic $\rightarrow$ mers aleatoriu (diferențiere)
        \end{itemize}
    \end{exampleblock}
    }
    \end{cminipage}
...
\begin{frame}{Ce urmează?}
    ...
    \begin{block}{Capitolul 1: Procese stochastice și staționaritate}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Procese Fundamentale}: zgomot alb și mers aleatoriu $\rightarrow$ blocuri pentru ARIMA
