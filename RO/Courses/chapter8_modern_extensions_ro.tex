    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Soluții moderne}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{LSTM}: Tipare secvențiale complexe, dependențe pe termen lung
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}
        ...
        \begin{block}{Parametrul Hurst $H$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \begin{itemize}\setlength{\itemsep}{0pt}
                    ...
                    \item $H > 0.5$: Persistență (urmărirea trendului)
...
\begin{frame}{Exponentul Hurst: interpretare vizuală}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{H $>$ 0.5}: Persistentă (urmărirea trendului)
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Estimarea parametrului $d$}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Implementare}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item În Python: pachetul \texttt{arch}, sau \texttt{statsmodels.tsa.arfima.ARFIMA}
...
\begin{frame}{Metrici de evaluare}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Metrici comune (cf.\ Capitolul 0 pentru definiții detaliate)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{RMSE}: $\sqrt{\frac{1}{n}\sum_{i=1}^n (y_i - \hat{y}_i)^2}$ $\Rightarrow$ Eroare în unități originale
                \item \textbf{MAE}: $\frac{1}{n}\sum_{i=1}^n |y_i - \hat{y}_i|$ $\Rightarrow$ Robust la outlieri
                \item \textbf{MAPE}: $\frac{100}{n}\sum_{i=1}^n \left|\frac{y_i - \hat{y}_i}{y_i}\right|$ $\Rightarrow$ Eroare procentuală
...
\begin{frame}{Bitcoin: evoluția prețului și randamentele}
    ...
    \begin{block}{Observații}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Creștere exponențială a prețului $\Rightarrow$ distribuție puternic \textbf{leptokurtotică}
            \item Randamentele zilnice: medie $\approx 0.15\%$, volatilitate $\approx 3.5\%$
            \item Volatility clustering evident $\Rightarrow$ perioadele de criză (2018, 2020, 2022)
...
\begin{frame}{Studiu de caz: prognoza prețului Bitcoin}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{De ce Bitcoin?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Volatilitate \textbf{extremă} și tipare complexe
...
\begin{frame}{Bitcoin: ACF și evidența pentru memorie lungă}
    ...
    \begin{block}{Analiză ACF}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ACF randamente: scădere rapidă $\Rightarrow$ memorie scurtă în medie
            ...
            \end{itemize}
            \item ARFIMA pe volatilitate $>$ ARMA $\Rightarrow$ captează persistența șocurilor
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Bitcoin: GARCH și managementul riscului}
    ...
    \begin{alertblock}{Concluzii -- Studiu Bitcoin}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Valoarea adăugată majoră: \textbf{modelarea volatilității} cu GARCH/EGARCH
            \item ARFIMA captează persistența în volatilitate; RF util pentru features neliniare
...
\begin{frame}{Bitcoin: estimare ARFIMA și comparație modele}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            {\footnotesize
            \begin{exampleblock}{Randamente BTC-USD (2019--2024)}
                \begin{tabular}{lccc}
                    \toprule
            ...
            {\scriptsize
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Hurst $\hat{d} = 0.13$; test: 2024-02--12; split 70/15/15
            \end{itemize}
            }
            }
        \end{column}
        \begin{column}{0.50\textwidth}
            \includegraphics[width=\textwidth, height=0.46\textheight, keepaspectratio]{ch8_btc_model_comparison.pdf}
        \end{column}
    \end{columns}
...
\begin{frame}{Energie: vizualizarea cererii și sezonalitatea multiplă}
    \vspace{-3mm}
    {\footnotesize
    \begin{block}{Tipare identificate}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Zilnic} (24h): vârf dimineață și seara, minim noaptea; \textbf{Săptămânal} (168h): consum redus în weekend
            \item \textbf{Anual} (8766h): vârf vara și iarna; SARIMA nu poate modela simultan 3 perioade!
        \end{itemize}
    \end{block}
    }
    \vspace{-1mm}
...
\begin{frame}{Studiu de caz: prognoza consumului de energie}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Provocări}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Tipare la scale temporale diferite
...
\begin{frame}{Energie: de ce Prophet și TBATS?}
    ...
    \begin{block}{Soluția: modele cu sezonalitate multiplă}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{TBATS}: perioade $[24, 168, 8766]$ $\Rightarrow$ Fourier pentru fiecare sezon; automat, bun pentru producție
            \item \textbf{Prophet}: sezonalitate aditivă/multiplicativă + regresori (temperatură, zile libere)
            \item \textbf{ARIMA clasic}: poate doar 1 sezon $\Rightarrow$ MAPE $\approx 8$--$10\%$
        \end{itemize}
    \end{block}
    }
    \vspace{-1mm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.42\textheight, keepaspectratio]{ch9_multiple_seasonality.pdf}
    \end{center}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Lecții învățate}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item TBATS: cel mai bun \textbf{out-of-the-box} $\Rightarrow$ fără reglaj de hiperparametri
            \end{itemize}
        \end{block}
        \vspace{0.1cm}
        \begin{alertblock}{Ce model alegem?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{LSTM/RF}: dacă ai $>$100.000 observații și tipare neliniare complexe
            \end{itemize}
        \end{alertblock}
        {\footnotesize
        \begin{exampleblock}{}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textit{Detalii complete despre Prophet și TBATS $\Rightarrow$ Capitolul 9}
            \end{itemize}
        \end{exampleblock}
        }
    \end{cminipage}
...
\begin{frame}{Formule principale -- Rezumat}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{ARFIMA(p,d,q) \& Memorie lungă}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\phi(L)(1-L)^d Y_t = \theta(L)\varepsilon_t$, \; $d \in \mathbb{R}$ ($d < 0.5$ pt.\ staționaritate)
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Fluxul recomandat de modelare}
            \begin{enumerate}\setlength{\itemsep}{2pt}
                ...
                \item Adaugă \textbf{features} $\Rightarrow$ Random Forest pentru relații neliniare
