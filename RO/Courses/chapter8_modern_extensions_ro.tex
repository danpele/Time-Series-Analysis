    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{La finalul acestui capitol, veți fi capabili să:}
            \begin{itemize}\setlength{\itemsep}{2pt}
                ...
                \item[\textcolor{MainBlue}{\textbf{3.}}] \textbf{Aplicați} Random Forest, XGBoost și LSTM pentru prognoza seriilor de timp
                \item[\textcolor{MainBlue}{\textbf{4.}}] \textbf{Analizați} arhitectura Temporal Fusion Transformer (TFT) și mecanismele sale de interpretabilitate
                \item[\textcolor{MainBlue}{\textbf{5.}}] \textbf{Evaluați} modelele fundaționale (TimeGPT, Chronos) și paradigma zero-shot
                \item[\textcolor{MainBlue}{\textbf{6.}}] \textbf{Comparați} ML vs.\ econometrie clasică în lumina competițiilor M4/M5
                \item[\textcolor{MainBlue}{\textbf{7.}}] \textbf{Alegeți} metoda potrivită în funcție de context și \textbf{implementați} în Python
            \end{itemize}
        \end{block}
    \end{cminipage}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Limitările modelelor ARIMA (AutoRegressive Integrated Moving Average)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Dificultăți cu \textbf{tipare complexe} și neliniare
        ...
        \begin{alertblock}{Soluții moderne}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Random Forest / XGBoost}: Relații neliniare, robustețe la outlieri
                \item \textbf{LSTM / TFT}: Tipare secvențiale complexe, atenție interpretabilă
                \item \textbf{Foundation Models}: TimeGPT, Chronos --- prognoză zero-shot
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{center}
            ...
            \begin{tabular}{l|c|c|c|c|c|c}
                \toprule
                \textbf{Caracteristică} & \textbf{ARIMA} & \textbf{ARFIMA} & \textbf{RF/XGB} & \textbf{LSTM} & \textbf{TFT} & \textbf{Found.} \\
                \midrule
                Memorie lungă & $\times$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
                Relații neliniare & $\times$ & $\times$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
                Interpretabilitate & $\checkmark$ & $\checkmark$ & $\sim$ & $\times$ & $\checkmark$ & $\times$ \\
                Date puține & $\checkmark$ & $\checkmark$ & $\times$ & $\times$ & $\times$ & $\checkmark$ \\
                Variabile exogene & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\sim$ \\
                Incertitudine & $\checkmark$ & $\checkmark$ & $\sim$ & $\times$ & $\checkmark$ & $\checkmark$ \\
                \bottomrule
            \end{tabular}
            }
        \end{center}
        ...
        \begin{exampleblock}{Regula de aur}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Începeți \textbf{simplu} (ARIMA), apoi creșteți complexitatea doar dacă este justificat de date și performanță.
            \end{itemize}
        \end{exampleblock}
    \end{cminipage}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Memorie scurtă (ARMA)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Sumă finită: $\sum_{k=0}^{\infty} |\rho_k| < \infty$ (mai precis, condiția este pe autocovarianțe: $\sum_{k=0}^{\infty} |\gamma(k)| < \infty$)
            \end{itemize}
        \end{block}
        ...
        \begin{alertblock}{Memorie lungă (ARFIMA)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Sumă infinită: $\sum_{k=0}^{\infty} |\rho_k| = \infty$ (pentru $0 < d < 0.5$, în regimul staționar)
            \end{itemize}
        \end{alertblock}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{defn}[ARFIMA]
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Parametru}: $d \in \mathbb{R}$ este parametrul de \textbf{diferențiere fracționară} (condiția de staționaritate: $d < 0.5$)
            \end{itemize}
        \end{defn}
        }
        \vspace{-0.1cm}
        ...
        \begin{block}{Cazuri particulare}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item $0.5 \leq d < 1$: Nestaționaritate, dar cu revenire la medie
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{center}
            \begin{tabular}{c|l|l}
                ...
                $d = 1$ & Scădere foarte lentă a ACF eșant. & Random walk \\
                \bottomrule
            \end{tabular}
        \end{center}
        ...
        \begin{block}{Parametrul Hurst $H$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item $H = 0.5$: Fără memorie lungă (comportament ARMA standard)
                    \item $H > 0.5$: Persistență (continuarea direcției, \emph{trend-following})
                    \item $H < 0.5$: Anti-persistență (revenire la medie)
                \end{itemize}
            \end{itemize}
        \end{block}
    \end{cminipage}
...
\begin{frame}{Exponentul Hurst: interpretare vizuală}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{H $<$ 0.5}: Revenire la medie \quad \textbf{H = 0.5}: Fără memorie lungă
            \item \textbf{H $>$ 0.5}: Persistență (continuarea direcției, \emph{trend-following})
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Exemplu date reale: analiza memoriei lungi EUR/RON}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Randamente}: $H \approx 0.55$, $d \approx 0.05$ --- aproape de limită, esențial memorie scurtă
            \item \textbf{Randamente pătrate}: $H \approx 0.69$, $d \approx 0.19$ --- memorie lungă în volatilitate
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Exemplu ARFIMA: Volatilitatea realizată S\&P 500}
    ...
    \begin{block}{Rezultate estimare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Date}: Randamentele zilnice S\&P 500 (Yahoo Finance, 2015--2024); Hurst: $H = 0.92$, $d = 0.42$ -- memorie lungă puternică
        \end{itemize}
    \end{block}
    \vspace{-1mm}
    \begin{alertblock}{Observație}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Volatilitatea are \textbf{memorie lungă} -- folosiți ARFIMA sau FIGARCH (Fractionally Integrated GARCH).
        \end{itemize}
    \end{alertblock}
    }
    \vspace{-1mm}
...
\begin{frame}{Estimarea parametrului $d$}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Implementare}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item În Python: pachetul \texttt{arch}, sau \texttt{statsmodels.tsa.arfima.ARFIMA} (clasa dedicată pentru $d$ fracționar)
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Ce este Random Forest?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item La fiecare nod, se selectează \textbf{aleator} un subset de variabile predictoare (\emph{features})
        ...
        \begin{exampleblock}{Avantaje pentru serii de timp}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Oferă \textbf{importanța variabilelor predictoare} (interpretabilitate)
...
\begin{frame}{Feature engineering: ilustrare}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Transformăm seria temporală în variabile predictoare: lag-uri, statistici pe fereastră mobilă (rolling)
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Feature Engineering pentru serii de timp}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Statistici pe fereastră mobilă} (\emph{rolling statistics}): medie mobilă, deviație standard
        ...
        \begin{alertblock}{Atenție: Scurgere informațională (\emph{data leakage})}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Nu folosiți informații din viitor în features
...
\begin{frame}{Importanța variabilelor predictoare și interpretare}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Importanța variabilelor predictoare (\emph{feature importance})}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Scăderea medie a impurității} (\emph{Mean Decrease Impurity}, MDI): Reducerea impurității la fiecare split
        ...
        \begin{block}{Interpretare tipică pentru serii de timp}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \texttt{lag\_1} foarte important $\Rightarrow$ Autocorelație puternică
...
\begin{frame}{Gradient Boosting și XGBoost pentru serii de timp}
    \begin{cminipage}{0.95\textwidth}
        {\small
        \begin{block}{De la Random Forest la Gradient Boosting}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item \textbf{Random Forest}: construiește arbori \emph{independenți} și face media (bagging)
                \item \textbf{Gradient Boosting}: construiește arbori \emph{secvențial}, fiecare corectând erorile precedentului
                \item \textbf{XGBoost} (Chen \& Guestrin, 2016): implementare optimizată cu regularizare $L_1$/$L_2$
            \end{itemize}
        \end{block}
        \begin{block}{Avantaje pentru serii de timp}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item Captează \textbf{interacțiuni neliniare} între variabile predictoare (lag-uri, calendar, exogene)
                \item \textbf{Importanța variabilelor}: identificarea automată a lag-urilor relevante
                \item \textbf{Performanță}: câștigător frecvent în competițiile M5 (Makridakis et al., 2022)
                \item \textbf{Limitare}: nu modelează dependența temporală nativ $\Rightarrow$ necesită ingineria variabilelor predictoare
            \end{itemize}
        \end{block}
        \begin{exampleblock}{Implementare Python}
            \texttt{from xgboost import XGBRegressor} --- aceleași variabile predictoare ca pentru Random Forest
        \end{exampleblock}
        }
    \end{cminipage}
\end{frame}
...
%=============================================================================
\section{LSTM: Învățare profundă (\emph{Deep Learning}) pentru serii de timp}
%=============================================================================
...
\begin{frame}{Portret de cercetător: Hochreiter \& Schmidhuber}
    ...
    \begin{columns}[T]
        \begin{column}{0.22\textwidth}
            \centering
            \includegraphics[width=0.95\textwidth, height=0.16\textheight, keepaspectratio]{photo_sepp_hochreiter.jpg}
            \\[0.02cm]
        ...
        \begin{column}{0.76\textwidth}
            \begin{block}{Biografie}
                {\scriptsize \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Sepp Hochreiter}: informatician austriac, profesor la JKU Linz; conducător al ELLIS Unit Linz
                    \item \textbf{J\"{u}rgen Schmidhuber}: informatician germano-elvețian, Director Științific IDSIA
    ...
    \begin{columns}[T]
        \begin{column}{0.22\textwidth}
            \centering
            \includegraphics[width=0.95\textwidth, height=0.16\textheight, keepaspectratio]{photo_juergen_schmidhuber.jpg}
            \\[0.02cm]
        ...
        \begin{column}{0.76\textwidth}
            \begin{exampleblock}{Contribuții principale}
                {\scriptsize
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{LSTM} (1997) --- arhitectură recurentă cu porți; rezolvă gradientul care dispare
                    \item \textbf{Analiza gradientului} (Hochreiter, 1991) --- identificarea problemei fundamentale
                    \item \textbf{Poarta forget} (Gers et al., 2000) --- extensie crucială pentru LSTM
                    \item Fundament pentru modelarea secvențelor în NLP, voce și serii de timp
                \end{itemize}}
            \end{exampleblock}
        \end{column}
    \end{columns}
...
\begin{frame}{De la neuronul biologic la cel artificial}
    ...
    \begin{block}{Analogia}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Dendrite} $\Rightarrow$ \textbf{Intrări} $x_i$ \quad \textbf{Sinapse} $\Rightarrow$ \textbf{Ponderi} $w_i$
            \item \textbf{Soma} $\Rightarrow$ \textbf{Sumă + Activare} \quad \textbf{Axon} $\Rightarrow$ \textbf{Ieșire} $y$
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Rețele neuronale recurente -- RNN (Recurrent Neural Network)}
    \begin{block}{Ideea de bază}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Au \textbf{memorie internă} (stare ascunsă, \emph{hidden state})
    ...
    \begin{alertblock}{Problema gradientului care dispare (\emph{vanishing gradient})}
        \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Celula LSTM: Diagrama detaliată}
    \vspace{-3mm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.65\textheight, keepaspectratio]{ch8_lstm_cell.pdf}
    \end{center}
    \vspace{-0.4cm}
    {\footnotesize
    \begin{columns}[T]
        \begin{column}{0.32\textwidth}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Poarta de uitare} (\emph{Forget Gate}) $f_t$
        ...
        \begin{column}{0.32\textwidth}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Poarta de intrare} (\emph{Input Gate}) $i_t$
        ...
        \begin{column}{0.32\textwidth}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Poarta de ieșire} (\emph{Output Gate}) $o_t$
...
\begin{frame}{LSTM: long short-term memory}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Soluția LSTM}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Celule speciale cu 3 porți: \textbf{Forget} ($f_t$) -- ce uităm, \textbf{Input} ($i_t$) -- ce adăugăm, \textbf{Output} ($o_t$) -- ce transmitem
            \end{itemize}
        \end{block}
        \vspace{-2mm}
        \begin{block}{Ecuațiile LSTM}
            {\footnotesize
                \vspace{-3mm}
                \begin{align*}
            f_t &= \sigma(W_f \cdot [h_{t-1}, x_t] + b_f) & \text{(Forget)} \\[-1mm]
            i_t &= \sigma(W_i \cdot [h_{t-1}, x_t] + b_i) & \text{(Input)} \\[-1mm]
            \tilde{C}_t &= \tanh(W_C \cdot [h_{t-1}, x_t] + b_C) & \text{(Candidate)} \\[-1mm]
            C_t &= f_t \odot C_{t-1} + i_t \odot \tilde{C}_t & \text{(Cell state)} \\[-1mm]
            o_t &= \sigma(W_o \cdot [h_{t-1}, x_t] + b_o) & \text{(Output)} \\[-1mm]
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{exampleblock}{De ce LSTM?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Învață \textbf{tipare complexe} și neliniare
        ...
        \begin{alertblock}{Dezavantaje}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item model de tip \emph{cutie neagră} -- interpretabilitate redusă
                \item Sensibil la \textbf{hiperparametri}
                \item Poate face \textbf{supraajustare} (\emph{overfitting}) ușor
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}

\begin{frame}{Temporal Fusion Transformer (TFT)}
    \quantlet{TSA\_ch8\_tft}{https://github.com/QuantLet/TSA/tree/main/TSA_ch8/TSA_ch8_tft}
    \vspace{-2mm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{Arhitectura TFT (Lim, Ar{\i}k, Loeff \& Pfister, 2021)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Variable Selection Networks}: importanța automată a variabilelor via GRN --- ponderi învățate, eliminând features irelevante
                \item \textbf{Static covariate encoders}: variabile invariante în timp (ID magazin, categorie) condiționează procesarea temporală
                \item \textbf{Procesare temporală}: encoder--decoder LSTM pentru tipare locale și secvențiale
                \item \textbf{Atenție multi-head interpretabilă}: identifică explicit care pași temporali contribuie la prognoză
                \item \textbf{Regresie cuantilică}: predicția simultană a cuantilelor ($\tau = 0.1, 0.5, 0.9$), intervale de incertitudine native
            \end{itemize}
        \end{block}
        \vspace{-2mm}
        \begin{exampleblock}{Inovația cheie: interpretabilitate încorporată în arhitectură}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Ponderile de atenție $\Rightarrow$ care momente din trecut contează cel mai mult
                \item Scoruri de selecție a variabilelor $\Rightarrow$ care features sunt relevante
                \item Performanță \emph{state-of-the-art} pe benchmark-uri: electricitate, trafic, retail
            \end{itemize}
        \end{exampleblock}
        }
    \end{cminipage}
...
\begin{frame}{TFT --- Interpretabilitate și Comparație}
    \quantlet{TSA\_ch8\_tft\_architecture}{https://github.com/QuantLet/TSA/tree/main/TSA_ch8/TSA_ch8_tft_architecture}
    \vspace{-2mm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{Mecanisme de interpretabilitate TFT}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Scoruri de importanță}: învățate automat prin Variable Selection Networks
                \item \textbf{Tipare de atenție temporală}: heatmap-uri --- ce pași din trecut influențează prognoza
            \end{itemize}
        \end{block}
        \vspace{-2mm}
        \begin{block}{Comparație cu alte arhitecturi deep learning}
            \begin{center}
            {\footnotesize
            \begin{tabular}{l|c|c|c|c}
                \toprule
                \textbf{Model} & \textbf{Interpretabil} & \textbf{Covariate} & \textbf{Multi-orizont} & \textbf{Cuantile} \\
                \midrule
                N-BEATS (Oreshkin+, 2020) & Parțial & Nu & Da & Nu \\
                DeepAR (Salinas+, 2020) & Nu & Da & Da & Da \\
                \textbf{TFT} (Lim+, 2021) & \textbf{Da} & \textbf{Da (3 tipuri)} & \textbf{Da} & \textbf{Da} \\
                \bottomrule
            \end{tabular}
            }
            \end{center}
        \end{block}
        \vspace{-2mm}
        \begin{alertblock}{Perspectivă critică}
            Modelele simple (ETS, ARIMA) rămân competitive cu deep learning pe multe tipuri de date (Zeng et al., 2023).
        \end{alertblock}
        {\tiny Ref: Lim, Ar{\i}k, Loeff \& Pfister (2021), \textit{Int.\ J.\ Forecasting}, 37(4), 1748--1764.}
        }
    \end{cminipage}
\end{frame}

\begin{frame}{Modele fundaționale pentru serii de timp}
    \vspace{-3mm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{Paradigmă: pre-antrenare masivă, zero-shot sau fine-tuning}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Analog cu GPT/BERT: model pre-antrenat pe corpus masiv, utilizabil direct fără re-antrenare
            \end{itemize}
        \end{block}
        \vspace{-2mm}
        \begin{block}{TimeGPT (Garza \& Mergenthaler-Canseco, 2024)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Antrenat pe \textbf{100+ miliarde} puncte temporale (finanțe, energie, retail, meteo)
                \item Arhitectură Transformer, prognoză \emph{zero-shot} via API (Nixtla)
            \end{itemize}
        \end{block}
        \vspace{-2mm}
        \begin{block}{Chronos (Ansari et al., 2024, Amazon)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Tokenizarea valorilor în intervale discrete; arhitectură T5 pre-antrenată
                \item Dimensiuni: Mini (20M) $\to$ Large (710M parametri); prognoză \emph{zero-shot}
            \end{itemize}
        \end{block}
        \vspace{-2mm}
        \begin{block}{TimesFM (Google, 2024)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{200M parametri}, pre-antrenat pe 100B puncte temporale reale + sintetice
                \item Arhitectură decoder-only cu input/output patching; prognoză \emph{zero-shot}
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{Foundation Models: Avantaje, Limite și Perspective}
    \vspace{-2mm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{exampleblock}{Avantaje}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Fără expertiză de domeniu}: modelul a ``văzut'' deja mii de tipuri de serii
                \item \textbf{Implementare instantanee}: un singur apel API, fără pipeline de antrenare
                \item \textbf{Transfer learning cross-domain}: cunoștințe transferabile între domenii
            \end{itemize}
        \end{exampleblock}
        \vspace{-2mm}
        \begin{alertblock}{Limitări}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Cutie neagră}: mai puțin interpretabile --- greu de justificat în reglementare
                \item \textbf{Cost computațional} al pre-antrenării (dar inferența e ieftină)
                \item \textbf{Sensibilitate la schimbări de distribuție}: performanța degradează pe regimuri noi
            \end{itemize}
        \end{alertblock}
        \vspace{-2mm}
        \begin{block}{Modele emergente}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Lag-Llama} (Rasul+, 2024), \textbf{Moirai} (Woo+, 2024), \textbf{TimesFM} (Google, 2024)
                \item Complementare econometriei tradiționale, nu înlocuitoare
                \item \textit{Pentru arhitecturi detaliate și comparații, vezi \textbf{Capitolul 11}}
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{Competiția M5: ML vs.\ Econometrie}
    \quantlet{TSA\_ch8\_m5\_competition}{https://github.com/QuantLet/TSA/tree/main/TSA_ch8/TSA_ch8_m5_competition}
    \vspace{-2mm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{Competiția M5 (Makridakis, Spiliotis \& Assimakopoulos, 2022)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{42.840 serii de timp} (date vânzări Walmart, 30.490 participanți)
                \item \textbf{Câștigător}: \textbf{LightGBM} cu inginerie intensivă a variabilelor predictoare
                \item ML a \textbf{dominat} primele poziții, dar ETS/ARIMA rămân \textbf{competitive per serie individuală}
            \end{itemize}
        \end{block}
        \vspace{-2mm}
        \begin{block}{Context: evoluția competițiilor M}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{M4} (2018): câștigător --- hibridul \textbf{ES-RNN} (Smyl, 2020); ML \emph{pur} a performat slab
                \item \textbf{M3} $\to$ \textbf{M4} $\to$ \textbf{M5}: avantajul ML crește progresiv
                \item Factor decisiv M5: \textbf{cross-learning} --- un model pe mii de serii simultan
            \end{itemize}
        \end{block}
        \vspace{-2mm}
        \begin{alertblock}{Lecție}
            Abordările hibride (statistică + ML) au depășit atât metodele clasice \emph{pure}, cât și ML \emph{pur}.
        \end{alertblock}
        }
    \end{cminipage}
\end{frame}

\begin{frame}{ML vs.\ Econometrie: Sinteză}
    \quantlet{TSA\_ch8\_timegpt}{https://github.com/QuantLet/TSA/tree/main/TSA_ch8/TSA_ch8_timegpt}
    \vspace{-2mm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{Comparație sistematică}
            \begin{center}
            \begin{tabular}{l|c|c}
                \toprule
                \textbf{Criteriu} & \textbf{Econometrie clasică} & \textbf{Machine Learning} \\
                \midrule
        ...
        \end{block}
        \vspace{-2mm}
        \begin{exampleblock}{Recomandare: abordări hibride}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item \textbf{Ensemble statistic + ML}: media ponderată ARIMA + LightGBM reduce varianța prognozei
                \item \textbf{Features statistice pentru ML}: reziduurile ARIMA, exponentul Hurst, parametrul $d$ ca variabile predictoare suplimentare pentru gradient boosting
                \item \textbf{Reconciliere ierarhică}: modele statistice la nivel agregat + ML la nivel granular
            \end{itemize}
        \end{exampleblock}
        \vspace{-1mm}
        {\tiny Ref: Makridakis, Spiliotis \& Assimakopoulos (2022), \textit{International Journal of Forecasting}, 38(4), 1325--1346.}
        }
    \end{cminipage}
\end{frame}
...
\begin{frame}{Metrici de evaluare}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Metrici comune}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textit{(Metricile RMSE, MAE, MAPE --- definite în Capitolul 0)}
                \item \textbf{MASE} (Mean Absolute Scaled Error): Comparat cu benchmark naiv --- metric suplimentar pentru comparații între serii
            \end{itemize}
        \end{block}
...
\begin{frame}{Studiu de caz: Bitcoin --- volatilitate și memorie lungă}
    \begin{cminipage}{0.95\textwidth}
        \begin{alertblock}{Acest studiu de caz este prezentat în detaliu în Capitolul 10}
            Analiza completă a Bitcoin (teste de staționaritate, modelare GARCH, exponent Hurst, comparație modele) este disponibilă în \textbf{Capitolul 10: Recapitulare Comprehensivă}.
        \end{alertblock}

        \vspace{0.2cm}

        \begin{block}{Relevanța pentru Capitolul 8}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Bitcoin ilustrează \textbf{memorie lungă} în volatilitate ($H \approx 0.65$--$0.70$)
                \item ARFIMA captează persistența șocurilor mai bine decât ARMA
                \item Combinație optimă: ARFIMA-GARCH + features exogene via Random Forest
            \end{itemize}
        \end{block}
    \end{cminipage}
\end{frame}

\begin{frame}{Studiu de caz: Consum de energie --- sezonalități multiple}
    \begin{cminipage}{0.95\textwidth}
        \begin{alertblock}{Acest studiu de caz este prezentat în detaliu în Capitolul 9}
            Analiza completă a consumului de energie (sezonalități multiple, TBATS, Prophet, comparație modele) este disponibilă în \textbf{Capitolul 9: Prophet și TBATS}.
        \end{alertblock}
        ...
        \begin{block}{Relevanța pentru Capitolul 8}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Sezonalitate multiplă (zilnică, săptămânală, anuală) --- SARIMA nu poate modela simultan
                \item Prophet + regresori reduc MAPE cu $\sim$50\% față de SARIMA
                \item LSTM/RF: utile dacă ai $>$100.000 observații și tipare neliniare complexe
            \end{itemize}
        \end{block}
    \end{cminipage}
\end{frame}
...
%=============================================================================
% REZUMAT FORMULE PRINCIPALE
%=============================================================================
\begin{frame}{Formule principale -- Rezumat}
    \vspace{-3mm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{ARFIMA(p,d,q) \& Memorie lungă}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\phi(L)(1-L)^d Y_t = \theta(L)\varepsilon_t$, \; $d \in \mathbb{R}$ ($d < 0.5$ pentru staționaritate)
                \item ACF: $\rho_k \sim C \cdot k^{2d-1}$ \quad Hurst: $d = H - 0.5$ \quad $H > 0.5$: persistență
            \end{itemize}
        \end{block}
        \vspace{-2mm}
        \begin{block}{Random Forest \& LSTM}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item RF: $\hat{y} = \frac{1}{B}\sum_{b=1}^{B} T_b(x)$ \quad $B$ arbori, features aleatorii
                \item LSTM: $f_t = \sigma(W_f[h_{t-1}, x_t] + b_f)$, \; $C_t = f_t \odot C_{t-1} + i_t \odot \tilde{C}_t$ \quad (Forget, Input, Output gates)
            \end{itemize}
        \end{block}
        \vspace{-2mm}
        \begin{block}{Metrici evaluare \& Time Series CV}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item RMSE, MAE, MAPE \textit{(vezi Capitolul 0 pentru definițiile complete)}
                \item Walk-forward validation --- Train $\Rightarrow$ Test (temporal split)
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{De ce EUR/RON?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Tipare influențate de \textbf{factori macroeconomici}
...
\begin{frame}{Vizualizarea seriei EUR/RON}
    ...
    \end{center}
    \quantlet{TSA\_ch8\_eurron\_foundation}{https://github.com/QuantLet/TSA/tree/main/TSA_ch8/TSA_ch8_eurron_foundation}
\end{frame}
...
\begin{frame}{Analiză ACF: randamente vs randamente pătrate}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Date}: Randamentele zilnice EUR/RON (Yahoo Finance, 2019--2025)
            \item \textbf{Stânga}: ACF randamente --- scădere rapidă; \textbf{Dreapta}: ACF randamente pătrate --- scădere lentă
            \item Indică \textbf{volatility clustering} (efecte ARCH)
        \end{itemize}
    \end{block}
    }
    \vspace{-1mm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.48\textheight, keepaspectratio]{ch8_case_acf_analysis.pdf}
    \end{center}
    \quantlet{TSA\_ch8\_eurron\_foundation}{https://github.com/QuantLet/TSA/tree/main/TSA_ch8/TSA_ch8_eurron_foundation}
\end{frame}
...
\begin{frame}{Rezultate test memorie lungă -- EUR/RON}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Output script (\texttt{generate\_ch8\_eurron\_foundation.py})}
            {\footnotesize
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \texttt{Phillips-Perron p-value: 0.0000} (randamentele sunt staționare)
                \item \texttt{Exponentul Hurst (H): 0.5476}
                \item \texttt{Parametrul d estimat: 0.0476}
                \item \texttt{Hurst randamente pătrate: H = 0.6896, d = 0.1896}
            \end{itemize}
            }
        \end{block}
        ...
        \begin{alertblock}{Interpretare}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item $H \approx 0.55$: aproape de limită, ușoară persistență
                \item $d \approx 0.05$: esențialmente \textbf{memorie scurtă} -- ARMA poate fi suficient
                \item \textbf{Randamente pătrate}: $H \approx 0.69$, $d \approx 0.19$ -- memorie lungă în volatilitate!
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
...
\begin{frame}{LSTM: curba de învățare}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Date}: Cursul EUR/RON (2019--2025), 100 epoci, MSE loss
            \item Training loss scade rapid, apoi se stabilizează; validation loss urmărește --- fără overfitting sever
        \end{itemize}
    \end{block}
    }
    \vspace{-1mm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.48\textheight, keepaspectratio]{ch8_case_lstm_training.pdf}
    \end{center}
    \quantlet{TSA\_ch8\_eurron\_foundation}{https://github.com/QuantLet/TSA/tree/main/TSA_ch8/TSA_ch8_eurron_foundation}
\end{frame}
...
\begin{frame}{Vizualizare: predicții vs valori reale}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Perioada de test EUR/RON --- ARIMA, RF, MLP/LSTM vs valori reale
            \item Toate modelele captează tiparul general; niciun model nu prezice perfect vârfurile de volatilitate (\textbf{eficiența pieței})
        \end{itemize}
    \end{block}
    }
    \vspace{-1mm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.48\textheight, keepaspectratio]{ch8_case_predictions.pdf}
    \end{center}
    \quantlet{TSA\_ch8\_eurron\_foundation}{https://github.com/QuantLet/TSA/tree/main/TSA_ch8/TSA_ch8_eurron_foundation}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{center}
            ...
            \begin{tabular}{l|c|c|c|c}
                ...
                \midrule
                ARIMA(1,1,1) & 0.0139 & 0.0038 & 24.32 & Da \\
                ARFIMA(1,$d$,1) & 0.0172 & 0.0088 & 0.16 & Da \\
                Random Forest & 0.0314 & 0.0104 & 0.58 & Parțial \\
                MLP/LSTM & 0.0162 & 0.0072 & 0.52 & Nu \\
                \bottomrule
            \end{tabular}
            }
        \end{center}
        {\scriptsize \textit{Prognoză rolling 1-pas pe niveluri de preț; split temporal 70/15/15; $d = 0.048$ (Hurst).}}

        \vspace{0.1cm}

        \begin{alertblock}{Concluzii}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item ARIMA obține cel mai mic \textbf{RMSE} --- greu de depășit pe piață eficientă
                \item ARFIMA și MLP/LSTM sunt aproape, dar \textbf{nu mai bune} decât ARIMA
                \item Random Forest supraaproximează ușor pe niveluri de preț
                \item Pentru EUR/RON, simplitatea câștigă --- \textbf{principiul parsimoniei} confirmat
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Comparație modele: metrici de performanță}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item EUR/RON (2019--2025): ARIMA vs ARFIMA vs RF vs MLP/LSTM
            \item \textbf{Stânga}: RMSE (mai mic = mai bun) --- ARIMA obține cel mai mic RMSE
            \item \textbf{Dreapta}: Timp antrenare (scală log) --- trade-off acuratețe vs. computație
        \end{itemize}
    \end{block}
    }
    \vspace{-1mm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.48\textheight, keepaspectratio]{ch8_case_comparison.pdf}
    \end{center}
    \quantlet{TSA\_ch8\_eurron\_foundation}{https://github.com/QuantLet/TSA/tree/main/TSA_ch8/TSA_ch8_eurron_foundation}
\end{frame}

%=============================================================================
% Foundation Models: Studiu de Caz EUR/RON
% (Tratament detaliat mutat în Capitolul 11 — vezi chapter11_llm_foundation_models_ro.tex)
%=============================================================================

\begin{frame}{Foundation Models: Previzualizare EUR/RON}
    \begin{cminipage}{0.95\textwidth}
    \vspace{-2mm}
    {\footnotesize
    \begin{block}{Rezultate rapide: Zero-Shot pe EUR/RON}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Am aplicat \textbf{Chronos} și \textbf{TimesFM} zero-shot pe EUR/RON (2019--2025)
            \item Ambele modele prognozează \emph{fără antrenare} --- transfer learning pur
            \item Pe această piață eficientă, \textbf{ARIMA clasic rămâne un baseline puternic}
        \end{itemize}
    \end{block}
    \vspace{-2mm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.35\textheight, keepaspectratio]{ch8_foundation_comparison.pdf}
    \end{center}
    \vspace{-2mm}
    \begin{alertblock}{Pentru un tratament complet, vezi Capitolul 11}
        {\scriptsize Capitolul 11 oferă o analiză detaliată a arhitecturilor foundation models (Chronos, TimesFM, Lag-Llama, Moirai, TimeGPT), strategii de tokenizare, zero-shot vs fine-tuning și studii de caz extinse pe multiple domenii.}
    \end{alertblock}
    }
    \end{cminipage}
    \quantlet{TSA\_ch8\_foundation\_comparison}{https://github.com/QuantLet/TSA/tree/main/TSA_ch8/TSA_ch8_foundation_comparison}
\end{frame}
...
\begin{frame}{Studiu de Caz 2: Consumul de Energie --- Introducere}
    \begin{cminipage}{0.95\textwidth}
    \vspace{-2mm}
    \begin{block}{Set de date: Consumul Orar de Electricitate ENTSO-E}
        {\small
        \begin{itemize}\setlength{\itemsep}{1pt}
            \item \textbf{Sursă}: Platforma de Transparență ENTSO-E (date europene de electricitate)
            \item \textbf{Variabilă}: Consumul total orar de electricitate (MW) pentru o țară europeană
            \item \textbf{Perioadă}: 2015--2020, aproximativ 2{,}160 observații zilnice agregate
            \item \textbf{Frecvență}: Zilnică (agregată din date orare)
        \end{itemize}
        }
    \end{block}
    \vspace{-2mm}
    \begin{alertblock}{Provocarea Prognozei}
        {\small
        \begin{itemize}\setlength{\itemsep}{1pt}
            \item \textbf{Sezonalități multiple}: săptămânală (5 zile lucrătoare + weekend) și anuală (vară/iarnă)
            \item \textbf{Efecte neliniare}: temperatura, sărbătorile și activitatea economică influențează consumul
            \item \textbf{Schimbări structurale}: lockdown-urile COVID-19 au cauzat scăderi fără precedent în 2020
            \item \textbf{Obiectiv}: Compararea abordărilor clasice (SARIMA, ARFIMA) vs.\ ML (Random Forest, LSTM)
        \end{itemize}
        }
    \end{alertblock}
    \vspace{-2mm}
    \begin{exampleblock}{De Ce Acest Set de Date?}
        {\small Prognoza energetică este o aplicație cheie din lumea reală: operatorii de rețea au nevoie de prognoze precise pe termen scurt pentru a echilibra cererea și oferta și a evita penele de curent.}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{center}
            \begin{tabular}{l|c|c|l}
                ...
                \hline
                1 & \textcolor{Forest}{\textbf{Random Forest}} & \textcolor{Forest}{\textbf{2.2\%}} & Cel mai bun: captează tipare neliniare \\
                ...
                5 & \textcolor{Purple}{\textbf{SARIMA}} & \textcolor{Purple}{\textbf{14.6\%}} & Dificultăți cu tiparele \\
            \end{tabular}
        \end{center}
        }
        \begin{center}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{LSTM/Deep Learning}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Resurse computaționale disponibile, tipare ascunse
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{Exemplu 2: indicele BET (Bursa București)}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Rezultate pe randamente BVB.RO (2019--2025, RMSE în \%)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item ARFIMA(1,$d$,1): 1.35 -- captează memoria lungă ($H = 0.67$, $d = 0.17$)
                \item ARIMA(1,0,1): 1.38
                \item MLP/LSTM: 1.40
                \item Random Forest: 1.44
            \end{itemize}
        \end{exampleblock}
    \end{cminipage}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Fluxul recomandat de modelare}
            \begin{enumerate}\setlength{\itemsep}{2pt}
                \item Începeți cu \textbf{ARIMA} ca baseline (simplu, rapid, interpretabil)
                \item Testați \textbf{memorie lungă} $\Rightarrow$ ARFIMA dacă $d$ semnificativ
                \item Adăugați \textbf{features} $\Rightarrow$ Random Forest pentru relații neliniare
                \item Doar cu date multe ($>10.000$ obs.) și resurse $\Rightarrow$ LSTM
            \end{enumerate}
        \end{block}
        ...
        \begin{alertblock}{Lecții din studiile de caz}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{EUR/RON}: Piață eficientă $\Rightarrow$ diferențe mici între modele
                \item \textbf{Bitcoin}: Volatilitatea beneficiază de memorie lungă (ARFIMA)
                \item \textbf{Energie}: RF cel mai bun $\Rightarrow$ tipare neliniare complexe
                \item \textbf{Inflație}: Date puține $\Rightarrow$ ARFIMA superior ML
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Exercițiu AI: Gândire critică}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Atenție}: Codul generat de AI poate produce cod funcțional cu aparență de corectitudine. \textit{Asta nu înseamnă că e corect.}
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Ce am învățat}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Random Forest / XGBoost}: Relații neliniare, câștigătoare M5
                \item \textbf{LSTM / TFT}: Deep learning cu atenție interpretabilă și regresie cuantilică
                \item \textbf{Foundation Models}: TimeGPT, Chronos --- paradigma zero-shot
                \item \textbf{Lecția M4/M5}: abordările hibride (statistică + ML) sunt cele mai robuste
            \end{itemize}
        \end{block}

        \begin{alertblock}{Recomandări practice}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Începeți cu modele \textbf{simple} (ARIMA) ca baseline
                \item Folosiți \textbf{Time Series CV} pentru evaluare corectă
                \item ML necesită \textbf{feature engineering} atent (lag-uri, calendar, rolling stats)
                \item Considerați \textbf{foundation models} pentru prototipare rapidă (zero-shot)
                \item Abordarea hibridă: features statistice + ML = cel mai bun compromis
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{Întrebarea 3: Răspuns}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-3mm}
        \begin{center}
            \includegraphics[width=0.98\textwidth, height=0.52\textheight, keepaspectratio]{ch8_quiz3_lstm_gates.pdf}
        \end{center}
        \vspace{-3mm}
        {\small
        \begin{exampleblock}{Răspuns: (B)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Porțile LSTM controlează fluxul de informație, preservând gradientul pe secvențe lungi; RNN simplu pierde semnalul după $\sim$10--20 pași
            \end{itemize}
        \end{exampleblock}
        }
...
\begin{frame}{Întrebarea 4}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Variante de răspuns}
            ...
            \textcolor{MainBlue}{\textbf{(A)}} LSTM --- deep learning captează toate tiparele\\[3pt]
...
\begin{frame}{Întrebarea 5: Răspuns}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-3mm}
        \begin{center}
            \includegraphics[width=0.98\textwidth, height=0.52\textheight, keepaspectratio]{ch8_quiz5_data_leakage.pdf}
        \end{center}
        \vspace{-3mm}
        {\small
        \begin{exampleblock}{Răspuns: (B)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Exemple: medii mobile centrate (valori viitoare), k-fold standard (amestecă ordinea temporală), statistici pe tot setul înainte de split
            \end{itemize}
        \end{exampleblock}
        }
...
\begin{frame}{Ce urmează?}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Extensii și subiecte avansate}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Prophet} (Facebook/Meta) pentru sezonalitate multiplă
                \item \textbf{Neural Prophet}: Prophet + rețele neuronale
                \item \textbf{Conformal prediction}: intervale de incertitudine model-agnostice
                \item \textbf{Reconciliere ierarhică}: prognoze coerente la multiple niveluri de agregare
...
\begin{frame}{Bibliografie II}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Transformers, Foundation Models și Competiții}
            {\small
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Garza, A., \& Mergenthaler-Canseco, M. (2024). TimeGPT-1, arXiv:2310.03589.
                \item Ansari, A.F., et al. (2024). Chronos: Learning the Language of Time Series, arXiv:2403.07815.
                \item Makridakis, S., Spiliotis, E., \& Assimakopoulos, V. (2022). M5 Accuracy Competition: Results, Findings, and Conclusions, \textit{International Journal of Forecasting}, 38(4), 1325--1346.
            \end{itemize}
            }
        \end{block}
    \end{cminipage}
...
\begin{frame}{Bibliografie III}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Modele cu prag, regim-switching și referințe suplimentare}
            {\footnotesize
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Hansen, B.E. (2011). Threshold Autoregression in Economics, \textit{Statistics and Its Interface}, 4(2).
                \item Hamilton, J.D. (1989). A New Approach to Nonstationary Time Series, \textit{Econometrica}, 57(2).
                \item Petropoulos, F., et al. (2022). Forecasting: Theory and Practice, \textit{Int.\ J.\ Forecasting}, 38(3).
                \item Smyl, S. (2020). Hybrid Exponential Smoothing and RNN, \textit{Int.\ J.\ Forecasting}, 36(1).
            \end{itemize}
            }
        \end{block}
        \vspace{-2mm}
        \begin{exampleblock}{Resurse online și cod}
            ...
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{GitHub TSA}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch8} -- Cod Python pentru acest capitol
            \end{itemize}
            }
        \end{exampleblock}
    \end{cminipage}
