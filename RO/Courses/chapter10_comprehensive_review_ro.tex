        ...
        \begin{column}{0.48\textwidth}
            \begin{exampleblock}{Aplicații}
                \begin{itemize}\setlength{\itemsep}{3pt}
                    ...
                    \item \textbf{Proiect de Cercetare Integrator}
                    \item Quiz
                \end{itemize}
            \end{exampleblock}
        \end{column}
    ...
    \begin{columns}[T]
        ...
        \begin{alertblock}{Atenție}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Raportați metricile obținute în afara eșantionului (\emph{out-of-sample})
            \end{itemize}
        \end{alertblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Evaluarea prognozelor dincolo de RMSE}
    ...
    \begin{block}{Metrici alternative}
        \begin{itemize}\setlength{\itemsep}{4pt}
            \item \textbf{MASE} (Mean Absolute Scaled Error): $\dfrac{\text{MAE}_{\text{model}}}{\text{MAE}_{\text{naïve}}}$\,; \; $<1 \Rightarrow$ bate naïve\\[2pt]
            {\footnotesize\textit{Notă}: Numitorul MASE se calculează pe datele de \textbf{antrenament} (MAE-ul metodei naive in-sample, $\frac{1}{T-1}\sum_{t=2}^{T}|y_t - y_{t-1}|$), nu pe datele de test (Hyndman \& Koehler, 2006)}
    ...
    \begin{columns}[T]
        ...
        \begin{alertblock}{Rezultate Bitcoin (GARCH volatilitate)}
            ...
            \begin{itemize}\setlength{\itemsep}{2pt}
                ...
                \item DA 28.7\%: direcția volatilității e dificilă (sub 50\% -- modelul prognozează direcția mai rău decât o monedă aruncată la întâmplare)
            \end{itemize}
        \end{alertblock}
        ...
        \begin{block}{Interpretare}
            \begin{itemize}\setlength{\itemsep}{4pt}
                ...
                \item \textbf{DA 28.7\%}: direcția volatilității e extrem de dificil de prognozat
...
\begin{frame}{Bitcoin: definirea problemei}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Caracteristicile Datelor}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Yahoo Finance (BTC-USD), Ian 2019 -- Ian 2025
                \item Zilnic, $\approx 2.200$ observații
            \end{itemize}
        \end{block}

        \begin{block}{Fapte stilizate}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Medie $\approx 0$, cozi groase (kurtosis $> 3$)
        ...
        \begin{alertblock}{Observație}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Randamentele financiare}: imprevizibile în medie, predictibile în varianță
...
\begin{frame}{Specificarea modelului GARCH}
    ...
    \begin{defn}[Modelul GARCH(p,q)]
        ...
        \vspace{-0.2cm}
        \begin{align*}
            r_t &= \mu + \varepsilon_t, \quad \varepsilon_t = \sigma_t z_t, \quad z_t \sim N(0,1) \\[0.1cm]
            \sigma_t^2 &= \omega + \sum_{i=1}^{q}\alpha_i \varepsilon_{t-i}^2 + \sum_{j=1}^{p}\beta_j \sigma_{t-j}^2
        \end{align*}
        \vspace{-0.4cm}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Condiții}: $\omega > 0$, $\alpha_i \geq 0$, $\beta_j \geq 0$, $\sum \alpha_i + \sum \beta_j < 1$
        \end{itemize}
    \end{defn}
    \vspace{-1mm}
    {\footnotesize
...
\begin{frame}{GARCH: Staționaritate și varianța necondiționată}
    ...
    \begin{exampleblock}{Derivare}
        ...
        \begin{align*}
            ...
            \bar{\sigma}^2 &= \omega + (\alpha_1 + \beta_1)\bar{\sigma}^2 \;\;\Rightarrow\;\; \bar{\sigma}^2 = \frac{\omega}{1 - \alpha_1 - \beta_1}
        \end{align*}
        \vspace{-4mm}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Ipoteză cheie}: $\E[\varepsilon_{t-1}^2] = \E[\sigma_{t-1}^2 z_{t-1}^2] = \E[\sigma_{t-1}^2]\E[z_{t-1}^2] = \bar{\sigma}^2$, deoarece $z_{t-1} \perp \sigma_{t-1}^2$ prin construcția modelului GARCH ($z_t$ este i.i.d.\ și independent de $\sigma_t$)
        \end{itemize}
        \vspace{-2mm}
    \end{exampleblock}
    }
...
\begin{frame}{Bitcoin: selectarea modelului pe setul de validare}
    ...
    \begin{tabular}{lcccl}
        ...
        \midrule
        GARCH(1,1) & 6994.8 & 7020.6 & \textbf{2.638} & \cellcolor{Forest!20}\textbf{Cel mai bun} \\
        GARCH(2,1) & 6993.7 & 7024.6 & 2.640 & \\
        GJR-GARCH(1,1) & 6983.7 & 7014.6 & 2.669 & \\
        EGARCH(1,1) & --- & --- & --- & Eșuat$^*$ \\
        \bottomrule
    \end{tabular}
    \end{center}
...
\begin{frame}{Bitcoin: împărțirea datelor și staționaritate}
    ...
    \begin{alertblock}{De ce contează staționaritatea}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{GARCH}: necesită o serie slab staționară ca variabilă de intrare
            \item \textbf{Prețuri vs Randamente}: prețurile urmează un mers aleatoriu (\emph{random walk}), randamentele sunt staționare
        \end{itemize}
    \end{alertblock}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Student-t: alegerea corectă}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item $\Delta$AIC = 509 --- evidență statistică \textbf{covârșitoare}
...
\begin{frame}{VaR vs expected shortfall: normal vs Student-t}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ES măsoară pierderea medie când VaR este depășit; Student-t: VaR/ES mai mari decât sub normală
        \end{itemize}
    \end{block}
    }
    \vspace{-1mm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.38\textheight, keepaspectratio]{garch_var_es_comparison.pdf}
    \end{center}
    \vspace{-1mm}
    ...
    \begin{block}{Definiții formale}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{ES} (Basel III/IV): $\text{ES}_\alpha = \E[-r_t \mid r_t \leq -\text{VaR}_\alpha] = \frac{1}{\alpha}\int_0^{\alpha} \text{VaR}_u \, du$; ES este \textbf{coerentă}, VaR nu
        \end{itemize}
    \end{block}
    }
\end{frame}
...
\begin{frame}{Value at Risk: exemplu numeric}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Scalare pentru perioade mai lungi}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\text{VaR}_{h\text{ zile}} = \text{VaR}_{1\text{ zi}} \cdot \sqrt{h}$ \quad (presupune randamente i.i.d., ceea ce contrazice ipoteza GARCH)
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Concluzii}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item GJR $\approx$ GARCH: efect de levier absent pentru Bitcoin
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Extensii}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{GJR-GARCH}: efect de levier
                \item \textbf{EGARCH}: șocuri asimetrice
                \item \textbf{MS-GARCH}: regimuri
                \item HAR, Hybrid GARCH + ML
            \end{itemize}
        \end{exampleblock}
        }
    \end{columns}
...
\begin{frame}{Bitcoin: Concluzii}
    \begin{columns}[T]
        ...
        \begin{alertblock}{Lecții metodologice}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Principiul parcimoniei (GARCH(1,1) vs variante)
...
\begin{frame}{Overfitting în alegerea lui $K$}
    \begin{columns}[T]
        ...
        \begin{block}{Pentru datele analizate}
            \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Rupturi structurale: abordare formală}
    \begin{columns}[T]
        ...
        \begin{alertblock}{Problemă}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item ADF poate confunda o \textbf{ruptură structurală} cu o \textbf{rădăcină unitară}
...
\begin{frame}{Prophet: când să-l folosiți}
    \vspace{-2mm}
    {\small
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{Cazuri de utilizare ideale}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Componente \textbf{interpretabile}
            \end{itemize}
        \end{block}
        \vspace{-1mm}
        \begin{alertblock}{Atenție: Rupturi structurale}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Validați pe date out-of-sample.
            \end{itemize}
        \end{alertblock}
...
\begin{frame}{VAR vs VECM (Vector Error Correction Model): cointegrare}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Problemă}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Dacă variabilele sunt $I(1)$ $\Rightarrow$ VAR pe niveluri produce regresii false
            \end{itemize}
        \end{block}
        \vspace{-0.15cm}
        \begin{defn}[VECM]
...
\begin{frame}{Cauzalitatea Granger: Definiție formală}
    ...
    \begin{defn}[Cauzalitatea Granger]
        ...
        unde $\mathcal{F}_t^{X,Y}$ include trecutul lui $X$ și $Y$, iar $\mathcal{F}_t^{Y}$ doar trecutul lui $Y$.
    \end{defn}
    \vspace{-1mm}
    {\small
    \begin{alertblock}{Observație importantă}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item ``$X$ Granger-cauzează $Y$'' $\Rightarrow$ $X$ conține informație utilă pentru prognoza lui $Y$; \textbf{nu} implică cauzalitate structurală
        \end{itemize}
    \end{alertblock}
    \vspace{-1mm}
    \begin{exampleblock}{Procedura de testare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Testul F (sau Wald): $H_0$: coeficienții lag-urilor lui $X$ sunt simultan zero în ecuația lui $Y$; respingem $H_0$ $\Rightarrow$ $X$ Granger-cauzează $Y$
        \end{itemize}
    \end{exampleblock}
    }
\end{frame}
...
\begin{frame}{Bune practici pentru prognoza aplicată}
    ...
    \begin{columns}[T]
        ...
        \begin{alertblock}{Greșeli frecvente}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Contaminarea setului de test
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{AI poate}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Selecta modele (AutoML, căutare pe grilă / \emph{grid search})
...
\begin{frame}{Exercițiu AI: Gândire critică}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{\footnotesize Prompt de testat în ChatGPT / Claude / Copilot}
            {\scriptsize
            ``Descarcă de pe FRED vânzările lunare cu amănuntul din SUA (seria RSXFS) din 2010-01 până în 2024-12 (180 obs.). Fă o analiză completă: descompunere, teste de staționaritate, selecție model (compară ETS, SARIMA și Prophet), prognoză pe 12 luni, evaluare cu RMSE/MAE/MASE pe un split temporal 70/15/15. Cod Python de calitate publicabilă.''
            }
        \end{block}
        \vspace{-2mm}
        {\scriptsize
        \begin{block}{Exercițiu}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                ...
                \item Împărțirea train/test este corectă? Există data leakage?
        ...
        \begin{alertblock}{Atenție}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Codul generat de AI poate produce cod funcțional cu aparență de corectitudine --- \textit{asta nu înseamnă că e corect}
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{Sinteză: de la teorie la practică}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{tabular}{@{}l l l l@{}}
            ...
            Ch8 & Extensii & ARFIMA, RF, LSTM & Memorie lungă \\
...
\begin{frame}{Întrebarea 1: Răspuns}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-3mm}
        \begin{center}
            \includegraphics[width=0.98\textwidth, height=0.52\textheight, keepaspectratio]{ch10_quiz1_model_selection.pdf}
        \end{center}
        \vspace{-3mm}
        {\small
        \begin{exampleblock}{Răspuns: (B)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item ARIMA modelează nivelul, Prophet sezonalitatea, VAR relațiile --- niciunul nu modelează varianța direct
            \end{itemize}
        \end{exampleblock}
        }
...
\begin{frame}{Întrebarea 2: Răspuns}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-3mm}
        \begin{center}
            \includegraphics[width=0.98\textwidth, height=0.52\textheight, keepaspectratio]{ch10_quiz2_overfitting.pdf}
        \end{center}
        \vspace{-3mm}
        {\small
        \begin{exampleblock}{Răspuns: (B)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Modelul memorează zgomotul; soluție: model mai simplu, validare pe set separat
            \end{itemize}
        \end{exampleblock}
        }
...
\begin{frame}{Întrebarea 5: Răspuns}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-3mm}
        \begin{center}
            \includegraphics[width=0.98\textwidth, height=0.52\textheight, keepaspectratio]{ch10_quiz5_long_seasonality.pdf}
        \end{center}
        \vspace{-3mm}
        {\small
        \begin{exampleblock}{Răspuns: (C)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Termenii Fourier cu $K=3$ folosesc doar 6 parametri; Prophet/TBATS gestionează automat
            \end{itemize}
        \end{exampleblock}
        }
...
%=============================================================================
% PROIECT DE CERCETARE INTEGRATOR
%=============================================================================
\section{Proiect de Cercetare Integrator}

\begin{frame}{Proiect: Prognoza Inflației în România}
    \quantlet{TSA\_ch10\_inflation\_forecast}{https://github.com/QuantLet/TSA/tree/main/TSA_ch10/TSA_ch10_inflation_forecast}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Obiectiv}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Aplicarea \textbf{întregii metodologii} (Cap.\ 0--9) pentru prognoza inflației IPC din România
                \item Studiu empiric complet: de la colectarea datelor la evaluarea prognozei
            \end{itemize}
        \end{block}
        ...
        \begin{columns}[T]
            \column{0.5\textwidth}
            \begin{exampleblock}{Surse de date}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{BNR}: rata dobânzii de politică monetară, cursul EUR/RON, masa monetară M3
                    \item \textbf{INS}: IPC (indicele prețurilor de consum), PIB trimestrial, rata șomajului
                    \item \textbf{Eurostat}: HICP, prețuri energie, prețuri import
                    \item \textbf{BVB}: indicele BET, randamente financiare
                \end{itemize}
            \end{exampleblock}

            \column{0.5\textwidth}
            \begin{alertblock}{Design experimental}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Eșantion}: 2005M01 -- 2024M12 (240 obs.\ lunare)
                    \item \textbf{Antrenament}: 2005M01 -- 2022M12 (216 obs.)
                    \item \textbf{Out-of-sample}: ultimii 24 luni (2023--2024) pentru evaluare
                    \item \textbf{Variabila țintă}: rata inflației IPC anuală (\% y-o-y)
                \end{itemize}
            \end{alertblock}
        \end{columns}
        }
...
\begin{frame}{Etapa 1: Analiză Exploratorie (Cap.\ 0--1)}
    \quantlet{TSA\_ch10\_exploratory}{https://github.com/QuantLet/TSA/tree/main/TSA_ch10/TSA_ch10_exploratory}
    \begin{cminipage}{0.95\textwidth}
        {\footnotesize
        \begin{columns}[T]
            \column{0.5\textwidth}
            \begin{block}{Descompunere și vizualizare}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Descompunere \textbf{STL} a inflației: trend + sezonalitate + rezidual
                    \item Grafice ale seriilor: IPC, PIB, EUR/RON, M3, BET
                    \item Identificarea regimurilor: pre-criză, criză 2008, criză COVID, inflație 2022+
                \end{itemize}
            \end{block}

            \vspace{0.15cm}

            \begin{exampleblock}{Analiză spectrală}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Periodograma: periodicități dominante (12 luni, 6 luni)
                    \item Spectrul încrucișat inflație--curs valutar
                \end{itemize}
            \end{exampleblock}

            \column{0.5\textwidth}
            \begin{alertblock}{Teste de staționaritate}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{ADF}: $H_0$: rădăcină unitară
                    \item \textbf{KPSS}: $H_0$: staționaritate
                    \item \textbf{Phillips-Perron}: robust la heteroskedasticitate
                    \item Aplicate pe: IPC, PIB, EUR/RON, M3, BET
                    \item Determinare ordine de integrare $d$ și $D$
                \end{itemize}
            \end{alertblock}

            \vspace{0.15cm}

            \begin{block}{ACF/PACF și diagnostic}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Funcțiile ACF/PACF: structura de dependență
                    \item Teste de normalitate (Jarque-Bera)
                    \item Teste de heteroskedasticitate (ARCH-LM)
                \end{itemize}
            \end{block}
        \end{columns}
        }
...
\begin{frame}{Etapa 2: Modele Univariate (Cap.\ 2--4)}
    \quantlet{TSA\_ch10\_univariate\_models}{https://github.com/QuantLet/TSA/tree/main/TSA_ch10/TSA_ch10_univariate_models}
    \begin{cminipage}{0.95\textwidth}
        {\footnotesize
        \begin{columns}[T]
            \column{0.5\textwidth}
            \begin{block}{ARIMA --- Box-Jenkins}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Identificare}: ACF/PACF + criterii informaționale
                    \item \textbf{Estimare}: MLE pentru $\text{ARIMA}(p,d,q)$
                    \item \textbf{Diagnostic}: Ljung-Box pe reziduuri, Q-Q plot
                    \item Selecție automată: \texttt{auto\_arima} (AIC minim)
                \end{itemize}
            \end{block}

            \vspace{0.15cm}

            \begin{exampleblock}{SARIMA --- Sezonalitate}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item $\text{SARIMA}(p,d,q)(P,D,Q)_{12}$
                    \item Captarea sezonalității lunare ($s=12$)
                    \item Diferențiere sezonieră: $(1-L^{12})$
                \end{itemize}
            \end{exampleblock}

            \column{0.5\textwidth}
            \begin{alertblock}{Modele benchmark}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Random walk}: $\hat{y}_{t+h} = y_t$
                    \item \textbf{Medie mobilă}: MA(12)
                    \item \textbf{ETS}: Error-Trend-Seasonal (Holt-Winters)
                    \item \textbf{Drift}: random walk cu tendință
                \end{itemize}
            \end{alertblock}
            ...
            \begin{block}{Evaluare out-of-sample}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item RMSE, MAE, MAPE pe fereastra 2023--2024
                    \item Rolling window vs.\ expanding window
                    \item \textbf{Exercițiu}: testați dacă supradiferențierea degradează prognoza
                \end{itemize}
            \end{block}
        \end{columns}
        }
...
\begin{frame}{Etapa 3: Modele Multivariate (Cap.\ 6--7)}
    \quantlet{TSA\_ch10\_multivariate\_models}{https://github.com/QuantLet/TSA/tree/main/TSA_ch10/TSA_ch10_multivariate_models}
    \vspace{-0.3cm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{columns}[T]
            \column{0.5\textwidth}
            \begin{block}{VAR: Inflație + Dobândă + EUR/RON + M3}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Selecție lag}: AIC, BIC, HQ
                    \item \textbf{Cauzalitate Granger}: ce variabile prezic inflația?
                    \item \textbf{IRF}: răspunsul inflației la un șoc al dobânzii BNR
                    \item \textbf{FEVD}: cât din varianța inflației e explicată de fiecare variabilă?
                    \item Stabilitate: rădăcinile polinomului caracteristic $|\lambda_i| < 1$
                \end{itemize}
            \end{block}

            \vspace{0.1cm}

            \begin{exampleblock}{Diagnostic VAR}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Teste Portmanteau pe reziduuri multivariate
                    \item Normalitate multivariată (Jarque-Bera)
                    \item Teste ARCH multivariate
                \end{itemize}
            \end{exampleblock}

            \column{0.5\textwidth}
            \begin{alertblock}{Cointegrare Johansen}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Test \textbf{Trace} și \textbf{Maximum Eigenvalue}
                    \item $H_0$: $r = 0$ (fără cointegrare)
                    \item Relație de echilibru pe termen lung:
                    \item[] $\beta' \mathbf{Y}_t = \beta_1 \pi_t + \beta_2 i_t + \beta_3 e_t + \beta_4 m_t$
                    \item Interpretare economică: regula Taylor?
                \end{itemize}
            \end{alertblock}

            \vspace{0.1cm}

            \begin{block}{VECM --- Corecția erorii}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Dacă există cointegrare $\Rightarrow$ VECM
                    \item $\Delta \mathbf{Y}_t = \boldsymbol{\alpha}\boldsymbol{\beta}'\mathbf{Y}_{t-1} + \sum_{i=1}^{p-1}\boldsymbol{\Gamma}_i\Delta\mathbf{Y}_{t-i} + \boldsymbol{\varepsilon}_t$
                    \item Viteza de ajustare $\boldsymbol{\alpha}$: cât de rapid revine sistemul la echilibru?
                \end{itemize}
            \end{block}
        \end{columns}
        }
...
\begin{frame}{Etapa 4: Volatilitate și Machine Learning (Cap.\ 5, 8--9)}
    \quantlet{TSA\_ch10\_volatility\_ml}{https://github.com/QuantLet/TSA/tree/main/TSA_ch10/TSA_ch10_volatility_ml}
    \begin{cminipage}{0.95\textwidth}
        {\footnotesize
        \begin{columns}[T]
            \column{0.5\textwidth}
            \begin{block}{GARCH pe EUR/RON}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Modelarea \textbf{incertitudinii} cursului valutar
                    \item GARCH(1,1), EGARCH, GJR-GARCH
                    \item Distribuție: Normal vs.\ Student-$t$ vs.\ Skew-$t$
                    \item Volatilitatea condiționată ca predictor al inflației
                \end{itemize}
            \end{block}

            \vspace{0.15cm}

            \begin{exampleblock}{Prophet / NeuralProphet}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Sezonalitate multiplă + tendință
                    \item Changepoints automate (regimuri inflaționiste)
                    \item NeuralProphet: componente AR neuronale
                \end{itemize}
            \end{exampleblock}

            \column{0.5\textwidth}
            \begin{alertblock}{Random Forest / XGBoost}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Features}: lag-uri inflație $(y_{t-1}, \ldots, y_{t-12})$
                    \item Variabile macro: dobândă, curs, M3, șomaj
                    \item Features calendar: lună, trimestru
                    \item Importanța variabilelor (feature importance)
                    \item Hiperparametri: CV temporal (TimeSeriesSplit)
                \end{itemize}
            \end{alertblock}

            \vspace{0.15cm}

            \begin{block}{Ensemble și incertitudine}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Ensemble}: media ponderată a modelelor
                    \item Ponderare: inverse RMSE weighting
                    \item \textbf{Conformal prediction}: intervale de predicție model-agnostice, garanție de acoperire
                \end{itemize}
            \end{block}
        \end{columns}
        }
...
\begin{frame}{Etapa 5: Evaluare și Concluzii}
    \quantlet{TSA\_ch10\_evaluation}{https://github.com/QuantLet/TSA/tree/main/TSA_ch10/TSA_ch10_evaluation}
    \vspace{-0.3cm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{Comparație ``Horse Race'' --- Out-of-sample 2023--2024}
            \vspace{-0.1cm}
            \begin{center}
            \begin{tabular}{@{}l c c c c@{}}
                \toprule
                \textbf{Model} & \textbf{RMSE} & \textbf{MAE} & \textbf{MAPE (\%)} & \textbf{DM test $p$-value} \\
                \midrule
                Random Walk & \ldots & \ldots & \ldots & --- \\
                SARIMA$(p,d,q)(P,D,Q)_{12}$ & \ldots & \ldots & \ldots & \ldots \\
                VAR$(p)$ & \ldots & \ldots & \ldots & \ldots \\
                VECM & \ldots & \ldots & \ldots & \ldots \\
                XGBoost & \ldots & \ldots & \ldots & \ldots \\
                Ensemble & \ldots & \ldots & \ldots & \ldots \\
                \bottomrule
            \end{tabular}
            \end{center}
            \vspace{-0.2cm}
            \hfill{\tiny \textit{Completați cu rezultatele proprii. DM test: Diebold-Mariano vs.\ Random Walk.}}
        \end{block}
        }
        ...
        \begin{columns}[T]
            \column{0.5\textwidth}
            \begin{exampleblock}{Testare formală}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Diebold-Mariano}: superioritate statistică a prognozei
                    \item \textbf{Model Confidence Set} (Hansen, Lunde \& Nason, 2011): mulțimea modelelor echivalent-superioare
                    \item Giacomini-White: testare condiționată
                \end{itemize}
            \end{exampleblock}

            \column{0.5\textwidth}
            \begin{alertblock}{Concluzii și livrabil}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Ce abordare funcționează cel mai bine pentru inflația din România? De ce?
                    \item Combinarea modelelor bate modelele individuale?
                    \item \textbf{Livrabil}: raport de 15--20 pagini + cod Python/R reproductibil
                \end{itemize}
            \end{alertblock}
        \end{columns}
        }
...
\begin{frame}{Rezumatul cursului}
\begin{block}{Ce am învățat}
\begin{itemize}
    \item Selecția modelului depinde de caracteristicile datelor: staționaritate, sezonalitate, volatilitate
    \item Metodologia Box-Jenkins oferă un cadru sistematic pentru modelarea seriilor de timp
    \item Evaluarea corectă necesită testare out-of-sample și cross-validare temporală
\end{itemize}
\end{block}

\begin{alertblock}{Important}
Niciun model nu câștigă peste tot. Potriviți modelul la date: ARIMA pentru tendințe, SARIMA pentru sezonalitate, GARCH pentru volatilitate, VAR/VECM pentru dinamici multivariate, Prophet/TBATS pentru tipare complexe. Validați întotdeauna out-of-sample!
\end{alertblock}
\end{frame}
...
\begin{frame}{Ce Urmează? Capitolul 11 --- LLMs și Foundation Models}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{Frontiera prognozei seriilor de timp}
        \begin{itemize}\setlength{\itemsep}{2pt}
            \item[\textcolor{MainBlue}{\textbf{1.}}] \textbf{Arhitectura Transformer} --- self-attention, codificare pozițională și adaptarea lor la serii de timp
            \item[\textcolor{MainBlue}{\textbf{2.}}] \textbf{Foundation Models} --- Chronos, TimesFM, Lag-Llama, Moirai, TimeGPT: arhitecturi și strategii de tokenizare
            \item[\textcolor{MainBlue}{\textbf{3.}}] \textbf{Prognoză Zero-Shot} --- predicții fără antrenare și când să faceți fine-tuning
            \item[\textcolor{MainBlue}{\textbf{4.}}] \textbf{Studii de caz practice} --- EUR/RON, cerere de energie și volatilitate financiară cu foundation models
            \item[\textcolor{MainBlue}{\textbf{5.}}] \textbf{Benchmark-uri și selecția modelelor} --- navigarea peisajului crescând al modelelor pre-antrenate
        \end{itemize}
    \end{block}

    \begin{alertblock}{De ce contează}
        Foundation models redefinesc abordarea prognozei --- permit predicții zero-shot între domenii, fără antrenare specifică. Capitolul 11 oferă instrumentele pentru evaluarea și aplicarea lor.
    \end{alertblock}
    \end{cminipage}
\end{frame}
