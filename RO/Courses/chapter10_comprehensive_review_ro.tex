...
\begin{frame}{Bitcoin: dovezi pentru GARCH}
    ...
    \begin{block}{Observație}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Randamentele pătrate $r_t^2$ prezintă autocorelație semnificativă $\succ$ efecte GARCH
            \item ACF descreștere lentă $\succ$ persistență ridicată a volatilității
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{VaR Backtesting: comparație modele}
    \vspace{-0.1cm}
    {\footnotesize
    \begin{block}{Rezultate Bitcoin --- VaR 5\% (T = 329 zile, așteptat: 16,5 încălcări)}
        \begin{center}
        \begin{tabular}{lcccccc}
            \toprule
            \textbf{Model} & \textbf{AIC} & \textbf{Încălcări} & \textbf{Rată} & \textbf{Kupiec $p$} & \textbf{Chr.\ $p$} & \textbf{Concluzie} \\
            \midrule
            GARCH(1,1)-N & 9769 & 12 & 3,6\% & 0,238 & 1,000 & Prea conservator \\
            \rowcolor{Forest!15} GARCH(1,1)-t & \textbf{9260} & \textbf{17} & \textbf{5,2\%} & \textbf{0,890} & 0,272 & \textcolor{Forest}{\textbf{Cel mai bun}} \\
            GJR-GARCH(1,1)-t & 9260 & 17 & 5,2\% & 0,890 & 0,272 & $\gamma \approx 0$ (simetric) \\
            Hist.\ Simulation & --- & 16 & 4,9\% & 0,909 & 0,216 & Bun, non-parametric \\
            \bottomrule
        \end{tabular}
        \end{center}
    \end{block}
    ...
    \begin{columns}[T]
        ...
        \begin{alertblock}{Lecții practice}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Distribuția inovațiilor contează: \textbf{$\Delta$AIC = 509}
...
\begin{frame}{VaR 1\% Backtesting: comparație modele}
    ...
    \begin{block}{Rezultate Bitcoin --- VaR 1\% (T = 329 zile, așteptat: 3,3 încălcări)}
        ...
        \begin{tabular}{lcccccc}
            ...
            \textbf{Model} & \textbf{AIC} & \textbf{Încălcări} & \textbf{Rată} & \textbf{Kupiec $p$} & \textbf{Chr.\ $p$} & \textbf{Concluzie} \\
            \midrule
            GARCH(1,1)-N & 9769 & 1 & 0,3\% & 0,137 & 1,000 & Acceptabil \\
            GARCH(1,1)-t & \textbf{9260} & 0 & 0,0\% & 1,000 & 1,000 & Acceptabil \\
            GJR-GARCH(1,1)-t & 9260 & 0 & 0,0\% & 1,000 & 1,000 & Acceptabil \\
            Hist.\ Simulation & --- & 4 & 1,2\% & 0,704 & 1,000 & Acceptabil \\
            \bottomrule
        \end{tabular}
        \end{center}
    \end{block}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Concluzii VaR 1\%}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Toate modelele trec} Kupiec ($p > 0{,}05$)
                \item Cu doar 3,3 încălcări așteptate, testul are \textbf{putere statistică scăzută}
                \item Nu putem distinge semnificativ între 0 și 4 încălcări
            \end{itemize}
        \end{exampleblock}
        ...
        \begin{alertblock}{Lecție: 5\% vs 1\%}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item La 5\% ($\approx$16 încălcări): test puternic, Student-t cel mai bun
                \item La 1\% ($\approx$3 încălcări): test slab, modele \textbf{indistinguibile}
                \item \textbf{Concluzie}: puterea testului crește cu $\alpha$ și $T$
            \end{itemize}
        \end{alertblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Pete solare: ciclul solar de 11 ani}
    ...
    \begin{block}{Observație}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item ACF periodică $\succ$ sezonalitate lungă, ideală pentru termeni Fourier
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Funcții de răspuns la impuls (IRF)}
    ...
    \begin{block}{Efecte}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\uparrow$ PIB $\succ$ $\downarrow$ Șomaj (Okun), $\uparrow$ Inflație (cerere), Fed crește rata
        \end{itemize}
    \end{block}
    }
    \begin{center}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Concluzii empirice}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Fourier}: $K=3$ armonici, validat pe set de test
