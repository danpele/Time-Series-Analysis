...
\begin{frame}{VAR: Prognoza (Train/Val/Test)}
    ...
    \begin{block}{Prognoză Rolling One-Step-Ahead}
        VAR captează dinamică PIB-Șomaj. Șocul COVID vizibil în perioadă validare (2020).
    \end{block}
    }
...
\begin{frame}{Quiz rapid}
    \begin{enumerate}
        ...
        \item Care este diferența dintre \textit{overfitting} pe setul de antrenament și performanța pe setul de test?

        \vspace{0.3cm}

        \item De ce este importantă separarea date în train/validation/test?

        \vspace{0.3cm}

        \item Cauzalitatea Granger este echivalentă cu cauzalitatea reală?

        \vspace{0.3cm}

        \item Ce model folosiți pentru o serie cu sezonalitate lungă (ex: 365 zile)?
    \end{enumerate}
\end{frame}
...
\begin{frame}{Răspunsuri quiz}
    {\small
    \begin{enumerate}
        \item \textbf{GARCH} (sau variante EGARCH/GJR-GARCH). Aceste modele captează volatility clustering și cozile groase ale distribuției randamentelor.

        \vspace{0.2cm}

        \item \textbf{Overfitting}: Modelul memorează zgomotul din datele de antrenament, obținând erori mici pe train dar erori mari pe date noi (test). Performanța pe test reflectă capacitatea reală de generalizare.

        \vspace{0.2cm}

        \item \textbf{Train/Validation/Test}: Train-ul estimează parametrii, validarea selectează modelul (ordin, hiperparametri), iar testul evaluează performanța finală fără a fi privit în prealabil.

        \vspace{0.2cm}

        \item \textbf{Nu}. Cauzalitatea Granger testează doar dacă $X$ trecut îmbunătățește predicția lui $Y$ --- este un test de conținut predictiv, nu de cauzalitate structurală sau economică.

        \vspace{0.2cm}

        \item \textbf{Termeni Fourier} (adăugați la ARIMA) sau \textbf{TBATS/Prophet}. SARIMA nu poate gestiona eficient perioade sezoniere foarte mari ($s=365$).
    \end{enumerate}
    }
\end{frame}
