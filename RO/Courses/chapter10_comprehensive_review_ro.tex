...
\begin{frame}{Evaluarea prognozelor dincolo de RMSE}
    \begin{alertblock}{RMSE nu este universal optim}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Metrica trebuie aleasă în funcție de \textbf{obiectivul economic}
        \end{itemize}
    \end{alertblock}

    \vspace{0.1cm}

    \begin{block}{Alte metrici relevante}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{MASE} (Mean Absolute Scaled Error) --- comparație cu naïve
            \item \textbf{Directional Accuracy} --- direcția corectă a schimbării
            \item \textbf{Quantile Loss} --- pentru VaR și prognoze probabilistice
            \item \textbf{CRPS} (Continuous Ranked Probability Score) --- distribuție completă
        \end{itemize}
    \end{block}
...
\begin{frame}{Compararea formală a prognozelor: Diebold--Mariano}
    ...
    \begin{alertblock}{Problemă}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item RMSE mai mic $\neq$ diferență statistic semnificativă
        \end{itemize}
    \end{alertblock}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Mesaj cheie}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $|DM|$ mare $\Rightarrow$ diferență semnificativă
                \item Compararea modelelor trebuie să fie \textbf{statistic fundamentată}
            \end{itemize}
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{GARCH: Distribuții pentru inovații}
    ...
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{alertblock}{Crypto: de ce contează}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Kurtosis $> 3$ (cozi groase)
                \item Evenimente extreme frecvente
        ...
        \begin{exampleblock}{Implicație practică}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Distribuția Student-t produce estimări VaR \textbf{mai realiste}
                \item Numărul de grade de libertate $\nu$ controlează grosimea cozilor
            \end{itemize}
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Aplicație: Value-at-Risk condiționat}
    \begin{defn}[VaR condiționat la nivel $\alpha$]
        ...
        \]
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\sigma_{t+1}^2$ provine din modelul GARCH
            \item $z_{\alpha}$ = cuantila distribuției alese (Normal sau Student-t)
        \end{itemize}
    \end{defn}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Insight cheie}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Randamentele sunt greu predictibile
                \item Volatilitatea este \textbf{predictibilă}
            \end{itemize}
        \end{block}
        ...
        \begin{exampleblock}{Concluzie}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item GARCH este \textbf{model de risc}, nu model de randament
                \item VaR = aplicația directă
            \end{itemize}
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Validarea modelului de risc: Backtesting}
    ...
    \begin{block}{Kupiec Test (Unconditional Coverage)}
        ...
        {(1-\hat{p})^{T-x} \hat{p}^x}
        \right) \sim \chi^2(1), \quad x = \text{încălcări}, \; \hat{p} = x/T
        \]
    \end{block}
    ...
    \begin{columns}[T]
        ...
        \begin{alertblock}{Interpretare}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Prea multe încălcări $\Rightarrow$ risc subestimat
                \item Prea puține $\Rightarrow$ model prea conservator
            \end{itemize}
        \end{alertblock}
    \end{columns}
    ...
    \begin{exampleblock}{Principiu}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Un model de risc trebuie \textbf{validat}, nu doar estimat
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Rupturi structurale: abordare formală}
    \begin{columns}[T]
        ...
        \column{0.5\textwidth}
        \begin{exampleblock}{Trade-off practic}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{SARIMA}: stabil, parcimonios, dar presupune parametri constanți
                \item \textbf{Prophet}: flexibil, detectează changepoints automat
            \end{itemize}
        \end{exampleblock}
...
\begin{frame}{Stabilitatea modelului VAR}
    \begin{block}{Condiția de stabilitate}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Toate valorile proprii ale \textbf{matricei companion} trebuie să fie în interiorul cercului unitate:
        \end{itemize}
        \[
    ...
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{alertblock}{Dacă nu este îndeplinită}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Model instabil
                \item IRF explozive
                \item Prognoze nesustenabile
            \end{itemize}
        \end{alertblock}

        \column{0.5\textwidth}
        \begin{exampleblock}{Verificare practică}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \texttt{results.is\_stable()} în statsmodels
                \item Verificarea stabilității este \textbf{obligatorie} înainte de IRF
            \end{itemize}
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{VAR vs VECM: cointegrare}
    ...
    \begin{block}{Problemă}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Test Johansen: verifică existența relațiilor de cointegrare
        \end{itemize}
    \end{block}
    ...
    \begin{alertblock}{Mesaj cheie}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item VAR este adecvat doar pentru serii \textbf{staționare}
            \item Ignorarea cointegrării produce \textbf{regresii spurioase}
        \end{itemize}
    \end{alertblock}
\end{frame}
