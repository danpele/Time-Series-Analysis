...
\begin{frame}{VaR 1\% Backtesting: comparație modele}
    ...
    \begin{block}{Rezultate Bitcoin --- VaR 1\% (T = 329 zile, așteptat: 3,3 încălcări)}
        ...
        \begin{tabular}{lcccccc}
            ...
            \textbf{Model} & \textbf{AIC} & \textbf{Încălcări} & \textbf{Rată} & \textbf{Kupiec $p$} & \textbf{Chr.\ $p$} & \textbf{Concluzie} \\
            \midrule
            GARCH(1,1)-N & 9769 & 1 & 0,3\% & 0,137 & 1,000 & Prea conservator \\
            GARCH(1,1)-t & \textbf{9260} & 0 & 0,0\% & 1,000 & 1,000 & Prea conservator \\
            GJR-GARCH(1,1)-t & 9260 & 0 & 0,0\% & 1,000 & 1,000 & Prea conservator \\
            \rowcolor{Forest!15} Hist.\ Simulation & --- & \textbf{4} & \textbf{1,2\%} & \textbf{0,704} & 1,000 & \textcolor{Forest}{\textbf{Cel mai precis}} \\
            \bottomrule
        \end{tabular}
        \end{center}
    \end{block}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Concluzii VaR 1\%}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Modelele parametrice: \textbf{prea conservatoare} la 1\%
                \item GARCH-t/GJR-t: 0 încălcări $\succ$ cozi prea groase
                \item Hist.\ Simulation: 1,2\% $\approx$ 1\% --- cel mai bun
            \end{itemize}
        \end{exampleblock}
        ...
        \begin{alertblock}{Lecție: 5\% vs 1\%}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item La 5\%: Student-t excelează (acoperire perfectă)
                \item La 1\%: Hist.\ Simulation mai precisă
                \item \textbf{Concluzie}: modelul optim depinde de $\alpha$!
            \end{itemize}
        \end{alertblock}
    \end{columns}
\end{frame}
