...
\begin{frame}{Întrebarea 1}
    ...
    \begin{block}{Variante de răspuns}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} SARIMA poate gestiona doar o singură perioadă sezonieră $s$ la un moment dat\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} SARIMA necesită erori normal distribuite pentru sezonalități multiple\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} SARIMA poate gestiona sezonalități multiple, dar necesită mai multe date\\[3pt]

        \textcolor{MainBlue}{\textbf{(D)}} SARIMA funcționează doar cu date lunare sau trimestriale
...
\begin{frame}{Întrebarea 1: Răspuns}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item SARIMA gestionează doar \textbf{o singură} perioadă sezonieră $s$. Nu se pot seta $s=24$ (zilnic) și $s=168$ (săptămânal) simultan într-un singur model SARIMA.
    \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{Întrebarea 2}
    ...
    \begin{block}{Variante de răspuns}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} Trend, Bayes, Autoregresiv, Timp, Staționaritate\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} Sezonalitate Trigonometrică, Box-Cox, erori ARMA, Trend, componente Sezoniere\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} Taylor, Box-Cox, ARIMA, Transformare, Smoothing\\[3pt]

        \textcolor{MainBlue}{\textbf{(D)}} Trigonometric, Bayesian, ARMA, Trend, Analiză spectrală
...
\begin{frame}{Întrebarea 3}
    ...
    \begin{block}{Variante de răspuns}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} Modelul devine mai simplu și mai robust\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} Modelul captează tipare sezoniere mai complexe, dar riscă supraajustarea\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} Orizontul de prognoză crește proporțional\\[3pt]

        \textcolor{MainBlue}{\textbf{(D)}} Perioada sezonieră $s$ se schimbă automat
...
\begin{frame}{Întrebarea 4}
    ...
    \begin{block}{Variante de răspuns}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} $g(t)$ = volatilitate GARCH, $s(t)$ = test de staționaritate, $h(t)$ = heteroscedasticitate\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} $g(t)$ = creștere (trend cu changepoints), $s(t)$ = sezonalitate, $h(t)$ = efecte de sărbători\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} $g(t)$ = zgomot gaussian, $s(t)$ = netezire, $h(t)$ = termeni armonici\\[3pt]

        \textcolor{MainBlue}{\textbf{(D)}} $g(t)$ = gradient, $s(t)$ = densitate spectrală, $h(t)$ = exponent Hurst
...
\begin{frame}{Întrebarea 5}
    ...
    \begin{block}{Variante de răspuns}
        ...
        \textcolor{MainBlue}{\textbf{(A)}} Sezonalitate trigonometrică și transformare Box-Cox\\[3pt]

        \textcolor{MainBlue}{\textbf{(B)}} Selecție automată a parametrilor și netezire exponențială\\[3pt]

        \textcolor{MainBlue}{\textbf{(C)}} Efecte de sărbători, regresori externi, changepoints în trend și gestionare nativă a datelor lipsă\\[3pt]

        \textcolor{MainBlue}{\textbf{(D)}} Formulare spațiu-stare și modelare erori ARMA
...
\begin{frame}{Întrebarea 5: Răspuns}
    ...
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item Prophet oferă efecte de sărbători, regresori externi, changepoints în trend și gestionare nativă a datelor lipsă---caracteristici indisponibile în TBATS.
    \end{itemize}
    \end{exampleblock}
    }
