...
\begin{frame}{Ergodicitatea: fundamentul inferenței din date}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{thm}[Condiție Suficientă]
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Dacă $\sum_{h=0}^{\infty} |\gamma(h)| < \infty$ (autocovarianțe absolut sumabile), procesul este ergodic
            \end{itemize}
        \end{thm}
        ...
        \begin{alertblock}{Contraexemplu: staționar dar ne-ergodic}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Fie $Z \sim N(0,1)$ și definim $X_t = Z$ pentru orice $t$
                \item Procesul este strict staționar (distribuțiile finite-dimensionale nu depind de $t$)
                \item Dar $\bar{X}_T = Z$ pentru orice $T$ $\Rightarrow$ media temporală \textbf{nu} converge la $\mu = 0$
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{defn}[Densitatea spectrală de putere]
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \]
                \item $S(\omega)$ descompune varianța procesului pe frecvențe: $\gamma(0) = \int_{-\pi}^{\pi} S(\omega)\, d\omega$
            \end{itemize}
        \end{defn}
        \vspace{-0.2cm}
        \begin{columns}[T]
...
\begin{frame}{Mers aleator cu drift}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Diferențiere}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\Delta X_t = c + \varepsilon_t$ --- constantă plus zgomot alb $\Rightarrow$ seria diferențiată este staționară
            \end{itemize}
        \end{alertblock}
        \vspace{-2mm}
        \begin{exampleblock}{Importanța practică}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item PIB nominal, prețuri de acțiuni $\Rightarrow$ adesea modele ca RW (Random Walk --- mers aleatoriu) cu drift
