...
\begin{frame}{Experiment: ChatGPT vs Fundamentele}
    ...
    \begin{enumerate}\setlength{\itemsep}{1pt}
        \item \textbf{Model pe zgomot}: EUR/RON $\approx$ mers aleatoriu $\succ$ $\Delta X_t = \varepsilon_t$ e \textbf{zgomot alb}
            \\ {\footnotesize ACF$(k) \approx 0\ \forall k \geq 1$ $\succ$ niciun model nu bate prognoza naivă $\hat{X}_{t+1} = X_t$}
        \item \textbf{Semnificație falsă}: cu $T = 5000$, banda ACF $= \pm\frac{1{,}96}{\sqrt{T}} = \pm 0{,}028$
            \\ {\footnotesize $\hat\rho_1 = 0{,}03$ abia depășește banda $\succ$ a ajusta un model pe asta $=$ \textbf{supraajustare}}
        \item \textbf{ADF greșit specificat}: seria are drift dar ADF a rulat fără regresor de trend
            \\ {\footnotesize Specificație greșită $\succ$ putere redusă $\succ$ concluzie nesigură}
    \end{enumerate}
    }
    \vspace{-1mm}
    \begin{alertblock}{}
        {\small \textbf{Discuție}: Dacă prețurile urmează un mers aleatoriu, poate \textit{vreun} model IA să le prezică?}
    \end{alertblock}
\end{frame}

\begin{frame}{Audit cantitativ: Verificarea rezultatelor AI}
    \vspace{-2mm}
    {\small
    \begin{block}{Reproducem afirmațiile AI pe date reale EUR/RON (2019--2024, $T \approx 1300$)}
        \vspace{-1mm}
        {\footnotesize
        \begin{center}
        \begin{tabular}{lll}
            \toprule
            \textbf{Afirmație AI} & \textbf{Verificare} & \textbf{Verdict} \\
            \midrule
            ADF $p = 0{,}67$ pe niveluri & \texttt{adfuller(x, regression='ct')} & Plauzibil (depinde de lag-uri) \\
            $\hat\rho_1 = 0{,}03$ pe $\Delta x_t$ & Bandă: $\pm 1{,}96/\sqrt{1300} = \pm 0{,}054$ & \textcolor{Crimson}{În interiorul benzii!} \\
            ARMA RMSE $= 0{,}0043$ & RMSE naiv pe aceeași perioadă & Naiv $\approx 0{,}0044$ \\
            \bottomrule
        \end{tabular}
        \end{center}
        }
    \end{block}
    \vspace{-1mm}
    \textbf{Testul decisiv}: Diebold--Mariano
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item $H_0$: ARMA și naiv au \textbf{aceeași acuratețe de prognoză}
        \item Rezultat tipic: $p > 0{,}5$ $\succ$ nu putem respinge $H_0$
        \item Modelul ARMA nu adaugă \textbf{nimic} față de $\hat{X}_{t+1|t} = X_t$
    \end{itemize}

    \vspace{1mm}
    \begin{alertblock}{Lecție}
        Comparați întotdeauna cu \textbf{prognoza naivă}. Un model cu RMSE mic este inutil dacă mersul aleatoriu obține același rezultat.
    \end{alertblock}
    }
\end{frame}
