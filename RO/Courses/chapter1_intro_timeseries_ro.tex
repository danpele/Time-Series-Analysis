...
\begin{frame}{Ergodicitatea: fundamentul inferenței din date}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{defn}[Ergodicitate pentru Medie]
            Un proces staționar $\{X_t\}$ este \textbf{ergodic pentru medie} dacă
        ...
        \begin{alertblock}{De ce contează ergodicitatea?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Soluția}: Ergodicitatea permite estimarea lui $\mu$ din $\bar{X}_T$ (media temporală $\to$ media populației)
...
\begin{frame}{Densitatea spectrală: domeniul frecvențelor}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{defn}[Densitatea spectrală de putere]
            Pentru un proces staționar cu $\sum_{h=-\infty}^{\infty} |\gamma(h)| < \infty$, \textbf{densitatea spectrală} este:
                \vspace{-2mm}
                $$S(\omega) = \frac{1}{2\pi} \sum_{h=-\infty}^{\infty} \gamma(h)\, e^{-i\omega h} = \frac{1}{2\pi}\bigl[\gamma(0) + 2\sum_{h=1}^{\infty} \gamma(h)\cos(\omega h)\bigr], \;\; \omega \in [-\pi, \pi].$$
                \vspace{-4mm}
            Descompune varianța: $\gamma(0) = \int_{-\pi}^{\pi} S(\omega)\, d\omega$.
        \end{defn}
        \vspace{-2mm}
        \begin{columns}[T]
            \begin{column}{0.48\textwidth}
...
\begin{frame}{Procese local staționare}
    \quantlet{TSA\_ch1\_locally\_stationary}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_locally_stationary}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-2mm}
        {\footnotesize
...
\begin{frame}{Analiza wavelet}
    \quantlet{TSA\_ch1\_wavelets}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_wavelets}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-2mm}
        {\footnotesize
        ...
        \begin{block}{Transformata wavelet discretă (DWT)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Analiză multirezoluție} (MRA): $X(t) = \sum_{j} D_j(t) + S_J(t)$, unde $D_j$ = detalii la scala $j$, $S_J$ = aproximare
            \end{itemize}
        \end{block}
        }
        \vspace{-1mm}
        {\footnotesize
        \begin{exampleblock}{Wavelet-uri uzuale și avantaje}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Wavelet-uri: \textbf{Haar} (discontinuu, simplu), \textbf{Daubechies} (db4, compact), \textbf{Morlet} (complex, bun pt.\ frecvențe)
                \item \textbf{Avantaj principal} față de Fourier: localizare \textit{simultană} timp-frecvență (principiul Heisenberg)
                \item Fourier: rezoluție perfectă în frecvență, zero în timp; wavelets: compromis adaptiv
            \end{itemize}
        \end{exampleblock}
        }
        \vspace{-1mm}
...
\begin{frame}{Spectrograma și scalograma}
    \quantlet{TSA\_ch1\_spectrogram}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_spectrogram}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-2mm}
        {\footnotesize
        ...
        \begin{alertblock}{Compromisul rezoluție timp--frecvență}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Planul timp-frecvență: STFT $\to$ \textit{dale rectangulare egale}; wavelets $\to$ \textit{dale adaptive} (înguste sus, largi jos)
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{Aplicații wavelet în econometrie}
    \quantlet{TSA\_ch1\_wavelet\_coherence}{https://github.com/QuantLet/TSA/tree/main/TSA_ch1/TSA_ch1_wavelet_coherence}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-2mm}
        {\footnotesize
        ...
        \vspace{-1mm}
        {\footnotesize
        \begin{exampleblock}{Pachete software}
            \begin{itemize}\setlength{\itemsep}{0pt}
        ...
        \vspace{-1mm}
        {\scriptsize \textcolor{gray}{Referințe: Gençay, R., Selçuk, F. \& Whitcher, B. (2002). \textit{An Introduction to Wavelets and Other Filtering Methods in Finance and Economics}, Academic Press.}}
    \end{cminipage}
\end{frame}
