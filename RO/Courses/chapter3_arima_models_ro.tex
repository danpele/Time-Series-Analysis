    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{La finalul acestui capitol, veți fi capabili să:}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                ...
                \item[\textcolor{MainBlue}{\textbf{5.}}] \textbf{Evaluați} prognozele prin metoda ferestrei mobile
...
\begin{frame}{De ce ARIMA? Datele nestaționare sunt pretutindeni}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.52\textheight, keepaspectratio]{ch3_motivation_nonstationary.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
    \begin{exampleblock}{}
...
\begin{frame}{Exemplu: PIB real SUA}
    ...
    \begin{block}{Observații}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Trend} ascendent clar $\Rightarrow$ media nu este constantă
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Distincție importantă}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Eliminarea trendului prin regresie pentru un proces cu rădăcină unitară duce la specificare eronată
                \item Diferențierea unui proces staționar în trend duce, de asemenea, la specificare eronată
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Demonstrație}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item $\Rightarrow$ $\Var(Y_t) = \sum_{i=1}^{t} \sigma^2 = \boxed{t\sigma^2}$
            \end{itemize}
        \end{block}

        \begin{alertblock}{Nestaționaritate}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Varianța depinde de $t$ $\Rightarrow$ încalcă cerința staționarității ($\Var(Y_t) = \gamma(0)$ constant)
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{Demonstrație: autocovarianța mersului aleatoriu}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Demonstrație}
            ...
            \vspace{-0.4cm}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Doar termenii cu $i = j$ sunt nenuli (când $i \leq t-k$)
            \end{itemize}
        \end{block}

        \begin{exampleblock}{ACF}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\rho(k) = \frac{\Cov(Y_t, Y_{t-k})}{\sqrt{\Var(Y_t)\Var(Y_{t-k})}} = \frac{(t-k)\sigma^2}{\sqrt{t\sigma^2 (t-k)\sigma^2}} = \sqrt{\frac{t-k}{t}}$
            \end{itemize}
        \end{exampleblock}
        }
        }
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{$Y_t = \mu + Y_{t-1} + \varepsilon_t$ \quad $\Leftrightarrow$ \quad $Y_t = Y_0 + \mu t + \sum_{i=1}^{t} \varepsilon_i$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Fără drift} (albastru): oscilează aleator în jurul valorii zero; \textbf{Cu drift} $\mu > 0$ (roșu): trend ascendent
                \item Ambele sunt nestaționare --- drift-ul adaugă trend determinist la evoluția stochastică
            \end{itemize}
        \end{block}
        }
...
\begin{frame}{Simularea mersurilor aleatorii}
    ...
    \begin{block}{Observații}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Stânga}: Mersuri aleatorii pure $\Rightarrow$ fără drift, evoluează imprevizibil
            \item \textbf{Dreapta}: Cu drift $\Rightarrow$ trend ascendent în medie
...
\begin{frame}{Exemplu: diferențierea unui mers aleator}
    \begin{cminipage}{0.95\textwidth}
        {\small
        \begin{exampleblock}{Mers aleator la zgomot alb}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Fie $Y_t = Y_{t-1} + \varepsilon_t$ (mers aleator). Luând prima diferență:
                    $$\Delta Y_t = Y_t - Y_{t-1} = \varepsilon_t$$
                \item Prima diferență este zgomot alb $\Rightarrow$ un proces staționar.
            \end{itemize}
        \end{exampleblock}
...
\begin{frame}{Demonstrație: diferențierea induce staționaritatea}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Demonstrație: Mers aleator cu drift $Y_t = \mu + Y_{t-1} + \varepsilon_t$}
            \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{ACF: detectarea nestaționarității}
    ...
    \begin{exampleblock}{}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Sus}: ACF mers aleator scade foarte lent $\Rightarrow$ rădăcină unitate
            \item \textbf{Jos}: După diferențiere, ACF se anulează $\Rightarrow$ staționar
        \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{Diferențierea în practică: exemplul PIB}
    ...
    \begin{exampleblock}{}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Stânga}: PIB în valori absolute $\Rightarrow$ trend ascendent clar (nestaționar)
            \item \textbf{Dreapta}: Rata de creștere PIB (diferența logaritmică) $\Rightarrow$ fluctuează în jurul mediei
...
\begin{frame}{Componentele ARIMA}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Cazuri speciale}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item ARIMA(p,0,q) = ARMA(p,q) $\Rightarrow$ staționar
                \item ARIMA(0,1,0) = Mers aleator
                \item ARIMA(0,1,1) = IMA(1,1) $\Rightarrow$ netezire exponențială
                \item ARIMA(1,1,0) = ARI(1,1) $\Rightarrow$ AR(1) diferențiat
            \end{itemize}
        \end{block}
        }
        }
...
\begin{frame}{Testul ADF}
    ...
    \begin{block}{Observație}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Stânga}: serie staționară $\Rightarrow$ ADF respinge rădăcina unitate
            \item \textbf{Dreapta}: nestaționară $\Rightarrow$ ADF nu respinge
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{Pasul 1: Determinarea lui $d$}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Procedură}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                \item Reprezentați grafic seria de timp $\Rightarrow$ căutați trenduri, varianță în schimbare
                \item Examinați ACF $\Rightarrow$ descreștere lentă sugerează nestaționaritate
...
\begin{frame}{Diagnosticul modelului ARIMA}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Verificări esențiale (aceleași ca pentru ARMA, cf.\ Cap.\ 2)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \begin{enumerate}\setlength{\itemsep}{0pt}
                    ...
                    \item \textbf{Testul Ljung-Box}: p-value $> 0.05$ $\Rightarrow$ fără autocorelație
        ...
        \begin{alertblock}{Acțiuni dacă diagnosticul indică probleme}
            \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Testul Ljung-Box}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Dacă testul indică probleme}
            \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Intervale de prognoză}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Proprietate a seriilor I(1)}
            \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Prognoze pe termen lung pentru ARIMA}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Comportament când $h \to \infty$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Cu drift $c$}: Prognoze punctuale $\Rightarrow$ trend liniar; IC (Interval de Încredere) $\Rightarrow$ lățimea crește cu $\sqrt{h}$
                \item \textbf{Fără drift}: Prognoze punctuale $\Rightarrow$ converg la ultima valoare observată; IC $\Rightarrow$ tot cresc nelimitat
            \end{itemize}
        \end{block}
...
\begin{frame}{Prognoză cu fereastră mobilă: fixă vs expandabilă}
    \vspace{-2mm}
    \begin{center}
    \begin{tikzpicture}[scale=0.75, transform shape]
        % Title for Fixed Window
        \node[font=\bfseries\small, color=IDAred] at (0, 3.8) {Fereastră Fixă (Mobilă)};
    ...
    \begin{block}{Evaluarea acurateții prognozei în afara eșantionului}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Mimează scenariul de prognoză în timp real; oferă multiple erori pentru evaluare
        \end{itemize}
    \end{block}
    }
\end{frame}
...
\begin{frame}{Prognoză cu fereastră mobilă: exemplu pas cu pas}
    \begin{cminipage}{0.95\textwidth}
        {\small
        \vspace{-0.3cm}
        {\footnotesize
        ...
        \begin{block}{Date la momentul $T$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $Y_{T-2} = 100$, $Y_{T-1} = 103$, $Y_T = 108$ $\Rightarrow$ $\Delta Y_{T-1} = 3$, $\Delta Y_T = 5$
            \end{itemize}
        \end{block}
...
\begin{frame}{Interval de încredere: exemplu numeric}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Notă}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item IC se lărgește pe măsură ce orizontul de prognoză crește.
            \end{itemize}
        \end{alertblock}
        }
        }
...
\begin{frame}{Ilustrație: fereastră mobilă}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.50\textheight, keepaspectratio]{ch3_rolling_forecast.pdf}
    \end{center}
    \vspace{-0.3cm}
    {\footnotesize
    \begin{exampleblock}{}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Fereastra mobilă menține estimarea modelului actualizată
        \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{Pasul 2: testarea rădăcinii unitate}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Concluzie}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item PIB este $I(1)$ $\Rightarrow$ Folosim $d = 1$ în modelul ARIMA
            \end{itemize}
        \end{alertblock}
        }
        }
...
\begin{frame}{Pasul 5: diagnosticul modelului}
    ...
    \begin{block}{Analiza reziduurilor}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Ljung-Box: $Q(10) = 5.8$, p-value $= 0.83$ --- fără autocorelație
            \item JB (Jarque-Bera): $156.4$, p $< 0.001$ --- non-normal (outlier COVID)
            \item \textbf{Concluzie}: trece verificările de autocorelație
        \end{itemize}
    \end{block}
    }
...
\begin{frame}{Pasul 7: Prognoză cu fereastră mobilă (Train/Val/Test)}
    \vspace{-0.2cm}
    \begin{center}
    ...
    \vspace{-0.2cm}
    {\footnotesize
    \begin{block}{Prognoză 1-pas înainte cu fereastră mobilă (expandabilă, IC 95\%)}
        \begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{\footnotesize Exercițiu}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                ...
                \item Intervalele de încredere se lărgesc cu orizontul? (proprietate fundamentală I(1))
            \end{enumerate}
        \end{block}
        }
        \vspace{-2mm}
...
\begin{frame}{Întrebarea 1: Răspuns}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Răspuns corect: (B) $\Var(Y_t) = t \cdot \sigma^2$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Varianța mersului aleatoriu crește liniar în timp $\Rightarrow$ de aceea mersurile aleatorii sunt nestaționare
            \end{itemize}
        \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Întrebarea 2: Răspuns}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Răspuns corect: (C) 2 ori}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item I($d$) înseamnă ``integrată de ordin $d$'' $\Rightarrow$ necesită $d$ diferențe pentru staționaritate
            \end{itemize}
        \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Întrebarea 4: Răspuns}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Răspuns corect: (B) Scade exponențial}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item ARIMA(1,1,0) $\Rightarrow$ $\Delta Y_t$ urmează AR(1) cu ACF $\rho_k = \phi_1^k$ (descreștere geometrică)
            \end{itemize}
        \end{exampleblock}
    \end{cminipage}
