    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Provocarea}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Nestaționaritate}: Multe serii economice/financiare sunt I(1) --- PIB, prețuri acțiuni, cursuri valutare au rădăcini unitare
                \item \textbf{Regresia standard}: Cu variabile I(1) $\Rightarrow$ rezultate false; diferențierea pierde informația pe termen lung
            \end{itemize}
        \end{block}
        ...
        \begin{alertblock}{Soluția: Cointegrarea}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Trend stochastic comun}: Unele serii nestaționare se mișcă împreună pe termen lung --- această relație poate fi modelată.
            \end{itemize}
        \end{alertblock}
...
\begin{frame}{Regresia falsă: exemplu vizual}
    ...
    \begin{alertblock}{Atenție}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Rezultat}: Două mersuri aleatorii complet independente prezintă $R^2$ ridicat doar din întâmplare; $R^2$ ridicat apare frecvent, iar $R^2 \to 1$ când $T \to \infty$ (Phillips, 1986)
...
\begin{frame}{Problema regresiei false}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Granger \& Newbold (1974)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Configurare}: Regresia unui mers aleatoriu pe un alt mers aleatoriu independent
                \begin{itemize}
        ...
        \begin{alertblock}{Interpretare economică}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \begin{itemize}
                    ...
                    \item Dar sunt \textit{trase înapoi} spre echilibru în timp
                \end{itemize}
        ...
        \begin{block}{Câte relații de cointegrare?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \begin{itemize}\setlength{\itemsep}{0pt}
                    ...
                    \item $r = k$: Toate variabilele sunt de fapt $I(0)$ --- inconsistent cu ipoteza $I(1)$
                \end{itemize}
            \end{itemize}
        \end{block}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Pasul 2: Testarea staționarității reziduurilor}
            \begin{itemize}\setlength{\itemsep}{-1pt}
                ...
                \item \textbf{Important}: Regresia auxiliară \emph{nu} include constantă, deoarece reziduurile OLS au media zero prin construcție
                \item \textbf{Ipoteze}:
                \begin{itemize}\setlength{\itemsep}{-1pt}
...
\begin{frame}{Teorema Reprezentării Granger (1983)}
    \quantlet{TSA\_ch7\_granger\_theorem}{https://github.com/QuantLet/TSA/tree/main/TSA_ch7/TSA_ch7_granger_theorem}
    \begin{cminipage}{0.95\textwidth}
        {\small
        \begin{block}{Enunț formal}
            Fie $\mathbf{Y}_t$ un vector $K \times 1$ de variabile $I(1)$. Dacă $\mathbf{Y}_t$ este cointegrat cu rang de cointegrare $r$ ($0 < r < K$), cu $\boldsymbol{\Pi} = \boldsymbol{\alpha}\boldsymbol{\beta}'$, atunci:
            \begin{enumerate}\setlength{\itemsep}{1pt}
                \item \textbf{Există} o reprezentare VECM: $\Delta \mathbf{Y}_t = \boldsymbol{\alpha}\boldsymbol{\beta}'\mathbf{Y}_{t-1} + \sum_{j=1}^{p-1}\boldsymbol{\Gamma}_j \Delta\mathbf{Y}_{t-j} + \boldsymbol{\varepsilon}_t$
                \item \textbf{Există} o reprezentare MA (Wold): $\Delta \mathbf{Y}_t = \mathbf{C}(L)\boldsymbol{\varepsilon}_t$, cu $\mathbf{C}(1) \neq \mathbf{0}$, $\text{rang}(\mathbf{C}(1)) = K - r$
                \item \textbf{Reciproc}: Dacă $\Delta\mathbf{Y}_t$ admite o reprezentare MA cu $\text{rang}(\mathbf{C}(1)) = K - r < K$, atunci $\mathbf{Y}_t$ este cointegrat CI(1,1) cu rang $r$
            \end{enumerate}
        \end{block}
        \begin{alertblock}{Implicație fundamentală}
            Cointegrarea $\Leftrightarrow$ Corecția erorilor $\Leftrightarrow$ Trend comun. Cele trei reprezentări sunt \textbf{echivalente}.
        \end{alertblock}
        }
    \end{cminipage}
\end{frame}

\begin{frame}{Demonstrație (schiță) --- Teorema Reprezentării Granger}
    \quantlet{TSA\_ch7\_granger\_theorem}{https://github.com/QuantLet/TSA/tree/main/TSA_ch7/TSA_ch7_granger_theorem}
    \vspace{-0.3cm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{1. Reprezentarea medie mobilă (MA)}
            Din VECM $\Delta \bY_t = \balpha\bbeta'\bY_{t-1} + \sum_{j=1}^{p-1}\bGamma_j\Delta\bY_{t-j} + \bepsilon_t$ se obține prin inversare:
            \vspace{-0.2cm}
            \[
                \Delta \bY_t = \mathbf{C}(L)\bepsilon_t, \quad \text{unde } \mathbf{C}(1) = \bbeta_\perp\!\bigl(\balpha_\perp'(\mathbf{I}_K - \textstyle\sum_{i=1}^{p-1}\bGamma_i)\bbeta_\perp\bigr)^{-1}\!\balpha_\perp'
            \]
            \vspace{-0.3cm}

            cu $\balpha_\perp, \bbeta_\perp$ complementele ortogonale ($\balpha'\balpha_\perp = \mathbf{0}$, $\bbeta'\bbeta_\perp = \mathbf{0}$), iar $\text{rang}(\mathbf{C}(1)) = K - r$.
        \end{block}

        \vspace{-0.15cm}

        \begin{block}{2. Reprezentarea trendurilor comune (Beveridge--Nelson)}
            Prin sumarea $\Delta\bY_s$ de la $s=1$ la $t$:
            \vspace{-0.2cm}
            \[
                \bY_t = \mathbf{C}(1)\sum_{s=1}^{t}\bepsilon_s + \mathbf{C}^*(L)\bepsilon_t + \bY_0
            \]
            \vspace{-0.3cm}
            \begin{itemize}\setlength{\itemsep}{-1pt}
                \item $\mathbf{C}(1)\sum_{s=1}^{t}\bepsilon_s$: \textbf{trenduri stocastice} (mersuri aleatorii), sursa nestaționarității
                \item $\mathbf{C}^*(L)\bepsilon_t$: \textbf{componentă staționară}
                \item Deoarece $\text{rang}(\mathbf{C}(1)) = K - r$, există exact $K - r$ \textbf{trenduri comune}
            \end{itemize}
        \end{block}
        ...
        \begin{alertblock}{Consecință}
            $K$ variabile $I(1)$ cu rang de cointegrare $r$ $\Rightarrow$ $K - r$ trenduri stocastice comune. Cointegrarea \textit{elimină} trenduri: $\bbeta'\bY_t$ anulează componenta nestationară deoarece $\bbeta'\mathbf{C}(1) = \mathbf{0}$. {\upshape\textit{Ref.\ Johansen (1991), Teorema 4.2.}}
        \end{alertblock}
        }
    \end{cminipage}
\end{frame}
...
\begin{frame}{Capcane frecvente}
    \begin{cminipage}{0.95\textwidth}
        \begin{alertblock}{Puncte de atenție}
            \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Pasul 3: Estimare VECM}
    ...
    \begin{block}{Model și ecuații stilizate}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Interpretare}: Rata pe termen scurt se ajustează rapid ($\alpha_1 = -0.15$); rata pe termen lung aproape slab exogenă ($\alpha_2 \approx 0$)
        \end{itemize}
    \end{block}
    }
    \vspace{-2mm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.48\textheight, keepaspectratio]{ch7_case_vecm.pdf}
    \end{center}
...
\begin{frame}{Extensie: Cointegrare cu ruptură structurală}
    \vspace{-0.2cm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{alertblock}{Limitare a testelor standard}
            Testele Engle-Granger și Johansen presupun \textbf{stabilitatea} relației de cointegrare pe întreaga perioadă. Dacă relația se schimbă (e.g., criză financiară, schimbare de regim), aceste teste \textbf{pierd putere}.
        \end{alertblock}

        \vspace{-0.1cm}

        \begin{block}{Testul Gregory-Hansen (1996)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Testează $H_0$: \textbf{fără cointegrare} vs.\ $H_1$: cointegrare cu \textbf{o ruptură structurală} la un moment necunoscut $\tau$
                \item Trei variante:
                    \begin{itemize}\setlength{\itemsep}{-1pt}
                        \item \textbf{C}: schimbare de nivel (constantă)
                        \item \textbf{C/T}: schimbare de nivel și trend
                        \item \textbf{C/S}: schimbare de regim (toți coeficienții)
                    \end{itemize}
                \item \textbf{Procedură}: Se calculează statistica ADF pentru fiecare punct de ruptură posibil $\tau \in [0{,}15T,\; 0{,}85T]$; se raportează $\inf_\tau \text{ADF}(\tau)$
                \item Valorile critice: tabelate de Gregory \& Hansen (1996)
            \end{itemize}
        \end{block}

        \vspace{-0.1cm}

        \begin{exampleblock}{Aplicații tipice}
            Paritatea puterii de cumpărare (PPP) cu rupturi la schimbări de regim valutar, relația Fisher cu schimbări de politică monetară
        \end{exampleblock}
        }
    \end{cminipage}
\end{frame}

\begin{frame}{Cointegrare cu prag (Threshold Cointegration)}
    \quantlet{TSA\_ch7\_threshold\_ecm}{https://github.com/QuantLet/TSA/tree/main/TSA_ch7/TSA_ch7_threshold_ecm}
    \vspace{-0.3cm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{alertblock}{Motivație}
            Modelele liniare ECM presupun \textbf{viteza de ajustare constantă} indiferent de mărimea sau semnul deviației de la echilibru. În realitate, ajustarea poate fi \textbf{asimetrică}: rapidă când deviația este mare, lentă sau inexistentă când este mică (costuri de tranzacție, rigidități).
        \end{alertblock}

        \vspace{-0.1cm}

        \begin{block}{Modelul Threshold ECM (Balke \& Fomby, 1997)}
            Fie $z_t = Y_t - \hat{\bbeta}'X_t$ eroarea de echilibru. Modelul cu prag:
            \vspace{-0.2cm}
            \[
                \Delta Y_t = \rho_1\, z_{t-1}\, \mathbb{I}(z_{t-1} \leq \gamma) \;+\; \rho_2\, z_{t-1}\, \mathbb{I}(z_{t-1} > \gamma) \;+\; \bepsilon_t
            \]
            \vspace{-0.3cm}
            \begin{itemize}\setlength{\itemsep}{-1pt}
                \item $\gamma$: \textbf{pragul} (estimat endogen); $\mathbb{I}(\cdot)$: funcția indicator
                \item \textbf{Regimul 1} ($z_{t-1} \leq \gamma$): ajustare lentă sau inactivă ($\rho_1 \approx 0$)
                \item \textbf{Regimul 2} ($z_{t-1} > \gamma$): ajustare rapidă ($\rho_2 < 0$, semnificativ)
            \end{itemize}
        \end{block}

        \vspace{-0.1cm}

        \begin{exampleblock}{Testare și aplicații}
            \begin{itemize}\setlength{\itemsep}{-1pt}
                \item \textbf{Testul sup-LM} (Hansen \& Seo, 2002): $H_0$: ECM liniar vs.\ $H_1$: threshold ECM
                \item Aplicații: transmiterea prețurilor pe piețele de mărfuri, legea prețului unic, asimetria pe piețele energetice
            \end{itemize}
        \end{exampleblock}
        }
    \end{cminipage}
...
\begin{frame}{Modelul cu tranziție lină (STECM)}
    \quantlet{TSA\_ch7\_stecm}{https://github.com/QuantLet/TSA/tree/main/TSA_ch7/TSA_ch7_stecm}
    \vspace{-0.3cm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{Smooth Transition ECM (Kapetanios, Shin \& Snell, 2006)}
            Generalizare a modelului cu prag prin \textbf{tranziție continuă} între regimuri:
            \vspace{-0.2cm}
            \[
                \Delta Y_t = \rho\, z_{t-1}\, G(z_{t-d};\, \gamma,\, c) + \bepsilon_t
            \]
            \vspace{-0.35cm}

            cu funcția de tranziție \textbf{logistică}:
            \vspace{-0.2cm}
            \[
                G(z;\, \gamma,\, c) = \bigl[1 + \exp\bigl(-\gamma(z - c)\bigr)\bigr]^{-1}, \quad \gamma > 0
            \]
            \vspace{-0.2cm}
        \end{block}
        ...
        \begin{block}{Cazuri speciale și variante}
            \begin{itemize}\setlength{\itemsep}{-1pt}
                \item $\gamma \to 0$: $G \to 0{,}5$ constant $\Rightarrow$ ECM \textbf{liniar} (caz particular)
                \item $\gamma \to \infty$: $G \to \mathbb{I}(z > c)$ $\Rightarrow$ model cu \textbf{prag} (salt discret)
                \item \textbf{ESTAR} (Exponential STAR): $G(z) = 1 - \exp(-\gamma\, z^2)$ --- ajustare \textbf{simetrică}: rapidă pentru deviații mari (ambele semne), lentă în apropierea echilibrului
            \end{itemize}
        \end{block}

        \vspace{-0.15cm}

        \begin{alertblock}{Testul KSS (Kapetanios, Shin \& Snell, 2003)}
            Test de rădăcină unitară neliniară, pre-condiție pentru STECM. Testează $H_0$: rădăcină unitară vs.\ $H_1$: proces ESTAR staționar global. Bazat pe aproximare Taylor de ordinul I a funcției de tranziție.
        \end{alertblock}
        }
    \end{cminipage}
\end{frame}

\begin{frame}{Cointegrare neliniară: sinteză}
    \quantlet{TSA\_ch7\_nonlinear\_cointegration}{https://github.com/QuantLet/TSA/tree/main/TSA_ch7/TSA_ch7_nonlinear_cointegration}
    \vspace{-0.35cm}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{Comparație: ECM liniar vs.\ Threshold vs.\ STECM}
            \vspace{-0.15cm}
            \begin{center}
            \begin{tabular}{@{}l c c c@{}}
                \toprule
                \textbf{Criteriu} & \textbf{ECM liniar} & \textbf{Threshold ECM} & \textbf{STECM} \\
                \midrule
                Ajustare & simetrică & asimetrică & graduală \\
                Tranziție & --- & discretă (salt) & continuă (lină) \\
                Nr.\ regimuri & 1 & 2 (sau mai multe) & continuum \\
                Viteză ajustare & constantă $\rho$ & $\rho_1 \neq \rho_2$ & $\rho \cdot G(z)$ variabilă \\
                Parametru cheie & $\rho$ & $\gamma$ (prag) & $\gamma$ (viteză tranziție) \\
                \bottomrule
            \end{tabular}
            \end{center}
            \vspace{-0.15cm}
        ...
        \begin{alertblock}{Când să utilizați modele neliniare?}
            \begin{itemize}\setlength{\itemsep}{-2pt}
                \item \textbf{Pas 1}: Estimați ECM liniar; testați reziduurile cu \textbf{testul BDS} sau \textbf{RESET} pentru neliniaritate
                \item \textbf{Pas 2}: Dacă se respinge liniaritatea, estimați modelul cu prag (test sup-LM) sau STECM
                \item \textbf{Pas 3}: Comparați prin criterii informaționale (AIC, BIC) și putere predictivă out-of-sample
            \end{itemize}
        \end{alertblock}
        ...
        \begin{exampleblock}{Instrumente software}
            \begin{itemize}\setlength{\itemsep}{-2pt}
                \item \textbf{R}: pachetele \texttt{tsDyn} (TVECM, SETAR), \texttt{apt} (threshold cointegration), \texttt{nardl} (NARDL)
                \item \textbf{Python}: \texttt{statsmodels} (ECM liniar) + implementare proprie STECM; \texttt{arch} pentru teste neliniare
            \end{itemize}
        \end{exampleblock}
        }
    \end{cminipage}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Răspuns: (B)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item $R^2$ ridicat \emph{nu} implică relație reală între variabile $I(1)$.
            \end{itemize}
        \end{exampleblock}
        }
