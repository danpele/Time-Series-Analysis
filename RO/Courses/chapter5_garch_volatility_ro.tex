...
\begin{frame}{Teorie vs empirică: decalajul kurtosis}
    ...
    \begin{alertblock}{Decalajul kurtosis}
        ...
        \end{center}
        \vspace{-2mm}
        ARCH(1) cu inovații gaussiene nu poate reproduce kurtosis-ul extrem observat în datele financiare.
    \end{alertblock}
    \vspace{-0.1cm}
...
\begin{frame}{Modelul GARCH(p,q) --- Bollerslev (1986)}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item $\alpha_i$ = reacția la șocuri recente (coeficienți news)
...
\small
    ...
    \begin{alertblock}{Condiție de staționaritate}
        Necesită $\alpha + \beta < 1$ pentru varianță necondiționată finită.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Estimarea modelelor GARCH}
    ...
    \begin{block}{Metoda verosimilității maxime (MLE)}
        Log-verosimilitate (normală): $\ell(\theta) = -\frac{T}{2} \ln(2\pi) - \frac{1}{2} \sum_{t=1}^{T} \left[ \ln(\sigma_t^2) + \frac{\varepsilon_t^2}{\sigma_t^2} \right]$
    \end{block}
    ...
    \begin{alertblock}{Notă practică}
        Distribuția Student-t oferă de regulă o potrivire mai bună pentru randamentele financiare datorită cozilor groase (kurtosis $> 3$).
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Quasi-MLE și robustețe}
    ...
    \begin{block}{Ce se întâmplă dacă distribuția este greșit specificată?}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Quasi-MLE (QMLE)}: maximizează log-verosimilitatea gaussiană chiar dacă $z_t$ nu este normal
            \item Observație cheie: avem nevoie doar ca \textit{primele două momente condiționate} să fie corecte
        \end{itemize}
    \end{block}
    \vspace{-0.2cm}
    ...
    \begin{alertblock}{Erori standard sandwich (robuste)}
        Erorile standard trebuie corectate:
        \[
            \hat{V}_{\text{robust}} = \hat{A}^{-1} \hat{B} \hat{A}^{-1} \quad \text{(White, 1982)}
        \]
        unde $\hat{A}$ = Hessiana, $\hat{B}$ = produsul exterior al scorurilor. În Python: \texttt{result.summary()} raportează SE robuste implicit.
    \end{alertblock}
    \vspace{-0.15cm}
...
\begin{frame}{Provocări practice în estimare}
    ...
    \begin{alertblock}{Regulă practică}
        Dacă BFGS și Nelder-Mead dau estimări \textit{diferite} ale parametrilor, modelul poate fi greșit specificat sau datele insuficiente.
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Valori tipice pentru GARCH(1,1)}
    ...
    \begin{alertblock}{Observații}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Bitcoin: $\alpha$ mai mare $\succ$ reacție mai rapidă la news
        \end{itemize}
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Schimbări de regim în volatilitate}
    ...
    \begin{block}{Problemă}
        GARCH presupune un \textbf{singur regim} cu parametri constanți. Dar volatilitatea prezintă adesea \textbf{rupturi structurale} (ex.\ criza 2008, COVID-19).
    \end{block}
    \vspace{-0.2cm}
    ...
    \end{defn}
    \vspace{-0.2cm}
    \begin{exampleblock}{Testul ICSS Inclan-Tiao}
        \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Leverage effect}
    ...
    {\scriptsize
        \begin{block}{Definiție}
            \textbf{Leverage effect}: Șocurile negative cresc volatilitatea \textbf{mai mult} decât cele pozitive de aceeași magnitudine.
        \end{block}
...
\small
    \begin{block}{Specificații comune}
        Prima de risc poate intra sub diferite forme: (1) $r_t = \mu + \lambda \sigma_t + \varepsilon_t$; (2) $r_t = \mu + \lambda \sigma_t^2 + \varepsilon_t$; (3) $r_t = \mu + \lambda \ln(\sigma_t^2) + \varepsilon_t$
    \end{block}
    \vspace{-0.1cm}
    ...
    \vspace{-0.1cm}
    \begin{rmk}
        GARCH-M este utilizat în evaluarea activelor, optimizarea portofoliului și testarea CAPM.
    \end{rmk}
\end{frame}
...
\begin{frame}{Testul Diebold-Mariano}
    ...
    \begin{block}{Motivație}
        Date fiind două modele de volatilitate concurente, care prognozează mai bine?
    \end{block}
    \vspace{-0.15cm}
    \begin{defn}[Diebold-Mariano (1995)]
        Definim diferența de pierdere: $d_t = L(e_{1t}) - L(e_{2t})$
        unde $e_{it} = \sigma_t^2 - \hat{\sigma}_{it}^2$ și $L(\cdot)$ este o funcție de pierdere (ex.\ $L(e) = e^2$).
        \[
    ...
    \begin{alertblock}{Note practice}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Folosiți erori standard HAC (Newey-West) dacă $d_t$ este autocorelat
        \end{itemize}
    \end{alertblock}
    \begin{exampleblock}{Exemplu}
        EGARCH-t vs GARCH-N pe S\&P 500: testul DM cu pierdere QLIKE respinge de regulă $H_0$ în favoarea EGARCH-t.
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Regresia Mincer-Zarnowitz}
    ...
    \begin{block}{Test de eficiență a prognozei}
        Regresăm volatilitatea realizată pe volatilitatea prognozată:
        \[
    ...
    \vspace{-0.1cm}
    \begin{defn}[Ipoteze]
        $H_0: a = 0, \; b = 1$ \quad (prognoză nedirecționată și eficientă)

        Test prin F-test sau Wald. Respingere $\succ$ prognoza este biasată sau ineficientă.
    \end{defn}
    \vspace{-0.1cm}
    ...
    \begin{alertblock}{Atenție}
        Varianța realizată ($r_t^2$) este un proxy zgomotos pentru $\sigma_t^2$ $\succ$ $R^2$ de regulă mic ($< 0.10$). Preferați $RV_t$ când e disponibil.
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Prognoza volatilității --- vizualizare}
    ...
    {\scriptsize
    \begin{itemize}\setlength{\itemsep}{0pt}
        \item Prognoza converge exponențial către $\bar{\sigma}^2$; viteza depinde de $\alpha + \beta$
...
\begin{frame}{Prognoza cu GARCH(1,1)}
    ...
    \begin{block}{Prognoză multi-pas}
        Pentru $h > 1$: $\E_T[\sigma_{T+h}^2] = \bar{\sigma}^2 + (\alpha + \beta)^{h-1} (\sigma_{T+1}^2 - \bar{\sigma}^2)$
    ...
    \begin{exampleblock}{Convergență}
        $\lim_{h \to \infty} \E_T[\sigma_{T+h}^2] = \bar{\sigma}^2$ --- prognoza converge către varianța necondiționată!
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Volatilitate condiționată vs necondiționată}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Volatilitate condiționată} $\sigma_t$ (GARCH): variabilă în timp, reacționează la news
...
\begin{frame}{Volatilitate realizată și HAR-RV}
    ...
    \begin{block}{Volatilitate realizată (RV)}
        Cu randamente intra-zilnice $r_{t,i}$ la frecvența $\Delta$ ($M$ observații pe zi):
        \[
            RV_t = \sum_{i=1}^{M} r_{t,i}^2
        \]
        $RV_t$ este o măsură \textbf{fără model}, \textbf{observabilă} a varianței zilnice (vs.\ GARCH: latentă).
    \end{block}
    \vspace{-0.2cm}
    ...
    \end{defn}
    \vspace{-0.15cm}
    \begin{alertblock}{Avantaj cheie}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Regresie OLS simplă care capturează \textbf{memoria lungă} în volatilitate
            \item Traderi eterogeni (orizonturi zilnice/săptămânale/lunare) explică dinamica multi-scală
            \item Adesea depășește GARCH când datele intra-zilnice sunt disponibile (rand.\ la 5 min, $M \approx 78$/zi)
...
\begin{frame}{Realized GARCH}
    ...
    \vspace{-0.4cm}
    \begin{block}{Motivație}
        GARCH standard tratează volatilitatea ca \textbf{latentă}. Cu date de frecvență înaltă, o putem \textbf{observa} prin $RV_t$.
    \end{block}
    \vspace{-0.15cm}
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \begin{block}{Value at risk (VaR)}
                ...
                Pierderea maximă cu probabilitate $1-\alpha$.
            \end{block}

            \begin{block}{Expected shortfall (ES)}
                $\text{ES}_\alpha = \E[-r | r < -\text{VaR}_\alpha]$

                Pierderea medie când VaR este depășit.
            \end{block}
        \end{column}
        \begin{column}{0.5\textwidth}
...
\begin{frame}{Value at risk --- exemplu numeric}
    ...
    \begin{exampleblock}{Calculul VaR}
        Portofoliu: \textbf{1.000.000 EUR}, volatilitate prognozată $\hat{\sigma}_{T+1} = 1.5\%$
    \end{exampleblock}
    ...
    \begin{alertblock}{Scalare pentru perioade mai lungi}
        $\text{VaR}_{h\text{ zile}} = \text{VaR}_{1\text{ zi}} \cdot \sqrt{h}$ --- presupune randamente i.i.d.
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Value at risk --- distribuție Student-t}
    ...
    \begin{block}{De ce Student-t?}
        Distribuția normală \textbf{subestimează} riscul de coadă. Student-t cu $\nu$ grade de libertate capturează mai bine cozile groase (kurtosis $> 3$).
    \end{block}
...
\begin{frame}{VaR backtesting: testul Kupiec}
    \begin{block}{Testul acoperirii necondiționate}
        Testează dacă rata de încălcare observată este egală cu rata așteptată $p$ (ex.\ 1\% pentru VaR 1\%).

        \vspace{0.2cm}

        Fie $N$ = numărul de încălcări VaR, $T$ = totalul observațiilor, $\hat{p} = N/T$.

        \vspace{0.2cm}

        \textbf{Statistica Likelihood Ratio:}
        \[
...
\begin{frame}{VaR backtesting: testul Christoffersen}
    ...
    \begin{block}{Testul acoperirii condiționate}
        Testează atât \textbf{acoperirea necondiționată} cât și \textbf{independența} încălcărilor.

        Încălcările trebuie să fie independente --- fără clusterizare a excepțiilor!
    \end{block}
    \vspace{-0.1cm}
...
\begin{frame}{Backtesting-ul Expected Shortfall}
    ...
    \begin{alertblock}{Observație cheie}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Mai puternic dar necesită eșantioane mari ($T > 1000$); alt.: bootstrap McNeil \& Frey (2000)
            \item ES sub Student-$t$: $\text{ES}_\alpha^{t_\nu} = -\mu + \sigma \cdot \frac{\nu + (t_{\alpha,\nu})^2}{\nu - 1} \cdot \frac{f_{t_\nu}(t_{\alpha,\nu})}{\alpha}$ --- cozi groase $\succ$ ES mai mare
        \end{itemize}
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Basel IV: de la VaR la ES}
    ...
    \begin{exampleblock}{Echivalență sub normalitate}
        ...
        \]
        $\succ$ Sub normalitate, ES 97.5\% $\approx$ VaR 99\%. Sub cozi groase, ES $\gg$ VaR.
    \end{exampleblock}
    \vspace{-0.1cm}
...
\small
    \begin{block}{De ce modelare combinată?}
        \textbf{Corelație serială} $\succ$ ARMA pentru medie; \textbf{Volatility clustering} $\succ$ GARCH pentru varianță.
    \end{block}
    \vspace{-0.1cm}
