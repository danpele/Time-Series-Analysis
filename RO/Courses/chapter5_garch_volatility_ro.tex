    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Aplicații directe în finanțe}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Capital economic / reglementar}: FRTB, stress testing
...
\begin{frame}{Portret de cercetător: Engle \& Bollerslev}
    ...
    \vspace{-0.1cm}
    \begin{center}
        {\scriptsize\textcolor{MainBlue}{\textbf{Aceasta este baza econometriei moderne a volatilității și a managementului cantitativ al riscului.}}}
    \end{center}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{ARCH(1): $\sigma_t^2 = \omega + \alpha_1 \varepsilon_{t-1}^2$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Kurtosis}: $\kappa = 3 \cdot \dfrac{1 - \alpha_1^2}{1 - 3\alpha_1^2}$ (dacă $\alpha_1^2 < 1/3$; sub ipoteza $z_t \sim N(0,1)$)
                \item Kurtosis $> 3$ pentru $\alpha_1 > 0$ $\Rightarrow$ \textbf{cozi groase}!
                \item \textbf{Notă}: Formula presupune inovații gaussiene ($z_t \sim N(0,1)$); sub alte distribuții (e.g., Student-$t$), kurtosisul necondiționat se modifică
            \end{itemize}
        \end{block}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Structura unui model GARCH(1,1)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Ecuația varianței}: $\sigma_t^2 = \underbrace{\omega}_{\text{bază}} + \underbrace{\alpha \varepsilon_{t-1}^2}_{\text{reacție la șoc}} + \underbrace{\beta \sigma_{t-1}^2}_{\text{persistență}}$
            \end{itemize}
        \end{block}
        \vspace{-1.5mm}
...
\begin{frame}{De ce funcționează GARCH: intuiția centrală}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Ideea fundamentală}
            {\footnotesize
            Randamentele financiare sunt greu de prognozat, dar \textbf{volatilitatea lor este prognozabilă}:
            \[
        ...
        \begin{exampleblock}{Mecanismul GARCH(1,1)}
            {\footnotesize
            \[
                \sigma_t^2 = \underbrace{\omega}_{\substack{\text{nivel de}\\\text{echilibru}}} + \underbrace{\alpha\, r_{t-1}^2}_{\substack{\text{informație}\\\text{nouă (șoc)}}} + \underbrace{\beta\, \sigma_{t-1}^2}_{\substack{\text{memoria}\\\text{volatilității}}}
            \]
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Ieri a fost o zi turbulentă?} ($r_{t-1}^2$ mare) $\Rightarrow$ volatilitatea de azi crește
                \item \textbf{Ieri volatilitatea era deja ridicată?} ($\sigma_{t-1}^2$ mare) $\Rightarrow$ persistă și azi
                \item Pe termen lung: volatilitatea revine la echilibru $\bar{\sigma}^2 = \omega / (1 - \alpha - \beta)$
            \end{itemize}
            }
        \end{exampleblock}
        \vspace{-0.15cm}
        ...
        \begin{alertblock}{De ce este important}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Fără GARCH: VaR constant $\Rightarrow$ \textbf{subestimează} riscul în crize, \textbf{supraestimează} în perioade calme
                \item Cu GARCH: VaR dinamic $\Rightarrow$ \textbf{se adaptează} la condițiile pieței $\Rightarrow$ management de risc realist
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Ce se întâmplă dacă distribuția este greșit specificată?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{QMLE}: maximizează log-verosimilitatea gaussiană chiar dacă $z_t$ nu este normal; necesită doar ca \textit{primele două momente condiționate} să fie corecte
            \end{itemize}
        \end{block}
        \vspace{-0.3cm}
        ...
        \begin{alertblock}{Erori standard sandwich (robuste)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Corecție: $\hat{V}_{\text{robust}} = \hat{A}^{-1} \hat{B} \hat{A}^{-1}$ (White, 1982); în Python: SE robuste implicit
            \end{itemize}
        \end{alertblock}
        \vspace{-0.3cm}
        \begin{exampleblock}{QMLE vs MLE complet}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{QMLE}: implicit robust --- consistent chiar sub specificare greșită
...
\begin{frame}{IGARCH: Staționaritate strictă vs.\ staționaritate în covarianță}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \vspace{-1mm}
        \begin{rmk}
            Distincția strict staționar / staționar în covarianță este analogă cu procesele cu cozi grele: un proces poate avea distribuția marginală bine definită fără a avea momente finite de ordin superior.
        \end{rmk}
        }
    \end{cminipage}
\end{frame}
...
\begin{frame}{Schimbări de regim în volatilitate}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Implicații practice}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Dacă rupturile sunt prezente, persistența GARCH ($\alpha + \beta$) este \textbf{distorsionată în sus} $\Rightarrow$ IGARCH fals
                \item Soluție: estimare GARCH separată per regim, ferestre rulante, sau GARCH Markov ($K=2$)
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Ghid de alegere}
            \begin{itemize}\setlength{\itemsep}{1pt}
                ...
                \item \textbf{EGARCH}: modelează $\ln(\sigma_t^2)$ $\Rightarrow$ \textbf{nu necesită restricții de pozitivitate}; captează leverage prin $\gamma$; \textcolor{Forest}{preferat pentru acțiuni}
        ...
        \vspace{-1mm}
        \begin{center}
            {\footnotesize\textcolor{MainBlue}{\textbf{Modelele nu sunt o listă --- sunt un set de alegeri motivate de date.}}}
        \end{center}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Testul Diebold-Mariano}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Note practice}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $H_0$: acuratețe predictivă egală; respingere $\Rightarrow$ un model este semnificativ mai bun
                \item Funcții de pierdere: MSE, MAE, QLIKE --- QLIKE robustă (Patton, 2011); SE HAC dacă $d_t$ autocorelat
            \end{itemize}
        \end{alertblock}
        \vspace{-0.35cm}
        \begin{exampleblock}{Exemplu}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item EGARCH-t vs GARCH-N pe S\&P 500: testul DM cu QLIKE respinge de regulă $H_0$ în favoarea EGARCH-t
            \end{itemize}
        \end{exampleblock}
    \end{cminipage}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Interpretare}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item $b > 1$: subestimează volatilitatea; $b < 1$: supraestimează; $a > 0$: bias ascendent
            \end{itemize}
        \end{exampleblock}
        \vspace{-0.2cm}
...
\begin{frame}{Volatilitate condiționată vs necondiționată}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Zone roșii: $\sigma_t > \bar{\sigma}$ (perioade de stres); zone albastre: $\sigma_t < \bar{\sigma}$ (perioade de volatilitate redusă)
        \end{itemize}
    \end{block}
    }
    \begin{center}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Scalare pentru perioade mai lungi}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Atenție}: Sub modelele GARCH, randamentele \textbf{nu} sunt i.i.d.\ (sunt condiționat heteroscedastice) $\Rightarrow$ regula $\sqrt{h}$ este doar o \textbf{aproximare}; pentru VaR multi-perioadă, folosiți prognoza GARCH iterată
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Backtesting-ul Expected Shortfall}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{defn}[Testul Z Acerbi-Sz\'ekely (2014)]
            $Z_{\text{ES}} = \frac{1}{n_\alpha} \sum_{t: r_t < -\text{VaR}_t} \frac{r_t}{\text{ES}_t} + 1$.\quad Sub $H_0$ (model ES corect): $\E[Z_{\text{ES}}] = 0$; se respinge dacă $Z_{\text{ES}} < z_{\alpha/2}$
        \end{defn}
        \vspace{-1.5mm}
        \begin{alertblock}{Observație}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Spre deosebire de backtesting VaR (binar), backtesting ES folosește \textit{magnitudinea} pierderilor
                \item Mai puternic dar necesită eșantioane mari ($T > 1000$); alternativă: bootstrap McNeil \& Frey (2000)
                \item ES sub Student-$t$: cozi groase $\Rightarrow$ ES mai mare decât sub normalitate
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Checklist practic: modelarea GARCH pas cu pas}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-0.2cm}
        {\footnotesize
        \begin{block}{Workflow recomandat}
            \begin{enumerate}\setlength{\itemsep}{1pt}
        ...
        \begin{alertblock}{Capcane frecvente}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Overfitting}: GARCH(1,1) este suficient în 90\% din cazuri --- ordine mai mari rareori îmbunătățesc prognoza
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{Pasul 3: Parametri estimați -- comparație}
    ...
    {\scriptsize
        ...
        \begin{alertblock}{Interpretare}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item EGARCH $\gamma = -0.12$ \textbf{semnificativ} $\Rightarrow$ \textbf{leverage effect confirmat}
...
\begin{frame}{Pasul 3: Volatilitatea condiționată -- Bitcoin}
    ...
    {\footnotesize
        \begin{block}{Diagnostice GJR-GARCH(1,1)-t}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Ljung-Box pe $z_t^2$: p-value $= 0.41$ $\Rightarrow$ \textbf{efecte ARCH eliminate}
                \item Vârfuri volatilitate: martie 2020 (COVID), mai 2022 (Terra/Luna)
                \item Volatilitate zilnică: de la 1\% (perioadele de volatilitate redusă) la $>$15\% (crize)
            \end{itemize}
        \end{block}
    }
    \begin{center}
...
\begin{frame}{Modelul DCC-GARCH multivariat}
    \quantlet{TSA\_ch5\_dcc\_garch}{https://github.com/QuantLet/TSA/tree/main/TSA_ch5/TSA_ch5_dcc_garch}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-3mm}
        {\scriptsize
        \setlength{\abovedisplayskip}{2pt}\setlength{\belowdisplayskip}{2pt}
        \begin{block}{Motivație: De la volatilitate univariată la corelații dinamice}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item GARCH univariat modelează volatilitatea \textbf{unei singure} serii
                \item În practică, portofoliile conțin \textbf{multiple active} cu corelații variabile în timp
                \item DCC-GARCH (Engle, 2002) modelează simultan volatilitățile \emph{și} corelațiile
            \end{itemize}
        \end{block}
        \vspace{-1.5mm}
        \begin{block}{Structura DCC în două etape}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                \item \textbf{Etapa 1}: Estimarea GARCH univariat pentru fiecare serie $\Rightarrow$ $\mathbf{D}_t = \text{diag}(\sigma_{1,t}, \ldots, \sigma_{K,t})$
                \item \textbf{Etapa 2}: Modelarea corelațiilor condiționate:
                $\mathbf{Q}_t = (1 - a - b)\bar{\mathbf{Q}} + a\,\mathbf{u}_{t-1}\mathbf{u}_{t-1}' + b\,\mathbf{Q}_{t-1}$,
                unde $\mathbf{u}_t = \mathbf{D}_t^{-1}\boldsymbol{\varepsilon}_t$ sunt reziduurile standardizate
            \end{enumerate}
        \end{block}
        \vspace{-1.5mm}
        \begin{block}{Matricea de corelație și covarianță condiționate}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Corelație: $\mathbf{R}_t = \mathbf{Q}_t^{*-1}\,\mathbf{Q}_t\,\mathbf{Q}_t^{*-1}$, cu $\mathbf{Q}_t^{*} = \text{diag}\!\bigl(\sqrt{q_{11,t}},\, \ldots,\, \sqrt{q_{KK,t}}\bigr)$
                \item Covarianță: $\mathbf{H}_t = \mathbf{D}_t\,\mathbf{R}_t\,\mathbf{D}_t$
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{Estimare DCC-GARCH}
    \quantlet{TSA\_ch5\_dcc\_estimation}{https://github.com/QuantLet/TSA/tree/main/TSA_ch5/TSA_ch5_dcc_estimation}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-3mm}
        {\scriptsize
        \setlength{\abovedisplayskip}{2pt}\setlength{\belowdisplayskip}{2pt}
        \begin{block}{Estimare QML în două etape (Engle, 2002; Engle \& Sheppard, 2001)}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                \item \textbf{Etapa 1} --- Estimarea parametrilor GARCH univariați:
                \vspace{-1mm}
                \[
                ...
                \]
                \vspace{-3mm}
                \item \textbf{Etapa 2} --- Estimarea parametrilor de corelație $(a, b)$:
                \vspace{-1mm}
                \[
        ...
        \end{block}
        \vspace{-1.5mm}
        \begin{block}{Proprietăți ale estimatorului}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Estimatorul QML este \textbf{consistent} chiar și sub misspecificarea distribuției condiționate
                \item Erori standard robuste via \textbf{sandwich estimator} (Bollerslev \& Wooldridge, 1992)
                \item Avantaj computațional: reduce dimensiunea problemei de la $O(K^2)$ la $O(K) + O(2)$
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{Proprietăți și extensii DCC}
    \quantlet{TSA\_ch5\_dcc\_extensions}{https://github.com/QuantLet/TSA/tree/main/TSA_ch5/TSA_ch5_dcc_extensions}
    \begin{cminipage}{0.95\textwidth}
        \vspace{-0.2cm}
        {\scriptsize
        \begin{block}{Pozitiv-definire}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\mathbf{H}_t$ este \textbf{pozitiv definită} dacă $\bar{\mathbf{Q}}$ este pozitiv definită și $a + b < 1$, $a \geq 0$, $b \geq 0$
                \item Normalizarea prin $\mathbf{Q}_t^{*}$ garantează $\rho_{ij,t} \in [-1, 1]$
            \end{itemize}
        \end{block}
        \vspace{-1.5mm}
        \begin{block}{DCC asimetric (Cappiello, Engle \& Sheppard, 2006)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Extensie cu \textbf{termen de levier} pentru a capta creșterea corelațiilor în perioade de criză:
            \end{itemize}
            \vspace{-2mm}
            \[
                \mathbf{Q}_t = (1\!-\!a\!-\!b)\bar{\mathbf{Q}} - g\bar{\mathbf{N}} + a\,\mathbf{u}_{t-1}\mathbf{u}_{t-1}' + b\,\mathbf{Q}_{t-1} + g\,\mathbf{n}_{t-1}\mathbf{n}_{t-1}'
            \]
            \vspace{-4mm}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\mathbf{n}_t = \mathbb{I}(\mathbf{u}_t < 0) \odot \mathbf{u}_t$ (componenta negativă), $\bar{\mathbf{N}} = \E[\mathbf{n}_t \mathbf{n}_t']$
            \end{itemize}
        \end{block}
        \vspace{-1.5mm}
        \begin{exampleblock}{Aplicații}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Optimizarea portofoliului}: ponderi Markowitz cu $\mathbf{H}_t$ variabilă în timp
                \item \textbf{Risc sistemic}: corelații condiționate ca măsură de contagiune financiară
                \item \textbf{Hedging dinamic}: rapoarte optime de acoperire $h_t^{*} = \sigma_{12,t} / \sigma_{22,t}$
            \end{itemize}
        \end{exampleblock}
        }
    \end{cminipage}
...
\begin{frame}{GARCH multivariat: extensii pentru portofolii}
    \begin{cminipage}{0.95\textwidth}
        \small
        \begin{block}{De ce GARCH multivariat?}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Portofoliile necesită matricea de covarianță condiționată $\mathbf{H}_t = \mathrm{Cov}(\mathbf{r}_t \mid \mathcal{F}_{t-1})$
                \item $K$ active $\Rightarrow$ $K(K+1)/2$ elemente unice în $\mathbf{H}_t$ $\Rightarrow$ explozie dimensională
            \end{itemize}
        \end{block}
        \vspace{-0.2cm}
        \begin{columns}[T]
            \begin{column}{0.48\textwidth}
                \begin{exampleblock}{DCC (Engle, 2002)}
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        ...
                        \item $\mathbf{D}_t = \mathrm{diag}(\sigma_{1,t}, \ldots, \sigma_{K,t})$ --- GARCH univariat
                        \item $\mathbf{R}_t$ --- corelații dinamice (2 parametri)
                        \item \textbf{Avantaj}: estimare în 2 etape, scalabil
                        \item \textbf{Limită}: corelații simetrice
                    \end{itemize}
                \end{exampleblock}
            \end{column}
            \begin{column}{0.48\textwidth}
                \begin{alertblock}{BEKK (Engle-Kroner, 1995)}
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        ...
                        \item Garantează $\mathbf{H}_t \succ 0$ (pozitiv-definită)
                        \item \textbf{Avantaj}: model complet flexibil
                        \item \textbf{Limită}: $K^2$ parametri per matrice $\Rightarrow$ practic doar $K \leq 3$
                    \end{itemize}
                \end{alertblock}
            \end{column}
        \end{columns}
        ...
        \begin{block}{Implementare Python}
            \texttt{from arch.univariate import ARCHModel} --- DCC via \texttt{rmgarch} (R) sau \texttt{mgarch} (Python/Stata)
        \end{block}
        }
    \end{cminipage}
\end{frame}
