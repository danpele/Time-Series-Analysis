    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{La finalul acestui capitol, veți fi capabili să:}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                ...
                \item Specificați și estimați modele \textbf{VAR(p)} (Vector Autoregresiv)
        ...
        \begin{column}{0.48\textwidth}
            \begin{exampleblock}{Aplicații}
                \begin{itemize}\setlength{\itemsep}{3pt}
                    ...
                    \item Extensii: Cauzalitate Modernă
...
\begin{frame}{Principiu: variabilele interacționează}
    \vspace{-0.2cm}
    {\footnotesize
    \begin{block}{Exemple}
        \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{De ce modelele univariate nu sunt suficiente}
    ...
    \begin{alertblock}{Problema}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ARIMA tratează fiecare variabilă \textbf{izolat}; ignoră \textbf{interacțiunile} și \textbf{efectele de retroacțiune}
        \end{itemize}
    \end{alertblock}
    \vspace{-2mm}
    \begin{exampleblock}{Exemple}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item PIB--Șomaj, Dobândă--Inflație, Acțiuni--Volum, Curs--Balanță comercială
        \end{itemize}
    \end{exampleblock}
    }
    \vspace{-1mm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.45\textheight, keepaspectratio]{ch6_motivation_univariate.pdf}
    \end{center}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Concepte fundamentale}
            ...
            \begin{enumerate}\setlength{\itemsep}{1pt}
                ...
                \item \textbf{Cauzalitate Granger}: Ajută $X$ la prognozarea lui $Y$?
...
\begin{frame}{Notația seriilor de timp multivariate}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Întrebări fundamentale}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                \item Ajută $Y_1$ la prognozarea lui $Y_2$? (Cauzalitate Granger)
...
\begin{frame}{Proprietăți ale covarianței încrucișate}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Funcția de covarianță încrucișată}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Pentru variabilele $Y_{it}$ și $Y_{jt}$:
        ...
        \begin{exampleblock}{Pentru cazul bivariat}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Interpretare}:
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item Diagonale: ACF (Autocorrelation Function) obișnuite
        ...
        \begin{column}{0.76\textwidth}
            \begin{block}{Biografie}
                {\footnotesize \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Christopher Sims}: econometrist american la Princeton
                    \item Premiul Nobel (2011) ,,pentru cercetări empirice privind cauza și efectul în macroeconomie''
                    \item \textbf{Clive Granger}: economist britanic-american la UC San Diego
                    \item Premiul Nobel (2003) ,,pentru metode de analiză a seriilor economice cu tendințe comune (cointegrare)''
                \end{itemize}}
            \end{block}
        \end{column}
    \end{columns}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Definiție}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item $\bepsilon_t$: vector $K \times 1$ de termeni de eroare cu $\E[\bepsilon_t] = \mathbf{0}$, $\E[\bepsilon_t \bepsilon_t'] = \bSigma$, $\E[\bepsilon_t \bepsilon_s'] = \mathbf{0}$ pentru $t \neq s$ (decorelare temporală)
            \end{itemize}
        \end{block}
    \end{cminipage}
\end{frame}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Ecuație cu ecuație}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{Esențial}: Fiecare ecuație include lag-uri ale \textbf{tuturor} variabilelor.
            \end{itemize}
        \end{exampleblock}
    \end{cminipage}
\end{frame}
...
%=============================================================================
% VAR BAYESIAN (BVAR) --- BLOC DOCTORAL (3 SLIDE-URI)
%=============================================================================

\begin{frame}{VAR Bayesian (BVAR) --- Priori Minnesota}
    \quantlet{TSA\_ch6\_bvar}{https://github.com/QuantLet/TSA/tree/main/TSA_ch6/TSA_ch6_bvar}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{alertblock}{Problema: Blestemul dimensionalității în VAR}
            VAR($p$) cu $K$ variabile necesită estimarea a $K + pK^2$ parametri.
            Cu $K{=}5$, $p{=}4$: $105$ parametri; cu $K{=}20$, $p{=}4$: $1620$ parametri $\Rightarrow$ supraajustare severă (\emph{overfitting}).
        \end{alertblock}
        \begin{block}{Abordarea Bayesiană: prior $+$ verosimilitate $\to$ posterior}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item \textbf{Teorema lui Bayes}: $p(\bA, \bSigma \mid \bY) \propto p(\bY \mid \bA, \bSigma)\; p(\bA, \bSigma)$
                \item Prior-ul introduce informație suplimentară, regularizând estimarea $\Rightarrow$ \emph{shrinkage}
                \item Echivalent cu un estimator de tip ridge/Tikhonov cu structură economică
            \end{itemize}
        \end{block}
        \begin{block}{Priori Minnesota (Litterman, 1986; Doan, Litterman \& Sims, 1984)}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item \textbf{Medie a priori} pentru coeficienți:
                    $\E[A_{ij}^{(\ell)}] = \begin{cases} \delta_i & \text{dacă } i=j,\; \ell=1 \quad \text{(mers aleatoriu: } \delta_i=1\text{)} \\ 0 & \text{altfel (lag-uri proprii superioare și efecte încrucișate)} \end{cases}$
                \item \textbf{Varianță a priori} cu structură ierarhică:
                    $\Var\bigl(A_{ij}^{(\ell)}\bigr) = \begin{cases} \bigl(\lambda / \ell^{d}\bigr)^2 & \text{dacă } i = j \quad \text{(lag propriu)} \\[4pt] \bigl(\lambda \theta / \ell^{d}\bigr)^2 \cdot \bigl(\hat{\sigma}_i^2 / \hat{\sigma}_j^2\bigr) & \text{dacă } i \neq j \quad \text{(efect încrucișat)} \end{cases}$
                \item $\lambda$ --- \emph{tightness} (intensitatea contracției globale):
                    $\lambda \to 0$: prior dominant $\Rightarrow$ mers aleatoriu;\;
                    $\lambda \to \infty$: OLS fără restricții
                \item $\theta \in (0,1]$ --- contracție suplimentară pe efectele încrucișate ($\theta < 1$ penalizează)
                \item $d \in \{1, 2\}$ --- rata de descreștere cu lag-ul (\emph{lag decay}); $d{=}1$ liniar, $d{=}2$ pătratic
                \item $\hat{\sigma}_i^2 / \hat{\sigma}_j^2$ --- scalare pentru a ține cont de diferențele de scală între variabile
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{Estimare și Selecție în BVAR}
    \quantlet{TSA\_ch6\_bvar\_estimation}{https://github.com/QuantLet/TSA/tree/main/TSA_ch6/TSA_ch6_bvar_estimation}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{Posterior conjugat: Normal--Inverse-Wishart (NIW)}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item Sub priori Minnesota cu matrice de covarianță $\bSigma$ cunoscută (sau condiționat pe $\bSigma$):
                    \vspace{2pt}

                    $\text{vec}(\bA) \mid \bY, \bSigma \;\sim\; \mathcal{N}\!\bigl(\text{vec}(\bar{\bA}),\; \bSigma \otimes \bar{V}\bigr)$
                    \vspace{2pt}

                    unde $\bar{V} = \bigl(V_0^{-1} + \bX'\bX\bigr)^{-1}$ și $\bar{\bA} = \bar{V}\bigl(V_0^{-1}\bA_0 + \bX'\bY\bigr)$
                \item $V_0$ --- matricea de precizie a priori (diagonală sub Minnesota); $\bA_0$ --- media a priori
                \item Posterior pe $\bSigma$: $\bSigma \mid \bY \sim \mathcal{IW}\bigl(\bar{S},\, \bar{\nu}\bigr)$ cu $\bar{S} = S_0 + (\bY - \bX\bar{\bA})'(\bY - \bX\bar{\bA})$
            \end{itemize}
        \end{block}
        \begin{block}{Selecția hiperparametrilor}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item \textbf{Verosimilitatea marginală} (\emph{marginal likelihood}):
                    $p(\bY \mid \lambda, \theta, d) = \int\!\!\int p(\bY \mid \bA, \bSigma)\; p(\bA, \bSigma \mid \lambda, \theta, d)\; d\bA\; d\bSigma$
                \item Sub prior NIW conjugat, $p(\bY \mid \lambda, \theta, d)$ are formă analitică (matricial-$t$)
                \item \textbf{Optimizare}: căutare pe grilă (\emph{grid search}) peste $(\lambda, \theta, d)$ sau maximizare numerică
                \item \textbf{Alternativ}: validare încrucișată pseudo-out-of-sample pe fereastră mobilă
            \end{itemize}
        \end{block}
        \begin{block}{Simulare posterioară pentru priori non-conjugate}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item Când priori-ul nu este conjugat (ex: Stochastic Search Variable Selection --- SSVS), se utilizează \textbf{eșantionarea Gibbs}:
                    \vspace{2pt}

                    \quad (1)\; $\bA^{(s)} \sim p(\bA \mid \bSigma^{(s-1)}, \bY)$
                    \qquad (2)\; $\bSigma^{(s)} \sim p(\bSigma \mid \bA^{(s)}, \bY)$
                \item Ref.: Koop \& Korobilis (2010) --- \emph{Bayesian Multivariate Time Series Methods for Empirical Macroeconomics}
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{Extensii BVAR și Aplicații}
    \quantlet{TSA\_ch6\_bvar\_extensions}{https://github.com/QuantLet/TSA/tree/main/TSA_ch6/TSA_ch6_bvar_extensions}
    \begin{cminipage}{0.95\textwidth}
        {\scriptsize
        \begin{block}{Large BVAR (Ba\'{n}bura, Giannone \& Reichlin, 2010)}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item Sisteme cu $K = 100{+}$ variabile: $\lambda$ se scalează cu dimensiunea sistemului ($\lambda \propto \sqrt{K}$)
                \item Performanță de prognoză comparabilă cu modelele factoriale (\emph{factor models})
                \item Principiul \emph{,,blessing of dimensionality''}: mai multe serii $\Rightarrow$ prior mai informativ
            \end{itemize}
        \end{block}
        \begin{block}{BVAR cu parametri variabili în timp (TVP-BVAR)}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item Coeficienți ce evoluează: $\bA_t = \bA_{t-1} + \boldsymbol{\eta}_t$, \quad $\boldsymbol{\eta}_t \sim \mathcal{N}(\mathbf{0}, \mathbf{Q})$ \quad (\emph{random walk})
                \item Volatilitate stocastică: $\log \sigma_{i,t}^2 = \log \sigma_{i,t-1}^2 + \zeta_{i,t}$ \quad (Primiceri, 2005)
                \item Capturează schimbări structurale, regimuri de politică monetară
                \item Estimare prin filtre Kalman + eșantionare MCMC (Carter \& Kohn, 1994)
            \end{itemize}
        \end{block}
        \begin{block}{Steady-State BVAR (Villani, 2009)}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item Prior specificat pe \textbf{mediile necondiționate} $\boldsymbol{\mu} = (\mathbf{I} - \bA_1 - \cdots - \bA_p)^{-1}\bc$ în loc de coeficienți
                \item Permite încorporarea directă a opiniilor experților privind nivelurile de echilibru (ex.: ținta de inflație)
            \end{itemize}
        \end{block}
        \begin{exampleblock}{Aplicații și software}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item \textbf{Bănci centrale}: Federal Reserve (model BVAR de prognoză), BCE, Riksbank (Ramses II --- DSGE-BVAR)
                \item \textbf{R}: \texttt{bvartools}, \texttt{BVAR}, \texttt{bvarsv}, \texttt{mfbvar} (frecvență mixtă)
                \item \textbf{Python}: \texttt{statsmodels} (suport limitat); \texttt{PyFlux}, \texttt{BVARmml}
                \item \textbf{MATLAB}: BEAR Toolbox (BCE), BVAR Toolbox (Dieppe, Legrand, van Roye, 2016)
            \end{itemize}
        \end{exampleblock}
        }
    \end{cminipage}
...
\begin{frame}{Forma companion}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Conversia VAR(p) la VAR(1)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Orice VAR($p$) poate fi scris ca VAR(1) în \textbf{forma companion}: $\boldsymbol{\xi}_t = \mathbf{A} \boldsymbol{\xi}_{t-1} + \mathbf{v}_t$
            \end{itemize}
        \end{block}
        }
        \vspace{-2mm}
        ...
        \begin{exampleblock}{Pentru VAR(2)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Forma}:
                    ...
                    \vspace{-4mm}
                \item \textbf{Dimensiune}: $\mathbf{A}$ este $Kp \times Kp$
            \end{itemize}
        \end{exampleblock}
        }
        \vspace{-2mm}
        ...
        \begin{alertblock}{Produsul Kronecker $\otimes$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Dacă $\mathbf{A}$ este $m \times n$ și $\mathbf{B}$ este $p \times q$, atunci $\mathbf{A} \otimes \mathbf{B}$ este matricea $mp \times nq$:
            \end{itemize}
            \vspace{-2mm}
            ...
            \vspace{-4mm}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{Utilizare}: $\text{vec}(\boldsymbol{\Sigma}_Y) = (\mathbf{I}_{K^2} - \mathbf{A} \otimes \mathbf{A})^{-1}\, \text{vec}(\boldsymbol{\Sigma}_\varepsilon)$ --- matricea de covarianță a VAR staționar
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Pentru VAR(1)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Exemplu: Pentru $\bA_1 = \bigl(\begin{smallmatrix} 0.5 & 0.1 \\ 0.2 & 0.3 \end{smallmatrix}\bigr)$, valorile proprii sunt $\lambda_1 \approx 0.57$ și $\lambda_2 \approx 0.23$ --- ambele $< 1$ $\Rightarrow$ stabil.
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}

\begin{frame}{Condiția de stabilitate: exemplu numeric}
    \begin{cminipage}{0.95\textwidth}
        {\small
        \begin{block}{Pentru $\bA = \begin{pmatrix} 0.7 & 0.2 \\ -0.1 & 0.6 \end{pmatrix}$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Polinomul caracteristic: $\det(\bA - \lambda \mathbf{I}) = 0$
            \end{itemize}
            \vspace{-0.2cm}
            \begin{multline*}
                \det\begin{pmatrix} 0.7 - \lambda & 0.2 \\ -0.1 & 0.6 - \lambda \end{pmatrix} = (0.7-\lambda)(0.6-\lambda) + 0.02 = 0 \\
                \implies \lambda^2 - 1.3\lambda + 0.44 = 0
            \end{multline*}
        \end{block}

        \begin{exampleblock}{Soluție}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\lambda = \frac{1.3 \pm \sqrt{1.69 - 1.76}}{2} = 0.65 \pm 0.132i$
                \item $|\lambda| = \sqrt{0.65^2 + 0.132^2} = \sqrt{0.44} = 0.663 < 1$ \quad $\checkmark$ Stabil.
            \end{itemize}
        \end{exampleblock}
        }
    \end{cminipage}
...
\begin{frame}{Media unui VAR staționar}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Exemplu}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Dacă $\mathbf{c} = \begin{pmatrix} 0.5 \\ 0.3 \end{pmatrix}$ și $\bA = \begin{pmatrix} 0.7 & 0.2 \\ -0.1 & 0.6 \end{pmatrix}$:
            \end{itemize}
            \vspace{-0.3cm}
            \begin{align*}
...
\begin{frame}{Structura covarianței pentru VAR(1)}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Matricea varianță-covarianță $\boldsymbol{\Gamma}(0)$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Pentru VAR(1), varianța satisface \textbf{ecuația discretă Lyapunov}:
        ...
        \begin{block}{Autocovarianța la lag-ul $h$}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Formula: $\boldsymbol{\Gamma}(h) = \bA^h \boldsymbol{\Gamma}(0), \quad h \geq 0$
                \item Autocovarianțele scad geometric cu valorile proprii ale lui $\bA$
            \end{itemize}
        \end{block}

        \begin{alertblock}{Rezolvarea ecuației Lyapunov}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Se rezolvă prin vectorizare:
                    $\text{vec}(\boldsymbol{\Gamma}(0)) = (\mathbf{I}_{K^2} - \bA \otimes \bA)^{-1} \text{vec}(\bSigma)$
                \item $\otimes$ denotă produsul Kronecker
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{Estimarea VAR}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Estimarea OLS}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Fiecare ecuație poate fi estimată prin \textbf{OLS} (Ordinary Least Squares -- metoda celor mai mici pătrate) \textbf{separat}:
                    \vspace{-0.15cm}
                    $$\hat{\bA} = \left(\sum_{t=1}^{T} \bY_t \bY_{t-1}'\right) \left(\sum_{t=1}^{T} \bY_{t-1} \bY_{t-1}'\right)^{-1}$$
                    \vspace{-0.2cm}
                \item Eficient, deoarece toate ecuațiile au \textbf{aceiași regresori}
            \end{itemize}
        \end{block}

        \begin{block}{Matricea de covarianță}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Estimatorul: $\hat{\bSigma} = \frac{1}{T-Kp-1} \sum_{t=1}^{T} \hat{\bepsilon}_t \hat{\bepsilon}_t'$
                \item Erorile $\varepsilon_{1t}$ și $\varepsilon_{2t}$ pot fi \textbf{corelate contemporan}
            \end{itemize}
        \end{block}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Selecția lag-ului: exemplu}
    ...
    \begin{block}{Observații}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item AIC și BIC pentru lag $p = 1, \ldots, 10$ (pot sugera ordine diferite); valori mici = ajustare mai bună; ambele selectează $p = 2$
        \end{itemize}
    \end{block}
    }
    \vspace{-1mm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.48\textheight, keepaspectratio]{ch6_lag_selection.pdf}
    \end{center}
...
\begin{frame}{Selecția ordinului lag-ului}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Criterii informaționale}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Alegem $p$ care minimizează:
            \end{itemize}
            \vspace{-0.2cm}
            \begin{align*}
                \text{AIC}(p) &= \ln|\hat{\bSigma}_p| + \frac{2pK^2}{T} \\[-0.05cm]
                \text{BIC}(p) &= \ln|\hat{\bSigma}_p| + \frac{pK^2 \ln T}{T} \\[-0.05cm]
                \text{HQ}(p) &= \ln|\hat{\bSigma}_p| + \frac{2pK^2 \ln\ln T}{T} \quad \text{(Hannan-Quinn)}
            \end{align*}
            \vspace{-0.2cm}
            {\scriptsize
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{unde:} $\hat{\bSigma}_p$ = matricea de covarianță a reziduurilor, $K$ = nr.\ variabile, $p$ = nr.\ lag-uri, $T$ = dimensiunea eșantionului
            \end{itemize}
            }
        \end{block}
        }
        ...
        \begin{block}{Îndrumări}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item AIC tinde să selecteze modele \textbf{mai mari} (prognoză); BIC selectează modele \textbf{mai mici} (selecție consistentă)
...
\begin{frame}{Modele VAR restricționate}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Testarea restricțiilor}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Folosim testul raportului de verosimilitate (LR --- Likelihood Ratio):
...
%=============================================================================
% SECȚIUNEA 3: CAUZALITATE GRANGER
%=============================================================================
...
\begin{frame}{Ce este cauzalitatea Granger?}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Clive Granger (1969, premiul Nobel 2003)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item ``$X$ \textbf{cauzează Granger} pe $Y$'' dacă valorile trecute ale lui $X$ ajută la prognozarea lui $Y$
                \item Criteriul esențial: \textbf{dincolo de} ce pot prezice valorile trecute ale lui $Y$ singure
            \end{itemize}
        \end{block}
        ...
        \begin{alertblock}{Distincție importantă: Cauzalitate Granger $\neq$ cauzalitate reală}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \emph{nu} implică cauzare economică/structurală
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Statistica testului: Testul Wald}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item $RSS_R$ (Residual Sum of Squares): suma pătratelor reziduurilor din modelul restricționat (fără lag-urile lui $Y_2$)
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{center}
            \begin{tikzpicture}[
                ...
                \draw[arrow] (x1) -- (y1);
                \node[below=0.02cm of x1, xshift=1.6cm, font=\footnotesize] {Unidirecțională: $X \rightarrow Y$};

                % Unidirectional Y -> X
                \node[box] (x2) at (0, 0) {$X$};
                \node[box] (y2) at (3.2, 0) {$Y$};
                \draw[arrow] (y2) -- (x2);
                \node[below=0.02cm of x2, xshift=1.6cm, font=\footnotesize] {Unidirecțională: $Y \rightarrow X$};
                ...
                \node[below=0.02cm of x3, xshift=1.6cm, font=\footnotesize] {Bidirecțională: $X \leftrightarrow Y$};
        ...
        \begin{block}{Exemple economice}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Prețurile acțiunilor $\Rightarrow$ Volumul tranzacționat (bidirecțională)
...
\begin{frame}{Corelație încrucișată: ilustrare vizuală}
    ...
    \begin{block}{Interpretare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Stânga: două serii înrudite; Dreapta: CCF (Cross-Correlation Function --- funcția de corelație încrucișată)
            \item CCF relevă că $X$ precede $Y$ (corelații semnificative la lag-uri pozitive)
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Test cauzalitate Granger: exemplu numeric}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Calculul testului}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $T = 100$, $RSS_U = 45.2$, $RSS_R = 52.8$:
            \end{itemize}
            \vspace{-0.2cm}
            $$F = \frac{(52.8 - 45.2)/2}{45.2/(100 - 5)} = \frac{3.8}{0.476} = 7.98$$
            \vspace{-0.2cm}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $F_{0.05}(2, 95) = 3.09$ $\Rightarrow$ \textbf{Respingem $H_0$}: Banii cauzează Granger producția.
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
...
\begin{frame}{Cauzalitate instantanee}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Testarea în VAR}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Testăm $\sigma_{12} \neq 0$ în $\bSigma = \begin{pmatrix} \sigma_1^2 & \sigma_{12} \\ \sigma_{12} & \sigma_2^2 \end{pmatrix}$
                \item $\sigma_{12} = 0$ $\Rightarrow$ fără cauzalitate instantanee
            \end{itemize}
        \end{block}

        \begin{alertblock}{Interpretare}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Cauze posibile: \textbf{șocuri comune} sau \textbf{agregarea datelor}
                \item Nu neapărat efecte contemporane reale
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Testul exogenității de bloc}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Exemplu: Cauzează variabilele financiare (rate ale dobânzii, prețuri acțiuni) Granger variabilele reale (PIB, șomaj)?\vspace{-1mm}
            \end{itemize}
        \end{block}

        \begin{exampleblock}{Statistica testului}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\chi^2 = T \left(\ln|\hat{\bSigma}_R| - \ln|\hat{\bSigma}_U|\right) \sim \chi^2_{K_1 \cdot K_2 \cdot p}$
                \item unde $K_1$ = variabile ``cauzate'', $K_2$ = variabile ``cauzatoare''
            \end{itemize}
        \end{exampleblock}
    \end{cminipage}
\end{frame}
...
%=============================================================================
% SECȚIUNEA 4: FUNCȚII DE RĂSPUNS LA IMPULS
%=============================================================================
...
\begin{frame}{Ce sunt funcțiile de răspuns la impuls?}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Reprezentarea MA($\infty$)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Un VAR(p) stabil: $\bY_t = \boldsymbol{\mu} + \sum_{i=0}^{\infty} \boldsymbol{\Phi}_i \bepsilon_{t-i}$
                \item $\boldsymbol{\Phi}_i$ = \textbf{răspunsurile la impuls} la orizontul $i$
            \end{itemize}
        \end{block}
        }
    \end{cminipage}
        ...
        \begin{block}{Interpretare}
            \begin{itemize}\setlength{\itemsep}{1pt}
                \item $[\boldsymbol{\Phi}_h]_{ij}$: Efectul asupra lui $Y_i$ la $t+h$
                    ...
                    \end{itemize}
                \item VAR stabil: $\boldsymbol{\Phi}_h \to \mathbf{0}$ când $h \to \infty$
                    \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Descompunerea Cholesky: Cum funcționează}
    \vspace{-2mm}
    \begin{cminipage}{0.95\textwidth}
        {\footnotesize
        \begin{block}{Exemplu numeric}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Fie $\bSigma = \bigl(\begin{smallmatrix} 4 & 2 \\ 2 & 5 \end{smallmatrix}\bigr)$. Factorul Cholesky $\mathbf{P}$ (inferior triunghiular) cu $\bSigma = \mathbf{P}\mathbf{P}'$:
            \end{itemize}
            \vspace{-2mm}
            \begin{align*}
            \mathbf{P} &= \begin{pmatrix} 2 & 0 \\ 1 & 2 \end{pmatrix}
            \;\Rightarrow\;
            \mathbf{P}\mathbf{P}' = \begin{pmatrix} 2 & 0 \\ 1 & 2 \end{pmatrix}\begin{pmatrix} 2 & 1 \\ 0 & 2 \end{pmatrix} \\
            &= \begin{pmatrix} 4 & 2 \\ 2 & 5 \end{pmatrix} = \bSigma \;\checkmark
            \end{align*}
            \vspace{-4mm}
        \end{block}
        \vspace{-1mm}
        \begin{exampleblock}{Interpretare: Ce implică ordinea}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $P_{21} = 1 \neq 0$: Un șoc unitar la $Y_1$ are \textbf{efect imediat asupra $Y_2$} (impact = 1)
                \item $P_{12} = 0$: Un șoc la $Y_2$ \textbf{nu afectează contemporan} pe $Y_1$
                \item $Y_1$ este ``mai exogenă'' --- afectează $Y_2$ instantaneu, dar nu și invers
            \end{itemize}
        \end{exampleblock}
        \vspace{-1mm}
        \begin{alertblock}{Inversarea ordinii ($Y_2$ primul)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Schimbarea ordinii dă un $\mathbf{P}$ diferit și IRF-uri diferite
                \item \textbf{Teoria economică} ghidează ordinea --- ex., PIB înainte de șomaj (legea lui Okun)
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{Intervale de încredere pentru IRF}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Surse de incertitudine}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item IRF sunt funcții de parametrii estimați $\hat{\bA}_1, \ldots, \hat{\bA}_p$ $\Rightarrow$ au \textbf{incertitudine de eșantionare}
            \end{itemize}
        \end{block}
        \vspace{-1mm}
        ...
        \begin{alertblock}{Procedura Bootstrap}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                ...
                \item Extragem cu înlocuire $\{\hat{\bepsilon}_t^*\}$; generăm eșantion bootstrap cu VAR estimat
                \item Reestimăm și calculăm IRF; repetăm de $B$ ori
                \item Folosim percentilele pentru IC (intervale de încredere)
            \end{enumerate}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{VAR structural (SVAR)}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Motivație}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Șocurile VAR standard $\bepsilon_t$ sunt inovații de \textbf{formă redusă} $\Rightarrow$ combinații liniare de șocuri structurale
        ...
        \begin{exampleblock}{Relația cu forma redusă}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\bepsilon_t = \mathbf{B}_0^{-1} \mathbf{u}_t \quad \Rightarrow \quad \bSigma = \mathbf{B}_0^{-1} (\mathbf{B}_0^{-1})'$
            \end{itemize}
        \end{exampleblock}
        }
    \end{cminipage}
...
%=============================================================================
% SECȚIUNEA 5: DESCOMPUNEREA VARIANȚEI ERORII DE PROGNOZĂ
%=============================================================================
...
%=============================================================================
% SECȚIUNEA 6: DIAGNOSTICAREA VAR
%=============================================================================
...
\begin{frame}{Diagnosticarea reziduurilor}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{De ce contează}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Reziduuri autocorelate $\Rightarrow$ estimări ineficiente
...
\begin{frame}{Testarea corelației seriale}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Testul Portmanteau (Ljung-Box)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Statistica de test:
                    \vspace{-2mm}
                    $$Q_h = T(T+2) \sum_{j=1}^{h} \frac{1}{T-j} \text{tr}\!\bigl(\hat{\mathbf{C}}_j' \hat{\mathbf{C}}_0^{-1} \hat{\mathbf{C}}_j \hat{\mathbf{C}}_0^{-1}\bigr)$$
                    \vspace{-4mm}
                \item $\hat{\mathbf{C}}_j = \frac{1}{T}\sum_{t=j+1}^{T} \hat{\bepsilon}_t \hat{\bepsilon}_{t-j}'$. \; Sub $H_0$: $Q_h \sim \chi^2_{K^2(h-p)}$
            \end{itemize}
        \end{block}

        \begin{block}{Testul LM (Lagrange Multiplier) Breusch-Godfrey}
            \begin{itemize}\setlength{\itemsep}{0pt}
        ...
        \begin{alertblock}{Dacă este respins}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Creșterea ordinului lag-ului $p$ sau adăugarea de variabile suplimentare
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{Testarea heteroscedasticității}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Testul ARCH-LM (Autoregressive Conditional Heteroscedasticity)}
            \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Testarea normalității}
    ...
    \begin{cminipage}{0.95\textwidth}
        {\small
        \begin{block}{Testul JB --- Jarque-Bera (univariat)}
            \vspace{-2mm}
            $$JB = \frac{T}{6}\left(S^2 + \frac{(\kappa-3)^2}{4}\right) \sim \chi^2_2$$
            \vspace{-4mm}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item unde $S$ = asimetrie, $\kappa$ = kurtosis
            \end{itemize}
        \end{block}
        \vspace{-1mm}
        \begin{block}{Normalitate multivariată (Doornik-Hansen)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Transformăm reziduurile și testăm asimetria și kurtosis comune:
            \end{itemize}
            \vspace{-2mm}
                    \begin{multline*}
                    DH = s_1'(\boldsymbol{\Omega}^{-1/2})'(\boldsymbol{\Omega}^{-1/2})s_1 \\
                    + \; s_2'(\boldsymbol{\Omega}^{-1/2})'(\boldsymbol{\Omega}^{-1/2})s_2 \sim \chi^2_{2K}
                    \end{multline*}
            \vspace{-4mm}
        \end{block}
        }
        ...
        \begin{alertblock}{Notă}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Normalitatea este adesea respinsă în datele financiare; folosiți erori standard robuste dacă non-normalitatea este severă
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{Grafic rezumat diagnostic}
    ...
    \begin{block}{Observații}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ACF reziduurilor nu prezintă autocorelație semnificativă
            \item Histograma aproximează distribuția normală
            \item Q-Q plot urmează linia de 45°
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Eroarea de prognoză și MSE}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Eroarea de prognoză la $h$ pași}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\mathbf{e}_{T+h|T} = \bY_{T+h} - \hat{\bY}_{T+h|T} = \sum_{j=0}^{h-1} \bA^j \bepsilon_{T+h-j}$
            \end{itemize}
        \end{block}

        \begin{block}{Matricea MSE (Mean Squared Error --- eroarea medie pătratică)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\text{MSE}(\hat{\bY}_{T+h|T}) = \E[\mathbf{e}_{T+h|T} \mathbf{e}_{T+h|T}'] = \sum_{j=0}^{h-1} \bA^j \bSigma (\bA^j)'$
            \end{itemize}
        \end{block}
        ...
        \begin{alertblock}{Regiuni de încredere comune}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Pentru mai multe variabile, folosim elipsoizi:
                    \vspace{-0.15cm}
                    \begin{multline*}
                    (\bY_{T+h} - \hat{\bY}_{T+h|T})' [\text{MSE}(\hat{\bY}_{T+h|T})]^{-1} \\
                    \times\; (\bY_{T+h} - \hat{\bY}_{T+h|T}) \leq \chi^2_{K,\alpha}
                    \end{multline*}
                    \vspace{-0.2cm}
            \end{itemize}
        \end{alertblock}
...
\begin{frame}{Evaluare out-of-sample: VAR vs.\ AR}
    ...
    \begin{block}{Metodologie generală}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Metrică: RMSE \textit{(vezi Capitolul 0)}
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Evaluarea prognozei}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Evaluare out-of-sample}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Împărțim datele: eșantion de estimare (1 la $T_1$) și eșantion de testare ($T_1+1$ la $T$)
                \item Erorile de prognoză: $e_{t+h} = Y_{t+h} - \hat{Y}_{t+h|t}$
            \end{itemize}
        \end{block}
        \vspace{-1mm}
        \begin{block}{Metrici comune \textit{\normalfont(vezi Capitolul 0 pentru definițiile complete)}}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{RMSE}: penalizează erorile mari \quad \textbf{MAE}: robust la outlieri \quad \textbf{MAPE}: procentual, independent de scală
            \end{itemize}
        \end{block}
        \vspace{-1mm}
        \begin{exampleblock}{Testul Diebold-Mariano}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Testează dacă prognozele VAR sunt semnificativ mai bune decât alternativa:
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Rezultatele testului: PIB și șomaj}
            ...
            \begin{tabular}{lcccc}
                ...
                \midrule
                PIB $\nRightarrow$ Șomaj & 8.42 & (2, 95) & 0.0004 & Respingem \\
                Șomaj $\nRightarrow$ PIB & 2.15 & (2, 95) & 0.1220 & Nu respingem \\
                \bottomrule
            \end{tabular}
            \end{center}
        \end{block}
...
\begin{frame}{Pasul 1: Analiză preliminară}
    ...
    \begin{block}{Rezultate}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item PIB: ACF scade rapid $\Rightarrow$ staționar; Șomaj: ACF persistent (ADF --- Augmented Dickey-Fuller: $p = 0.02$)
...
\begin{frame}{Pasul 3: Testul Granger de cauzalitate}
    ...
    \begin{block}{Rezultate}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item PIB $\Rightarrow$ Șomaj: $F = 17.35$, $p < 0.001$ $\Rightarrow$ PIB ``cauzează Granger'' Șomajul
            \item Șomaj $\Rightarrow$ PIB: $F = 38.93$, $p < 0.001$ $\Rightarrow$ cauzalitate bidirecțională (Legea lui Okun)
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Pasul 4: Funcții de răspuns la impuls (IRF)}
    ...
    \begin{block}{Rezultate IRF}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Șoc PIB $\Rightarrow$ efect negativ persistent asupra șomajului (Legea lui Okun, $>$20 trim.)
            \item Șoc Șomaj $\Rightarrow$ efect pozitiv de scurtă durată asupra PIB (recuperare, 2--3 trim.)
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Pasul 6: Diagnosticarea reziduurilor}
    ...
    \begin{block}{Diagnostic}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Non-normalitate semnificativă (JB respins) $\Rightarrow$ datorită valorilor extreme COVID-19
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Pasul 9: Comparație out-of-sample -- AR vs VAR}
    ...
    \begin{block}{Metodologie}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Prognoză recursivă: estimare pe $[1, \ldots, t]$, prognoză $\hat{y}_{t+1|t}$
            \item RMSE: $\text{RMSE}_t = \sqrt{\frac{1}{t}\sum_{s=1}^{t} e_s^2}$
...
\begin{frame}{Exercițiu AI: Gândire critică}
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{\footnotesize Prompt de testat în ChatGPT / Claude / Copilot}
            {\footnotesize
            ``Descarcă de pe FRED: rata trimestrială de creștere a PIB real SUA (A191RL1Q225SBEA) și rata lunară a șomajului (UNRATE, agregată trimestrial) din 2000-Q1 până în 2024-Q4 (100 observații). Testează cauzalitatea Granger în ambele direcții, estimează un model VAR și calculează funcțiile impuls-răspuns ortogonalizate. Vreau cod Python complet.''
            }
        \end{block}
        \vspace{-2mm}
        {\footnotesize
        ...
        \begin{enumerate}\setlength{\itemsep}{0pt}
            ...
            \item Cum selectează ordinul lag-urilor? Compară AIC, BIC, HQ?
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Modele VAR}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Estimare prin OLS ecuație cu ecuație; necesită staționaritate
            \end{itemize}
        \end{block}
        \vspace{-0.1cm}
        \begin{block}{Cauzalitate Granger}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Testează dacă $X$ ajută la prognozarea lui $Y$ dincolo de istoricul propriu al lui $Y$
    ...
    \begin{cminipage}{0.95\textwidth}
        \begin{alertblock}{Subiecte pentru studiu aprofundat}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{VECM} (Vector Error Correction Model): Modele cu corecția erorii pentru sisteme cointegrate
...
\begin{frame}{Întrebarea 1}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Variante de răspuns}
            ...
            \textcolor{MainBlue}{\textbf{(A)}} Da, deoarece toate elementele diagonale sunt mai mici decât 1\\[3pt]

            \textcolor{MainBlue}{\textbf{(B)}} Da, deoarece toate valorile proprii sunt în interiorul cercului unitate\\[3pt]

            \textcolor{MainBlue}{\textbf{(C)}} Nu, deoarece suma coeficienților depășește 1\\[3pt]

            \textcolor{MainBlue}{\textbf{(D)}} Nu poate fi determinat fără a cunoaște $\bSigma$
...
\begin{frame}{Întrebarea 1: Răspuns}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Răspuns: (B)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\lambda_1 = 0.879$, $\lambda_2 = 0.421$ $\Rightarrow$ ambele $|\lambda| < 1$ $\Rightarrow$ stabil.
            \end{itemize}
        \end{exampleblock}
        }
...
\begin{frame}{Întrebarea 2}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Variante de răspuns}
            ...
            \textcolor{MainBlue}{\textbf{(B)}} Valorile trecute ale lui $X$ conțin informații utile pentru prognozarea lui $Y$\\[3pt]

            \textcolor{MainBlue}{\textbf{(C)}} $Y$ nu poate cauza Granger pe $X$\\[3pt]

            \textcolor{MainBlue}{\textbf{(D)}} Corelația între $X$ și $Y$ este pozitivă
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Răspuns: (B)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item X trecut ajută la prognozarea lui Y
            \end{itemize}
        \end{exampleblock}
        }
...
\begin{frame}{Întrebarea 3}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Variante de răspuns}
            ...
            \textcolor{MainBlue}{\textbf{(A)}} Magnitudinea răspunsurilor la impuls\\[3pt]

            \textcolor{MainBlue}{\textbf{(B)}} Viteza cu care șocurile dispar\\[3pt]

            \textcolor{MainBlue}{\textbf{(C)}} Care variabile pot răspunde contemporan la care șocuri\\[3pt]

            \textcolor{MainBlue}{\textbf{(D)}} Numărul de lag-uri în VAR
        ...
        \begin{block}{Numărare detaliată}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item VAR(1) cu $K=2$ variabile:
                    \vspace{-0.2cm}
                    $$\begin{pmatrix} Y_{1t} \\ Y_{2t} \end{pmatrix} \!=\! \underbrace{\begin{pmatrix} c_1 \\ c_2 \end{pmatrix}}_{\text{2 par.}} \!+\! \underbrace{\begin{pmatrix} a_{11} & a_{12} \\ a_{21} & a_{22} \end{pmatrix}}_{\text{4 par.}} \!\begin{pmatrix} Y_{1,t-1} \\ Y_{2,t-1} \end{pmatrix} \!+\! \begin{pmatrix} \varepsilon_{1t} \\ \varepsilon_{2t} \end{pmatrix}$$
                    \vspace{-0.2cm}
...
\begin{frame}{Întrebarea 5}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Variante de răspuns}
            ...
            \textcolor{MainBlue}{\textbf{(A)}} 35\% din varianța totală a variabilei 1 este explicată de variabila 2\\[3pt]

            \textcolor{MainBlue}{\textbf{(B)}} 35\% din varianța erorii de prognoză la $h$ pași a variabilei 1 se datorează șocurilor la variabila 2\\[3pt]

            \textcolor{MainBlue}{\textbf{(C)}} Corelația între variabilele 1 și 2 la lag-ul $h$ este 0.35\\[3pt]

            \textcolor{MainBlue}{\textbf{(D)}} Variabila 2 explică 35\% din răspunsul la impuls al variabilei 1
...
%=============================================================================
% EXTENSII: CAUZALITATE MODERNĂ
%=============================================================================
\section{Extensii: Cauzalitate Modernă}

\begin{frame}{De la Granger la Pearl: Paradigme ale Cauzalității}
    \quantlet{TSA\_ch6\_pearl\_causality}{https://github.com/QuantLet/TSA/tree/main/TSA_ch6/TSA_ch6_pearl_causality}
    \vspace{-0.3cm}
    {\scriptsize
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Cauzalitate Granger (1969): cauzalitate predictivă}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item $X$ cauzează Granger $Y$ dacă trecutul lui $X$ îmbunătățește predicția lui $Y$ dincolo de trecutul lui $Y$
                    \item \textbf{Limitare}: corelație $\neq$ cauzalitate; confunderii pot genera cauzalitate Granger \textbf{falsă}
                    \item \textit{Exemplu}: vânzările de înghețată ``cauzează Granger'' înecurile (factor comun: temperatura)
                \end{itemize}
            \end{block}

            \begin{alertblock}{Distincția fundamentală}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Granger} = precedență temporală + predictibilitate incrementală
                    \item \textbf{Pearl} = mecanisme structurale (intervenții)
                \end{itemize}
            \end{alertblock}
        \end{column}

        \begin{column}{0.48\textwidth}
            \begin{exampleblock}{Cauzalitate structurală (Pearl, 2009)}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Grafuri Aciclice Direcționate (DAG)}: codifică structura cauzală
                    \item \textbf{do-calculus}: $P(Y \mid do(X{=}x)) \neq P(Y \mid X{=}x)$
                    \item \textbf{Intervenție vs.\ observație}:
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item ``Ce se întâmplă dacă \textbf{fixăm} $X{=}x$'' (intervenție)
                        \item ``Ce observăm când $X{=}x$'' (condiționare)
                    \end{itemize}
                \end{itemize}
            \end{exampleblock}

            \begin{block}{Exemplu DAG}
                \begin{center}
                    Temperatură $\rightarrow$ Înghețată \\[2pt]
                    Temperatură $\rightarrow$ Înecuri \\[2pt]
                    {\footnotesize Înghețată $\not\!\rightarrow$ Înecuri (cauzalitate falsă)}
                \end{center}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{do-Calculus și Grafuri Cauzale}
    \quantlet{TSA\_ch6\_do\_calculus}{https://github.com/QuantLet/TSA/tree/main/TSA_ch6/TSA_ch6_do_calculus}
    \vspace{-0.4cm}
    {\scriptsize
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Modelul Cauzal Structural (SCM)}
                \vspace{-0.2cm}
                \[
                ...
                \vspace{-0.4cm}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item $\text{Pa}_i$ = părinții lui $X_i$ în DAG
                    \item $U_i$ = termen de zgomot (exogen)
                    \item Fiecare ecuație = un mecanism cauzal autonom
                \end{itemize}
            \end{block}

            \begin{alertblock}{Cele trei reguli ale do-calculus (Pearl, 2009)}
                \begin{enumerate}\setlength{\itemsep}{0pt}
                    \item Inserare/ștergere observații
                    \item Schimbarea acțiune $\leftrightarrow$ observație
                    \item Inserare/ștergere acțiuni
                \end{enumerate}
            \end{alertblock}
        \end{column}

        \begin{column}{0.48\textwidth}
            \begin{exampleblock}{Criteriul back-door}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Mulțimea $Z$ blochează toate căile back-door de la $X$ la $Y$:
                \end{itemize}
                \vspace{-0.2cm}
                \[
                ...
                \]
                \vspace{-0.3cm}
                {\footnotesize Ajustare standard -- elimină confunderea.}
            \end{exampleblock}

            \begin{block}{Criteriul front-door}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Utilizat când criteriul back-door nu este satisfăcut
                    \item Necesită un mediator $M$: $X \!\rightarrow\! M \!\rightarrow\! Y$
                \end{itemize}
            \end{block}

            \begin{block}{Legătura cu SVAR}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Identificarea structurală în VAR $\approx$ presupunerea unui DAG specific
                    \item Restricțiile de ordonare Cholesky $=$ un DAG recursiv
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{PCMCI: Inferență Cauzală pentru Serii de Timp}
    \quantlet{TSA\_ch6\_pcmci}{https://github.com/QuantLet/TSA/tree/main/TSA_ch6/TSA_ch6_pcmci}
    \vspace{-0.3cm}
    {\scriptsize
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{Provocare: efecte cauzale lagate și contemporane}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Seriile de timp au atât efecte cauzale \textbf{contemporane}, cât și \textbf{întârziate} (lagate)
                    \item Autocorrelația puternică distorsionează testele standard de independență condițională
                \end{itemize}
            \end{block}

            \begin{alertblock}{Algoritmul PCMCI (Runge et al., 2019)}
                \textbf{Pasul 1 -- PC (selecție condiționată):}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Pentru fiecare variabilă $X_t^j$, testează:
                    \item $X_{t-\tau}^i \perp\!\!\!\perp X_t^j \mid S$ \quad (mulțimea de condiționare $S$)
                    \item Utilizează corelație parțială sau IMC (informație mutuală condițională)
                \end{itemize}
            \end{alertblock}
        \end{column}

        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Pasul 2 -- MCI (independență condițională momentană)}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item $X_{t-\tau}^i \rightarrow X_t^j$ dacă și numai dacă:
                \end{itemize}
                \vspace{-0.2cm}
                {\footnotesize
                \[
                    X_{t-\tau}^i \not\!\perp\!\!\!\perp X_t^j \;\Big|\; \widehat{\text{Pa}}(X_t^j)\!\setminus\!\{X_{t-\tau}^i\},\; \widehat{\text{Pa}}(X_{t-\tau}^i)
                \]}
                \vspace{-0.4cm}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item Controlează autocorrelația (avantaj major față de algoritmul PC standard)
                \end{itemize}
            \end{alertblock}

            \begin{exampleblock}{Rezultat și implementare}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item \textbf{Output}: graf cauzal cu lag-uri, intensități ale legăturilor și $p$-valori
                    \item \textbf{Software}: pachetul Python \texttt{tigramite} (Runge)
                    \item \textbf{Referință}: Runge et al.\ (2019), \textit{Science Advances}
                \end{itemize}
            \end{exampleblock}
        \end{column}
    \end{columns}
...
\begin{frame}{Comparație și Aplicații}
    \quantlet{TSA\_ch6\_causality\_comparison}{https://github.com/QuantLet/TSA/tree/main/TSA_ch6/TSA_ch6_causality_comparison}
    \vspace{-0.3cm}
    {\scriptsize
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Comparație între metodele de inferență cauzală}
            \vspace{-0.1cm}
            {\footnotesize
            \begin{tabular}{@{}l c c c c@{}}
                \hline
                \textbf{Metodă} & \textbf{Tip cauzalitate} & \textbf{Confounderi} & \textbf{Non-liniar} & \textbf{Contemporan} \\
                \hline
                Granger (1969) & Predictivă & Nu controlează & Nu$^*$ & Nu \\
                SVAR & Structural & Restricții & Nu & Da \\
                PC / FCI & Grafic & Da & Nu$^*$ & Da \\
                PCMCI (Runge) & Grafic temporal & Da & Da (IMC) & Da \\
                CCM$^\dagger$ & Sisteme dinamice & Da & Da & Nu \\
                \hline
            \end{tabular}}\\[3pt]
            {\footnotesize $^*$Extensii: Granger non-liniar, PC bazat pe kernel. \quad $^\dagger$Convergent Cross Mapping (Sugihara et al., 2012).}
        \end{block}
        ...
        \begin{columns}[T]
            \begin{column}{0.48\textwidth}
                \begin{exampleblock}{Aplicații}
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item \textbf{Climatologie}: rețele cauzale între variabile climatice (Runge et al., 2019)
                        \item \textbf{Neuroștiință}: conectivitate cerebrală din date fMRI
                        \item \textbf{Finanțe}: transmiterea riscului sistemic, estimarea efectelor politicii monetare
                        \item \textbf{Economie}: mecanismele de transmisie a politicii monetare
                    \end{itemize}
                \end{exampleblock}
            \end{column}
            ...
            \begin{column}{0.48\textwidth}
                \begin{block}{Referințe cheie}
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        ...
                        \item Runge, J.\ et al.\ (2019). Detecting and quantifying causal associations in large nonlinear time series datasets, \textit{Science Advances}, 5(11)
                        \item Spirtes, P., Glymour, C.\ \& Scheines, R.\ (2000). \textit{Causation, Prediction, and Search}, MIT Press
                    \end{itemize}
                \end{block}
            \end{column}
        \end{columns}
...
\begin{frame}{Formule principale -- Rezumat}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            ...
            \begin{block}{FEVD}
                \begin{itemize}\setlength{\itemsep}{0pt}
                    \item {\small $\text{FEVD}_{jk}(h) = \frac{\sum_{i=0}^{h-1}(\mathbf{e}'_j\bPhi_i\mathbf{P}\mathbf{e}_k)^2}{\sum_{i=0}^{h-1}\mathbf{e}'_j\bPhi_i\bSigma\bPhi'_i\mathbf{e}_j}$}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Resurse online și cod}
            ...
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item \textbf{GitHub TSA}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch6} -- Cod Python pentru acest capitol
            \end{itemize}
            }
        \end{exampleblock}
    \end{cminipage}
