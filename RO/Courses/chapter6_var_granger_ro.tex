    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{alertblock}{Pentru VAR(1)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item Exemplu: Pentru $\bA_1 = \bigl(\begin{smallmatrix} 0.5 & 0.1 \\ 0.2 & 0.3 \end{smallmatrix}\bigr)$, valorile proprii sunt $\lambda_1 \approx 0.54$ și $\lambda_2 \approx 0.26$ --- ambele $< 1$ $\Rightarrow$ stabil!
            \end{itemize}
        \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Condiția de stabilitate: exemplu numeric}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{exampleblock}{Soluție}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $\lambda = \frac{1.3 \pm \sqrt{1.69 - 1.76}}{2} = 0.65 \pm 0.132i$, \quad
...
\begin{frame}{Ce este cauzalitatea Granger?}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Clive Granger (1969, premiul Nobel 2003)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item ``$X$ \textbf{cauzează Granger} pe $Y$'' dacă valorile trecute ale lui $X$ ajută la prezicerea lui $Y$
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Testul Portmanteau (Ljung-Box)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                ...
                \item $\hat{\mathbf{C}}_j = \frac{1}{T}\sum_{t=j+1}^{T} \hat{\bepsilon}_t \hat{\bepsilon}_{t-j}'$. \quad Sub $H_0$: $Q_h \sim \chi^2_{K^2(h-p)}$
            \end{itemize}
        \end{block}
...
\begin{frame}{Testarea normalității}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Testul JB --- Jarque-Bera (univariat)}
            ...
            \vspace{-4mm}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item unde $S$ = asimetrie, $K$ = curtoză
            \end{itemize}
        \end{block}
        \vspace{-1mm}
...
\begin{frame}{Evaluare out-of-sample: VAR vs.\ AR}
    ...
    \begin{block}{Metodologie generală}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Metrică: $\text{RMSE}$ (Root Mean Squared Error) $= \sqrt{\tfrac{1}{h}\sum e_i^2}$
        \end{itemize}
    \end{block}
    }
    \begin{center}
...
\begin{frame}{Evaluarea prognozei}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Metrici comune}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item \textbf{RMSE}: $\sqrt{\frac{1}{n}\sum e_{t+h}^2}$ \quad \textbf{MAE} (Mean Absolute Error): $\frac{1}{n}\sum |e_{t+h}|$ \quad \textbf{MAPE} (Mean Absolute Percentage Error): $\frac{100}{n}\sum \left|\frac{e_{t+h}}{Y_{t+h}}\right|$
            \end{itemize}
        \end{block}
        \vspace{-1mm}
...
\begin{frame}{Întrebarea 2}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Variante de răspuns}
            ...
            \textcolor{MainBlue}{\textbf{(B)}} Valorile trecute ale lui $X$ conțin informații utile pentru prezicerea lui $Y$\\[3pt]
