...
\begin{frame}{De ce modelele univariate nu sunt suficiente}
    ...
    \begin{alertblock}{Problema}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ARIMA (Autoregressive Integrated Moving Average) tratează fiecare variabilă \textbf{izolat}
            \item Ignoră \textbf{interacțiunile} și \textbf{efectele de retroacțiune}
        \end{itemize}
    \end{alertblock}
    \vspace{-1mm}
...
\begin{frame}{Selecția lag-ului: exemplu}
    ...
    \begin{block}{Observații}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Date reale SUA (FRED --- Federal Reserve Economic Data): PIB și Șomaj, $T = 140$ trimestre
            \item Criterii informaționale: AIC (Akaike Information Criterion) și BIC (Bayesian Information Criterion) pentru lag $p = 1, \ldots, 10$ (pot sugera ordine diferite)
...
\begin{frame}{Descompunerea Cholesky: Cum funcționează}
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Exemplu numeric}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Fie matricea de covarianță a reziduurilor VAR:
                    $\bSigma = \begin{pmatrix} 4 & 2 \\ 2 & 5 \end{pmatrix}$
                \item Factorul Cholesky $\mathbf{P}$ (inferior triunghiular) cu $\bSigma = \mathbf{P}\mathbf{P}'$:
            \end{itemize}
            \[
        ...
        \begin{alertblock}{Inversarea ordinii ($Y_2$ primul)}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item Schimbarea ordinii variabilelor dă un $\mathbf{P}$ diferit și IRF-uri diferite
                \item \textbf{Teoria economică} trebuie să ghideze ordinea --- ex., PIB înainte de șomaj (legea lui Okun: șocurile de producție afectează piața muncii)
            \end{itemize}
        \end{alertblock}
        }
    \end{cminipage}
...
\begin{frame}{Intervale de încredere pentru IRF}
    ...
    \begin{cminipage}{0.95\textwidth}
        ...
        \begin{block}{Surse de incertitudine}
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item IRF sunt funcții de parametrii estimați $\hat{\bA}_1, \ldots, \hat{\bA}_p$
                \item Prin urmare, au \textbf{incertitudine de eșantionare}
            \end{itemize}
        \end{block}
        ...
        \begin{alertblock}{Procedura Bootstrap}
            \begin{enumerate}\setlength{\itemsep}{0pt}
                ...
                \item Repetăm de $B$ ori; folosim percentilele pentru IC (intervale de încredere)
            \end{enumerate}
        \end{alertblock}
        }
    \end{cminipage}
