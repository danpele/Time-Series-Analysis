...
\begin{frame}{Descompunerea Cholesky: Cum funcționează}
    ...
    \begin{block}{Exemplu numeric}
        Fie matricea de covarianță a reziduurilor VAR:
        $\bSigma = \begin{pmatrix} 4 & 2 \\ 2 & 5 \end{pmatrix}$.
        Factorul Cholesky $\mathbf{P}$ (inferior triunghiular) cu $\bSigma = \mathbf{P}\mathbf{P}'$:
        \[
    ...
    \begin{alertblock}{Inversarea ordinii ($Y_2$ primul)}
        Schimbarea ordinii variabilelor dă un $\mathbf{P}$ diferit și IRF-uri diferite. De aceea \textbf{teoria economică} trebuie să ghideze ordinea --- ex., PIB înainte de șomaj (legea lui Okun: șocurile de producție afectează piața muncii, nu invers pe termen scurt).
    \end{alertblock}
    }
    \end{cminipage}
\end{frame}
