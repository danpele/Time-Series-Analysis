...
\begin{frame}{Bitcoin: Evaluarea Finală pe Setul de Test}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Performanță Test}
            MAE = 1,82, RMSE = 2,14\\
            Prognoza urmărește bine volatilitatea realizată.
        \end{exampleblock}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Fapte Stilizate Financiare}
            \begin{enumerate}
                ...
                \item \textbf{Efect leverage}: Randamente negative $\rightarrow$ volatilitate mai mare
...
\begin{frame}{Pete Solare: Rezultate Prognoză}
    ...
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{Model}
            ARIMA(2,0,1) + 3 termeni Fourier captează dinamica ciclului de 11 ani.
        \end{block}

        \column{0.5\textwidth}
        \begin{exampleblock}{Performanță Test}
            RMSE = 29,68, MAE = 27,35. Modelul urmărește tiparul general al ciclului.
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Pete Solare: Concluzii Cheie}
    \begin{columns}[T]
        ...
        \begin{alertblock}{Alegerea lui K}
            Începeți cu $K=1$, creșteți până când eroarea de validare nu mai scade. K prea mare = overfitting.
        \end{alertblock}
...
\begin{frame}{Șomajul: Diagnosticare SARIMA}
    ...
    \begin{columns}[T]
        ...
        \begin{exampleblock}{Ljung-Box p = 0,66}
            Fără autocorelație. Model bine specificat.
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Modelul Prophet}
    \begin{defn}[Descompunerea Prophet]
        ...
        unde $g(t)$ = trend, $s(t)$ = sezonalitate, $h(t)$ = sărbători, $\sigma^2$ = varianța zgomotului (estimată).
    \end{defn}
...
\begin{frame}{Șomajul: Rezultate Prognoză Prophet}
    ...
    \begin{block}{Concluzie Cheie}
        Prophet se adaptează prin detectare changepoint. \textbf{Test RMSE = 0,12} (la fel ca SARIMA).
    \end{block}
\end{frame}
...
\begin{frame}{Prophet: Când să-l Folosești}
    \begin{columns}[T]
        ...
        \begin{block}{Cazuri de Utilizare Ideale}
            \begin{itemize}
                \item Date cu \textbf{rupturi structurale}
        ...
        \begin{alertblock}{Nu Este Ideal Pentru}
            \begin{itemize}
                \item Date financiare de înaltă frecvență
                \item Date fără trend/sezonalitate clară
                \item Serii de timp foarte scurte
            \end{itemize}
        \end{alertblock}
...
\begin{frame}{VAR: Prognoză (Train/Val/Test)}
    ...
    \begin{block}{Prognoză Rolling One-Step-Ahead}
        VAR captează dinamica PIB-Șomaj. Șocul COVID vizibil în perioada test.
    \end{block}
\end{frame}
...
\begin{frame}{VAR: Rezultate Set Test}
    \begin{block}{Performanță Set Test pe Variabile}
        ...
        \begin{tabular}{lccc}
            ...
            \midrule
            Creștere PIB & 0,90 & 0,81 & 50\% \\
            Șomaj & 0,43 & 0,35 & 50\% \\
            Inflație & 0,58 & 0,51 & 70\% \\
            Rata Fed & 1,81 & 1,77 & 90\% \\
            \midrule
            \textbf{Medie} & \textbf{0,93} & \textbf{0,86} & \textbf{65\%} \\
            \bottomrule
        \end{tabular}
        \end{center}
    \end{block}
...
\begin{frame}{Cadrul de Selectare a Modelului}
    ...
    \begin{tikzpicture}[scale=0.7, transform shape,
        ...
        \node[decision, below left=1cm and 1.3cm of q1] (q2) {Rupturi\\structurale?};
        \node[decision, below right=1cm and 1.3cm of q1] (q3) {Date\\financiare?};
        ...
        \draw[arrow] (start) -- (q1);
        \draw[arrow] (q1) -- node[right] {\scriptsize Da} (var);
        \draw[arrow] (q1) -- node[above left] {\scriptsize Nu} (q2);
        \draw[arrow] (q2) -- node[left] {\scriptsize Da} (prophet);
        \draw[arrow] (q2) -- node[above right] {\scriptsize Nu} (q3);
        \draw[arrow] (q3) -- node[left] {\scriptsize Vol.} (garch);
        \draw[arrow] (q3) -- node[right] {\scriptsize Nivel} (arima);
    \end{tikzpicture}
    \end{center}
\end{frame}
...
\begin{frame}{Sumar: Comparație Modele}
    \begin{columns}[T]
        ...
        \begin{tabular}{llll}
            ...
            \midrule
            Bitcoin & Volatilitate & GARCH & 2,14 \\
            Pete solare & Sezonalitate & Fourier & 29,68 \\
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Rezultatele Noastre}
            \begin{itemize}
                ...
                \item Fourier: RMSE=29,68 (cicluri)
                \item Prophet: RMSE=0,12 (rupturi)
                \item VAR: RMSE mediu=0,93 (multi)
            \end{itemize}
        \end{block}

        \column{0.5\textwidth}
        \begin{exampleblock}{Insight Cheie}
            Fiecare model excelează în domeniul său. Arta constă în potrivirea modelului cu caracteristicile datelor.
        \end{exampleblock}
    \end{columns}
\end{frame}
