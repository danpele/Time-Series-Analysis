...
\begin{frame}{Cadrul Train/Validation/Test}
    ...
    \begin{tikzpicture}[scale=0.95]
        ...
        \node at (4.5,0) {\textbf{Antrenament (70\%)}};
        ...
        \node[below, MainBlue] at (4.5,-1) {\small Estimare parametri};

        \draw[thick, ->, Orange] (10.25,-1) -- (10.25,-0.6);
        \node[below, Orange] at (10.25,-1) {\small Selectare model};

        \draw[thick, ->, Crimson] (12.75,-1) -- (12.75,-0.6);
        \node[below, Crimson] at (12.75,-1) {\small Evaluare finală};

        \draw[thick, ->] (0,-2) -- (14,-2);
        \node[right] at (14,-2) {Timp};
        \node[left] at (0,-2) {$t_0$};
    \end{tikzpicture}
    \end{center}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Set Validare}
            \begin{itemize}
                ...
                \item Ajustare hiperparametri
                \item Selectare abordare
            \end{itemize}
        \end{block}
        ...
        \begin{block}{Set Test}
            \begin{itemize}
                \item \textbf{Păstrat} până la final
                \item Evaluare nebiasată
                \item Raportare metrici
            \end{itemize}
        \end{block}
    \end{columns}
\end{frame}
...
\begin{frame}{Metrici de Evaluare}
    \begin{defn}[Metrici ale Erorii de Prognoză]
        Fie $y_t$ valorile reale și $\hat{y}_t$ prognozele pentru $t = 1, \ldots, n$:
        \begin{align}
    ...
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{exampleblock}{Când să Folosim Fiecare}
            \begin{itemize}
        ...
        \begin{alertblock}{Atenție}
            \begin{itemize}
                ...
                \item Comparați modele pe \textbf{același} set test
...
\begin{frame}{Prognoza GARCH: Rolling vs Multi-Step}
    \begin{alertblock}{De ce Prognoze Rolling One-Step-Ahead?}
        Prognozele GARCH multi-step converg la \textbf{varianța necondiționată}:
        \begin{equation}
            \lim_{h \to \infty} \E[\sigma_{t+h}^2 | \mathcal{F}_t] = \bar{\sigma}^2 = \frac{\omega}{1 - \alpha - \beta}
        \end{equation}
        Aceasta produce o prognoză \textbf{linie dreaptă}---inutilă pentru managementul dinamic al riscului!
    \end{alertblock}
    ...
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{Prognoză Multi-Step}
            \begin{itemize}
                \item O singură estimare model
                \item Prognoză $h$ pași înainte
                \item Converge la $\bar{\sigma}^2$
                \item \textcolor{Crimson}{Apare ca linie dreaptă}
            \end{itemize}
        \end{block}
        ...
        \begin{exampleblock}{Rolling One-Step-Ahead}
            \begin{itemize}
                \item Re-estimare la fiecare $t$
                \item Prognoză doar 1 pas înainte
                \item Captează dinamica volatilității
                \item \textcolor{Forest}{Prognoze dinamice}
            \end{itemize}
        \end{exampleblock}
    \end{columns}
    ...
    \begin{center}
        \textit{Prognozele rolling sunt practica standard în managementul riscului financiar (VaR, ES).}
    \end{center}
\end{frame}
...
\begin{frame}{Pete Solare: Definirea Problemei}
    \begin{block}{Întrebarea de Cercetare}
        Cum modelăm \textbf{cicluri sezoniere lungi} care depășesc capacitatea SARIMA?
    \end{block}
    ...
    \begin{columns}[T]
        ...
        \textbf{Caracteristicile Datelor}
        \begin{itemize}
            \item Sursă: Statsmodels (Wolfer)
            \item Perioadă: 1900 -- 2008
            \item Frecvență: Anuală
            \item Observații: 109 ani
        \end{itemize}

        \vspace{0.3cm}

        \textbf{Caracteristică Cunoscută}
        \begin{itemize}
            \item \textbf{Ciclul Schwabe}: $\approx$ 11 ani
            \item Amplitudine neregulată
            \item Valori non-negative
        \end{itemize}

        \column{0.5\textwidth}
        \begin{alertblock}{Provocarea}
            SARIMA standard $(p,d,q)(P,D,Q)_s$ necesită:
            \begin{itemize}
                \item $s = 11$ pentru date anuale
                \item $(P + D + Q) \times 11$ lag-uri sezoniere
                \item \textbf{Prea mulți parametri!}
            \end{itemize}
        \end{alertblock}
        ...
        \begin{exampleblock}{Soluție}
            Folosim \textbf{termeni Fourier} ca regresori exogeni în ARIMA.
        \end{exampleblock}
    \end{columns}
\end{frame}

\begin{frame}{Termeni Fourier pentru Sezonalitate}
    \begin{defn}[Reprezentarea Fourier]
        Un tipar sezonier cu perioada $s$ poate fi aproximat prin:
        \begin{equation}
            S_t = \sum_{k=1}^{K}\left[\alpha_k \sin\left(\frac{2\pi k t}{s}\right) + \beta_k \cos\left(\frac{2\pi k t}{s}\right)\right]
        \end{equation}
        unde $K \leq \lfloor s/2 \rfloor$ este numărul de perechi armonice.
    \end{defn}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Avantaje}
            \begin{itemize}
                \item Doar $2K$ parametri (nu $s$)
                \item Gestionează \textbf{orice} perioadă sezonieră
                \item Tipar sezonier neted
                \item $K$ controlează flexibilitatea
            \end{itemize}
        \end{block}

        \column{0.5\textwidth}
        \begin{exampleblock}{Structura Modelului}
            ARIMA$(p,d,q)$ cu regresori Fourier:
            \begin{equation}
                y_t = \underbrace{S_t}_{\text{Fourier}} + \underbrace{\eta_t}_{\text{ARIMA}}
            \end{equation}
            unde $\eta_t$ urmează dinamica ARIMA.
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Șomajul: Rupturi Structurale}
    \begin{block}{Întrebarea de Cercetare}
        Cum modelăm serii de timp cu \textbf{schimbări structurale bruște}?
    \end{block}
    ...
    \begin{columns}[T]
        ...
        \begin{itemize}
            ...
            \item Perioadă: 2010 -- 2025
            \item Frecvență: Lunară
            \item Observații: $\approx 180$ luni
        \end{itemize}

        \vspace{0.2cm}

        \textbf{Statistici Cheie}
        \begin{itemize}
            \item Minim pre-COVID: 3,5\%
            \item Vârf COVID (Apr 2020): \textbf{14,8\%}
        ...
        \begin{alertblock}{Provocarea}
            \begin{itemize}
                \item Aprilie 2020: Cea mai mare creștere lunară din istoria SUA
                \item ARIMA tradițional tratează asta ca outlier
                \item Avem nevoie de model care se \textbf{adaptează} la rupturi structurale
            \end{itemize}
        \end{alertblock}
        ...
        \begin{exampleblock}{Soluție}
            \textbf{Prophet} cu detectare automată a punctelor de schimbare.
        \end{exampleblock}
    \end{columns}
\end{frame}
...
\begin{frame}{VAR: Motivație}
    \begin{block}{Întrebarea de Cercetare}
        Cum modelăm \textbf{interdependențele dinamice} între mai multe variabile economice?
    \end{block}
    ...
    \begin{columns}[T]
        \column{0.5\textwidth}
        \textbf{Variabile (FRED)}
        \begin{itemize}
            \item Creștere PIB (YoY \%)
            \item Rata Șomajului (\%)
            \item Inflația (CPI YoY \%)
            \item Rata Fed Funds (\%)
        \end{itemize}
        ...
        \textbf{Date}
        \begin{itemize}
            \item Perioadă: 2000 -- 2025
            \item Frecvență: Trimestrială
            \item Observații: $\approx 100$ trimestre
        \end{itemize}

        \column{0.5\textwidth}
        \begin{exampleblock}{Relații Economice}
            \begin{itemize}
        ...
        \begin{alertblock}{De ce VAR?}
            Fiecare variabilă poate fi \textbf{atât cauză cât și efect} al celorlalte.
        \end{alertblock}
    \end{columns}
\end{frame}
...
\begin{frame}{Analiza Cauzalității Granger}
    \begin{defn}[Cauzalitate Granger]
        Variabila $X$ \textbf{cauzează Granger} $Y$ dacă valorile trecute ale lui $X$ ajută la predicția lui $Y$, dincolo de propriul trecut al lui $Y$.

        \vspace{0.2cm}
        \textit{Notă: Cauzalitatea Granger $\neq$ cauzalitate adevărată. Măsoară conținutul predictiv.}
    \end{defn}

    \vspace{0.3cm}

    \begin{center}
    \textbf{P-values Cauzalitate Granger (rând $\rightarrow$ coloană)}
    \begin{tabular}{lcccc}
        \toprule
        & \textbf{PIB} & \textbf{Șomaj} & \textbf{Inflație} & \textbf{Rata Fed} \\
        \midrule
        Creștere PIB & --- & \cellcolor{Forest!20}0,076 & 0,309 & 0,698 \\
        Șomaj & \cellcolor{Forest!20}\textbf{0,045} & --- & 0,093 & 0,857 \\
        Inflație & 0,545 & 0,665 & --- & 0,834 \\
        Rata Fed & 0,286 & 0,317 & \cellcolor{Forest!20}0,087 & --- \\
        \bottomrule
    \end{tabular}
    \end{center}
    ...
    \begin{exampleblock}{Concluzie Cheie}
        Șomajul cauzează Granger PIB ($p = 0,045$), consistent cu Legea Okun.
    \end{exampleblock}
\end{frame}

\begin{frame}{Funcții de Răspuns la Impuls}
    \begin{defn}[Funcția de Răspuns la Impuls]
        IRF urmărește efectul unui șoc de o unitate la variabila $j$ asupra variabilei $i$ pe $h$ perioade:
        \begin{equation}
    ...
    \begin{block}{Răspuns la Șocul PIB}
        Un șoc pozitiv la creșterea PIB:
        \begin{itemize}
            \item \textbf{Șomaj}: Scade (Legea Okun confirmată)
            \item \textbf{Inflație}: Crește cu întârziere (cerere-pull)
            \item \textbf{Rata Fed}: Crește după 2-3 trimestre (Regula Taylor)
        \end{itemize}
    \end{block}
    ...
    \begin{alertblock}{Interpretare Economică}
        VAR captează mecanismul clasic de transmisie macroeconomică de la output la ocupare, prețuri și politică monetară.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Cadrul de Selectare a Modelului}
    ...
    \begin{tikzpicture}[
        ...
        \node[decision, below left=1.5cm and 2cm of q1] (q2) {Rupturi\\structurale?};
        \node[decision, below right=1.5cm and 2cm of q1] (q3) {Date\\financiare?};
        ...
        \draw[arrow] (start) -- (q1);
        \draw[arrow] (q1) -- node[right] {Da} (var);
        \draw[arrow] (q1) -- node[above left] {Nu} (q2);
        \draw[arrow] (q2) -- node[left] {Da} (prophet);
        \draw[arrow] (q2) -- node[above right] {Nu} (q3);
        \draw[arrow] (q3) -- node[left] {Vol.} (garch);
        \draw[arrow] (q3) -- node[right] {Nivel} (arima);
    \end{tikzpicture}
    \end{center}
\end{frame}
...
\begin{frame}{Sumar: Comparație Modele}
    ...
    \begin{tabular}{lllll}
        \toprule
        \textbf{Studiu de Caz} & \textbf{Provocare} & \textbf{Model} & \textbf{Caracteristică} & \textbf{Test RMSE} \\
        \midrule
        Bitcoin & Clustering volatilitate & GARCH(1,1) & Prognoze rolling & 2,21 \\
        Pete solare & Sezonalitate lungă & ARIMA + Fourier & Termeni Sin/Cos & 48,51 \\
        Șomaj & Ruptură structurală & Prophet & Changepoints & 0,42 \\
        Economic & Serii multiple & VAR(2) & Dinamică încrucișată & 1,32 (med) \\
        \bottomrule
    \end{tabular}
    \end{center}
    ...
    \begin{exampleblock}{Principiu Cheie}
        \textbf{Potriviți modelul cu caracteristicile datelor.} Niciun model nu domină---alegeți în funcție de:
        \begin{itemize}
            \item Natura problemei de prognoză (nivel vs. volatilitate)
            \item Proprietățile datelor (sezonalitate, rupturi, serii multiple)
            \item Cerințele de interpretabilitate
        \end{itemize}
    \end{exampleblock}
\end{frame}
