...
\begin{frame}{Exercițiu AI 1: Audit descompunere}
    ...
    \begin{enumerate}\setlength{\itemsep}{1pt}
        \item Calculați $\frac{\text{amplitudine sezonieră}}{\text{nivel}}$ pentru 2015 vs 2023. Dacă raportul e aproximativ constant, ce tip de descompunere implică?
        \item Dacă $X_t = T_t \cdot S_t \cdot \varepsilon_t$, arătați că $\ln X_t = \underbrace{\ln T_t}_{\text{trend}} + \underbrace{\ln S_t}_{\text{sezon}} + \underbrace{\ln \varepsilon_t}_{\text{reziduu}}$ este aditivă.
        \item Aplicați STL pe $\ln X_t$. Cum verificați varianța constantă a reziduurilor? (Indiciu: Breusch--Pagan sau grafic $|\hat\varepsilon_t|$ vs $t$.)
        \item Care sunt consecințele folosirii STL aditiv pe date multiplicative pentru prognoză?
    \end{enumerate}
    }
\end{frame}
...
\end{verbatim}
    ...
    \vspace{-2mm}
    {\small
    \textbf{Găsiți trei erori critice}:
    \begin{enumerate}\setlength{\itemsep}{1pt}
        \item \texttt{center=True} înseamnă $\text{MA}_{12}(t)$ folosește $X_{t-5}, \ldots, X_{t+6}$ --- \textbf{șase valori viitoare} se scurg în caracteristică. Care este setarea corectă?
        \item MAPE e calculat pe \textbf{valorile ajustate in-sample}, nu pe prognoze out-of-sample. Dacă MAPE real OOS $= 8{,}3\%$, ce explică diferența de $7\times$?
        \item Model sezonier aditiv pe date cu amplitudine crescătoare. Ce grafic diagnostic relevă aceasta? (Indiciu: $|S_t|$ vs nivelul $T_t$.)
    \end{enumerate}

    \vspace{1mm}
    \textbf{Corectare}: rescrieți cu \texttt{center=False}, împărțiți 80/20, calculați MAPE pe test, și comparați cu benchmark-ul naiv sezonier $\hat{X}_{t+h} = X_{t+h-12}$.
    }
\end{frame}
...
\begin{frame}{Exercițiu AI 3: Expertiza parametrilor de netezire}
    ...
    \textbf{Cerințe}:
    \begin{enumerate}\setlength{\itemsep}{1pt}
        \item Arătați că SES cu $\alpha \to 1$ dă $\hat{X}_{t+1|t} = X_t$ (prognoză naivă). Ce aproximează $\alpha = 0{,}98$?
        \item RMSE pe test \textit{crește} cu complexitatea: $4{,}8 \to 6{,}3 \to 9{,}1$. Numiți acest fenomen. Care coloană RMSE trebuie folosită la selecția modelului?
        \item $\gamma = 0{,}01$: indicii sezonieri aproape nu se actualizează. Ce se întâmplă dacă tiparul sezonier se schimbă (ex.\ post-COVID)?
        \item Ce model ați selecta și de ce? Calculați AIC $= T\ln(\text{SSE}/T) + 2k$ pentru fiecare.
    \end{enumerate}
    }
\end{frame}
