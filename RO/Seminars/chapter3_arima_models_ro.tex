...
\begin{frame}{Aplicații Practice}
    ...
    \begin{alertblock}{Provocarea}
        Datele financiare și economice sunt de obicei \textbf{integrate} (I(1) sau aproape de rădăcină unitară):
        \begin{itemize}
            \item Prețuri de acțiuni: mers aleatoriu în logaritmi
            \item Cursuri de schimb: mers aleatoriu
            \item Rate ale dobânzii: foarte persistente (aproape de rădăcină unitară)
        \end{itemize}
    \end{alertblock}
    }
\end{frame}
...
\begin{frame}{Ce Vom Învăța Astăzi}
    ...
    \begin{exampleblock}{La Sfârșitul Acestui Curs}
        Veți fi capabili să modelați și să prognozăți serii de timp nestaționare precum prețurile acțiunilor, PIB și cursurile de schimb folosind modele ARIMA.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{De Ce Contează Nestaționaritatea}
    ...
    \begin{block}{Consecințele Nestaționarității}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Momentele din eșantion (medie, varianță, ACF) nu sunt eștimatori consistenți
...
\begin{frame}{Tipuri de Nestaționaritate}
    ...
    \begin{alertblock}{Distincție Cheie}
        Identificarea corectă este crucială: eliminărea trendului prin regresie pentru un proces cu rădăcină unitară sau diferențierea unui proces staționar în trend duc ambele la specificăre greșită!
    \end{alertblock}
\end{frame}
...
\begin{frame}{Mers Aleatoriu: Ilustrație Vizuală}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.58\textheight, keepaspectratio]{ch3_def_random_walk.pdf}
    \end{center}
    \vspace{-0.2cm}
    \begin{alertblock}{Proprietăți Cheie}
        Stânga: Traiectorii multiple rătăcesc imprevizibil fără revenire la medie. Dreapta: Varianța $\Var(Y_t) = t\sigma^2$ crește liniar --- caracteristica definitorie a nestaționarității.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Mers Aleatoriu cu Drift: Ilustrație Vizuală}
    \begin{center}
        \includegraphics[width=0.9\textwidth, height=0.58\textheight, keepaspectratio]{ch3_def_random_walk_drift.pdf}
    \end{center}
    \vspace{-0.2cm}
    \begin{exampleblock}{Comparație}
        Fără drift (albastru): rătăcește în jurul lui zero fără direcție. Cu drift $\mu > 0$ (roșu): trend ascendent sistematic. Ambele sunt nestaționare --- drift-ul adaugă trend determinist la rătăcirea stochastică.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Procese Integrate}
    ...
    \begin{exampleblock}{Cazuri Comune}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item $I(2)$: A două diferență este staționară (mai rar)
        \end{itemize}
    \end{exampleblock}
    \end{minipage}
    }
...
\begin{frame}{Proces Integrat: Ilustrație Vizuală}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.58\textheight, keepaspectratio]{ch3_def_integrated.pdf}
    \end{center}
    \vspace{-0.2cm}
    \begin{block}{Ordinul de Integrare}
        $I(0)$: Staționar --- nicio diferențiere necesară. $I(1)$: O diferență necesară (mers aleatoriu). $I(2)$: Două diferențe necesare. Majoritatea seriilor economice sunt $I(0)$ sau $I(1)$.
    \end{block}
\end{frame}
...
\begin{frame}{Diagnostic ACF: Detectarea Nestaționarității}
    ...
    {\footnotesize
    \begin{itemize}
        \item \textbf{Sus}: ACF mers aleatoriu scade foarte lent $\Rightarrow$ rădăcină unitară
...
\begin{frame}{ARIMA: Ilustrație Vizuală}
    \begin{center}
        \includegraphics[width=0.85\textwidth, height=0.55\textheight, keepaspectratio]{ch3_def_arima.pdf}
    \end{center}
    \vspace{-0.2cm}
    \begin{exampleblock}{Interpretare}
        Sus: seria ARIMA originală (nestaționară). Jos: după diferențiere de $d$ ori, ACF/PACF dezvăluie ordinele AR și MA pentru componenta staționară.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Testul Dickey-Fuller}
    ...
    \begin{block}{Ipoteze}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $H_0$: $\gamma = 0$ (rădăcină unitară, $\phi = 1$, nestaționar)
...
\begin{frame}{Testul Augmented Dickey-Fuller (ADF)}
    ...
    \begin{block}{Alegerea Lungimii Lag-ului $k$}
        \begin{itemize}
            ...
            \item Începeți cu $k_{max}$, reduceți până ultimul lag este semnificâțiv
        \end{itemize}
    \end{block}
    }
\end{frame}
...
\begin{frame}{Testul ADF: Ilustrație Vizuală}
    ...
    \end{center}
    \vspace{-0.2cm}
    \small Stânga: serie staționară -- ADF respinge rădăcina unitară. Dreapta: nestaționară -- ADF nu respinge.
\end{frame}
...
\begin{frame}{Valori Critice ADF}
    ...
    \begin{block}{Regula de Decizie}
        ...
        \begin{itemize}
            ...
            \item Statistică de test $\geq$ valoare critică $\Rightarrow$ Nu respingem (rădăcină unitară)
        \end{itemize}
        }
    \end{block}
\end{frame}
...
\begin{frame}{Pasul 1: Determinarea lui $d$}
    ...
    \begin{block}{Procedură}
        \begin{enumerate}\setlength{\itemsep}{0pt}
            \item Reprezentăți grafic seria de timp -- căutați trenduri, varianță în schimbare
            \item Examinați ACF -- descreștere lentă sugerează nestaționaritate
            \item Aplicați teste de rădăcină unitară (ADF, KPSS)
...
\begin{frame}{Algoritmi Auto-ARIMA}
    ...
    \begin{block}{Cum Funcționează Auto-ARIMA}
        \begin{enumerate}\setlength{\itemsep}{0pt}
            \item Folosește teste de rădăcină unitară pentru a determina $d$
...
\begin{frame}{Metode de Estimare}
    ...
    \begin{block}{Estimarea prin Maximum de Verosimilitate (MLE)}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Oferă eștimatori consistenți, eficienți
...
\begin{frame}{Restricții asupra Parametrilor}
    ...
    \begin{alertblock}{Staționaritate și Invertibilitate}
        Modelul ARIMA eștimat ar trebui să satisfacă:
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Staționaritate AR}: Rădăcinile lui $\phi(z) = 0$ în afără cercului unitate
            \item \textbf{Invertibilitate MA}: Rădăcinile lui $\theta(z) = 0$ în afără cercului unitate
        \end{itemize}
    \end{alertblock}
    ...
    \begin{block}{Verificare în Practică}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Coeficienți eștimați cu erori standard
...
\begin{frame}{Analiza Reziduurilor}
    ...
    \begin{block}{Instrumente de Diagnostic}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{ACF/PACF rezidual}: Nu ar trebui să arate vârfuri semnificâțive
...
\begin{frame}{Testul Ljung-Box}
    {\small
    \hfill\begin{minipage}{0.9\textwidth}
    \begin{defn}[Statistică Q Ljung-Box]
    ...
    \begin{block}{Utilizare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item Grade de libertate ajustate pentru parametrii eștimați
    ...
    \begin{alertblock}{Dacă Testul Eșuează}
        Luați în considerare adăugarea de termeni AR sau MA, sau verificăți pentru rupturi structurale.
    \end{alertblock}
    \end{minipage}
    }
\end{frame}
...
%=============================================================================
\section{Prognoză cu ARIMA}

\begin{frame}{Prognoze Punctuale}
    {\small
    \hfill\begin{minipage}{0.9\textwidth}
    \begin{block}{Prognoză cu MSE Minim}
    ...
    \begin{exampleblock}{Prognoză ARIMA(1,1,1)}
...
\begin{frame}{Prognoză Rulantă: Concept}
    {\small
    \hfill\begin{minipage}{0.9\textwidth}
    \begin{block}{Ce este Prognoză Rulantă?}
        O tehnică pentru evaluarea acurateții prognozei în afără eșantionului:
        \begin{enumerate}\setlength{\itemsep}{0pt}
            \item Fixăm o \textbf{fereastră de antrenament} de dimensiune $w$
            \item Eștimăm modelul pe observațiile $t = 1, \ldots, w$
...
for t în range(window_size, len(y) - 1):
...
%=============================================================================
\section{Studiu de Caz: Prognoză PIB SUA}
...
\begin{frame}{Studiu de Caz: Analiză ARIMA Completă}
    ...
    {\small
    \begin{enumerate}
        \item \textbf{Pasul 1}: Vizualizarea datelor și verificărea staționarității
        \item \textbf{Pasul 2}: Aplicarea testelor de rădăcină unitară (ADF, KPSS)
...
\begin{frame}{Pasul 3: Identificarea Modelului prin ACF/PACF}
    ...
    \begin{block}{Analiza Seriei Diferențiate}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item ACF: Spike semnificâțiv la lag 1, apoi se anulează $\Rightarrow$ sugerează MA(1)
            \item PACF: Spike semnificâțiv la lag 1, scade $\Rightarrow$ sugerează AR(1)
            \item \textbf{Modele cândidate}: ARIMA(1,1,0), ARIMA(0,1,1), ARIMA(1,1,1)
        \end{itemize}
    \end{block}
    }
\end{frame}
...
\begin{frame}{Pasul 4: Estimarea Modelului}
    \vspace{-0.2cm}
    {\small
    \begin{block}{Comparărea Modelelor folosind Criterii Informaționale}
        \begin{center}
        \begin{tabular}{lccc}
            \toprule
    ...
    \begin{exampleblock}{Model Selectat: ARIMA(1,1,1)}
        ...
        \vspace{-0.3cm}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\hat{\phi}_1 = 0.35$ (SE = 0.09), semnificâțiv la 1\%
            \item $\hat{\theta}_1 = 0.58$ (SE = 0.08), semnificâțiv la 1\%
        \end{itemize}
    \end{exampleblock}
    }
\end{frame}
...
\begin{frame}{Pasul 6: Prognoză cu Intervale de Încredere}
    \vspace{-0.2cm}
    {\small
    ...
    \begin{exampleblock}{Prognoză la 1 Pas (2024T3)}
        \vspace{-0.2cm}
        \begin{align*}
...
\begin{frame}{Rezultatele Testului de Rădăcină Unitate}
    ...
    {\small
    \begin{itemize}
        \item PIB în niveluri: Nu putem respinge rădăcina unitară (nestaționar)
        \item Creștere PIB: Respingem rădăcina unitară la nivel de 1\% (staționar)
    \end{itemize}
    }
\end{frame}
...
\begin{frame}{Prognoză ARIMA: Real vs Prezis}
    \vspace{-0.2cm}
    \begin{center}
...
\begin{frame}{Studiu de Caz: Indice PIB (Simulat)}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.58\textheight, keepaspectratio]{ch3_case_raw_data.pdf}
    \end{center}
    \vspace{-0.2cm}
    \begin{block}{Descrierea Datelor}
        \textbf{Indice PIB trimestrial simulat (1960--2010)}: Serie nestaționară cu trend ascendent. Demonstrează necesitătea diferențierii înainte de modelarea ARMA.
    \end{block}
\end{frame}

\begin{frame}{Pasul 1: Testul ADF pentru Staționaritate}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.55\textheight, keepaspectratio]{ch3_case_adf_test.pdf}
    \end{center}
    \vspace{-0.2cm}
    \begin{alertblock}{Rezultate Test ADF}
        \textbf{Seria originală}: p-value mare $\Rightarrow$ nu respingem $H_0$ (rădăcină unitară prezentă).
        \textbf{Prima diferență}: p-value $< 0.01$ $\Rightarrow$ respingem $H_0$ $\Rightarrow$ $d = 1$ este suficient.
    \end{alertblock}
\end{frame}

\begin{frame}{Pasul 2: ACF/PACF Înainte și După Diferențiere}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.62\textheight, keepaspectratio]{ch3_case_acf_diff.pdf}
    \end{center}
    \vspace{-0.2cm}
    \begin{exampleblock}{Identificare}
        \textbf{Sus}: ACF cu descreștere lentă $\Rightarrow$ nestaționaritate.
        \textbf{Jos}: După diferențiere, ACF și PACF sugerează ARMA de ordin mic.
    \end{exampleblock}
\end{frame}

\begin{frame}{Pasul 3: Comparărea Modelelor ARIMA}
    \begin{center}
    ...
    \begin{block}{Selecția Modelului}
        Comparăm ARIMA(0,1,0), ARIMA(1,1,0), ARIMA(0,1,1), ARIMA(1,1,1). Modelul cu cel mai mic AIC este selectat.
    \end{block}
\end{frame}

\begin{frame}{Pasul 4: Verificarea Diagnostică}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.65\textheight, keepaspectratio]{ch3_case_diagnostics.pdf}
    \end{center}
    \vspace{-0.2cm}
    \begin{exampleblock}{Diagnostice ARIMA(1,1,1)}
        Reziduurile sunt aproximativ zgomot alb: nicio autocorelație semnificâțivă, distribuție aproape normală.
    \end{exampleblock}
\end{frame}

\begin{frame}{Pasul 5: Prognoză}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.58\textheight, keepaspectratio]{ch3_case_forecast.pdf}
    \end{center}
    \vspace{-0.2cm}
    \begin{block}{Rezultate}
        \begin{itemize}
            \item Prognozele ARIMA(1,1,1) urmează trendul datelor
            \item Intervalele de încredere se lărgesc cu orizontul (caracteristic pentru I(1))
            \item Incertitudinea crește deoarece erorile se acumulează prin diferențiere
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{Concluzii Cheie}
    ...
    \begin{block}{Puncte Principale}
        \begin{enumerate}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Testele de rădăcină unitară} (ADF, KPSS) ajută la determinarea lui $d$
            \item \textbf{Metodologia Box-Jenkins}: Identificare $\to$ Estimare $\to$ Diagnosticare
...
\begin{frame}{Întrebarea Quiz 3}
    ...
    \begin{enumerate}[(A)]
        ...
        \item Nu respingem $H_0$: seria probabil are rădăcină unitară
        \item Testul este neconcludent
    \end{enumerate}
\end{frame}

\begin{frame}{Întrebarea Quiz 3: Răspuns}
    \begin{exampleblock}{Răspuns Corect: (C) Nu respingem $H_0$: seria are rădăcină unitară}
