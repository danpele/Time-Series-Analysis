...
\begin{frame}{Test 1: Ordinul de Integrare}
    ...
    \begin{exampleblock}{Răspuns: C -- $I(2)$}
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Definiție}: $Y_t \sim I(d)$ dacă $\Delta^d Y_t$ este staționară dar $\Delta^{d-1} Y_t$ nu este.
        \item \textbf{Exemplu}: Dacă $Y_t$ urmează $\Delta^2 Y_t = \varepsilon_t$, atunci:
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item $\Delta Y_t = \Delta Y_{t-1} + \varepsilon_t$ (încă are rădăcină unitară)
            \item $\Delta^2 Y_t = \varepsilon_t$ (zgomot alb, staționară)
        \end{itemize}
        \item \textbf{În practică}: Nivelurile prețurilor pot fi $I(2)$ când inflația însăși este nestaționară.
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Test 2: Proprietățile Mersului Aleatoriu}
    ...
    \begin{exampleblock}{Răspuns: B -- $t \cdot \sigma^2$}
        \vspace{-0.2cm}
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.22\textheight, keepaspectratio]{sem3_rw_variance.pdf}
        \end{center}
        \vspace{-0.2cm}
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Demonstrație}: $Y_t = \sum_{i=1}^{t}\varepsilon_i \Rightarrow \Var(Y_t) = t\sigma^2$ (crește liniar $\Rightarrow$ nestaționară)
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Test 3: Ipotezele Testului ADF}
    ...
    \begin{exampleblock}{Răspuns: B -- Seria are o rădăcină unitară}
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Regresia ADF}: $\Delta Y_t = \alpha + \gamma Y_{t-1} + \sum_{j=1}^{p}\delta_j \Delta Y_{t-j} + \varepsilon_t$
        \item \textbf{Ipoteze}: $H_0: \gamma = 0$ (rădăcină unitară, nestaționară) vs $H_1: \gamma < 0$ (staționară)
        \item \textbf{Decizie}: Respingem $H_0$ dacă statistica $t <$ valoarea critică (de ex., $-2.86$ la 5\%).
        Folosește distribuția specială Dickey-Fuller, nu $t$ standard.
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Test 4: Notația ARIMA}
    ...
    \begin{exampleblock}{Răspuns: A -- AR(2) pe date diferențiate cu erori MA(1)}
        ...
        \item \textbf{ARIMA(2,1,1) se expandează la}:
        $(1-\phi_1 L - \phi_2 L^2)(1-L)Y_t = (1+\theta_1 L)\varepsilon_t$,
        sau echivalent: $(1-\phi_1 L - \phi_2 L^2)\Delta Y_t = (1+\theta_1 L)\varepsilon_t$
        \item \textbf{Interpretare}: Mai întâi diferențiem seria, apoi ajustăm ARMA(2,1) pe $\Delta Y_t$.
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Vizual: Procesul ARIMA}
    ...
    \vspace{-0.3cm}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.75\textheight, keepaspectratio]{ch3_def_arima.pdf}
    \end{center}
    \vspace{-0.3cm}
...
\begin{frame}{Test 5: Operatorul de Diferență}
    ...
    \begin{exampleblock}{Răspuns: B -- $Y_t - 2Y_{t-1} + Y_{t-2}$}
        {\small
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Expandare}: $(1-L)^2 = 1 - 2L + L^2$
        \item \textbf{Aplicare}: $(1-L)^2 Y_t = Y_t - 2Y_{t-1} + Y_{t-2}$
        \item \textbf{Notă}: Aceasta este egală cu $\Delta(\Delta Y_t) = \Delta Y_t - \Delta Y_{t-1}$, ``schimbarea schimbărilor''.
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Test 6: KPSS vs ADF}
    ...
    \begin{exampleblock}{Răspuns: B -- Ipoteze nule inversate}
        \vspace{-0.2cm}
        \begin{center}
            \includegraphics[width=0.85\textwidth, height=0.22\textheight, keepaspectratio]{sem3_adf_kpss.pdf}
        \end{center}
        \vspace{-0.2cm}
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Strategie}: Folosiți ambele teste împreună pentru inferență robustă.
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
    \quantlet{TSA\_ch3\_adf\_test}{https://github.com/QuantLet/TSA/tree/main/TSA_ch3/TSA_ch3_adf_test}
\end{frame}
...
\begin{frame}{Test 7: Supradiferențierea}
    ...
    \begin{exampleblock}{Răspuns: B -- Autocorelație negativă artificială}
        \vspace{-0.2cm}
        \begin{center}
            \includegraphics[width=0.95\textwidth, height=0.22\textheight, keepaspectratio]{sem3_overdifferencing.pdf}
        \end{center}
        \vspace{-0.2cm}
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Diagnostic}: ACF la lag 1 $\approx -0.5$ semnalează supradiferențiere. Reduceți $d$ cu 1.
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Test 8: Varianța Prognozei}
    ...
    \begin{exampleblock}{Răspuns: C -- Crește liniar cu $h$}
        {\small
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Prognoză}: $\hat{Y}_{T+h|T} = Y_T$. \textbf{Eroare}: $Y_{T+h} - \hat{Y}_{T+h|T} = \sum_{i=1}^{h} \varepsilon_{T+i}$
        \item \textbf{Varianță}: $\Var(Y_{T+h} - \hat{Y}_{T+h|T}) = h\sigma^2$
        \item \textbf{IC 95\%}: $Y_T \pm 1.96\sqrt{h}\sigma$ (se lărgește cu $\sqrt{h}$)
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Test 9: Puterea Testului de Rădăcină Unitară}
    ...
    \begin{exampleblock}{Răspuns: B -- Rădăcina aproape dar nu egală cu 1}
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Exemplu}: AR(1) cu $\phi = 0.95$ vs mers aleatoriu ($\phi = 1$)
        \item \textbf{Problemă}: Ambele au modele ACF similare (descreștere lentă), dar una este staționară.
        \item \textbf{Putere scăzută}: Probabilitate mare de eroare de tip II (eșec în respingerea lui $H_0$ fals).
        \item \textbf{Soluții}: dimensiuni mai mari ale eșantionului, testul Phillips-Perron (robust la heteroscedasticitate), teste de rădăcină unitară pe paneluri (serii multiple).
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Test 10: Selecția Modelului ARIMA}
    ...
    \begin{exampleblock}{Răspuns: B -- ARIMA(0,1,1)}
        \vspace{-0.2cm}
        \begin{center}
            \includegraphics[width=0.95\textwidth, height=0.18\textheight, keepaspectratio]{sem3_arima_flowchart.pdf}
        \end{center}
        \vspace{-0.3cm}
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Model}: ACF se anulează la lag 1, PACF descrește $\Rightarrow$ MA(1) pentru seria diferențiată. Model complet: ARIMA(0,1,1) = IMA(1,1)
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Test 11: Staționaritate în Trend vs Staționaritate în Diferențe}
    ...
    \begin{exampleblock}{Răspuns: B -- Eliminarea trendului determinist prin regresie}
        \vspace{-0.2cm}
        \begin{center}
            \includegraphics[width=0.95\textwidth, height=0.18\textheight, keepaspectratio]{sem3_trend_vs_diff.pdf}
        \end{center}
        \vspace{-0.3cm}
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Staționar în trend}: Eliminarea trendului prin regresie (șocurile sunt temporare). \textbf{Staționar în diferențe}: Diferențiere (șocurile sunt permanente). Tratamentul greșit afectează modelul.
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Test 12: Invertibilitatea ARIMA}
    ...
    \begin{exampleblock}{Răspuns: C -- Nestaționară și neinvertibilă}
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Staționaritate}: $d=1$ înseamnă o rădăcină unitară $\Rightarrow$ \textcolor{Crimson}{Nestaționară}
        \item \textbf{Invertibilitate}: Polinomul MA este $\theta(z) = 1 + 1.2z$; rădăcină: $z = -1/1.2 = -0.833$ (în interiorul cercului unitate). Invertibilitatea necesită rădăcina în afara cercului unitate, iar $|\theta_1| = 1.2 > 1$ $\Rightarrow$ \textcolor{Crimson}{Neinvertibilă}.
        \item \textbf{Corecție}: Rescrieți cu $\theta^* = 1/1.2 = 0.833$ și ajustați varianța.
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Test 13: Regresia Falsă}
    ...
    \begin{exampleblock}{Răspuns: B -- $R^2$ ridicat și statistici t semnificative (fals)}
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item \textbf{Granger \& Newbold (1974)}: Fenomenul regresiei false
        \item \textbf{Simptome}: $R^2$ ridicat (adesea $>$ 0.9) între serii neînrudite, statistici $t$ semnificative, statistică Durbin-Watson foarte scăzută ($\ll 2$), reziduuri nestaționare.
        \item \textbf{Soluții}: (1) Diferențiați ambele serii, sau (2) Testați pentru cointegrare
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Test 14: Prognoza pe Termen Lung}
    ...
    \begin{exampleblock}{Răspuns: C -- O extrapolare liniară a trendului}
        ...
        \item \textbf{Prognoza pe termen lung}: Pentru modelele I(1) cu derivă $c$:
        $\hat{Y}_{T+h} \approx Y_T + h \cdot \frac{c}{1-\phi_1}$
        \item \textbf{Diferențe cheie}: ARMA staționară: prognozele $\to$ media necondiționată;
        I(1) fără derivă: prognozele $\to$ ultima valoare (plată);
        I(1) cu derivă: prognozele $\to$ extrapolare liniară.
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Problema 2: Identificarea Modelului}
    ...
    \begin{exampleblock}{Soluție}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Model sugerat: ARIMA(0,1,1) sau IMA(1,1)}
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Problema 3: Ecuația ARIMA}
    ...
    \begin{exampleblock}{Soluție}
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item Expandând $(1-\phi_1 L)(1-L) = 1 - L - \phi_1 L + \phi_1 L^2 = 1 - (1+\phi_1)L + \phi_1 L^2$:
...
\begin{frame}{Problema 5: Intervale de Încredere}
    ...
    \begin{exampleblock}{Soluție}
        \begin{itemize}\setlength{\itemsep}{0pt}
        \item Pentru IMA(1,1), ponderile MA($\infty$) sunt $\psi_0 = 1$, $\psi_j = 1 + \theta_1$ pentru $j \geq 1$.
        \item \textbf{1 pas:} $\Var(e_{T+1}) = \sigma^2 \psi_0^2 = 4$, deci $SE = 2$

        $100.6 \pm 1.96(2) = \mathbf{[96.68, 104.52]}$
        \item \textbf{2 pași:} $\Var(e_{T+2}) = \sigma^2(\psi_0^2 + \psi_1^2) = 4(1 + 1.3^2) = 10.76$, $SE = 3.28$
...
\begin{frame}{Exercițiu AI: Gândire critică}
    ...
    \begin{block}{\footnotesize Prompt de testat în ChatGPT / Claude / Copilot}
        {\footnotesize
        ``Folosind yfinance, descarcă cursul de schimb lunar EUR/USD (EURUSD=X) din 2010-01 până în 2024-12 (180 observații). Testează prezența rădăcinii unitare cu ADF și KPSS. Determină ordinul de diferențiere $d$. Estimează un model ARIMA$(p,d,q)$ folosind atât analiza ACF/PACF, cât și auto\_arima. Compară cele două abordări. Împarte datele în antrenare (2010--2023) și test (2024) și evaluează prognoza pe 12 luni cu RMSE și MAE. Vreau cod Python complet cu grafice.''
        }
    \end{block}
    \vspace{-2mm}
    {\footnotesize
    ...
    \begin{enumerate}\setlength{\itemsep}{0pt}
        ...
        \item Folosește ambele teste (ADF și KPSS) pentru a decide $d$? Verifică supra-diferențierea?
        \item Rezultatele ACF/PACF coincid cu ordinele alese de auto\_arima? Dacă nu, de ce?
        \item Intervalele de încredere ale prognozei se lărgesc cu orizontul? (proprietate cheie I(1))
        \item Menționează ipoteza de mers aleatoriu (random walk) pentru cursul de schimb?
    \end{enumerate}
    }
    \vspace{-2mm}
    \begin{alertblock}{}
...
\begin{frame}{Puncte Cheie din Seminarul de Astăzi}
    ...
    \begin{exampleblock}{Următorul Seminar}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Exerciții practice Python cu date economice reale:
            \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Bibliografie II}
    ...
    \begin{exampleblock}{Resurse Online și Cod}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{GitHub TSA}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch3} --- Cod Python pentru acest seminar
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
