...
\begin{frame}{Test 3: Specificarea Testului ADF}
    \begin{cminipage}{0.95\textwidth}
    \begin{alertblock}{Întrebare}
        Când aplicați testul ADF pe datele PIB (care prezintă un trend ascendent clar), ce specificare trebuie folosită?
    \end{alertblock}
    \vspace{0.1cm}

    \only<1>{
    \begin{block}{Variante de răspuns}
        \textcolor{MainBlue}{\textbf{(A)}} Fără constantă, fără trend\\[3pt]
        \textcolor{MainBlue}{\textbf{(B)}} Cu constantă, fără trend\\[3pt]
        \textcolor{MainBlue}{\textbf{(C)}} Cu constantă și trend\\[3pt]
        \textcolor{MainBlue}{\textbf{(D)}} Nu contează specificarea
    \end{block}
    }
    \only<2>{
    \begin{exampleblock}{Răspuns: C -- Cu constantă și trend}
        \textbf{Regresia ADF cu trend}: $\Delta Y_t = \alpha + \beta t + \gamma Y_{t-1} + \sum \delta_j \Delta Y_{t-j} + \varepsilon_t$

        \textbf{Regula practică}:
        \begin{itemize}
            \item \textbf{Fără constantă}: serii cu medie zero (rar utilizat)
            \item \textbf{Cu constantă}: serii cu medie nenulă dar fără trend vizibil
            \item \textbf{Cu constantă + trend}: serii cu trend determinist vizibil (PIB, prețuri)
        \end{itemize}

        \textbf{Atenție}: Specificarea greșită reduce puterea testului!
    \end{exampleblock}
    }
    \end{cminipage}
\end{frame}
...
\begin{frame}{Test 5: Echivalența ARIMA}
    \begin{cminipage}{0.95\textwidth}
    \begin{alertblock}{Întrebare}
        Modelul ARIMA(0,1,1) fără constantă, $(1-L)Y_t = (1+\theta L)\varepsilon_t$, este echivalent cu:
    \end{alertblock}
    \vspace{0.1cm}

    \only<1>{
    \begin{block}{Variante de răspuns}
        \textcolor{MainBlue}{\textbf{(A)}} O netezire exponențială simplă (SES)\\[3pt]
        \textcolor{MainBlue}{\textbf{(B)}} Un model AR(1) staționar\\[3pt]
        \textcolor{MainBlue}{\textbf{(C)}} Un mers aleatoriu pur\\[3pt]
        \textcolor{MainBlue}{\textbf{(D)}} Un model MA(1) staționar
    \end{block}
    }
    \only<2>{
    \begin{exampleblock}{Răspuns: A -- O netezire exponențială simplă (SES)}
        \textbf{ARIMA(0,1,1)}: $Y_t = Y_{t-1} + \varepsilon_t + \theta\varepsilon_{t-1}$

        \textbf{SES}: $\hat{Y}_{t+1} = \alpha Y_t + (1-\alpha)\hat{Y}_t$ cu $\alpha = 1 + \theta$

        \begin{itemize}
            \item Când $\theta = 0$: mers aleatoriu pur (naive)
            \item Când $-1 < \theta < 0$: netezire ($0 < \alpha < 1$)
            \item Legătura fundamentală între abordarea stochastică și cea deterministă
        \end{itemize}

        \textbf{Concluzie}: SES este cazul optim al unui ARIMA(0,1,1)!
    \end{exampleblock}
    }
    \end{cminipage}
\end{frame}

\begin{frame}{Test 6: Matricea de Decizie ADF + KPSS}
    \begin{cminipage}{0.95\textwidth}
    \begin{alertblock}{Întrebare}
        ADF nu respinge $H_0$ (p = 0.15) și KPSS nu respinge $H_0$ (p = 0.08). Ce concluzie rezultă?
    \end{alertblock}
    \vspace{0.1cm}
    ...
    \begin{block}{Variante de răspuns}
        ...
        \textcolor{MainBlue}{\textbf{(B)}} Seria are o rădăcină unitară\\[3pt]
        \textcolor{MainBlue}{\textbf{(C)}} Rezultatele sunt neconcludente --- putere statistică insuficientă\\[3pt]
        \textcolor{MainBlue}{\textbf{(D)}} Ambele teste sunt greșite
    \end{block}
    }
    \only<2>{
    \begin{exampleblock}{Răspuns: C -- Rezultate neconcludente}
        \vspace{-0.2cm}
        \begin{center}
        \small
        \begin{tabular}{l|cc}
            & \textbf{ADF nu resp.} & \textbf{ADF resp.} \\
            \hline
            \textbf{KPSS nu resp.} & \textcolor{Orange}{\textbf{Neconcludent}} & \textcolor{Forest}{Staționară} \\
            \textbf{KPSS resp.} & \textcolor{Crimson}{Rădăcină unitară} & \textcolor{Orange}{\textbf{Neconcludent}} \\
        \end{tabular}
        \end{center}
        \vspace{-0.1cm}
        {\footnotesize
        \textbf{Soluții}: Eșantion mai mare, teste PP sau ERS, sau procedura secvențială --- diferențiați și retestați.
        }
    \end{exampleblock}
    }
...
\begin{frame}{Adevărat/Fals: Soluții}
    ...
    \begin{enumerate}\setlength{\itemsep}{0pt}
        ...
        \\ \textcolor{MediumGray}{$d$ diferențe pt. I($d$). I(2) = două rădăcini unitare.}
        \item ADF include întotdeauna un termen constant. \hfill \textcolor{Crimson}{\textbf{FALS}}
        \\ \textcolor{MediumGray}{Alegeți: fără constantă, doar constantă, sau constantă + trend.}
        \item ARIMA(0,1,0) = mers aleator. \hfill \textcolor{Forest}{\textbf{ADEVĂRAT}}
        \\ \textcolor{MediumGray}{$(1-L)Y_t = \varepsilon_t \Rightarrow Y_t = Y_{t-1} + \varepsilon_t$.}
        \item Diferențierea unei serii staționare $\to$ ``mai staționară.'' \hfill \textcolor{Crimson}{\textbf{FALS}}
        \\ \textcolor{MediumGray}{Supradiferențierea creează MA neinvertibil.}
        \item KPSS: $H_0$ = staționară. \hfill \textcolor{Forest}{\textbf{ADEVĂRAT}}
        \\ \textcolor{MediumGray}{Opus testului ADF ($H_0$ = rădăcină unitară).}
        \item ARIMA captează doar modele liniare. \hfill \textcolor{Forest}{\textbf{ADEVĂRAT}}
        \\ \textcolor{MediumGray}{Liniar în parametri. Neliniare $\to$ GARCH, rețele neuronale.}
    \end{enumerate}
    }
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}[shrink=5]{Exercițiu AI 2: Rafinarea Prompturilor pentru ARIMA}
    \begin{cminipage}{0.95\textwidth}
    \begin{block}{Sarcină}
...
\begin{frame}{Bibliografie II}
    ...
    \begin{exampleblock}{Resurse online și cod}
        ...
        \begin{itemize}
            ...
            \item \textbf{GitHub TSA}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch3} --- Cod Python pentru fiecare capitol
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
