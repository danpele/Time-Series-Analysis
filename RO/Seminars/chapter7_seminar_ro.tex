...
\input{preamble}
\subtitle{Seminar 7: Cointegrare \& VECM}
...
\begin{frame}{Test 1: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: C -- O combinație liniară este I(0)}
        ...
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Cheie}: $Y_t - \beta X_t \sim I(0)$ înseamnă că seriile au un trend stochastic comun. Combinația liniară (spread-ul) este staționară chiar dacă ambele serii sunt nestaționare.
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Test 2: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: B -- $R^2$ mare și coeficienți semnificativi (fals!)}
        ...
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Granger-Newbold (1974)}: Regresarea seriilor I(1) nerelaționate dă rezultate înșelătoare. Regulă: Dacă $R^2 > DW$, suspectați regresie falsă!
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Test 3: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: B -- Dacă reziduurile au rădăcină unitară}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Pasul 1}: Rulăm OLS: $Y_t = \alpha + \beta X_t + e_t$, salvăm reziduurile $\hat{e}_t$
            \item \textbf{Pasul 2}: Test ADF pe reziduuri: $\Delta \hat{e}_t = \rho \hat{e}_{t-1} + \ldots$
            \begin{itemize}\setlength{\itemsep}{0pt}
                \item $H_0$: $\rho = 0$ (rădăcină unitară $\Rightarrow$ fără cointegrare)
                \item $H_1$: $\rho < 0$ (staționar $\Rightarrow$ cointegrare!)
            \end{itemize}
            \item \textbf{Important}: Folosiți valorile critice Engle-Granger, nu ADF standard!
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Test 4: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: B -- Poate detecta relații de cointegrare multiple}
        \begin{center}
            \includegraphics[width=0.85\textwidth, height=0.50\textheight, keepaspectratio]{ch6_quiz4_johansen.pdf}
        \end{center}
        \vspace{-3mm}
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Avantaje Johansen}:
            \item Testează pentru $r = 0, 1, 2, \ldots, k-1$ vectori de cointegrare
            \item Verosimilitate maximă (mai eficient); nu necesită alegerea variabilei dependente
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Test 5: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: C -- Două relații de cointegrare}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Interpretarea rangului} pentru $k$ variabile:
            \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Test 6: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: B -- Coeficienții de ajustare (încărcare)}
        \begin{center}
            \includegraphics[width=0.85\textwidth, height=0.48\textheight, keepaspectratio]{ch6_quiz6_adjustment.pdf}
        \end{center}
        \vspace{-0.2cm}
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
...
\begin{frame}{Test 7: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: A -- $Y$ deasupra echilibrului; scade dacă $\alpha < 0$}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Mecanismul de corecție a erorilor}:
        \end{itemize}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{Convenție de semn}: $\alpha$ ar trebui să fie negativ pentru ca variabila dependentă să se miște înapoi spre echilibru.
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
    \quantlet{TSA\_ch7\_error\_correction}{https://github.com/QuantLet/TSA/tree/main/TSA_ch7/TSA_ch7_error_correction}
...
\begin{frame}{Test 8: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: B -- Variabila 2 este slab exogenă}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Exogenitate slabă}: Variabila nu răspunde la dezechilibru.
            \item \textbf{Exemplu: Ratele dobânzii}
            \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \end{itemize}
            \item \textbf{Implicație}: Putem estima o singură ecuație pentru variabila care se ajustează.
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Test 9: Răspuns}
    \begin{cminipage}{0.95\textwidth}
    \begin{exampleblock}{Răspuns: B -- Există cel mult un vector de cointegrare}
        {\footnotesize
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Notă}: (B) descrie ipoteza nulă $H_0: r \leq 1$, ceea ce este ceea ce testul testează. (C) descrie ipoteza alternativă $H_1: r > 1$, nu ceea ce testul ``testează dacă''.
        \end{itemize}
        }

        \vspace{0.2cm}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Procedura de testare secvențială}:
        \end{itemize}
        ...
        \vspace{0.2cm}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Statistica trace}:
        \end{itemize}
        $$\lambda_{\text{trace}}(r) = -T \sum_{i=r+1}^{k} \ln(1 - \hat{\lambda}_i)$$

        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Respingem $H_0$ dacă statistica trace $>$ valoarea critică.
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
    \quantlet{TSA\_ch7\_johansen\_eigenvalues}{https://github.com/QuantLet/TSA/tree/main/TSA_ch7/TSA_ch7_johansen_eigenvalues}
...
\begin{frame}{Test 10: Răspuns}
    ...
    \begin{exampleblock}{Răspuns: C -- Pierde informația pe termen lung}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Teorema Reprezentării Granger}: Dacă există cointegrare, reprezentarea VECM există și trebuie folosită.
        \end{itemize}
        ...
        \vspace{0.2cm}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Concluzie}: Diferențierea elimină relația pe termen lung pe care o reprezintă cointegrarea!
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Adevărat/Fals: Soluții}
    ...
    \begin{enumerate}\setlength{\itemsep}{1pt}
        ...
        {\footnotesize \textcolor{MediumGray}{Cazul standard CI(1,1): toate variabilele I(1), combinația liniară I(0). (Acesta este cazul standard CI(1,1). Mai general, cointegrarea poate exista între variabile cu ordine superioare de integrare, dar CI(1,1) este de departe cel mai frecvent în practică.)}}
...
\begin{frame}{Problema 1: Soluție}
    ...
    \begin{exampleblock}{Soluție: Da, sunt cointegrate}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Test}: $H_0$: Fără cointegrare (reziduurile au rădăcină unitară)
            \item \textbf{Statistica ADF}: $-3.92$
            \item \textbf{Valoarea critică (5\%)}: $-3.34$
            \item Deoarece $-3.92 < -3.34$, \textbf{respingem} $H_0$ la nivelul de 5\%.
            \item \textbf{Concluzie}: Reziduurile sunt staționare $\Rightarrow$ Există cointegrare!
            \item \textbf{Interpretare}: Consumul și venitul au un trend comun. Vectorul de cointegrare este aproximativ $(1, -0.85)$, consistent cu ipoteza venitului permanent.
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
    \quantlet{TSA\_ch7\_case\_cointegration}{https://github.com/QuantLet/TSA/tree/main/TSA_ch7/TSA_ch7_case_cointegration}
...
\begin{frame}{Problema 2: Soluție}
    ...
    \begin{exampleblock}{Soluție}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Termenul de corecție a erorilor}: $(r_{t-1} - R_{t-1})$ = spread-ul
            \item \textbf{Rata pe termen scurt} ($\alpha_r = -0.25$):
            \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \end{itemize}
            \item \textbf{Rata pe termen lung} ($\alpha_R = -0.02$):
            \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \end{itemize}
            \item \textbf{Interpretare economică}: Banca centrală (rata scurtă) se ajustează pentru a menține curba randamentelor.
        \end{itemize}
    \end{exampleblock}
    \end{cminipage}
...
\begin{frame}{Problema 3: Soluție}
    ...
    \begin{exampleblock}{Soluție: Rangul = 2}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Testare secvențială}:
        \end{itemize}
        \begin{enumerate}\setlength{\itemsep}{0pt}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{Concluzie}: $r = 2$ relații de cointegrare
...
\begin{frame}{Discuție: Cointegrare vs Corelație}
    ...
    \begin{exampleblock}{Exemplu}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Două mersuri aleatoare independente pot avea corelație $> 0.9$ pur întâmplător (corelație falsă). Dar NU sunt cointegrate---spread-ul lor este tot I(1).
            \item \textbf{Cointegrarea} implică o relație de echilibru pe termen lung semnificativă.
        \end{itemize}
    \end{exampleblock}
    }
    \end{cminipage}
...
\begin{frame}{Discuție: Alegerea Componentelor Deterministe}
    ...
    \begin{exampleblock}{Sfat Practic}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Începeți cu Cazul 3 (constantă nerestricționată). Verificați sensibilitatea la specificație. Folosiți raționament economic: au nivelurile trenduri?
        \end{itemize}
    \end{exampleblock}
    }
    \end{cminipage}
...
%=============================================================================
\begin{frame}{Exercițiu de Replicare: Johansen (1991)}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Obiectiv}
            Replicați analiza de cointegrare din Johansen (1991) pe date macroeconomice românești.
        \end{block}
        \begin{enumerate}\setlength{\itemsep}{2pt}
            {\small
            \item Descărcați date trimestriale (BNR/INS, 2005--2024): PIB real, consum privat, investiții
            \item Aplicați logaritmul natural tuturor seriilor
            \item Testați stationaritatea fiecărei serii (ADF, KPSS)
            \item Selectați lag-ul VAR optim (AIC, BIC) pe seriile în nivel
            \item Aplicați testul Johansen (\texttt{trace} și $\lambda_{\max}$) pentru rangul de cointegrare $r$
            \item Estimați VECM($r$) și interpretați vectorii de cointegrare $\hat{\boldsymbol{\beta}}$
            \item \textbf{Întrebare}: Există o relație de echilibru pe termen lung între consum, investiții și PIB în România?
            }
        \end{enumerate}
        {\tiny Ref: Johansen, S.\ (1991). Estimation and Hypothesis Testing of Cointegration Vectors in Gaussian VAR Models, \textit{Econometrica}, 59(6), 1551--1580.}
    \end{cminipage}
    \quantlet{TSA\_ch7\_case\_cointegration}{https://github.com/QuantLet/TSA/tree/main/TSA_ch7/TSA_ch7_case_cointegration}
\end{frame}

\begin{frame}{Exercițiu Monte Carlo: Dimensiunea Testului Johansen}
    \begin{cminipage}{0.95\textwidth}
        \begin{block}{Obiectiv}
            Evaluați dimensiunea empirică a testului Johansen trace pentru diferite $T$ și $K$.
        \end{block}
        \begin{enumerate}\setlength{\itemsep}{2pt}
            {\small
            \item Generați $B = 5000$ sisteme VAR(1) \textbf{fără cointegrare} ($r = 0$):
            \item[] $\Delta \mathbf{Y}_t = \boldsymbol{\varepsilon}_t$, $\boldsymbol{\varepsilon}_t \sim N(\mathbf{0}, \mathbf{I}_K)$  ($K$ mersuri aleatorii independente)
            \item Variați: $K \in \{2, 3, 5\}$ și $T \in \{50, 100, 200, 500\}$
            \item Pentru fiecare combinație, aplicați testul trace Johansen cu $H_0: r = 0$ la $\alpha = 5\%$
            \item Calculați rata empirică de respingere (ar trebui $\approx 5\%$)
            \item \textbf{Analizați}: (a) Testul este over-sized pentru $K = 5, T = 50$?
            \item[] (b) Cum se comportă dimensiunea pe măsură ce $T$ crește?
            \item[] (c) Repetați cu $r = 1$ (un vector de cointegrare) și evaluați puterea
            }
        \end{enumerate}
        {\footnotesize \textbf{Cod}: \texttt{from statsmodels.tsa.vector\_ar.vecm import coint\_johansen}}
    \end{cminipage}
    \quantlet{TSA\_ch7\_johansen\_eigenvalues}{https://github.com/QuantLet/TSA/tree/main/TSA_ch7/TSA_ch7_johansen_eigenvalues}
\end{frame}
%=============================================================================
...
\begin{frame}{Bibliografie II}
    ...
    \begin{exampleblock}{Resurse Online și Cod}
        ...
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item \textbf{GitHub TSA}: \url{https://github.com/QuantLet/TSA/tree/main/TSA_ch7} --- Cod Python pentru acest seminar
        \end{itemize}
        }
    \end{exampleblock}
    \end{cminipage}
