...
\begin{frame}{Întrebarea 11}
    ...
    \begin{block}{Ce descompune modelul CGARCH (Component GARCH)?}
        \begin{enumerate}[(A)]\setlength{\itemsep}{0pt}
            \item Randamentul în componentă sistematică și nesistematică
            \item Varianța condiționată în componentă permanentă și tranzitorie
            \item Distribuția inovațiilor în cozi stângi și drepte
            \item Volatilitatea în componentă simetrică și asimetrică
        \end{enumerate}
    \end{block}
...
\begin{frame}{Întrebarea 12}
    ...
    \begin{block}{În modelul FIGARCH(1,$d$,1), parametrul $d$ controlează:}
        \begin{enumerate}[(A)]\setlength{\itemsep}{0pt}
            \item Gradul de asimetrie a volatilității
            \item Numărul de lag-uri în ecuația varianței
            \item Viteza de descreștere a memoriei (integrare fracționară)
            \item Distribuția inovațiilor
        \end{enumerate}
    \end{block}
...
\begin{frame}{Problema 5: EWMA vs GARCH --- Persistența și Prognoza}
    ...
    \begin{block}{Enunț}
        ...
        Calculați:
        \begin{enumerate}[(a)]\setlength{\itemsep}{0pt}
            \item Prognoza $\sigma_{T+1}^2$ pentru fiecare model
            \item Varianța necondiționată $\bar{\sigma}^2$ pentru GARCH(1,1). De ce nu există pentru EWMA?
            \item Prognoza pe termen lung ($h \to \infty$) pentru fiecare model
        \end{enumerate}
    \end{block}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Soluție Problema 5}
    ...
    \begin{exampleblock}{Răspunsuri}
        \begin{enumerate}[(a)]\setlength{\itemsep}{0pt}
            ...
            \item $\bar{\sigma}^2_{\text{GARCH}} = \dfrac{0.000001}{1 - 0.05 - 0.93} = \dfrac{0.000001}{0.02} = 0.00005$
...
\begin{frame}{Problema 6: Comparare Modele Avansate}
    ...
    \begin{block}{Enunț}
        ...
        \vspace{0.3cm}
        \begin{enumerate}[(a)]\setlength{\itemsep}{0pt}
            \item Care model are cel mai bun AIC? Ce semnifică?
            \item Ce ne spune $\hat{\delta} = 1.3$ în APARCH?
            \item Ce ne spune $\hat{d} = 0.42$ în FIGARCH?
            \item Ce model ați recomanda și de ce?
        \end{enumerate}
    \end{block}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Soluție Problema 6}
    ...
    \begin{exampleblock}{Răspunsuri}
        \begin{enumerate}[(a)]\setlength{\itemsep}{0pt}
            \item \textbf{APARCH} are cel mai mic AIC ($-5{,}485$) $\Rightarrow$ cel mai bun compromis complexitate/fit.

            \vspace{0.15cm}

            \item $\hat{\delta} = 1.3$: puterea optimă este între $\sigma_t$ ($\delta=1$) și $\sigma_t^2$ ($\delta=2$). Piața nu se comportă exact ca GARCH ($\delta=2$) sau TGARCH ($\delta=1$) --- datele preferă o transformare intermediară.

            \vspace{0.15cm}

            \item $\hat{d} = 0.42$: memorie lungă moderată. ACF a $r_t^2$ decade hiperbolic, nu exponențial. Persistența nu este infinită (ca IGARCH), ci lentă --- șocurile vechi contează, dar tot mai puțin.

            \vspace{0.15cm}

            \item \textbf{Recomandare}: APARCH sau GJR-GARCH. APARCH are AIC-ul cel mai bun și captează asimetria + puterea optimă. Dacă simplitatea contează, GJR-GARCH este o alternativă bună (AIC apropiat, mai ușor de interpretat).
        \end{enumerate}
    \end{exampleblock}
    \end{cminipage}
    \quantlet{TSA\_ch5\_aparch\_sim}{https://github.com/QuantLet/TSA/tree/main/TSA_ch5/TSA_ch5_aparch_sim}
...
\begin{frame}{Rezumat Seminar}
    ...
    \begin{block}{Concepte Cheie}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item \textbf{ARCH/GARCH}: varianța condiționată depinde de șocuri și lag-uri ale varianței
            \item \textbf{EGARCH/GJR/TGARCH}: captează leverage effect (asimetrie)
            \item \textbf{IGARCH/EWMA}: persistență infinită, fără mean-reversion
            \item \textbf{CGARCH}: componentă permanentă + tranzitorie
            \item \textbf{FIGARCH}: memorie lungă ($0 < d < 1$) --- între GARCH și IGARCH
            \item \textbf{APARCH}: model unificator ($\delta$, $\gamma$ estimați din date)
        \end{itemize}
    \end{block}
    ...
    \begin{block}{Formule Importante}
        \begin{itemize}\setlength{\itemsep}{0pt}
            ...
            \item EWMA: $\sigma_t^2 = \lambda \sigma_{t-1}^2 + (1-\lambda) r_{t-1}^2$
    ...
    \begin{alertblock}{Sfat Practic}
        \begin{itemize}\setlength{\itemsep}{0pt}
            \item Folosiți distribuția Student-t. Verificați absența efectelor ARCH în reziduuri!
        \end{itemize}
    \end{alertblock}
    \end{cminipage}
\end{frame}
...
\begin{frame}{Exerciții pentru Acasă}
    ...
    \end{block}
    \begin{block}{Exercițiul 5}
        Pentru S\&P 500, estimați GARCH(1,1), GJR-GARCH, APARCH și comparați AIC. Ce valoare estimează APARCH pentru $\delta$? Este semnificativ diferit de 2?
    \end{block}
    }
    \end{cminipage}
