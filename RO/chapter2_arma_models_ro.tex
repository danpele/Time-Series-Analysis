...
\begin{frame}{Condiția de Staționaritate AR(1)}
    ...
    \textbf{Intuiție:}
    \begin{itemize}
        \item Dacă $|\phi| < 1$: șocurile se diminuează în timp $\succ$ staționar
        \item Dacă $|\phi| = 1$: mers aleatoriu $\succ$ nestaționar (rădăcină unitară)
        \item Dacă $|\phi| > 1$: proces exploziv $\succ$ nestaționar
    \end{itemize}
...
\begin{frame}{ACF și PACF AR(1): Teorie vs Eșantion}
    ...
    \begin{itemize}
        ...
        \item \textbf{PACF}: Un singur vârf la lag 1, apoi se anulează -- aceasta identifică AR(1)
...
\begin{frame}{AR(p): Ilustrație Vizuală}
    ...
    \begin{exampleblock}{Caracteristici AR(2)}
        \begin{itemize}
            ...
            \item PACF se anulează după lag 2 --- tiparul cheie de identificare
        \end{itemize}
    \end{exampleblock}
    }
...
\begin{frame}{Modelul AR(2)}
    ...
    \vspace{0.15cm}
    \textbf{PACF:} Se anulează după lag 2 ($\pi_k = 0$ pentru $k > 2$)
    \quantlet{TSA\_ch2\_ar2}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_ar2}
\end{frame}
...
\begin{frame}{MA(1): Ilustrație Vizuală}
    ...
    \begin{itemize}
        ...
        \item \textbf{Panoul drept}: ACF arată \textbf{anulare caracteristică după lag 1}
            \begin{itemize}
                \item Doar $\rho(1) \neq 0$; toate lagurile superioare sunt zero
                \item Această anulare bruscă este identificatorul cheie pentru modele MA
            \end{itemize}
...
\begin{frame}{Proprietățile MA(1)}
    ...
    \vspace{0.15cm}
    \textbf{Observație cheie:} ACF \textbf{se anulează} după lag 1
    \quantlet{TSA\_ch2\_ma1}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_ma1}
\end{frame}
...
\begin{frame}{ACF și PACF MA(1): Comparație Vizuală}
    ...
    {\footnotesize
    \begin{itemize}
        \item \textbf{ACF}: Un singur vârf la lag 1, apoi se anulează imediat -- semnătura cheie MA(1)
...
\begin{frame}{Modelul MA(q): Forma Generală}
    ...
    \begin{itemize}
        ...
        \item ACF se anulează după lag $q$: $\rho(h) = 0$ pentru $h > q$
...
\begin{frame}{MA(q): Ilustrație Vizuală}
    ...
    \end{center}
    \vspace{-0.2cm}
    \small Proces MA(3). Semnătura cheie: ACF se anulează după lag $q$ (aici, lag 3).
...
\begin{frame}{Quiz: Recunoașterea Tiparelor ACF/PACF}
    \begin{alertblock}{Întrebare}
        Observați: ACF are vârf la lag 1, apoi se anulează. PACF scade gradual. Ce model?
    \end{alertblock}
...
\begin{frame}{ARMA: Ilustrație Vizuală}
    ...
    \begin{itemize}
        ...
        \item \textbf{Tipar ACF}: Descreștere după primul lag (nu anulare bruscă ca MA pur)
            \begin{itemize}
                \item Primul lag influențat atât de $\phi$ cât și de $\theta$
                \item Lagurile următoare descresc geometric ca AR
            \end{itemize}
        \item \textbf{Tipar PACF}: De asemenea descreștere (nu anulare bruscă ca AR pur)
...
\begin{frame}{Tipare ACF/PACF: Ghid Vizual}
    ...
    {\footnotesize
    \begin{itemize}
        \item \textbf{AR}: ACF scade, PACF se anulează -- folosiți PACF pentru a identifica ordinul $p$
        \item \textbf{MA}: ACF se anulează, PACF scade -- folosiți ACF pentru a identifica ordinul $q$
...
\begin{frame}{Analiza Reziduurilor}
    ...
    \begin{itemize}
        ...
        \item Fără vârfuri semnificâțive $\succ$ zgomot alb
    \end{itemize}
...
\begin{frame}{Testul Ljung-Box}
    ...
    \textbf{Decizie:}
    \begin{itemize}
        \item p-value $> 0.05$ $\succ$ nu respingem $H_0$ $\succ$ reziduurile arată ca zgomot alb (bine!)
        \item p-value $< 0.05$ $\succ$ autocorelație semnificâțivă rămâne $\succ$ model inadecvat
    \end{itemize}
    \quantlet{TSA\_ch2\_diagnostics}{https://github.com/QuantLet/TSA/tree/main/TSA_ch2/TSA_ch2_diagnostics}
\end{frame}
