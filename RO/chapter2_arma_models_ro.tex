...
\begin{frame}{Zgomot Alb: Ilustrare Vizuală}
    ...
    \begin{block}{Caracteristici Cheie}
        \textbf{Stânga}: Seria fluctuează aleatoriu în jurul mediei zero, fără tipare. \textbf{Dreapta}: ACF arată doar un vârf la lag 0; toate celelalte autocorelații sunt în intervalul de încredere --- nicio structură de prezis.
    \end{block}
\end{frame}
...
\begin{frame}{AR(1): Ilustrație Vizuală}
    ...
    \end{center}
    \vspace{-0.2cm}
    \begin{block}{Tipare de Comportament}
        \textbf{$\phi$ pozitiv}: Fluctuații persistente, netede --- valorile tind să rămână pe aceeași parte a mediei. \textbf{$\phi$ negativ}: Comportament oscilant --- valorile alternează în jurul mediei.
    \end{block}
\end{frame}
...
\begin{frame}{Demonstrație: Media AR(1)}
    ...
    \begin{alertblock}{Cerință}
        Aceasta necesită $\phi \neq 1$. Dacă $\phi = 1$ (rădăcină unitară), media este nedefinită.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Demonstrație: Varianța AR(1)}
    ...
    \begin{exampleblock}{Notă}
        Necesită $|\phi| < 1$ pentru varianță pozitivă. Când $|\phi| \to 1$, varianța $\to \infty$.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{AR(p): Ilustrație Vizuală}
    ...
    \begin{exampleblock}{Caracteristici AR(2)}
        AR(2) poate prezentă comportament pseudo-ciclic când rădăcinile sunt complexe conjugate. ACF arată descreștere sinusoidală amortizată; PACF se întrerupe după lag 2 --- tiparul cheie de identificare.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Triunghiul de Staționaritate AR(2)}
    ...
    \begin{itemize}
        ...
        \item Punctele din afără acestei regiuni duc la procese nestaționare sau explozive
    \end{itemize}
    }
\end{frame}
...
\begin{frame}{MA(1): Ilustrație Vizuală}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.58\textheight, keepaspectratio]{ch2_def_ma1.pdf}
    \end{center}
    \vspace{-0.2cm}
    \begin{block}{Semnătura MA(1)}
        ACF are un singur vârf la lag 1, apoi se întrerupe imediat --- aceasta este semnătura caracteristică MA(1). PACF descreștere exponențială --- opusul AR(1).
    \end{block}
\end{frame}
...
\begin{frame}{Demonstrație: Maximul ACF pentru MA(1)}
    ...
    \begin{exampleblock}{Implicație}
        Dacă eștimați $|\hat{\rho}(1)| > 0.5$ din date, procesul \textbf{nu} este MA(1).
    \end{exampleblock}
\end{frame}
...
\begin{frame}{ACF și PACF MA(1): Comparăție Vizuală}
    \vspace{-0.3cm}
    \begin{center}
...
\begin{frame}{Invertibilitatea Modelelor MA}
    ...
    \begin{itemize}
        \item Asigură reprezentăre unică
        \item Necesară pentru prognoză și eștimare
...
\begin{frame}{Invertibilitate: Ilustrație Vizuală}
    ...
    \end{center}
    \vspace{-0.2cm}
    \small Stânga: invertibilitatea necesită rădăcini în afără cercului unitate. Dreapta: ponderile AR($\infty$) scad doar când $|\theta| < 1$.
\end{frame}
...
\begin{frame}{ARMA: Ilustrație Vizuală}
    \begin{center}
        \includegraphics[width=0.95\textwidth, height=0.58\textheight, keepaspectratio]{ch2_def_arma.pdf}
    \end{center}
    \vspace{-0.2cm}
    \begin{exampleblock}{Proces ARMA(1,1)}
        ACF descreștere exponențială după lag 1 (ca AR), dar punctul de pornire depinde atât de $\phi$ cât și de $\theta$. Ambele ACF și PACF scad --- necesită criterii informaționale pentru selecția modelului.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Tabel Rezumat pentru Identificarea Modelului}
    \vspace{-0.2cm}
    \begin{center}
        \includegraphics[width=0.85\textwidth, height=0.65\textheight, keepaspectratio]{model_identificâțion_table.pdf}
    \end{center}
    \vspace{-0.2cm}
...
\begin{frame}{Criterii Informaționale}
    ...
    \begin{itemize}
        ...
        \item Comparăți modele potrivite pe \textit{aceleași date}
    \end{itemize}
\end{frame}
...
%=============================================================================
\section{Eștimarea Parametrilor}

\begin{frame}{Prezentare Generală a Metodelor de Eștimare}
    ...
    \vspace{0.15cm}
    \textbf{2. Eștimarea prin Maximum de Verosimilitate (MLE)}
    \begin{itemize}
...
\begin{frame}{Comparăția Metodelor de Eștimare}
    \begin{center}
        \includegraphics[width=0.80\textwidth, height=0.70\textheight, keepaspectratio]{eștimation_comparison.pdf}
    \end{center}
\end{frame}
...
\begin{frame}{Ecuațiile Yule-Walker: Forma Matriceală}
    ...
    \vspace{0.15cm}
    \textbf{Eștimare:} Înlocuiți $\rho(k)$ cu autocorelațiile din eșantion $\hat{\rho}(k)$
\end{frame}
...
\begin{frame}{Demonstrație: Ecuațiile Yule-Walker}
    ...
    \begin{exampleblock}{Cazul Special AR(1)}
        $\rho(k) = \phi_1 \rho(k-1) = \phi_1^k$ (folosind $\rho(0) = 1$)
    \end{exampleblock}
\end{frame}

\begin{frame}{Eștimarea prin Maximum de Verosimilitate}
...
\begin{frame}{Diagnosticarea Reziduurilor: Exemplu}
    ...
    \begin{itemize}
        ...
        \item \textbf{ACF reziduurilor}: Fără vârfuri semnificâțive indică zgomot alb (potrivire bună)
...
\begin{frame}{Evaluarea Prognozei}
    ...
    \begin{enumerate}
        ...
        \item Comparăți prognozele cu valorile reale
    \end{enumerate}
...
\begin{frame}{Exemplu de Prognoză Train/Validare/Test}
    ...
    \begin{alertblock}{Bună Practică}
        Evaluați întotdeauna prognozele pe date nevăzute. Folosiți setul de antrenare pentru eștimare, setul de validare pentru selecția modelului și setul de test pentru evaluarea finală.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Prognoză cu Fereastră Mobilă (Rolling Forecast)}
    ...
    \begin{block}{Metodologia Rolling Forecast}
        \begin{itemize}
            \item \textbf{Fereastră fixă:} Re-eștimare model folosind cele mai recente $w$ observații
            \item \textbf{Fereastră expansivă:} Folosește toate datele disponibile până la origine
            \item Generează prognoză 1-pas înainte, mută fereastra, repetă
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{Rolling vs Prognoză Multi-Pas}
    ...
    \begin{exampleblock}{Diferențe Cheie}
        \begin{itemize}
            \item \textbf{Rolling 1-pas:} Mai precis, necesită re-eștimare frecventă
            \item \textbf{Multi-pas direct:} Eștimare model separat pentru fiecare orizont $h$
            \item \textbf{Multi-pas recursiv:} Iterează prognoze 1-pas (acumulare erori)
        \end{itemize}
    \end{exampleblock}
\end{frame}

\begin{frame}{Aplicație cu Date Reale: Comparăție Prognoze}
    \begin{center}
    ...
    \begin{block}{Considerații Practice}
        \begin{itemize}
            \item Datele reale prezintă adesea nestaționaritate, rupturi structurale
            \item Comparăți mai multe modele: ARMA, netezire exponențială, naive
            \item Folosiți validare încrucișată sau evaluare rolling pentru robustețe
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{Rezumat Flux de Lucru}
    \begin{enumerate}
        ...
        \item \textbf{Eștimare și diagnosticare}
        \begin{itemize}
...
\begin{frame}{Studiu de Caz: Petele Solare (Sunspots)}
    ...
    \begin{block}{Descrierea Datelor}
        \textbf{Numărul anual de pete solare (1700--2008)}: Set de date clasic în analiza seriilor de timp. Serie staționară cu cicluri de aproximativ 11 ani. Vom aplica metodologia Box-Jenkins completă.
    \end{block}
\end{frame}
...
\begin{frame}{Pasul 1: Analiza ACF/PACF}
    ...
    \begin{exampleblock}{Identificare}
        \begin{itemize}
            \item \textbf{ACF}: Descreștere lentă, sinusoidală --- sugerează AR
            \item \textbf{PACF}: Vârfuri semnificâțive la lag-urile 1, 2, 9 --- sugerează AR(9) sau AR(2)
            \item Seria pare staționară (nu necesită diferențiere, $d=0$)
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Pasul 2: Comparărea Modelelor}
    ...
    \begin{alertblock}{Selecția Modelului}
        Comparăm mai multe modele cândidate folosind criteriul AIC. Modelul \textbf{AR(9)} are cel mai mic AIC, capturând ciclul solar de 11 ani.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Pasul 3: Verificarea Diagnostică}
    ...
    \begin{block}{Diagnostice AR(9)}
        Reziduurile seamănă cu zgomot alb: medie zero, varianță constantă, ACF fără structură semnificâțivă, distribuție aproximativ normală.
    \end{block}
\end{frame}
...
\begin{frame}{Pasul 4: Prognoză}
    ...
    \begin{exampleblock}{Rezultate}
        \begin{itemize}
            \item Modelul AR(9) captează natura ciclică a petelor solare
            \item Intervalele de încredere de 95\% acoperă majoritatea valorilor reale
            \item RMSE pe setul de test: aproximativ 30 (rezonabil pentru această serie volatilă)
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Concluzii Cheie}
    \begin{enumerate}
        ...
        \begin{itemize}
            \item Staționaritate: rădăcinile lui $\phi(z)$ în afără cercului unitate
            \item PACF se întrerupe la lag $p$
        \end{itemize}

        \item \textbf{Modele MA(q):} Valoarea curentă depinde de $q$ șocuri trecute
        \begin{itemize}
            \item Întotdeauna staționar; invertibilitate: rădăcinile lui $\theta(z)$ în afără cercului unitate
        ...
        \item \textbf{Box-Jenkins:} Identificare $\to$ Eștimare $\to$ Diagnosticare $\to$ Prognoză
