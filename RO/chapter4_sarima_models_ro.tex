...
\begin{frame}{Testul HEGY: Formula de regresie (Trimestrial)}
    ...
    \begin{block}{Regresia auxiliara HEGY}
        Pentru date trimestriale ($s=4$), eștimam:
...
\begin{frame}{Testul Canova-Hansen: Formula}
    ...
    \begin{block}{Statistică de test LM}
        ...
        unde $\hat{\omega}_i$ = eștimator consistent al densității spectrale la frecvența $\lambda_i$.
    \end{block}
    \vspace{-0.1cm}
...
\begin{frame}{Diferenta sezonieră: Ilustrare vizuala}
    ...
    \begin{block}{Efectul Diferențierii Sezoniere}
        Stânga: Seria originală cu tipar sezonier clar. Dreapta: După $\Delta_{12} = (1-L^{12})$, tiparul sezonier este eliminăt. Comparăția an-la-an elimină efectele sezoniere.
    \end{block}
\end{frame}
...
\begin{frame}{Structura multiplicâțivă}
    ...
    \begin{alertblock}{Interpretare}
        Structura multiplicâțivă permite modelarea parsimonioasă a tiparelor sezoniere complexe cu puțini parametri.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Metode de estimare}
    ...
    \begin{block}{Considerații computationale}
        \begin{itemize}
            ...
            \item Parametrii sezonieri eștimati din lag-urile $s, 2s, \ldots$
...
\begin{frame}{Intervale de prognoză}
    \begin{block}{Cuantificarea incertitudinii}
        ...
        Varianta calculata din reprezentărea MA($\infty$).
    \end{block}
...
\begin{frame}{Pasul 1: Transformări}
    ...
    {\small
    \begin{itemize}
        \item Transformarea log stabilizează varianța (multiplicâțiv $\rightarrow$ aditiv)
...
\begin{frame}{Pasul 3: Comparărea modelelor}
    \vspace{-0.2cm}
    \begin{center}
...
\begin{frame}{Întrebarea 2: Răspuns}
    \begin{exampleblock}{Răspuns corect: (B) Schimbarea an-la-an}
        $(1 - L^{12})Y_t = Y_t - Y_{t-12}$ elimină tiparul sezonier prin comparărea acelorasi luni.
    \end{exampleblock}
    \vspace{0.2cm}
    \begin{center}
...
\begin{frame}{Întrebarea 4}
    \begin{alertblock}{Întrebare}
        ``Modelul Airline'' este SARIMA$(0,1,1)\times(0,1,1)_{12}$. Câți parametri trebuie eștimati (excluzand varianța)?
    \end{alertblock}
