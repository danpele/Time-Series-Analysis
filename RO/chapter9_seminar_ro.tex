...
\begin{frame}{Test 5: Prophet vs TBATS}
    ...
    \begin{enumerate}[A)]
        ...
        \item Când aveți nevoie de cel mai parsimonios model
...
\begin{verbatim}
model = Prophet(
    seasonality_mode='multiplicative',  # Amplitudine sezonieră crește
    changepoint_prior_scale=0.5,        # Trend mai flexibil
    yearly_seasonality=True,
    weekly_seasonality=False            # Date lunare
)
...
\end{verbatim}
    ...
    \begin{alertblock}{Decizie Cheie}
        Am folosit sezonalitate multiplicativă deoarece efectul decembrie este proporțional cu nivelul de bază al vânzărilor.
    \end{alertblock}
    }
\end{frame}
...
# Specificarea perioadelor sezoniere multiple
energy_msts <- msts(energy_data,
                    seasonal.periods = c(24, 168, 8760))

# Ajustare TBATS - selecție automată a termenilor Fourier
fit <- tbats(energy_msts)

# Prognoză săptămâna următoare
