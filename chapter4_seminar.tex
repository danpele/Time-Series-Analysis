...
\begin{frame}{Quiz 2: SARIMA Notation}
    ...
    \begin{exampleblock}{Answer: C -- ARIMA(1,1,1) with seasonal ARIMA(1,1,1) at period 12}
        \vspace{-0.2cm}
        \begin{center}
            \includegraphics[width=0.95\textwidth, height=0.5\textheight, keepaspectratio]{charts/sem4_sarima_notation.pdf}
        \end{center}
        \vspace{-0.2cm}
        {\footnotesize
...
\begin{frame}{Quiz 3: The Airline Model}
    ...
    \begin{exampleblock}{Answer: A -- 2 parameters ($\theta_1$ and $\Theta_1$)}
        \vspace{-0.2cm}
        \begin{center}
            \includegraphics[width=0.95\textwidth, height=0.5\textheight, keepaspectratio]{charts/sem4_airline_model.pdf}
        \end{center}
        \vspace{-0.2cm}
        {\footnotesize
        \textbf{Airline model}: $(1-L)(1-L^{12})Y_t = (1+\theta_1 L)(1+\Theta_1 L^{12})\varepsilon_t$ -- remarkably fits many seasonal series!
        }
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 4: ACF of Seasonal Data}
    ...
    \begin{exampleblock}{Answer: C -- At lags 12, 24, 36, ...}
        ...
        \textbf{ACF pattern for seasonal data}:
        \begin{itemize}
            \item Spikes at lag $s$: $\rho_{12} \neq 0$
            \item Spikes at lag $2s$: $\rho_{24} \neq 0$
            \item Spikes at lag $3s$: $\rho_{36} \neq 0$
        \end{itemize}

        \textbf{Diagnostic}: If ACF decays slowly at seasonal lags $\Rightarrow$ seasonal unit root ($D=1$)

        If ACF cuts off after lag $s$ $\Rightarrow$ seasonal MA ($Q=1$)
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 5: Multiplicative Structure}
    ...
    \begin{exampleblock}{Answer: B -- Regular and seasonal polynomials are multiplied}
        \textbf{Multiplicative SARIMA}:
        \[
        \underbrace{\phi(L)\Phi(L^s)}_{\text{AR}} \cdot \underbrace{(1-L)^d(1-L^s)^D}_{\text{Diff}} Y_t = \underbrace{\theta(L)\Theta(L^s)}_{\text{MA}} \varepsilon_t
        \]

        \textbf{Example}: $\phi(L)\Phi(L^{12}) = (1-\phi_1 L)(1-\Phi_1 L^{12})$
        \[
        = 1 - \phi_1 L - \Phi_1 L^{12} + \phi_1\Phi_1 L^{13}
        \]
...
\begin{frame}{Quiz 6: Seasonal vs Regular Differencing}
    ...
    \begin{exampleblock}{Answer: C -- Both trend and seasonal non-stationarity}
        \textbf{Combined differencing}:
        \[
        W_t = (1-L)(1-L^{12})Y_t = Y_t - Y_{t-1} - Y_{t-12} + Y_{t-13}
        \]

        \textbf{When needed}:
        \begin{itemize}
            \item ACF decays slowly at lags 1, 2, 3, ... $\Rightarrow$ need $d=1$
            \item ACF decays slowly at lags 12, 24, 36, ... $\Rightarrow$ need $D=1$
        \end{itemize}

        \textbf{Classic example}: Airline passengers, retail sales, energy demand
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 7: Detecting Seasonality from ACF}
    ...
    \begin{exampleblock}{Answer: C -- Seasonal unit root requiring $D=1$}
        \vspace{-0.2cm}
        \begin{center}
            \includegraphics[width=0.9\textwidth, height=0.48\textheight, keepaspectratio]{charts/sem4_seasonal_acf.pdf}
        \end{center}
        \vspace{-0.2cm}
        {\footnotesize
        \textbf{Left}: Stationary seasonal (fast decay at seasonal lags) \quad
        \textbf{Right}: Seasonal unit root (slow decay $\Rightarrow D=1$ needed)
        }
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 8: Multiplicative vs Additive Seasonality}
    ...
    \begin{exampleblock}{Answer: B -- Multiplicative seasonality, use $\log$ transformation}
        \vspace{-0.2cm}
        \begin{center}
            \includegraphics[width=0.95\textwidth, height=0.5\textheight, keepaspectratio]{charts/sem4_mult_add.pdf}
        \end{center}
        \vspace{-0.2cm}
        {\footnotesize
        \textbf{Multiplicative}: Diverging subseries lines $\Rightarrow$ Use log transformation!
        }
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 9: Seasonal Subseries Plot}
    ...
    \begin{exampleblock}{Answer: B -- Lines diverge (spread increases over time)}
        \textbf{Seasonal subseries plot}: Groups data by month, plots each month's values across years

        \textbf{Interpretation}:
        \begin{itemize}
            \item \textbf{Parallel lines}: Constant seasonal amplitude $\Rightarrow$ Additive
            \item \textbf{Diverging lines}: Growing amplitude $\Rightarrow$ Multiplicative
            \item \textbf{Horizontal lines}: No trend within months
        \end{itemize}
...
\begin{frame}{Quiz 10: Invertibility in SARIMA}
    ...
    \begin{exampleblock}{Answer: C -- Both $|\theta_1| < 1$ and $|\Theta_1| < 1$}
        \textbf{Invertibility condition}: All MA roots must be outside unit circle

        \textbf{Multiplicative MA}: $(1+\theta_1 L)(1+\Theta_1 L^{12})$

        \textbf{Roots}:
        \begin{itemize}
            \item Regular: $z = -1/\theta_1$ must satisfy $|z| > 1 \Leftrightarrow |\theta_1| < 1$
            \item Seasonal: $z^{12} = -1/\Theta_1$ must satisfy $|z| > 1 \Leftrightarrow |\Theta_1| < 1$
        \end{itemize}
...
\begin{frame}{Quiz 11: HEGY Test}
    ...
    \begin{exampleblock}{Answer: B -- Test for unit roots at different frequencies}
        ...
        Tests for unit roots at:
        \begin{itemize}
            \item Zero frequency ($\omega = 0$): trend unit root $\Rightarrow d = 1$
            \item Nyquist frequency ($\omega = \pi$): biannual cycle
            \item Seasonal frequencies: annual cycles $\Rightarrow D = 1$
        \end{itemize}

        \textbf{Decision}:
        \begin{itemize}
            \item Reject at all frequencies $\Rightarrow$ use seasonal dummies
            \item Don't reject seasonal $\Rightarrow$ use seasonal differencing
        \end{itemize}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 12: Seasonal MA Identification}
    ...
    \begin{exampleblock}{Answer: A -- SARIMA$(0,1,0) \times (0,1,1)_{12}$}
        ...
        \begin{tabular}{lcc}
            ...
            Regular lags & No spikes & No spikes \\
            Seasonal lags & Cuts off at $s$ & Decays
        \end{tabular}
        \end{center}

        \textbf{Interpretation}:
        \begin{itemize}
            \item No regular MA needed ($q = 0$)
            \item Seasonal MA(1) indicated ($Q = 1$)
        \end{itemize}
...
\begin{frame}{Quiz 13: Over-differencing}
    ...
    \begin{exampleblock}{Answer: B -- The series has been over-differenced}
        \textbf{Over-differencing signature}:
        \begin{itemize}
            \item ACF at lag 1 $\approx -0.5$ $\Rightarrow$ over-differenced at $d$
            \item ACF at lag $s$ $\approx -0.5$ $\Rightarrow$ over-differenced at $D$
        \end{itemize}

        \textbf{Why?} If $\Delta Y_t = \varepsilon_t$, then $\Delta^2 Y_t = \varepsilon_t - \varepsilon_{t-1}$

        This is MA(1) with $\theta = -1$, giving $\rho_1 = \frac{-1}{1+1} = -0.5$
...
\begin{frame}{Quiz 14: Forecasting Horizon}
    ...
    \begin{exampleblock}{Answer: B -- They grow without bound}
        \textbf{Unit root property}: Any unit root (regular or seasonal) causes unbounded forecast variance

        \textbf{For SARIMA with $D=1$}:
        \[
        \Var(\hat{Y}_{T+h} - Y_{T+h}) \to \infty \text{ as } h \to \infty
        \]

        \textbf{Intuition}: Seasonal shocks accumulate over time

        \textbf{Practical implication}: Long-range forecasts for seasonal integrated series have very wide confidence intervals
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 15: Seasonal Period Selection}
    ...
    \begin{exampleblock}{Answer: B -- $s = 7$ (weekly)}
        \textbf{Seasonal period selection}:
        \begin{center}
        \begin{tabular}{lll}
            \textbf{Data} & \textbf{Pattern} & \textbf{Period $s$} \\
            Daily & Weekly & 7 \\
            Daily & Annual & 365 (or 252 for trading) \\
            Monthly & Annual & 12 \\
            Quarterly & Annual & 4 \\
            Hourly & Daily & 24
        \end{tabular}
        ...
        \textbf{Rule}: $s$ = number of observations per cycle of the dominant pattern
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 16: Seasonal AR Component}
    ...
    \begin{exampleblock}{Answer: C -- 80\% explained by same period last year}
        \textbf{SAR(1) equation}: $(1 - \Phi_1 L^{12})Y_t = \varepsilon_t$

        Rearranged: $Y_t = \Phi_1 Y_{t-12} + \varepsilon_t$

        \textbf{With $\Phi_1 = 0.8$}: $Y_{Jan2024} = 0.8 \cdot Y_{Jan2023} + \varepsilon_t$

        \textbf{Interpretation}: Strong seasonal persistence --- each month's value is 80\% determined by the same month in the previous year

        \textbf{Stationarity}: Requires $|\Phi_1| < 1$ (here satisfied: $|0.8| < 1$)
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 17: Seasonal Stationarity}
    ...
    \begin{exampleblock}{Answer: B -- Has a seasonal unit root}
        \textbf{Model}: $(1 - L^{12})Y_t = \varepsilon_t$ when $\Phi_1 = 1$

        This implies: $Y_t = Y_{t-12} + \varepsilon_t$ (seasonal random walk)

        \textbf{Properties}:
        \begin{itemize}
            \item Variance grows with time (non-stationary)
            \item Each month follows its own random walk
            \item Need $D = 1$ to make stationary
        \end{itemize}

        \textbf{Analogy}: Like regular RW but at seasonal frequency
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 18: Model Comparison}
    ...
    \begin{exampleblock}{Answer: C -- AIC difference of 5 suggests Model A is substantially better}
        \textbf{AIC interpretation}: Lower AIC = better balance of fit and complexity

        \textbf{Rule of thumb} (Burnham \& Anderson):
        \begin{itemize}
            \item $\Delta$AIC $< 2$: Models are essentially equivalent
            \item $\Delta$AIC $2-10$: Some evidence for better model
            \item $\Delta$AIC $> 10$: Strong evidence for better model
        \end{itemize}
...
\begin{frame}{Quiz 19: Seasonal Patterns in Residuals}
    ...
    \begin{exampleblock}{Answer: B -- The seasonal component is inadequate}
        \textbf{Residual diagnostics}: Good residuals should be white noise (no significant ACF)

        \textbf{Seasonal ACF in residuals} means:
        \begin{itemize}
            \item Model hasn't fully captured seasonal structure
            \item Consider increasing $P$ or $Q$
            \item Verify $D$ is correct (not over/under-differenced)
        \end{itemize}
...
\begin{frame}{Quiz 20: Practical Forecasting}
    ...
    \begin{exampleblock}{Answer: B -- The observation from the same month last year}
        \textbf{Airline model forecast structure}: $(1-L)(1-L^{12})Y_t = (1+\theta L)(1+\Theta L^{12})\varepsilon_t$

        \textbf{For 13-month ahead forecast}:
        \begin{itemize}
            \item Most influential: $Y_{T-11}$ (same month last year)
            \item Also influenced by $Y_T$ (most recent) and $Y_{T-12}$
            \item MA shocks decay quickly, seasonal structure persists
        \end{itemize}
