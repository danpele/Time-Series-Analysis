...
\begin{frame}{Quiz 7: Detecting Seasonality from ACF}
    ...
    \pause
    \begin{exampleblock}{Answer: C}
        Slow decay of ACF at seasonal lags ($s, 2s, 3s, \ldots$) indicates a seasonal unit root. This requires seasonal differencing $(1-L^{12})$ to achieve stationarity. Fast cutoff at seasonal lags would suggest a stationary seasonal pattern.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 8: Multiplicative vs Additive Seasonality}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        When seasonal peaks and troughs grow with the level, we have multiplicative seasonality: $Y_t = T_t \times S_t \times \varepsilon_t$. Taking logarithms converts this to additive: $\log Y_t = \log T_t + \log S_t + \log \varepsilon_t$, making standard SARIMA applicable.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 9: Seasonal Subseries Plot}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        In a seasonal subseries plot, each line shows one month across all years. If the lines diverge (fan out), the spread between high and low months is increasing, indicating multiplicative seasonality. Parallel lines indicate additive seasonality.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 10: Invertibility in SARIMA}
    ...
    \pause
    \begin{exampleblock}{Answer: C}
        Both the regular MA polynomial $\theta(L) = 1 + \theta_1 L$ and seasonal MA polynomial $\Theta(L^s) = 1 + \Theta_1 L^{12}$ must have roots outside the unit circle. This requires $|\theta_1| < 1$ AND $|\Theta_1| < 1$ for invertibility.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 11: HEGY Test}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        The HEGY (Hylleberg-Engle-Granger-Yoo) test examines unit roots at different frequencies: zero frequency (trend unit root) and seasonal frequencies. It helps determine whether to use deterministic seasonality (dummies) or stochastic seasonality (seasonal differencing).
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 12: Seasonal MA Identification}
    ...
    \pause
    \begin{exampleblock}{Answer: A}
        ACF cutting off at lag 12 (not at lag 1) with decaying PACF at seasonal lags suggests only a seasonal MA(1) component with no regular MA. The model is SARIMA$(0,1,0) \times (0,1,1)_{12}$.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 13: Over-differencing}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        A large negative ACF at lag 1 (close to $-0.5$) or at lag $s$ after differencing is a classic sign of over-differencing. This introduces artificial negative autocorrelation. Solution: reduce $d$ or $D$ by one.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 14: Forecasting Horizon}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        With seasonal differencing ($D \geq 1$), the series has a seasonal unit root, meaning it's seasonally integrated. Like regular unit roots, this causes forecast uncertainty to grow without bound as the horizon increases.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 15: Seasonal Period Selection}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        The seasonal period should match the repeating pattern's frequency. Weekly patterns in daily data repeat every 7 days, so $s = 7$. Always choose $s$ based on the dominant seasonal cycle in your data.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 16: Seasonal AR Component}
    ...
    \pause
    \begin{exampleblock}{Answer: C}
        The seasonal AR coefficient $\Phi_1$ measures the dependence on the observation from one seasonal cycle ago. With $\Phi_1 = 0.8$, about 80\% of January's value (for example) is explained by last January's value, capturing persistence in seasonal patterns.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 17: Seasonal Stationarity}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        When $|\Phi_1| = 1$, the process has a seasonal unit root, meaning it's seasonally integrated (requires seasonal differencing). This is analogous to a regular unit root ($\phi = 1$) but at the seasonal frequency. The solution is to apply $(1-L^s)$ differencing.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 18: Model Comparison}
    ...
    \pause
    \begin{exampleblock}{Answer: C}
        An AIC difference of 5 is meaningful---Model A provides a better fit even after penalizing for its extra parameters. However, we should also check residual diagnostics and consider if the simpler model (B) is ``good enough'' for practical purposes.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 19: Seasonal Patterns in Residuals}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        Significant residual ACF at seasonal lags (12, 24, ...) indicates the SARIMA model hasn't fully captured the seasonal structure. Consider increasing $P$ or $Q$ in the seasonal component, or checking if $D$ is appropriate.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Quiz 20: Practical Forecasting}
    ...
    \pause
    \begin{exampleblock}{Answer: B}
        For SARIMA with $d=D=1$, the 13-month forecast heavily depends on the value from 12 months ago (same month last year) plus recent shocks. The seasonal structure ensures forecasts capture both trend and seasonal patterns, with seasonal memory being particularly important for seasonal-horizon forecasts.
    \end{exampleblock}
\end{frame}
