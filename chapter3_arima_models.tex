...
\begin{frame}{Augmented Dickey-Fuller (ADF) Test}
    \begin{block}{The Problem with Simple DF}
        If the true model has AR dynamics beyond AR(1), the DF test residuals will be autocorrelated, invalidating the test.
    \end{block}

    \vspace{0.3cm}

    \begin{defn}[ADF Test]
        Add lagged differences to absorb serial correlation:
    ...
    \begin{block}{Choosing Lag Length $k$}
        \begin{itemize}
            ...
            \item Start with $k_{max}$ and reduce until last lag is significant
            \item Ensure residuals are white noise
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{ADF Test Critical Values}
    \begin{table}
        ...
        \begin{tabular}{lccc}
            ...
            \bottomrule
        \end{tabular}
        \caption{Asymptotic critical values for ADF test (approximate)}
    \end{table}
    ...
    \begin{block}{Decision Rule}
        \begin{itemize}
            \item If test statistic $<$ critical value $\Rightarrow$ Reject $H_0$ (stationary)
            \item If test statistic $\geq$ critical value $\Rightarrow$ Fail to reject $H_0$ (unit root)
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{The KPSS Test}
    \begin{block}{Reversed Hypotheses}
        Unlike ADF, the KPSS test has:
        \begin{itemize}
            \item $H_0$: Series is stationary (possibly around a trend)
            \item $H_1$: Series has a unit root
        \end{itemize}
    \end{block}
    ...
    \begin{block}{KPSS Procedure}
        Decompose: $Y_t = \xi t + r_t + \varepsilon_t$

        where $r_t = r_{t-1} + u_t$ is a random walk.

        Test whether $\Var(u_t) = 0$ (stationarity).
    \end{block}

    \vspace{0.3cm}

    \begin{exampleblock}{Complementary Use}
        Use ADF and KPSS together:
        \begin{itemize}
            \item ADF rejects, KPSS doesn't reject $\Rightarrow$ Stationary
...
\begin{frame}{US Real GDP: A Non-Stationary Series}
    \begin{center}
        \includegraphics[width=0.95\textwidth]{charts/ch3_gdp_levels.pdf}
    \end{center}
...
\begin{frame}{Effect of Differencing}
    \begin{center}
        \includegraphics[width=0.95\textwidth]{charts/ch3_differencing.pdf}
    \end{center}
...
\begin{frame}{Unit Root Test Results}
    \begin{center}
        \includegraphics[width=0.85\textwidth]{charts/ch3_adf_test.pdf}
    \end{center}

    \vspace{0.2cm}
    \begin{itemize}
        \item GDP in levels: \textbf{Cannot reject} unit root (non-stationary)
        \item GDP growth: \textbf{Reject} unit root at 1\% level (stationary)
        \item Conclusion: GDP is I(1), need $d=1$ in ARIMA
    \end{itemize}
\end{frame}

\begin{frame}{ACF/PACF: Levels vs Differenced}
    \begin{center}
        \includegraphics[width=0.85\textwidth]{charts/ch3_acf_pacf.pdf}
    \end{center}

    \vspace{0.1cm}
    {\small
    \begin{itemize}
        \item \textbf{Top row}: Slow ACF decay in levels suggests non-stationarity
        \item \textbf{Bottom row}: After differencing, ACF/PACF help identify $p$ and $q$
    \end{itemize}
    }
\end{frame}
...
\begin{frame}{ARIMA Forecasting: Actual vs Predicted}
    \begin{center}
        \includegraphics[width=0.95\textwidth]{charts/ch3_arima_forecast.pdf}
    \end{center}

    \vspace{0.2cm}
    \begin{itemize}
        \item ARIMA(1,1,1) captures the trend dynamics
        \item Confidence intervals widen with forecast horizon (I(1) property)
    \end{itemize}
\end{frame}

\begin{frame}{Model Diagnostics}
    \begin{center}
        \includegraphics[width=0.85\textwidth]{charts/ch3_diagnostics.pdf}
    \end{center}

    \vspace{0.1cm}
    {\small
    \begin{itemize}
        \item Residuals appear random (no patterns)
        \item ACF within bounds -- no remaining autocorrelation
