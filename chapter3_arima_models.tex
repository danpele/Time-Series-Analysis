...
\begin{frame}[plain]
    ...
    \begin{center}
        ...
        {\LARGE\textbf{\textcolor{MainBlue}{Chapter 3: ARIMA Models for Non-Stationary Data}}}\\[0.5cm]
...
\begin{frame}{Example: Modeling GDP Growth}
    \begin{block}{Typical Workflow}
        \begin{enumerate}
            \item \textbf{Plot}: Observe trend in log(GDP)
            \item \textbf{Test}: ADF suggests I(1)
            \item \textbf{Difference}: Work with GDP growth rate
            \item \textbf{Identify}: ACF/PACF suggest ARIMA(1,1,0) or ARIMA(0,1,1)
            \item \textbf{Estimate}: Fit both, compare AIC
            \item \textbf{Diagnose}: Check residuals
            \item \textbf{Forecast}: Generate predictions with intervals
        \end{enumerate}
    \end{block}
\end{frame}
